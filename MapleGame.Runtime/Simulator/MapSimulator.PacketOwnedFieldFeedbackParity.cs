...
namespace HaCreator.MapSimulator
{
    ...
    {
        private readonly PacketFieldFeedbackRuntime _packetFieldFeedbackRuntime;
        ...
        private string ResolvePacketFieldFeedbackMobName(int mobId)
        {
            return ResolvePacketGuideMobName(mobId);
        }

        private int? ResolvePacketFieldFeedbackMobMaxHp(int mobId)
        {
            if (mobId <= 0)
            {
                return null;
            ...
            try
            {
                WzImage mobImage = runtimeServices.Assets.FindImage("Mob", $"{mobId.ToString(CultureInfo.InvariantCulture)}.img");
                if (mobImage == null)
                {
                    return null;
                }
        ...
        private string ResolvePacketFieldFeedbackItemName(int itemId)
        {
            return InventoryItemMetadataResolver.TryResolveItemName(itemId, runtimeServices.Catalog, out string itemName)
                ? itemName
                : $"Item {itemId}";
        }
        ...
        {
            RuntimeMiscDefinition clock = _runtimeMapDefinition?.Misc
                .FirstOrDefault(item => item.Kind == RuntimeMiscKind.Clock);
            if (clock == null || clock.Kind != RuntimeMiscKind.Clock)
            {
                return false;
            }
        ...
        private void EnsurePacketOwnedBossTimerAssetsLoaded()
        {
            ...
            WzImage mapEtcImage = runtimeServices.Assets.FindImage("Map", "Obj/etc.img")
                ?? runtimeServices.Assets.FindImage("Map", "etc.img");
        ...
        private void EnsureRemoteEvolRingFloatNoticeAssetsLoaded()
        {
            ...
            WzImage uiWindowImage = runtimeServices.Assets.FindImage("UI", "UIWindow2.img")
                ?? runtimeServices.Assets.FindImage("UI", "UIWindow.img");
        ...
        private List<PacketOwnedUiFrame> ResolvePacketOwnedSummonEffectFrames(byte effectId, out string sourceUol)
        {
            ...
            {
                WzImage image = runtimeServices.Assets.FindImage(categoryName, imageName);
        ...
        {
            WzImage soundImage = runtimeServices.Assets.FindImage("Sound", "Summon.img");
        ...
        private IReadOnlyList<PacketFieldSwindleWarningEntry> GetPacketOwnedSwindleWarningEntries()
        {
            ...
            WzImage swindleImage = runtimeServices.Assets.FindImage("Etc", "Swindle");
            ...
            foreach (PacketOwnedRewardRouletteLayerCandidate candidate in family)
            {
                ...
                if (!TryGetOrCreatePacketOwnedAnimationFrames(
                    ...
                    () => LoadPacketOwnedAnimationFrames(
                        ResolvePacketOwnedPropertyPath(
                            runtimeServices.Assets.FindImage(candidate.CategoryName, candidate.ImageName),
