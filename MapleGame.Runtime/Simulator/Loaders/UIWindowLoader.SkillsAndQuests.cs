...
namespace HaCreator.MapSimulator.Loaders
{
    public static partial class UIWindowLoader
    {
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            return CreateSkillWindow(uiWindow1Image, basicImage, soundUIImage, device, screenWidth, screenHeight, runtimeAssets);
        }
        ...
        /// </summary>
        public static void LoadSkillsForJob(UILoaderResourceCache resourceCache,
            SkillUI skillWindow,
            int jobId,
            ...
            int subJob = 0)
        {
            ...
            try
            {
                ReloadVisibleSkillRoots(resourceCache,
                    skillWindow,
                    jobId,
                    device,
                    learnedSkillIds,
        ...
        internal static void RefreshVisibleSkillRootsFromCurrentRecords(UILoaderResourceCache resourceCache,
            SkillUI skillWindow,
            int jobId,
            GraphicsDevice device,
            IRuntimeAssetSource runtimeAssets,
        ...
        /// </summary>
        public static void LoadBeginnerSkills(UILoaderResourceCache resourceCache, SkillUIBigBang skillWindow, WzFile skillWzFile, WzFile stringWzFile, GraphicsDevice device, IRuntimeAssetSource runtimeAssets)
        {
            // Default to beginner job
            LoadSkillsForJob(resourceCache, skillWindow, 0, device, runtimeAssets);
        }
        ...
        /// </summary>
        public static void LoadAllSkills(UILoaderResourceCache resourceCache, SkillUIBigBang skillWindow, WzFile skillWzFile, GraphicsDevice device, IRuntimeAssetSource runtimeAssets, int focusJobId = 0)
        {
            if (skillWindow == null)
                return;
            ...
            try
            {
                ...
                {
                    LoadSkillsForJob(resourceCache, skillWindow, focusJobId, device, runtimeAssets);
                    return;
                }
                ...
                var defaultIcon = SkillDataLoader.LoadJobIcon(resourceCache, 0, device, runtimeAssets);
                ...
                var availableBookIds = SkillDataLoader.GetAvailableSkillBookJobIds(skillWzFile, runtimeAssets);
...

                {
                    ...
                    {
                        var skills = SkillDataLoader.LoadSkillsForJob(resourceCache, resolvedBookJobId, device, runtimeAssets);
        ...
        private static IReadOnlyList<int> GetVisibleSkillRootIdsForJob(int jobId, int subJob, IEnumerable<int> learnedSkillIds, IRuntimeAssetSource runtimeAssets)
        {
            if (ShouldLoadFocusedJobOnly(jobId))
                return new[] { jobId };

            var availableRootIds = SkillDataLoader.GetAvailableSkillBookJobIds(null, runtimeAssets);
            ...
            var visibleRootIds = SkillRootVisibilityResolver.ResolveVisibleSkillRootIds(
                ...
                availableRootIds,
                learnedSkillIds,
                (rootId, skillId) => SkillDataLoader.SkillRootContainsSkill(rootId, skillId, runtimeAssets));
        ...
        private static void EnsureDefaultSkillBookHeader(UILoaderResourceCache resourceCache, SkillUIBigBang skillWindow, GraphicsDevice device, IRuntimeAssetSource runtimeAssets)
        {
            Texture2D defaultBookIcon = SkillDataLoader.LoadJobIcon(resourceCache, 0, device, runtimeAssets);
        ...
        private static void ReloadVisibleSkillRoots(UILoaderResourceCache resourceCache,
            SkillUIBigBang skillWindow,
            int jobId,
            GraphicsDevice device,
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            EnsureDefaultSkillBookHeader(resourceCache, skillWindow, device, runtimeAssets);

            IReadOnlyList<int> visibleRootIds = GetVisibleSkillRootIdsForJob(jobId, subJob, learnedSkillIds, runtimeAssets);
            if (!clearLoadedSkillTabs)
            {
            ...
            {
            EnsureSkillRootLoaded(resourceCache, skillWindow, skillRootId, device, runtimeAssets);
            }
        ...
        private static void EnsureSkillRootLoaded(UILoaderResourceCache resourceCache, SkillUIBigBang skillWindow, int skillRootId, GraphicsDevice device, IRuntimeAssetSource runtimeAssets)
        {
            ...
            if (!hasLoadedCurrentRoot)
            {
                ...
                {
                    foreach (SkillDisplayData skill in SkillDataLoader.LoadSkillsForJob(resourceCache, bookJobId, device, runtimeAssets))
                    {
            ...
            skillWindow.SetRecommendedSkillEntries(
                skillRootId,
                SkillDataLoader.LoadRecommendedSkillEntries(skillRootId, runtimeAssets));

            Texture2D jobIcon = SkillDataLoader.LoadJobIcon(resourceCache, skillRootId, device, runtimeAssets);
            if (jobIcon == null)
            {
                foreach (int bookJobId in GetSkillBookAliasesForJob(skillRootId).Distinct())
                {
                    jobIcon = SkillDataLoader.LoadJobIcon(resourceCache, bookJobId, device, runtimeAssets);
                    if (jobIcon != null)
                        break;
                }
            }
        ...
        private static void EnsureSkillRootLoaded(UILoaderResourceCache resourceCache, SkillUI skillWindow, int skillRootId, GraphicsDevice device, IRuntimeAssetSource runtimeAssets)
        {
            int tabIndex = GetSkillTabFromJobId(skillRootId);
            HashSet<int> aliasRootIds = GetSkillBookAliasesForJob(skillRootId).ToHashSet();
            bool hasLoadedCurrentRoot = skillWindow.TryGetDisplayedSkillRootId(tabIndex, out int displayedSkillRootId)
            ...
            skillWindow.SetRecommendedSkillEntries(
                ...
                loadedSkillIds == null
                    ? SkillDataLoader.LoadRecommendedSkillEntries(skillRootId, runtimeAssets)
                    : SkillDataLoader.LoadRecommendedSkillEntries(skillRootId, runtimeAssets, loadedSkillIds));
            skillWindow.SetJobInfo(tabIndex, jobIcon, SkillDataLoader.GetJobName(skillRootId));
        }
            ...
            IRuntimeDataServices runtimeServices)
        {
            if (isBigBang && uiWindow2Image != null)
            {
                return CreateQuestWindowBigBang(uiWindow2Image, uiWindow1Image, basicImage, soundUIImage, device, screenWidth, screenHeight, runtimeServices);
            }
            return CreateQuestWindow(uiWindow1Image, soundUIImage, device, screenWidth, screenHeight, runtimeServices);
        }
        ...
        public static QuestUIBigBang CreateQuestWindowBigBang(
            WzImage uiWindow2Image, WzImage uiWindow1Image, WzImage basicImage, WzImage soundUIImage,
            GraphicsDevice device, int screenWidth, int screenHeight, IRuntimeDataServices runtimeServices)
        {
            ...
            {
                return CreatePlaceholderQuestBigBang(device, screenWidth, screenHeight, runtimeServices);
            }
            ...
            QuestUIBigBang quest = new QuestUIBigBang(frame, device, runtimeServices);
        ...
        private static QuestUIBigBang CreatePlaceholderQuestBigBang(
            GraphicsDevice device,
            int screenWidth,
            int screenHeight,
            IRuntimeDataServices runtimeServices)
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            {
                return CreatePlaceholderQuest(device, screenWidth, screenHeight, runtimeServices);
            }
            ...
            QuestUI quest = new QuestUI(frame, device, runtimeServices);
        ...
        private static QuestUI CreatePlaceholderQuest(
            GraphicsDevice device,
            int screenWidth,
            int screenHeight,
            IRuntimeDataServices runtimeServices)
            ...
            IRuntimeDataServices runtimeServices)
        {
            QuestDetailWindow window = isBigBang
                ? CreateQuestDetailWindowBigBang(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, screenWidth, screenHeight, runtimeServices)
                : CreateQuestDetailWindowPreBigBang(uiWindow1Image, basicImage, soundUIImage, device, screenWidth, screenHeight, runtimeServices);
            ...
            return CreatePlaceholderQuestDetailWindow(device, basicImage, soundUIImage, screenWidth, screenHeight, runtimeServices);
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            QuestDetailWindow window = CreateQuestDetailWindowShell(device, frameTexture, screenWidth, screenHeight, runtimeServices);
        ...
        private static QuestDetailWindow CreateQuestDetailWindowShell(
            ...
            IRuntimeDataServices runtimeServices)
        {
            return new QuestDetailWindow(new DXObject(0, 0, frameTexture, 0), MapSimulatorWindowNames.QuestDetail, runtimeServices)
            {
                Position = new Point(
            ...
            UILoaderResourceCache resourceCache)
        {
            ...
            bool isVUpdate = runtimeAssets.FindImage("UI", "StatusBar3.img") != null;
            if (isVUpdate)
            {
                WzImage modernQuickSlotImage = runtimeAssets.FindImage("UI", "UIQuickSlot.img");
            ...
            {
                WzImage statusBarImage = runtimeAssets.FindImage("UI", "StatusBar.img");
            ...
            if (frameTexture == null)
            {
                ...
                frameTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, fallbackWidth, fallbackHeight));
            ...
            QuickSlotUI quickSlot = new QuickSlotUI(frame, device, runtimeServices);
            ...
            quickSlot.SetTooltipTextures(UILoader.LoadSkillTooltipTextures(device, runtimeAssets, resourceCache));
            quickSlot.SetTooltipOrigins(UILoader.LoadSkillTooltipOrigins(runtimeAssets));
            ...
            int screenHeight)
        {
            ...
            Texture2D selectionTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            UserInfoUI window = new UserInfoUI(new DXObject(0, 0, frameTexture, 0), false, runtimeServices)
            {
                Position = new Point(
                    Math.Max(40, (screenWidth / 2) - (frameTexture.Width / 2)),
                    Math.Max(40, (screenHeight / 2) - (frameTexture.Height / 2)))
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            UserInfoUI window = new UserInfoUI(new DXObject(0, 0, frameTexture, 0), true, runtimeServices)
            {
                Position = new Point(
                    Math.Max(40, (screenWidth / 2) - (frameTexture.Width / 2)),
                    Math.Max(40, (screenHeight / 2) - (frameTexture.Height / 2)))
