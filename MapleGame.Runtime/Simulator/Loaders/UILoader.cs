...
namespace HaCreator.MapSimulator.Loaders
{
    ...
    public static class UILoader
    {
        ...
        private static string BuildMinimapCacheKey(GraphicsDevice device, RuntimeMinimapData minimapData, float userScreenScaleFactor, bool isBigBang)
        {
            int mapId = minimapData?.MapId ?? 0;
            bool zeroMirror = minimapData?.ZeroSideOnly ?? false;
            int miniMapWidth = minimapData?.MiniMap?.Width ?? 0;
            int miniMapHeight = minimapData?.MiniMap?.Height ?? 0;
        ...
        private static UILoaderResourceCache EnsureResourceCache(UILoaderResourceCache resourceCache) =>
            resourceCache ?? throw new ArgumentNullException(nameof(resourceCache),
                "UI resources must be owned by the active simulator/client session.");

        private static void ApplySharedMinimapWindowPosition(MinimapUI minimap, UILoaderResourceCache resourceCache)
        {
            if (minimap == null)
            {
                return;
            ...
            minimap.WindowPositionChanged = position => resourceCache.SharedMinimapWindowPosition = position;
            minimap.SetWindowPosition(resourceCache.SharedMinimapWindowPosition);
        }
        ...
        public static Tuple<StatusBarUI, StatusBarChatUI> CreateStatusBarFromProperty(
            WzImage uiStatusBar, WzImage uiStatusBar2, WzImage uiStatusBar3, WzImage uiBasic, WzImage uiBuffIcon, GraphicsDevice device,
        ...
        {
            resourceCache = EnsureResourceCache(resourceCache);
            using IDisposable scope = GraphicsResourceCreationScope.Begin(resourceCache.OwnGraphicsResource);
            return CreateStatusBarFromPropertyCore(
                uiStatusBar, uiStatusBar2, uiStatusBar3, uiBasic, uiBuffIcon, device,
                UserScreenScaleFactor, renderParams, soundUIImage, bBigBang, runtimeAssets, resourceCache);
        }

        private static Tuple<StatusBarUI, StatusBarChatUI> CreateStatusBarFromPropertyCore(
            WzImage uiStatusBar, WzImage uiStatusBar2, WzImage uiStatusBar3, WzImage uiBasic, WzImage uiBuffIcon, GraphicsDevice device,
            float UserScreenScaleFactor, RenderParameters renderParams, WzImage soundUIImage, bool bBigBang,
            IRuntimeAssetSource runtimeAssets,
            UILoaderResourceCache resourceCache)
        {
            ...
            if (resourceCache.StatusBars.TryGetValue(statusBarCacheKey, out Tuple<StatusBarUI, StatusBarChatUI> cachedStatusBar))
            {
            ...
            if (uiStatusBar3 != null)
            {
                Tuple<StatusBarUI, StatusBarChatUI> statusBar3 = CreateStatusBar3FromProperty(
                    uiStatusBar3, uiBasic, uiBuffIcon, device, renderParams, soundUIImage, runtimeAssets, resourceCache);
                if (statusBar3 != null)
                {
                    resourceCache.StatusBars[statusBarCacheKey] = statusBar3;
                    return statusBar3;
                }
            }
            ...
            if (bBigBang)
            {
                ...
                if (mainBarProperties != null)
                {
                    ...
                    statusBar.SetBuffIconTextures(LoadBuffIconTextures(uiBuffIcon, device, resourceCache));
                    }
                    statusBar.SetCooldownMasks(LoadStatusBarCooldownMasks(device, isBigBang: true, runtimeAssets: runtimeAssets, resourceCache: resourceCache));
                    statusBar.SetTemporaryStatViewTexture(LoadStatusBarTemporaryStatViewTexture(device, isBigBang: true, runtimeAssets: runtimeAssets, resourceCache: resourceCache));
                    statusBar.SetTemporaryStatViewShadowTextures(LoadStatusBarTemporaryStatViewShadowTextures(device, isBigBang: true, runtimeAssets: runtimeAssets, resourceCache: resourceCache));
                    statusBar.SetTooltipTextures(LoadSkillTooltipTextures(device, runtimeAssets, resourceCache));
                    statusBar.SetTooltipOrigins(LoadSkillTooltipOrigins(runtimeAssets));
                    statusBar.SetWarningAnimations(
                        LoadStatusBarWarningAnimation(mainBarProperties?["aniHPGauge"] as WzSubProperty, device, resourceCache),
                        LoadStatusBarWarningAnimation(mainBarProperties?["aniMPGauge"] as WzSubProperty, device, resourceCache));
                    statusBar.SetKeyDownBarTextures(LoadKeyDownBarTextures(uiBasic, device, resourceCache));
                    ...
                    WzImage uiWindow2DialogImage = runtimeAssets.FindImage("UI", "UIWindow2.img");
                    WzImage uiBasicImage = runtimeAssets.FindImage("UI", "Basic.img");
                    (Dictionary<MapSimulatorChatTargetType, Texture2D> chatTargetTextures,
                        ...
                        LoadChatTargetTextures(subProperty_chatTarget, device, resourceCache);
                    ...
                    chatUI.SetPointNotificationAnimations(
                        LoadPointNotificationAnimation(mainBarProperties?["ApNotify"] as WzSubProperty, device, resourceCache),
                        LoadPointNotificationAnimation(mainBarProperties?["SpNotify"] as WzSubProperty, device, resourceCache));
                    ...
                    resourceCache.StatusBars[statusBarCacheKey] = result;
                    return result;
                }
            }
            else
            {
                ...
                if (baseProperties != null)
                {
                    ...
                    statusBar.SetBuffIconTextures(LoadBuffIconTextures(uiBuffIcon, device, resourceCache));
                    statusBar.SetCooldownMasks(LoadStatusBarCooldownMasks(device, isBigBang: false, runtimeAssets: runtimeAssets, resourceCache: resourceCache));
                    statusBar.SetTemporaryStatViewTexture(LoadStatusBarTemporaryStatViewTexture(device, isBigBang: false, runtimeAssets: runtimeAssets, resourceCache: resourceCache));
                    statusBar.SetTemporaryStatViewShadowTextures(LoadStatusBarTemporaryStatViewShadowTextures(device, isBigBang: false, runtimeAssets: runtimeAssets, resourceCache: resourceCache));
                    statusBar.SetTooltipTextures(LoadSkillTooltipTextures(device, runtimeAssets, resourceCache));
                    statusBar.SetTooltipOrigins(LoadSkillTooltipOrigins(runtimeAssets));
                    statusBar.SetWarningAnimations(
                        LoadStatusBarWarningAnimation(gaugeProperties?["hpFlash"] as WzSubProperty, device, resourceCache),
                        LoadStatusBarWarningAnimation(gaugeProperties?["mpFlash"] as WzSubProperty, device, resourceCache));
                    statusBar.SetKeyDownBarTextures(LoadKeyDownBarTextures(uiBasic, device, resourceCache));
            ...
            UILoaderResourceCache resourceCache)
        {
            ...
            statusBar.SetTooltipOrigins(LoadSkillTooltipOrigins(runtimeAssets));
            statusBar.SetWarningAnimations(
                LoadStatusBarWarningAnimation(status["aniHPGauge"] as WzSubProperty, device, resourceCache),
                LoadStatusBarWarningAnimation(status["aniMPGauge"] as WzSubProperty, device, resourceCache));
            statusBar.SetKeyDownBarTextures(LoadKeyDownBarTextures(uiBasic, device, resourceCache));
        ...
        private static Dictionary<string, Texture2D> LoadBuffIconTextures(WzImage uiBuffIcon, GraphicsDevice device, UILoaderResourceCache resourceCache)
        {
            if (uiBuffIcon == null)
            {
            ...
            if (resourceCache.BuffIconTextures.TryGetValue(cacheKey, out Dictionary<string, Texture2D> cachedTextures))
            {
            ...
            resourceCache.BuffIconTextures[cacheKey] = buffIconTextures;
            return buffIconTextures;
        }

        public static IReadOnlyDictionary<string, BuffIconCatalogEntry> LoadBuffIconCatalogEntries(
            WzImage uiBuffIcon, UILoaderResourceCache resourceCache)
        {
            resourceCache = EnsureResourceCache(resourceCache);
            if (uiBuffIcon == null)
            {
            ...
            if (resourceCache.BuffIconCatalog.TryGetValue(cacheKey, out IReadOnlyDictionary<string, BuffIconCatalogEntry> cachedCatalogEntries))
            {
            ...
            resourceCache.BuffIconCatalog[cacheKey] = catalogEntries;
            return catalogEntries;
        }

        internal static Texture2D[] LoadSkillTooltipTextures(GraphicsDevice device, IRuntimeAssetSource runtimeAssets, UILoaderResourceCache resourceCache)
        {
            ArgumentNullException.ThrowIfNull(runtimeAssets);
            resourceCache = EnsureResourceCache(resourceCache);
            if (device == null)
            ...
            if (resourceCache.SkillTooltipTextures.TryGetValue(cacheKey, out Texture2D[] cachedFrames))
            {
                return cachedFrames;
            }

            Texture2D[] tooltipFrames = new Texture2D[3];
            WzImage uiWindow2Image = runtimeAssets.FindImage("UI", "UIWindow2.img");
            ...
            resourceCache.SkillTooltipTextures[cacheKey] = tooltipFrames;
            return tooltipFrames;
        }

        internal static Point[] LoadSkillTooltipOrigins(IRuntimeAssetSource runtimeAssets)
        {
            ArgumentNullException.ThrowIfNull(runtimeAssets);
        ...
        private static Texture2D[] LoadStatusBarCooldownMasks(GraphicsDevice device, bool isBigBang, IRuntimeAssetSource runtimeAssets, UILoaderResourceCache resourceCache)
        {
            if (device == null)
            {
            ...
            if (resourceCache.StatusBarCooldownMasks.TryGetValue(cacheKey, out Texture2D[] cachedMasks))
            {
                return cachedMasks;
            }

            WzImage uiWindowImage = runtimeAssets.FindImage("UI", isBigBang ? "UIWindow2.img" : "UIWindow.img");
            ...
            resourceCache.StatusBarCooldownMasks[cacheKey] = masks;
            return masks;
        }

        private static Texture2D LoadStatusBarTemporaryStatViewTexture(GraphicsDevice device, bool isBigBang, IRuntimeAssetSource runtimeAssets, UILoaderResourceCache resourceCache)
        {
            if (device == null)
            {
                return null;
            ...
            if (resourceCache.StatusBarTemporaryStatView.TryGetValue(cacheKey, out Texture2D cachedTexture))
            {
            ...
            resourceCache.StatusBarTemporaryStatView[cacheKey] = texture;
            return texture;
        }

        private static Dictionary<int, Texture2D> LoadStatusBarTemporaryStatViewShadowTextures(GraphicsDevice device, bool isBigBang, IRuntimeAssetSource runtimeAssets, UILoaderResourceCache resourceCache)
        {
            if (device == null)
            {
            ...
            if (resourceCache.StatusBarTemporaryStatViewShadows.TryGetValue(cacheKey, out Dictionary<int, Texture2D> cachedTextures))
            {
                return cachedTextures;
            }
            ...
            resourceCache.StatusBarTemporaryStatViewShadows[cacheKey] = textures;
            return textures;
        }
        ...
        private static Dictionary<string, StatusBarKeyDownBarTextures> LoadKeyDownBarTextures(WzImage uiBasic, GraphicsDevice device, UILoaderResourceCache resourceCache)
        {
            ...
            if (resourceCache.KeyDownBarTextures.TryGetValue(cacheKey, out Dictionary<string, StatusBarKeyDownBarTextures> cachedTextures))
            {
                return cachedTextures;
            }
            ...
            resourceCache.KeyDownBarTextures[cacheKey] = keyDownBarTextures;
        ...
        private static StatusBarWarningAnimation LoadStatusBarWarningAnimation(WzSubProperty warningProperty, GraphicsDevice device, UILoaderResourceCache resourceCache)
        {
            ...
            if (resourceCache.WarningAnimations.TryGetValue(cacheKey, out StatusBarWarningAnimation cachedAnimation))
            {
            ...
            resourceCache.WarningAnimations[cacheKey] = animation;
            return animation;
        }
        ...
        private static (Dictionary<MapSimulatorChatTargetType, Texture2D> Textures, Dictionary<MapSimulatorChatTargetType, Point> Origins)
            LoadChatTargetTextures(WzSubProperty chatTargetProperty, GraphicsDevice device, UILoaderResourceCache resourceCache)
        {
            ...
            if (resourceCache.ChatTargetTextures.TryGetValue(cacheKey, out var cachedTextures))
            {
                return cachedTextures;
            }
            ...
            resourceCache.ChatTargetTextures[cacheKey] = result;
            return result;
        }
            ...
            UILoaderResourceCache resourceCache)
        {
            ...
            if (resourceCache.PointNotificationAnimations.TryGetValue(cacheKey, out StatusBarChatUI.StatusBarPointNotificationAnimation cachedAnimation))
            {
                return cachedAnimation;
            }
            ...
            resourceCache.PointNotificationAnimations[cacheKey] = animation;
            return animation;
        }
        ...
        public static MinimapUI CreateMinimapFromProperty(
            WzImage uiWindow1Image, WzImage uiWindow2Image, WzImage uiBasicImage, RuntimeMinimapData minimapData,
            GraphicsDevice device, float UserScreenScaleFactor,
            WzImage soundUIImage, bool bBigBang, UILoaderResourceCache resourceCache)
        {
            return CreateMinimapFromProperty(uiWindow1Image, uiWindow2Image, null, uiBasicImage, minimapData,
                device, UserScreenScaleFactor, soundUIImage, bBigBang, resourceCache);
        }

        public static MinimapUI CreateMinimapFromProperty(
            WzImage uiWindow1Image, WzImage uiWindow2Image, WzImage uiMapImage, WzImage uiBasicImage, RuntimeMinimapData minimapData,
            GraphicsDevice device, float UserScreenScaleFactor,
            WzImage soundUIImage, bool bBigBang, UILoaderResourceCache resourceCache)
        {
            resourceCache = EnsureResourceCache(resourceCache);
            using IDisposable scope = GraphicsResourceCreationScope.Begin(resourceCache.OwnGraphicsResource);
            return CreateMinimapFromPropertyCore(
                uiWindow1Image, uiWindow2Image, uiMapImage, uiBasicImage, minimapData,
                device, UserScreenScaleFactor, soundUIImage, bBigBang, resourceCache);
        }

        private static MinimapUI CreateMinimapFromPropertyCore(
            WzImage uiWindow1Image, WzImage uiWindow2Image, WzImage uiMapImage, WzImage uiBasicImage, RuntimeMinimapData minimapData,
            GraphicsDevice device, float UserScreenScaleFactor,
            WzImage soundUIImage, bool bBigBang, UILoaderResourceCache resourceCache)
        {
            if (minimapData?.MiniMap == null)
                return null;

            string minimapCacheKey = BuildMinimapCacheKey(device, minimapData, UserScreenScaleFactor, bBigBang);
            if (resourceCache.Minimap.TryGetValue(minimapCacheKey, out MinimapUI cachedMinimap))
            {
                ApplySharedMinimapWindowPosition(cachedMinimap, resourceCache);
            ...
            if (minimapData.ZeroSideOnly || MapConstants.IsZerosTemple(minimapData.MapId)) // zero's temple
            {
            ...
            System.Drawing.Bitmap miniMapImage = minimapData.MiniMap; // the original minimap image without UI frame overlay
            ...
            // Create Map mark
            System.Drawing.Bitmap mapMark = minimapData.MapMarkImage;
            ...
            string renderText = string.Format("{0}{1}{2}", minimapData.StreetName, Environment.NewLine, minimapData.MapName);
            ...
            if (bBigBang)
            {
                ...
                if (listNpcProperty != null)
                {
                    ...
                    foreach (RuntimeMinimapNpcMarker npc in minimapData.NpcMarkers)
                    {
            ...
            ApplySharedMinimapWindowPosition(minimapItem, resourceCache);
            ...
            resourceCache.Minimap[minimapCacheKey] = minimapItem;
            return minimapItem;
        }
