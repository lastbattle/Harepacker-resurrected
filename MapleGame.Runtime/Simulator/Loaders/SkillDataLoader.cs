...
namespace HaCreator.MapSimulator.Loaders
{
    ...
    {
        internal sealed class SkillDisplayCache
        {
            public readonly object Gate = new();
            public readonly Dictionary<int, List<SkillDisplayData>> ByJob = new();
            public readonly HashSet<int> MissingBooks = new();
        }

        private static readonly System.Runtime.CompilerServices.ConditionalWeakTable<IRuntimeAssetSource, SourceRuntimeAssetCatalog> ItemCatalogs = new();
        ...
        /// <summary>
        /// Load all skills for a specific job using the session asset source.
        /// </summary>
        ...
        public static List<SkillDisplayData> LoadSkillsForJob(UILoaderResourceCache resourceCache,
            int jobId,
            ...
            IRuntimeAssetSource assets)
        {
            ...
            SkillDisplayCache cache = resourceCache.SkillDisplays.GetOrAdd((assets, device), _ => new SkillDisplayCache());
            lock (cache.Gate)
            {
                if (cache.MissingBooks.Contains(jobId))
                {
                    return new List<SkillDisplayData>();
                }

                if (cache.ByJob.TryGetValue(jobId, out List<SkillDisplayData> cachedSkills))
                {
            ...
            // Load skill image through the configured session asset source.
            WzImage skillImage = assets?.FindImage("Skill", skillImageName);
            if (skillImage == null)
            {
                Debug.WriteLine($"[SkillDataLoader] Failed to load Skill/{skillImageName}");
                lock (cache.Gate)
                {
                    cache.MissingBooks.Add(jobId);
                }
                return skills;
            }
            ...
            WzImage stringImage = assets?.FindImage("String", "Skill.img");
            ...
            foreach (WzImageProperty prop in skillProperty.WzProperties)
            {
                if (prop is WzSubProperty skillEntry)
                {
                    try
                    {
                        SkillDisplayData skillData = LoadSingleSkill(resourceCache, skillEntry, stringImage, device, assets);
                        if (skillData != null)
                        {
            ...
            lock (cache.Gate)
            {
                cache.ByJob[jobId] = new List<SkillDisplayData>(skills);
            }

            return new List<SkillDisplayData>(skills);
        }

        public static List<SkillDisplayData> LoadGuildSkills(UILoaderResourceCache resourceCache, GraphicsDevice device, IRuntimeAssetSource assets)
        {
            return LoadSkillsFromImageName(resourceCache, "9100.img", device, assets);
        }
        ...
        {
            WzImage recommendSkillImage = assets?.FindImage("Etc", "RecommendSkill.img");
        ...
        /// </summary>
        public static IReadOnlyList<int> GetAvailableSkillBookJobIds(
            WzFile skillWzFile,
            IRuntimeAssetSource assets)
        {
            ...
            if (assets?.FindObject("Skill", string.Empty) is WzDirectory rootDirectory)
            {
            ...
            if (result.Count == 0)
            {
                foreach (var directory in assets?.GetDirectories("Skill") ?? Array.Empty<WzDirectory>())
                {
        ...
        public static bool SkillRootContainsSkill(int skillRootId, int skillId, IRuntimeAssetSource assets)
        {
            if (skillRootId < 0 || skillId <= 0)
                return false;

            WzImage skillImage = assets?.FindImage("Skill", GetSkillImageName(skillRootId));
            if (skillImage == null)
                return false;
        ...
        public static List<SkillDisplayData> LoadSkillsForJob(UILoaderResourceCache resourceCache,
            ...
            IRuntimeAssetSource assets)
        {
            return LoadSkillsForJob(resourceCache, jobId, device, assets);
        }

        /// <summary>
        /// Load beginner skills specifically (job 0)
        /// </summary>
        public static List<SkillDisplayData> LoadBeginnerSkills(UILoaderResourceCache resourceCache,
            WzFile skillWzFile, WzFile stringWzFile, GraphicsDevice device,
            IRuntimeAssetSource assets)
        {
            return LoadSkillsForJob(resourceCache, 0, device, assets);
        }

        /// <summary>
        /// Load beginner skills (simplified overload)
        /// </summary>
        public static List<SkillDisplayData> LoadBeginnerSkills(UILoaderResourceCache resourceCache, GraphicsDevice device, IRuntimeAssetSource assets)
        {
            return LoadSkillsForJob(resourceCache, 0, device, assets);
        }
        ...
        /// </summary>
        public static List<SkillDisplayData> LoadSkillsFromImage(UILoaderResourceCache resourceCache,
            ...
            IRuntimeAssetSource assets)
        {
            ...
            foreach (WzImageProperty prop in skillProperty.WzProperties)
            {
                if (prop is WzSubProperty skillEntry)
                {
                    try
                    {
                        SkillDisplayData skillData = LoadSingleSkill(resourceCache,
                            skillEntry, stringImage, device, assets);
                        if (skillData != null)
                        {
                            skills.Add(skillData);
                        }
        ...
        private static List<SkillDisplayData> LoadSkillsFromImageName(UILoaderResourceCache resourceCache,
            string skillImageName, GraphicsDevice device, IRuntimeAssetSource assets)
        {
            List<SkillDisplayData> skills = new List<SkillDisplayData>();
            ...
            WzImage stringImage = assets?.FindImage("String", "Skill.img");
            return LoadSkillsFromImage(resourceCache, skillImage, stringImage, device, assets);
        }

        /// <summary>
        /// Load a single skill from WZ data
        /// </summary>
        private static SkillDisplayData LoadSingleSkill(UILoaderResourceCache resourceCache,
            ...
            IRuntimeAssetSource assets)
        {
            ...
            Texture2D iconTexture = resourceCache.OwnSkillTexture(iconBitmap?.ToTexture2DAndDispose(device));
            if (iconTexture == null)
                return null;
            ...
            if (disabledIconProp != null)
            {
                ...
                disabledIconTexture = resourceCache.OwnSkillTexture(disabledBitmap?.ToTexture2DAndDispose(device));
            }
            ...
            if (mouseOverIconProp != null)
            {
                ...
                mouseOverIconTexture = resourceCache.OwnSkillTexture(mouseOverBitmap?.ToTexture2DAndDispose(device));
            }
            ...
            if (stringImage != null)
            {
                ...
                if (stringEntry != null)
                {
                    ...
                    levelDescriptions = BuildLevelDescriptions(assets, skillEntry, stringEntry, Math.Max(1, maxLevel), preserveFormatting: false);
                    formattedLevelDescriptions = BuildLevelDescriptions(assets, skillEntry, stringEntry, Math.Max(1, maxLevel), preserveFormatting: true);
                }
            }

            int requiredCharacterLevel = ResolveRequiredCharacterLevel(skillEntry);
            List<SkillRequirementDisplayData> requiredSkills = BuildRequiredSkills(resourceCache, skillEntry, stringImage, device, assets);
        ...
        private static List<SkillRequirementDisplayData> BuildRequiredSkills(UILoaderResourceCache resourceCache,
            WzSubProperty skillEntry,
            WzImage stringImage,
            GraphicsDevice device,
            IRuntimeAssetSource assets)
        {
            ...
            foreach (WzImageProperty requirementEntry in requirementNode.WzProperties)
            {
                ...
                requirements.Add(new SkillRequirementDisplayData
                {
                    ...
                    IconTexture = LoadSkillIcon(resourceCache, skillId, device, assets, out Point iconOrigin),
        ...
        private static Texture2D LoadSkillIcon(UILoaderResourceCache resourceCache,
            int skillId,
            GraphicsDevice device,
            IRuntimeAssetSource assets,
            out Point iconOrigin)
        {
            ...
            System.Drawing.Bitmap iconBitmap = iconProp.GetLinkedWzCanvasBitmap();
            return resourceCache.OwnSkillTexture(iconBitmap?.ToTexture2DAndDispose(device));
        }
        ...
        private static Dictionary<int, string> BuildLevelDescriptions(IRuntimeAssetSource assets,
            WzSubProperty skillEntry,
            ...
            bool preserveFormatting)
        {
            ...
            for (int level = 1; level <= maxLevel; level++)
            {
                ...
                string fallbackStats = BuildFallbackLevelDescription(assets, skillEntry, stringEntry, level);
        ...
        private static string BuildFallbackLevelDescription(IRuntimeAssetSource assets,
            WzSubProperty skillEntry,
            WzSubProperty stringEntry,
            int level)
        {
            ...
            AppendFallbackStatsFromNode(assets, builder, skillEntry, stringEntry, levelNode, commonNode, infoNode, level, levelNode, appendedKeys);
            AppendFallbackStatsFromNode(assets, builder, skillEntry, stringEntry, levelNode, commonNode, infoNode, level, commonNode, appendedKeys);
            AppendFallbackStatsFromNode(assets, builder, skillEntry, stringEntry, levelNode, commonNode, infoNode, level, infoNode, appendedKeys);
            AppendFallbackStatsInDefaultOrder(assets, builder, skillEntry, stringEntry, levelNode, commonNode, infoNode, level, appendedKeys);

            return builder.ToString().Trim();
        }

        internal static string BuildFallbackLevelDescriptionForTests(IRuntimeAssetSource assets,
            WzSubProperty skillEntry,
            WzSubProperty stringEntry,
            int level)
        {
            return BuildFallbackLevelDescription(assets, skillEntry, stringEntry, level);
        }

        private static void AppendFallbackStatsFromNode(IRuntimeAssetSource assets,
            ...
            ISet<string> appendedKeys)
        {
            ...
            foreach (WzImageProperty property in sourceNode.WzProperties)
            {
                ...
                if (!TryAppendFallbackStat(assets, builder, skillEntry, stringEntry, levelNode, commonNode, infoNode, level, statKey, useGenericFallback))
                    continue;
        ...
        private static void AppendFallbackStatsInDefaultOrder(IRuntimeAssetSource assets,
            StringBuilder builder,
            WzSubProperty skillEntry,
            WzSubProperty stringEntry,
            WzImageProperty levelNode,
            ...
            ISet<string> appendedKeys)
        {
            foreach (string statKey in FallbackSkillStatOrder)
            {
                ...
                if (!TryAppendFallbackStat(assets, builder, skillEntry, stringEntry, levelNode, commonNode, infoNode, level, statKey))
                    continue;
        ...
        private static bool TryAppendFallbackStat(IRuntimeAssetSource assets,
            StringBuilder builder,
            WzSubProperty skillEntry,
            WzSubProperty stringEntry,
            WzImageProperty levelNode,
            ...
            bool allowGenericFallback = true)
        {
            if (string.Equals(statKey, "itemCon", StringComparison.OrdinalIgnoreCase))
            {
                ...
                AppendItemConsumptionStat(assets, builder, skillEntry, levelNode, commonNode, infoNode, level, "itemCon", "itemConNo", "Consumes");
            ...
            {
                int originalLength = builder.Length;
                AppendItemConsumptionStat(assets, builder, skillEntry, levelNode, commonNode, infoNode, level, "itemConsume", null, "Consumes");
                return builder.Length != originalLength;
            }
        ...
        private static void AppendItemConsumptionStat(IRuntimeAssetSource assets,
            StringBuilder builder,
            WzSubProperty skillEntry,
            WzImageProperty levelNode,
            WzImageProperty commonNode,
            ...
            string label)
        {
            ...
            string value = FormatFallbackItemConsumptionValue(assets, itemId, quantity);
        ...
        internal static string FormatFallbackItemConsumptionValue(IRuntimeAssetSource assets, int itemId, int quantity, string resolvedItemName = null)
        {
            if (itemId <= 0)
                return string.Empty;
            ...
            string itemName = resolvedItemName != null
                ...
                : TryResolveFallbackItemName(assets, itemId);
        ...
        private static string TryResolveFallbackItemName(IRuntimeAssetSource assets, int itemId)
        {
            return InventoryItemMetadataResolver.TryResolveItemName(itemId, ItemCatalogs.GetValue(assets, source => new SourceRuntimeAssetCatalog(source)), out string resolvedName)
        ...
        public static Texture2D LoadJobIcon(UILoaderResourceCache resourceCache, int jobId, GraphicsDevice device, IRuntimeAssetSource assets)
        {
            string skillImageName = GetSkillImageName(jobId);
            if (string.IsNullOrEmpty(skillImageName))
                return null;
