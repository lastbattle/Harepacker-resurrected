...
namespace HaCreator.MapSimulator.Loaders
{
    ...
    /// </summary>
    public sealed class DamageNumberLoader
    {
        internal const string DamageNumberSpecialTextOwnerSetName = "NoRed0";

        // Cached digit sets
        private readonly Dictionary<string, DamageNumberDigitSet> _digitSets = new Dictionary<string, DamageNumberDigitSet>();
        ...
        public bool LoadDamageNumbers(GraphicsDevice device, WzImage basicEffImage)
        {
            ...
            foreach (string typeName in DamageTypeNames)
            {
                ...
                _digitSets[typeName] = digitSet; // Retain partially loaded sets for deterministic cleanup.
        ...
        public DamageNumberDigitSet GetDigitSet(DamageColorType colorType, DamageNumberSize size, bool isCritical)
        {
        ...
        /// <returns>The digit set, or null if not loaded</returns>
        public DamageNumberDigitSet GetDigitSetByName(string typeName)
        {
        ...
        /// </summary>
        public int LoadedSetCount => _digitSets.Values.Count(set => set.IsLoaded);
