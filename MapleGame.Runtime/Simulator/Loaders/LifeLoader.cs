...
namespace HaCreator.MapSimulator.Loaders
{
    ...
    public static class LifeLoader
    {
        ...
        private static readonly ConditionalWeakTable<TexturePool, ConditionalWeakTable<MobImgEntry, Lazy<CachedMobActionAssets>>> _cachedMobActionAssetsByDevice = new();
        private static readonly ConditionalWeakTable<TexturePool, ConditionalWeakTable<MobImgEntry, Lazy<CachedMobAttackAssets>>> _cachedMobAttackAssetsByDevice = new();
        private static readonly ConditionalWeakTable<IRuntimeAssetSource, ConcurrentDictionary<string, Lazy<MobImgEntry>>> _cachedMobImgEntries = new();
        private static readonly ConditionalWeakTable<IRuntimeAssetSource, ConcurrentDictionary<string, byte>> _missingMobSoundIds = new();
        ...
        private static readonly ConditionalWeakTable<TexturePool, ConditionalWeakTable<IRuntimeAssetSource, Lazy<CachedDoomMobAssets>>> _cachedDoomMobAssetsByDevice = new();
        ...
        public static MobItem CreateMobFromProperty(
            TexturePool texturePool, RuntimeLife runtimeLife, IRuntimeAssetSource assetSource, float UserScreenScaleFactor,
            GraphicsDevice device, SoundManager soundManager, ConcurrentBag<WzObject> usedProps)
        {
            ArgumentNullException.ThrowIfNull(runtimeLife);
            ArgumentNullException.ThrowIfNull(assetSource);
            string mobTemplateId = NormalizeMobTemplateId(runtimeLife.Id);
            MobImgEntry source = BuildMobImgEntry(
                mobTemplateId,
                new HashSet<string>(StringComparer.Ordinal),
                assetSource,
                ResolveRuntimeLifeImage(runtimeLife, assetSource, "Mob"));
            runtimeLife.SetMobDataIfMissing(MobData.Parse(source?.Image, runtimeLife.Id != null && int.TryParse(runtimeLife.Id, out int mobId) ? mobId : 0));

            // Create animation set to store frames per action
            MobAnimationSet animationSet = new MobAnimationSet();
            CachedDoomMobAssets doomAssets = GetOrBuildDoomMobAssets(texturePool, assetSource, device, usedProps);
            CachedMobActionAssets cachedActionAssets = GetOrBuildCachedMobActionAssets(texturePool, mobTemplateId, source, device, usedProps);
            CachedMobAttackAssets cachedAttackAssets = GetOrBuildCachedMobAttackAssets(texturePool, mobTemplateId, source, device, usedProps);
            ApplyCachedMobActionAssets(animationSet, cachedActionAssets, texturePool, runtimeLife.X, runtimeLife.Y, device);
            ...
            bool hasBossHpBar = runtimeLife.MobData?.HpTagColor > 0;
            if (!hasBossHpBar)
            {
                nameTooltip = MapSimulatorLoader.CreateNPCMobNameTooltip(
                    runtimeLife.DisplayName, runtimeLife.X, runtimeLife.Y, color_foreGround,
            ...
            LoadMobSounds(mobItem, runtimeLife.Id, soundManager, assetSource);
            ...
            IRuntimeAssetSource assetSource)
        {
            ...
            MobImgEntry source = GetMobImgEntry(normalizedTemplateId, assetSource);
            if (source == null)
            {
                return null;
            }
            ...
            IRuntimeAssetSource assetSource)
        {
            ...
            WzImage mobSoundImage = assetSource?.FindImage("Sound", "Mob");
        ...
        {
            if (string.IsNullOrWhiteSpace(mobTemplateId) || source == null || device == null)
            {
                return null;
            }

            // A texture pool identifies the upload lifetime; the source entry identifies
            // the data generation. Neither may reuse another session's cached frames.
            Lazy<CachedMobActionAssets> lazyAssets = _cachedMobActionAssetsByDevice
                .GetOrCreateValue(texturePool).GetValue(source,
            ...
            ConcurrentBag<WzObject> usedProps)
        {
            ...
            // A texture pool identifies the upload lifetime; the source entry identifies
            // the data generation. Neither may reuse another session's cached frames.
            Lazy<CachedMobAttackAssets> lazyAssets = _cachedMobAttackAssetsByDevice
                .GetOrCreateValue(texturePool).GetValue(source,
                    _ => new Lazy<CachedMobAttackAssets>(
                        () => BuildCachedMobAttackAssets(texturePool, mobTemplateId, source, device, usedProps),
                        System.Threading.LazyThreadSafetyMode.ExecutionAndPublication));

            return lazyAssets.Value;
        }

        private static MobImgEntry GetMobImgEntry(string mobTemplateId, IRuntimeAssetSource assetSource)
        {
            ...
            string templateId = NormalizeMobTemplateId(mobTemplateId);
            Lazy<MobImgEntry> lazyEntry = _cachedMobImgEntries.GetValue(assetSource, _ => new(StringComparer.Ordinal)).GetOrAdd(
                templateId,
                _ => new Lazy<MobImgEntry>(
                    () => BuildMobImgEntry(templateId, new HashSet<string>(StringComparer.Ordinal), assetSource),
                    System.Threading.LazyThreadSafetyMode.ExecutionAndPublication));

            return lazyEntry.Value;
        }

        private static MobImgEntry BuildMobImgEntry(
            string templateId,
            HashSet<string> activeTemplateIds,
            IRuntimeAssetSource assetSource = null,
            WzImage initialImage = null)
        {
            ...
            WzImage mobImage = initialImage ?? assetSource?.FindImage("Mob", templateId + ".img");
            if (mobImage == null)
            {
            ...
            {
                MobImgEntry linkedEntry = BuildMobImgEntry(linkedTemplateId, activeTemplateIds, assetSource);
                if (linkedEntry != null)
                {
            ...
            ConcurrentBag<WzObject> usedProps)
        {
            ...
            Lazy<CachedDoomMobAssets> lazyAssets = _cachedDoomMobAssetsByDevice.GetOrCreateValue(texturePool).GetValue(
                assetSource,
                _ => new Lazy<CachedDoomMobAssets>(
                    () => BuildDoomMobAssets(texturePool, assetSource, device, usedProps),
                    System.Threading.LazyThreadSafetyMode.ExecutionAndPublication));
        ...
        private static CachedDoomMobAssets BuildDoomMobAssets(
            ...
            ConcurrentBag<WzObject> usedProps)
        {
            WzImage doomImage = assetSource?.FindImage("Mob", "0100101.img");
            if (doomImage == null)
            {
                return null;
            }
            ...
            foreach (WzImageProperty childProperty in source.WzProperties)
            {
                ...
                foreach ((WzImageProperty hitNode, int sourceFrameIndex, bool isAttackFrameOwned) in hitNodes)
                {
                    ...
                    if (hitFrames.Count > 0)
                    {
                        ...
                        AddCachedAttackHitEffect(
                            ...
                            sourceFrameIndex,
                            isAttackFrameOwned,
                            sourceUol: ResolveMobAttackHitSourceUol(mobTemplateId, hitNode));
                        System.Diagnostics.Debug.WriteLine($"[LifeLoader] Loaded {hitFrames.Count} hit effect frames for mob {mobTemplateId} {actionName}");
                    }
                }
                ...
                if (ballNode != null)
                {
                    ...
                    if (ballFrames.Count > 0)
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine($"[LifeLoader] Loaded {ballFrames.Count} projectile frames for mob {mobTemplateId} {actionName}");
                    }
                }
                ...
                foreach (WzImageProperty attackEffectProperty in attackEffectProperties)
                {
                    ...
                    {
                        structuredEffectNode.SourceUol = BuildMobAttackSourceUol(mobTemplateId, attackEffectProperty);
                    ...
                    {
                        plainEffectNode.SourceUol = BuildMobAttackSourceUol(mobTemplateId, attackEffectProperty);
                        ...
                        if (isPrimaryEffect)
                        {
                            ...
                            cached.AttackEffectSourceUols[actionName] = BuildMobAttackSourceUol(mobTemplateId, attackEffectProperty);
                        }
        ...
        /// </summary>
        private static void LoadMobSounds(
            MobItem mobItem,
            string mobId,
            SoundManager soundManager,
            IRuntimeAssetSource assetSource)
        {
            ...
            if (mobSounds == null)
            {
                if (_missingMobSoundIds.GetValue(assetSource, _ => new(StringComparer.Ordinal)).TryAdd(mobId, 0))
                {
            ...
            GraphicsDevice device)
        {
            ...
            for (int i = 0; i < frameCanvases.Count; i++)
            {
                ...
                Texture2D texture = texturePool.LoadCanvasTexture(canvasProperty, device);
                if (texture == null)
                {
                    continue;
                }
            ...
            int requestedClientActionSetIndex = NpcClientActionSetLoader.AutomaticClientActionSetIndex)
        {
            ArgumentNullException.ThrowIfNull(runtimeLife);
            ArgumentNullException.ThrowIfNull(assetSource);
            WzImage source = ResolveRuntimeLifeImage(runtimeLife, assetSource, "Npc");
            source = ResolveLinkedNpcImage(source, assetSource);
            if (source == null)
            {
                // Optional employee NPCs may be absent from the selected data version.
                // Skip the actor before action/fallback loading touches the missing image.
                return null;
            }
            ...
            WzImageProperty editorStandProperty = source?[AnimationKeys.Stand];
            ...
            if (includeTooltips)
            {
                ...
                npcDescTooltip = MapSimulatorLoader.CreateNPCMobNameTooltip(
                    string.Empty, runtimeLife.X, runtimeLife.Y + NPC_FUNC_Y_POS, color_foreGround,
                    texturePool, UserScreenScaleFactor, device);
            }
        ...
        private static WzImage ResolveRuntimeLifeImage(RuntimeLife runtimeLife, IRuntimeAssetSource assetSource, string fallbackCategory)
        {
            string category = string.IsNullOrWhiteSpace(runtimeLife.Asset.Category)
                ? fallbackCategory
                : runtimeLife.Asset.Category;
            string path = runtimeLife.Asset.Path;
            if (string.IsNullOrWhiteSpace(path))
            {
                path = WzInfoTools.AddLeadingZeros(runtimeLife.Id ?? string.Empty, 7) + ".img";
            }
            return assetSource.FindImage(category, path);
        }

        private static WzImage ResolveLinkedNpcImage(WzImage source, IRuntimeAssetSource assetSource)
        {
            string linkedId = (WzInfoTools.GetRealProperty(source?["info"]?["link"]) as WzStringProperty)?.Value;
            if (string.IsNullOrWhiteSpace(linkedId))
            {
                return source;
            }
            string imageName = WzInfoTools.AddLeadingZeros(linkedId, 7) + ".img";
            return assetSource.FindImage("Npc", imageName) ?? source;
        }
