...
namespace HaCreator.MapSimulator.Loaders
{
    public static partial class UIWindowLoader
    {
        ...
        private static Texture2D CreatePixelTexture(GraphicsDevice device)
        {
            ...
            Texture2D texture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));
            texture.SetData(new[] { Color.White });
            return texture;
        }

        private static WzSubProperty ResolveUiSubPropertyFromStringPoolPath(IRuntimeAssetSource runtimeAssets, int stringPoolId, params string[] fallbackPathSegments)
        {
            string path = MapleStoryStringPool.GetOrNull(stringPoolId);
            WzSubProperty resolvedProperty = ResolveUiSubPropertyFromStringPath(runtimeAssets, path);
            return resolvedProperty ?? ResolveUiSubPropertyFromSegments(runtimeAssets, fallbackPathSegments);
        }

        private static WzSubProperty ResolveUiSubPropertyFromStringPath(IRuntimeAssetSource runtimeAssets, string path)
        {
            ...
            string[] segments = path
                ...
                .ToArray();
            return ResolveUiSubPropertyFromSegments(runtimeAssets, segments);
        }

        private static WzSubProperty ResolveUiSubPropertyFromSegments(IRuntimeAssetSource runtimeAssets, params string[] segments)
        {
            ...
            WzImage image = runtimeAssets.FindImage("ui", segments[0]);
            if (image == null)
            {
                return null;
            }
        ...
        {
            using IDisposable resourceCreationScope = manager?.BeginResourceCreationScope();
            ...
            WzImage basicEffectImage = runtimeAssets.FindImage("Effect", "BasicEff.img");
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            WzImage uiWindowImage = runtimeAssets.FindImage("ui", "UIWindow.img");
        ...
        {
            Texture2D texture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, width, height));
            ...
            int height = 132)
        {
            ...
            using Texture2D topTexture = LoadCanvasTexture(utilDialogProperty, "t", device)
                ?? LoadTextureFromUiPath(PacketOwnedRewardResultRuntime.GetUtilDlgNoticeTopResourcePath(), device, uiWindow2Image, uiWindowImage);
            using Texture2D centerTexture = LoadCanvasTexture(utilDialogProperty, "c", device)
            ...
            using Texture2D bottomTexture = LoadCanvasTexture(utilDialogProperty, "s", device)
            ...
            GraphicsDevice device)
        {
            ...
            Texture2D frameTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, frameWidth, frameHeight));
            ...
            GraphicsDevice device)
        {
            ...
            Texture2D croppedTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, width, height));
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            WzImage loginImage = runtimeAssets.FindImage("UI", "Login.img");
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            WzSubProperty uiWindowFadeYesNoProperty = uiWindowImage?["FadeYesNo"] as WzSubProperty
                                                     ?? fadeYesNoProperty;
            WzImage basicImage = runtimeAssets.FindImage("UI", "Basic.img");
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            RegisterLoginTitleWindow(manager, loginImage, soundUIImage, device, screenWidth, screenHeight, runtimeAssets);
            ...
            RegisterLoginCharacterSelectWindow(manager, basicImage, soundUIImage, device, screenWidth, screenHeight, runtimeAssets);
            RegisterConnectionNoticeWindow(manager, loginImage, basicImage, soundUIImage, device, screenWidth, screenHeight);
            RegisterLoginUtilityDialogWindow(manager, uiWindow2Image, loginImage, basicImage, soundUIImage, device, screenWidth, screenHeight, runtimeAssets);
        }
        ...
        public static UIWindowManager CreateUIWindowManager(
            ...
            IRuntimeDataServices runtimeServices,
            UILoaderResourceCache resourceCache,
            string storageAccountLabel = null,
            string storageAccountKey = null,
            ISimulatorProfileStorage profileStorage = null)
        {

            return CreateUIWindowManager(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage,

                null, null, null, device, screenWidth, screenHeight, isBigBang, runtimeServices, resourceCache, 900,
                storageAccountLabel, storageAccountKey, profileStorage); // Default to GM book (900 in data)
        }
            ...
            ISimulatorProfileStorage profileStorage = null)
        {
            ...
            UIWindowBase inventory = CreateInventoryWindowUnified(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, screenWidth, screenHeight, isBigBang, runtimeServices);
            UIWindowBase equip = CreateEquipWindowUnified(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, screenWidth, screenHeight, isBigBang, runtimeServices);
            UIWindowBase skill = CreateSkillWindowUnified(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, screenWidth, screenHeight, isBigBang, runtimeServices.Assets);
            UIWindowBase quest = CreateQuestWindowUnified(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, screenWidth, screenHeight, isBigBang, runtimeServices);
            QuestDetailWindow questDetail = CreateQuestDetailWindowUnified(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, screenWidth, screenHeight, isBigBang, runtimeServices);
            ...
            UserInfoUI characterInfo = CreateCharacterInfoWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, screenWidth, screenHeight, isBigBang, runtimeServices);
            QuickSlotUI quickSlot = CreateQuickSlotWindow(uiWindow1Image, uiWindow2Image, device, screenWidth, screenHeight, isBigBang, runtimeServices, resourceCache);
            ...
            if (skill is SkillUIBigBang skillBigBang)
            {
                ...
                LoadSkillsForJob(resourceCache, skillBigBang, jobId, device, runtimeServices.Assets);
            }
            else if (skill is SkillUI legacySkill)
            {
                ...
                LoadSkillsForJob(resourceCache, legacySkill, jobId, device, runtimeServices.Assets);
            }
            ...
            SimulatorStorageRuntime storageRuntime = new SimulatorStorageRuntime(
                initialAccountLabel: storageAccountLabel,
                initialAccountKey: storageAccountKey,
                profileStorage: profileStorage,
                runtimeCatalog: runtimeServices.Catalog);
            ...
            manager.RegisterLazyWindow(
                ...
                m =>
                {
                TrunkUI trunk = CreateTrunkWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, screenWidth, screenHeight, m.InventoryWindow as InventoryUI, storageRuntime, runtimeServices);
                    if (trunk != null)
                    {
            ...
            RegisterProgressionUtilityPlaceholderWindows(manager, uiWindow1Image, uiWindow2Image, mapleTvImage, basicImage, soundUIImage, device, screenWidth, screenHeight, storageRuntime, runtimeServices, resourceCache);

            RegisterSocialRoomWindows(manager, uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, screenWidth, screenHeight, runtimeServices);
            ...
            UILoaderResourceCache resourceCache)
        {
            ...
            IRuntimeAssetSource runtimeAssets = runtimeServices.Assets;
            ...
            manager.RegisterLazyWindow(MapSimulatorWindowNames.AdminShopWishList, m => RegisterAdminShopWishListWindow(m, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 3), y + cascade), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.CashAvatarPreview, m => RegisterCashAvatarPreviewWindow(m, basicImage, soundUIImage, device, new Point(x + (cascade * 8), y + cascade), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.CashShop, m => RegisterAdminShopWindow(m, uiWindow2Image, basicImage, soundUIImage, device, MapSimulatorWindowNames.CashShop, AdminShopServiceMode.CashShop, new Point(x + cascade, y + cascade), storageRuntime, runtimeServices));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.CashShopStage, m => RegisterCashServiceStageWindow(m, device, MapSimulatorWindowNames.CashShopStage, CashServiceWindowStageKind.CashShop, new Point(x + cascade, y + cascade), runtimeServices));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.Mts, m => RegisterAdminShopWindow(m, uiWindow2Image, basicImage, soundUIImage, device, MapSimulatorWindowNames.Mts, AdminShopServiceMode.Mts, new Point(x + (cascade * 2), y + (cascade * 2)), storageRuntime, runtimeServices));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.MtsStatus, m => RegisterCashServiceStageWindow(m, device, MapSimulatorWindowNames.MtsStatus, CashServiceWindowStageKind.ItemTradingCenter, new Point(x + (cascade * 2), y + (cascade * 2)), runtimeServices));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.SocialList, m => RegisterSocialListWindow(m, uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 2), y + (cascade * 5)), runtimeAssets, resourceCache));
            ...
            manager.RegisterLazyWindow(MapSimulatorWindowNames.ItemMaker, m => RegisterItemMakerWindow(m, uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 5), y + (cascade * 5)), runtimeServices));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.BookCollection, m => RegisterBookCollectionWindow(m, uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 6), y + (cascade * 5)), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.ItemUpgrade, m => RegisterItemUpgradeWindow(m, uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 6), y + (cascade * 6)), runtimeServices));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.VegaSpell, m => RegisterVegaSpellWindow(m, uiWindow1Image, basicImage, soundUIImage, device, new Point(x + (cascade * 7), y + (cascade * 6)), runtimeServices));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.MemoMailbox, m => RegisterMemoMailboxWindow(m, uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 7), y + (cascade * 4)), runtimeAssets));
            ...
            manager.RegisterLazyWindow(MapSimulatorWindowNames.QuestDelivery, m => RegisterQuestDeliveryWindow(m, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 6), y + (cascade * 3)), runtimeServices));
            ...
            manager.RegisterLazyWindow(MapSimulatorWindowNames.RepairDurability, m => RegisterRepairDurabilityWindow(m, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 6), y + (cascade * 4)), runtimeServices, resourceCache));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.QuestAlarm, m => RegisterQuestAlarmWindow(m, uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 8), y + (cascade * 8)), runtimeAssets));
            ...
            manager.RegisterLazyWindow(MapSimulatorWindowNames.NpcShop, m => RegisterPacketOwnedNpcShopWindow(m, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 2), y + (cascade * 6)), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.StoreBank, m => RegisterPacketOwnedStoreBankWindow(m, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 3), y + (cascade * 6)), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.BattleRecord, m => RegisterPacketOwnedBattleRecordWindow(m, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 4), y + (cascade * 6)), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.ShopScanner, m => RegisterShopScannerWindow(m, uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 5), y + (cascade * 6)), runtimeServices));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.KeyConfig, m => RegisterKeyConfigWindow(m, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 4), y + (cascade * 4)), runtimeAssets));
            ...
            manager.RegisterLazyWindow(MapSimulatorWindowNames.Ranking, m => RegisterRankingWindow(m, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 6), y + (cascade * 2)), runtimeAssets));
            ...
            manager.RegisterLazyWindow(MapSimulatorWindowNames.MedalQuestInfo, m => RegisterPacketOwnedRawFunctionOwnerWindow(m, uiWindow2Image, basicImage, soundUIImage, device, MapSimulator.PacketOwnedRawFunctionOwner.Medal, "Medal", "Packet-owned medal key owner routed from the raw KeyConfig palette id 26.", new Point(x + (cascade * 8), y + (cascade * 2)), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.ItemPot, m => RegisterPacketOwnedRawFunctionOwnerWindow(m, uiWindow2Image, basicImage, soundUIImage, device, MapSimulator.PacketOwnedRawFunctionOwner.ItemPot, "Item Pot", "Packet-owned item-pot key owner routed from the raw KeyConfig palette id 30.", new Point(x + (cascade * 8), y + (cascade * 3)), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.MagicWheel, m => RegisterPacketOwnedRawFunctionOwnerWindow(m, uiWindow2Image, basicImage, soundUIImage, device, MapSimulator.PacketOwnedRawFunctionOwner.MagicWheel, "Magic Wheel", "Packet-owned magic-wheel key owner routed from the raw KeyConfig palette id 32.", new Point(x + (cascade * 8), y + (cascade * 4)), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.Radio, m => RegisterRadioWindow(m, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 6), y + (cascade * 4)), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.DragonBox, m => RegisterDragonBoxWindow(m, basicImage, soundUIImage, device, new Point(x + (cascade * 8), y + (cascade * 3)), runtimeAssets));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.AccountMoreInfo, m => RegisterAccountMoreInfoWindow(m, uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 7), y + (cascade * 4)), runtimeAssets));
        }
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            manager.RegisterLazyWindow(MapSimulatorWindowNames.MiniRoom, m => RegisterSocialRoomWindow(m, CreateMiniRoomWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x, y), runtimeServices)));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.PersonalShop, m => RegisterSocialRoomWindow(m, CreatePersonalShopWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + cascade, y + cascade), runtimeServices)));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.EntrustedShop, m => RegisterSocialRoomWindow(m, CreateEntrustedShopWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 2), y + (cascade * 2)), runtimeServices)));
            manager.RegisterLazyWindow(MapSimulatorWindowNames.TradingRoom, m => RegisterSocialRoomWindow(m, CreateTradingRoomWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(x + (cascade * 3), y + (cascade * 3)), runtimeServices)));
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            UIWindowBase window = CreateRankingWindow(uiWindow2Image, basicImage, soundUIImage, device, position, runtimeAssets);
            if (window != null)
            {
                manager.RegisterCustomWindow(window);
            }
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            UIWindowBase window = CreateRadioWindow(uiWindow2Image, basicImage, soundUIImage, device, position, runtimeAssets);
            if (window != null)
            {
                manager.RegisterCustomWindow(window);
            }
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            {
                UIWindowBase categoryBaseWindow = CreateAdminShopWishListCategoryWindow(uiWindow2Image, basicImage, soundUIImage, device, position, runtimeAssets);
            ...
            {
                UIWindowBase searchResultBaseWindow = CreateAdminShopWishListSearchResultWindow(uiWindow2Image, basicImage, soundUIImage, device, position, runtimeAssets);
            ...
            UIWindowBase window = CreateAdminShopWishListWindow(uiWindow2Image, basicImage, soundUIImage, device, position, runtimeAssets);
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            UIWindowBase window = CreateCashServiceStageWindow(device, windowName, stageKind, position, runtimeServices);
            if (window != null)
            {
                manager.RegisterCustomWindow(window);
            }
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            UIWindowBase window = CreateDragonBoxWindow(basicImage, soundUIImage, device, position, runtimeAssets);
            if (window != null)
            {
                manager.RegisterCustomWindow(window);
            }
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            UIWindowBase window = CreateAccountMoreInfoWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, position, runtimeAssets);
            if (window != null)
            {
                manager.RegisterCustomWindow(window);
            }
        ...
        private static UIWindowBase CreateCashServiceStageWindow(
            ...
            IRuntimeDataServices runtimeServices)
        {
            var runtimeAssets = runtimeServices.Assets;
            string serviceImageName = stageKind == CashServiceWindowStageKind.CashShop
                ? "CashShop.img"
                : "ITC.img";
            WzImage serviceImage = runtimeAssets.FindImage("ui", serviceImageName);
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            UIWindowBase window = CreateCashAvatarPreviewWindow(basicImage, soundUIImage, device, position, runtimeAssets);
            if (window != null)
            {
                manager.RegisterCustomWindow(window);
            }
        ...
        {
            using IDisposable resourceCreationScope = manager?.BeginResourceCreationScope();
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.CashShopCharacter, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.CashShopLocker, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.CashShopInventory, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.CashShopTab, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.CashShopList, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.CashShopBest, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.CashShopStatus, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.CashShopItemSearch, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.CashShopOneADay, runtimeAssets);
        }
        ...
        {
            using IDisposable resourceCreationScope = manager?.BeginResourceCreationScope();
            RegisterCashServiceModalOwnerWindow(manager, uiWindow1Image, uiWindow2Image, device, screenWidth, screenHeight, MapSimulatorWindowNames.CashCouponDialog, runtimeAssets);
            RegisterCashServiceModalOwnerWindow(manager, uiWindow1Image, uiWindow2Image, device, screenWidth, screenHeight, MapSimulatorWindowNames.CashPurchaseConfirmDialog, runtimeAssets);
            RegisterCashServiceModalOwnerWindow(manager, uiWindow1Image, uiWindow2Image, device, screenWidth, screenHeight, MapSimulatorWindowNames.CashReceiveGiftDialog, runtimeAssets);
            RegisterCashServiceModalOwnerWindow(manager, uiWindow1Image, uiWindow2Image, device, screenWidth, screenHeight, MapSimulatorWindowNames.CashNameChangeLicenseDialog, runtimeAssets);
            RegisterCashServiceModalOwnerWindow(manager, uiWindow1Image, uiWindow2Image, device, screenWidth, screenHeight, MapSimulatorWindowNames.CashTransferWorldLicenseDialog, runtimeAssets);
        }
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            Texture2D frameTexture = ResolveCashServiceModalFrameTexture(uiWindow1Image, uiWindow2Image, device, windowName, runtimeAssets)
        ...
        private static Texture2D ResolveCashServiceModalFrameTexture(
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            WzImage cashShopImage = runtimeAssets.FindImage("ui", "CashShop.img");
        ...
        private static Texture2D CreateCashPurchaseConfirmFrameTexture(WzImage cashShopImage, GraphicsDevice device)
        {
            ...
            Texture2D frameTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, width, height));
        ...
        {
            using IDisposable resourceCreationScope = manager?.BeginResourceCreationScope();
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.ItcCharacter, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.ItcSale, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.ItcPurchase, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.ItcInventory, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.ItcTab, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.ItcSubTab, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.ItcList, runtimeAssets);
            RegisterCashShopStageChildWindow(manager, basicImage, soundUIImage, device, position, MapSimulatorWindowNames.ItcStatus, runtimeAssets);
        }
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            UIWindowBase window = CreateCashShopStageChildWindow(basicImage, soundUIImage, device, position, windowName, runtimeAssets);
            if (window != null)
            {
                manager.RegisterCustomWindow(window);
            }
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            UIWindowBase itemMaker = CreateItemMakerWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, position, manager.InventoryWindow as IInventoryRuntime, runtimeServices);
            ...
            ConfigureProductionEnhancementAnimationDisplayer(manager, device, hammerProperty: null, runtimeAssets: runtimeServices.Assets);
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            UIWindowBase bookCollectionWindow = CreateBookCollectionWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, position, runtimeAssets);
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            SeedStarterEnhancementInventory(manager.InventoryWindow as IInventoryRuntime, runtimeServices.Catalog);
            UIWindowBase itemUpgrade = CreateItemUpgradeWindow(
                ...
                device,
                position,
                manager.InventoryWindow as IInventoryRuntime, runtimeServices.Catalog);
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            UIWindowBase vegaSpellWindow = CreateVegaSpellWindow(uiWindow1Image, basicImage, soundUIImage, device, position, runtimeServices);
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            UIWindowBase questAlarm = CreateQuestAlarmWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, position, runtimeAssets);
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            UIWindowBase questDeliveryWindow = CreateQuestDeliveryWindow(uiWindow2Image, basicImage, soundUIImage, device, position, runtimeServices);
            ...
            UILoaderResourceCache resourceCache)
        {
            ...
            UIWindowBase repairWindow = CreateRepairDurabilityWindow(uiWindow2Image, basicImage, soundUIImage, device, position, runtimeServices, resourceCache);
            ...
            UILoaderResourceCache resourceCache)
        {
            ...
            UIWindowBase guildRankWindow = CreateGuildRankWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(position.X + 54, Math.Max(24, position.Y - 6)), runtimeAssets, resourceCache);
            ...
            UIWindowBase guildMarkWindow = CreateGuildMarkWindow(uiWindow1Image, uiWindow2Image, basicImage, soundUIImage, device, new Point(position.X + 64, Math.Max(24, position.Y - 2)), runtimeAssets, resourceCache);
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            SocialRoomRuntime runtime = SocialRoomRuntime.CreateMiniRoomSample(runtimeServices);
            ...
            ConfigureMiniRoomOmokAssets(window, omokProperty, commonProperty, device, runtimeAssets);
            if (window == null)
            {
                return CreatePlaceholderUtilityWindow(
                    basicImage,
        ...
        private static void ConfigureMiniRoomOmokAssets(SocialRoomWindow window, WzSubProperty omokProperty, WzSubProperty commonProperty, GraphicsDevice device, IRuntimeAssetSource runtimeAssets)
        {
            ...
            window.SetMiniRoomOmokRoundEffectFrames(LoadMiniRoomOmokRoundEffectFrames(device, runtimeAssets));
        }

        private static IReadOnlyDictionary<int, SocialRoomWindow.OmokRoundEffectFrame[]> LoadMiniRoomOmokRoundEffectFrames(GraphicsDevice device, IRuntimeAssetSource runtimeAssets)
        {
            ...
            WzSubProperty effectRoot = ResolveUiSubPropertyFromStringPoolPath(runtimeAssets, 0x9E8, "UIWindow.img", "MinigameTable", "Effect");
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            SocialRoomRuntime runtime = SocialRoomRuntime.CreatePersonalShopSample(runtimeServices);
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            SocialRoomRuntime runtime = SocialRoomRuntime.CreateEntrustedShopSample(runtimeServices);
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            SocialRoomRuntime runtime = SocialRoomRuntime.CreateTradingRoomSample(runtimeServices);
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            WzImage cashShopImage = runtimeAssets.FindImage("ui", "CashShop.img");
            WzImage itcImage = runtimeAssets.FindImage("ui", "ITC.img");
            ...
            switch (windowName)
            {
                ...
                case MapSimulatorWindowNames.CashShopOneADay:
                    window.SetContentBounds(new Rectangle(275, 95, 412, 430));
                    WzSubProperty oneADayProperty = ResolveUiSubPropertyFromSegments(runtimeAssets, "OneADay.img", "CSOneADay");
            ...
            IRuntimeDataServices runtimeServices)
        {
            ...
            Texture2D pixel = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));

            pixel.SetData(new[] { Color.White });



            ItemMakerUI itemMaker = new ItemMakerUI(new DXObject(0, 0, frameTexture, 0), pixel, runtimeServices)
            {
                Position = position
            };
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            window.SetTooltipDescription(ResolveQuestAlarmTooltipDescription(runtimeAssets));
        ...
        private static string ResolveQuestAlarmTooltipDescription(IRuntimeAssetSource runtimeAssets)
        {
            try
            {
                WzImage tooltipHelpImage = runtimeAssets.FindImage("String", "ToolTipHelp.img");
        ...
        private static UIWindowBase CreateQuestDeliveryWindow(
            ...
            WzImage soundUIImage,
            GraphicsDevice device,
            Point position, IRuntimeDataServices runtimeServices)
        {
            WzSubProperty utilDialogProperty = uiWindow2Image?["UtilDlgEx"] as WzSubProperty;
            ...
            QuestDeliveryWindow window = new QuestDeliveryWindow(
                ...
                device, runtimeServices.Catalog)
            {
                Position = position
            };
            ...
            UILoaderResourceCache resourceCache)
        {
            ...
            window.SetTooltipTextures(UILoader.LoadSkillTooltipTextures(device, runtimeAssets, resourceCache));
            window.SetTooltipOrigins(UILoader.LoadSkillTooltipOrigins(runtimeAssets));

            WzSubProperty equipTooltipProperty = ResolveRepairEquipTooltipProperty(uiWindow2Image, runtimeAssets);
        ...
        private static WzSubProperty ResolveRepairEquipTooltipProperty(WzImage uiWindow2Image, IRuntimeAssetSource runtimeAssets)
        {
        ...
        private static UIWindowBase CreateItemUpgradeWindow(
            ...
            GraphicsDevice device,
            Point position,
            IInventoryRuntime inventory, IRuntimeAssetCatalog catalog)
        {
            using IDisposable resourceCreationScope = manager?.BeginResourceCreationScope();
            ...
            ItemUpgradeUI itemUpgrade = new ItemUpgradeUI(frame, catalog)
            {
                Position = position
            };
            ...
            Point position, IRuntimeDataServices runtimeServices)
        {
            ...
            VegaSpellUI window = new VegaSpellUI(frame10, device, runtimeServices)
            {
                Position = position
            };
        ...
        private static IDXObject CreateAccountMoreInfoClientSizedFrame(
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            string currentDirectoryPath = runtimeAssets.Version?.DirectoryPath;
            ...
            bool enforceRequestedSize = false)
        {
            ...
            Color[] clientFrameData = ComposeAccountMoreInfoBackgroundPixelsForTesting(
                ...
                sourceOffset.X,
                sourceOffset.Y);
            Texture2D clientSizedTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, expandedWidth, expandedHeight));
            ...
            Point sourceOffset)
        {
            ...
            Color[] composedData = ComposeAccountMoreInfoBackgroundPixelsForTesting(
                ...
                sourceOffset.X,
                sourceOffset.Y);
            Texture2D composedTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, composedWidth, composedHeight));
            ...
            Point position)
        {
            ...
            Texture2D pixelTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));
            ...
            Point position)
        {
            ...
            Texture2D frameTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, 332, 272));
        ...
        private static void SeedStarterEnhancementInventory(IInventoryRuntime inventory, IRuntimeAssetCatalog catalog)
        {
            if (inventory == null)
            {
                return;
            ...
            foreach (int itemId in ItemUpgradeUI.GetStarterEnhancementEquipItemIds(catalog))
            {
        ...
        private static void SeedStarterCompanionInventory(IInventoryRuntime inventory, GraphicsDevice device, IRuntimeAssetSource runtimeAssets)
        {
            if (inventory == null)
            {
                return;
            ...
            for (int i = 0; i < starterEquipIds.Length; i++)
            {
                ...
                {
                    ItemId = itemId,
                    ItemTexture = companionItem?.ItemTexture ?? LoadSeedInventoryItemIcon(itemId, device, runtimeAssets),
                    Quantity = 1,
                    MaxStackSize = 1,
        ...
        private static Texture2D LoadSeedInventoryItemIcon(int itemId, GraphicsDevice device, IRuntimeAssetSource runtimeAssets)
        {
            if (device == null || runtimeAssets == null || !InventoryItemMetadataResolver.TryResolveImageSource(itemId, out string category, out string imagePath))
            {
                return null;
            }
