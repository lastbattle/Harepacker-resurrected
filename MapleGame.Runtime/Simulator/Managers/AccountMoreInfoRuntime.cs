...
namespace HaCreator.MapSimulator.Managers
{
    ...
    internal sealed class AccountMoreInfoRuntime
    {
        ...
        private readonly object _countryNameCatalogSync = new();
        private readonly IRuntimeAssetSource _assets;
        private bool _countryNameCatalogLoaded;
        private bool _hasCountryNameCatalogData;
        private IReadOnlyDictionary<int, string> _areaGroupDisplayNames = new Dictionary<int, string>();
        private IReadOnlyDictionary<int, IReadOnlyDictionary<int, string>> _areaDetailDisplayNames = new Dictionary<int, IReadOnlyDictionary<int, string>>();
        private IReadOnlyList<int> _areaGroupItemParams = Array.Empty<int>();
        private IReadOnlyDictionary<int, IReadOnlyList<int>> _areaDetailItemParams = new Dictionary<int, IReadOnlyList<int>>();
        ...
        internal AccountMoreInfoRuntime(IRuntimeAssetSource assets = null)
        {
            _assets = assets;
        }
        ...
        internal AccountMoreInfoOwnerSnapshot BuildSnapshot()
        {
            return new AccountMoreInfoOwnerSnapshot
            {
                ...
                AreaGroupText = ResolveAreaGroupComboTextForSession(_areaGroup),
                AreaDetailText = ResolveAreaDetailComboTextForSession(_areaGroup, _areaDetail),
        ...
        private string ResolveAreaGroupComboTextForSession(int areaCode)
        {
            EnsureCountryNameCatalogLoaded();
            return _areaGroupDisplayNames.TryGetValue(areaCode, out string displayName)
                && !string.IsNullOrWhiteSpace(displayName)
                ? displayName
                : ResolveRegionComboText(areaCode);
        }

        private string ResolveAreaDetailComboTextForSession(int areaGroup, int areaCode)
        {
            EnsureCountryNameCatalogLoaded();
            return _areaDetailDisplayNames.TryGetValue(areaGroup, out IReadOnlyDictionary<int, string> detailDisplayNames)
        ...
        private void EnsureCountryNameCatalogLoaded()
        {
        ...
        private void TryPopulateCountryNameCatalog(
            ...
            IDictionary<int, IReadOnlyList<int>> areaDetailItemParams)
        {
            ...
            TryPopulateCountryNameCatalogFromImage(
                _assets?.FindImage("Etc", "CountryName.img"),
                areaGroups,
                areaDetails,
                areaDetailItemParams);
        }
        ...
        private int AdjustAreaGroup(int currentAreaGroup, int delta)
        {
            EnsureCountryNameCatalogLoaded();
        ...
        private int AdjustAreaDetail(int currentAreaGroup, int currentAreaDetail, int delta)
        {
            EnsureCountryNameCatalogLoaded();
        ...
        private int ResolveLoadedAreaGroup(int requestedAreaGroup)
        {
            EnsureCountryNameCatalogLoaded();
        ...
        private int ResolveLoadedAreaDetail(int selectedAreaGroup, int requestedAreaDetail)
        {
            EnsureCountryNameCatalogLoaded();
        ...
        private IReadOnlyList<AccountMoreInfoComboItem> BuildAreaGroupComboItems()
        {
            EnsureCountryNameCatalogLoaded();
            return BuildCountryNameComboItems(
                _hasCountryNameCatalogData ? _areaGroupItemParams : new[] { 0 },
                ResolveAreaGroupComboTextForSession);
        }

        private IReadOnlyList<AccountMoreInfoComboItem> BuildAreaDetailComboItems(int selectedAreaGroup)
        {
            EnsureCountryNameCatalogLoaded();
            ...
            return BuildCountryNameComboItems(
                itemParams,
                value => ResolveAreaDetailComboTextForSession(selectedAreaGroup, value));
        }
