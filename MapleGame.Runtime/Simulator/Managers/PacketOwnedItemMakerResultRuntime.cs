...
namespace HaCreator.MapSimulator.Managers
{
    ...
    internal static class PacketOwnedItemMakerResultRuntime
    {
        ...
        public static string BuildStatusMessage(
            ...
            IRuntimeAssetCatalog catalog = null)
        {
            if (result == null)
            {
            ...
            {
                return $"Created {ResolveItemName(craftedItemId, catalog)} x{craftedQuantity}.";
            }

            if (result.ResultType == 3)
            {
                return FormatDisassemblyYieldSummary(result, catalog);
            }

            if (result.DisassembledItemId > 0)
            {
                return FormatDisassemblyNotice(result.DisassembledItemId, catalog);
            }
        ...
        public static IReadOnlyList<string> BuildFeedbackLines(
            PacketOwnedItemMakerResult result,
            IRuntimeAssetCatalog catalog = null)
        {
            if (result == null)
            ...
            switch (result.ResultType)
            {
                ...
                case 2:
                    ...
                    {
                        AppendPrimaryTargetGainLine(lines, result.TargetItemId, result.TargetItemCount, catalog);
                    }

                    AppendSecondaryRewardLines(lines, result.RewardItems, catalog);
                    AppendSecondaryBonusLines(lines, result.BonusItemIds, catalog);
                    if (result.HasAuxiliaryItem)
                    {
                        AppendSecondaryGainLine(lines, result.AuxiliaryItemId, 1, catalog);
                    }
                ...
                case 3:
                    ...
                    {
                        AppendPrimaryTargetGainLine(lines, result.TargetItemId, 1, catalog);
                    }

                    AppendSecondaryGainLine(lines, result.GeneratedItemId, result.GeneratedItemCount, catalog);
                    break;

                case 4:
                    if (result.DisassembledItemId > 0)
                    {
                        lines.Add(FormatDisassemblySuccess(result.DisassembledItemId, catalog));
                    }

                    AppendPrimaryRewardLines(lines, result.RewardItems, catalog);
                    AppendMesoLossLine(lines, result.MesoDelta);
                    break;
            }
        ...
        private static void AppendPrimaryRewardLines(
            List<string> lines,
            IReadOnlyList<PacketOwnedItemMakerResultItemEntry> items,
            IRuntimeAssetCatalog catalog)
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                AppendPrimaryTargetGainLine(lines, item.ItemId, item.Quantity, catalog);
            }
        }

        private static void AppendSecondaryRewardLines(
            List<string> lines,
            IReadOnlyList<PacketOwnedItemMakerResultItemEntry> items,
            IRuntimeAssetCatalog catalog)
        {
            ...
            {
                PacketOwnedItemMakerResultItemEntry item = items[i];
                AppendSecondaryGainLine(lines, item.ItemId, item.Quantity, catalog);
            }
        }

        private static void AppendSecondaryBonusLines(
            List<string> lines,
            IReadOnlyList<int> bonusItemIds,
            IRuntimeAssetCatalog catalog)
        {
            ...
            {
                AppendSecondaryGainLine(lines, bonusItemIds[i], 1, catalog);
            }
        }

        private static void AppendPrimaryTargetGainLine(
            List<string> lines,
            int itemId,
            int quantity,
            IRuntimeAssetCatalog catalog)
        {
            ...
            string format = MapleStoryStringPool.GetCompositeFormatOrFallback(
                ...
                out _);
            string itemTypeName = ResolveItemTypeName(itemId, catalog);
            string itemName = ResolveItemName(itemId, catalog);
            lines.Add(string.Format(CultureInfo.InvariantCulture, format, itemTypeName, itemName, clampedQuantity));
        }

        private static void AppendSecondaryGainLine(
            List<string> lines,
            int itemId,
            int quantity,
            IRuntimeAssetCatalog catalog)
        {
            ...
            string format = MapleStoryStringPool.GetCompositeFormatOrFallback(
                ...
                out _);
            lines.Add(string.Format(CultureInfo.InvariantCulture, format, ResolveItemName(itemId, catalog), clampedQuantity));
        }
        ...
        private static string FormatDisassemblySuccess(int itemId, IRuntimeAssetCatalog catalog)
        {
            string format = MapleStoryStringPool.GetCompositeFormatOrFallback(
                ...
                maxPlaceholderCount: 1,
                out _);
            return string.Format(CultureInfo.InvariantCulture, format, ResolveItemName(itemId, catalog));
        }

        private static string FormatDisassemblyNotice(int itemId, IRuntimeAssetCatalog catalog)
        {
            string format = MapleStoryStringPool.GetCompositeFormatOrFallback(
        ...
        private static string FormatDisassemblyYieldSummary(
            PacketOwnedItemMakerResult result,
            IRuntimeAssetCatalog catalog)
        {
            ...
            if (result.TargetItemId > 0)
            {
                rewardParts.Add(string.Format(
                    ...
                    "{0} x{1}",
                    ResolveItemName(result.TargetItemId, catalog),
            ...
            if (result.GeneratedItemId > 0)
            {
                rewardParts.Add(string.Format(
                    CultureInfo.InvariantCulture,
                    "{0} x{1}",
                    ResolveItemName(result.GeneratedItemId, catalog),
        ...
        internal static string ResolveItemName(int itemId, IRuntimeAssetCatalog catalog = null)
        {
            return catalog?.TryGetItemName(itemId, out RuntimeItemName itemInfo) == true
                && !string.IsNullOrWhiteSpace(itemInfo?.Name)
                ? itemInfo.Name
                : $"Item #{itemId}";
        }

        internal static string ResolveItemTypeName(int itemId, IRuntimeAssetCatalog catalog = null)
        {
            return catalog?.TryGetItemName(itemId, out RuntimeItemName itemInfo) == true
