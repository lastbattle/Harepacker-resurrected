...
namespace HaCreator.MapSimulator.Managers
{
    ...
    public class RenderingManager
    {
        ...
        private readonly Func<Point> _getMapCenter;
        ...
        public RenderingManager(Func<Point> getMapCenter, Func<int> getWidth, Func<int> getHeight)
        {
            _getMapCenter = getMapCenter ?? throw new ArgumentNullException(nameof(getMapCenter));
        ...
        public void DrawTooltips(in RenderContext context, MouseState mouseState)
        {
            ...
            for (int i = 0; i < tooltipCount; i++)
            {
                ...
                if (tooltip.Tooltip.Bounds.Width > 0 && tooltip.Tooltip.Bounds.Height > 0)
                {
                    Rectangle tooltipRect = tooltip.Tooltip.Bounds;
                ...
                tooltip.Draw(context.SpriteBatch, context.SkeletonMeshRenderer, context.GameTime,
                    context.MapShiftX, context.MapShiftY, mapCenter.X, mapCenter.Y,
                    null,
                    context.RenderParams,
                    context.TickCount);
            }
        ...
        public void DrawDebugOverlays(in RenderContext context)
        {
            ...
            _transportField?.DrawDebug(context.SpriteBatch, context.DebugTexture,
                context.MapShiftX, context.MapShiftY, mapCenter.X, mapCenter.Y, _fontDebugValues);
        }
        ...
        private void DrawPortalDebugInfo(in RenderContext context)
        {
            ...
            for (int i = 0; i < _portalsArray.Length; i++)
            {
                ...
                RuntimePortal instance = portalItem.Portal;
                Rectangle rect = new Rectangle(
                    instance.X - (int)context.ShiftCenter.X - (portalItem.Width - 20),
                    instance.Y - (int)context.ShiftCenter.Y - portalItem.Height,
                    portalItem.Width + 40,
                    portalItem.Height);
        ...
        private void DrawReactorDebugInfo(in RenderContext context)
        {
            ...
            for (int i = 0; i < _reactorsArray.Length; i++)
            {
                ...
                RuntimeReactor instance = reactorItem.RuntimeReactor;
                Rectangle rect = new Rectangle(
                    reactorItem.CurrentWorldX - (int)context.ShiftCenter.X - (reactorItem.Width - 20),
                    reactorItem.CurrentWorldY - (int)context.ShiftCenter.Y - reactorItem.Height,
                    Math.Max(80, reactorItem.Width + 40),
                    Math.Max(120, reactorItem.Height));
                ...
                if (reactorItem.CanUpdateDebugText(context.TickCount, 1000))
                {
                    ...
                    _debugStringBuilder.Append(" x: ").Append(rect.X).Append('\n');
                    _debugStringBuilder.Append(" y: ").Append(rect.Y).Append('\n');
                    _debugStringBuilder.Append(" id: ").Append(instance.Id).Append('\n');
        ...
        private void DrawMobDebugInfo(in RenderContext context)
        {
            ...
            for (int i = 0; i < _mobsArray.Length; i++)
            {
                ...
                RuntimeLife instance = mobItem.RuntimeLife;
                int mobX = mobItem.CurrentX;
                int mobY = mobItem.CurrentY;

                Rectangle rect = new Rectangle(
                    mobX - (int)context.ShiftCenter.X - (mobItem.Width - 20),
                    mobY - (int)context.ShiftCenter.Y - mobItem.Height,
                    Math.Max(100, mobItem.Width + 40),
                    Math.Max(140, mobItem.Height));
        ...
        private void DrawNpcDebugInfo(in RenderContext context)
        {
            ...
            {
                NpcItem npcItem = _npcsArray[i];
                RuntimeLife instance = npcItem.RuntimeLife;

                Rectangle rect = new Rectangle(
                    instance.X - (int)context.ShiftCenter.X - (npcItem.Width - 20),
                    instance.Y - (int)context.ShiftCenter.Y - npcItem.Height,
                    Math.Max(100, npcItem.Width + 40),
                    Math.Max(120, npcItem.Height));
                ...
                if (npcItem.CanUpdateDebugText(context.TickCount, 1000))
                {
                    ...
                    _debugStringBuilder.Append(" x: ").Append(rect.X).Append('\n');
                    _debugStringBuilder.Append(" y: ").Append(rect.Y).Append('\n');
                    _debugStringBuilder.Append(" id: ").Append(instance.Id);
