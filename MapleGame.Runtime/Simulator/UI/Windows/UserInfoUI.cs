...
namespace HaCreator.MapSimulator.UI
{
    ...
    public sealed class UserInfoUI : UIWindowBase
    {
        ...
        public UserInfoUI(IDXObject frame, bool isBigBang, IRuntimeDataServices runtimeServices)
            : base(frame)
        {
            _defaultFrame = frame;
        ...
        private string GetEquippedItemName(EquipSlot slot, CharacterBuild build = null)
        {
            ...
            if (build != null)
            {
                ...
                {
            return InventoryItemMetadataResolver.TryResolveItemName(profileRideItemId, _runtimeServices.Catalog, out string itemName) &&
        ...
        {
            if (sprite?.GraphicsDevice == null || itemId <= 0)
            {
                return null;
            }
        ...
        private string ResolveRemotePetDisplayName(int petItemId)
        {
            return InventoryItemMetadataResolver.TryResolveItemName(petItemId, _runtimeServices.Catalog, out string itemName) &&
                   !string.IsNullOrWhiteSpace(itemName)
                ? itemName
                : $"Pet item {petItemId}";
        }
