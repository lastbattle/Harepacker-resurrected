...
namespace HaCreator.MapSimulator.UI
{
    ...
    public class EquipUIBigBang : UIWindowBase, IEquipmentPendingChangeWindow
    {
        ...
        public EquipUIBigBang(IDXObject frame, GraphicsDevice device, IRuntimeDataServices runtimeServices)
            : base(frame)
        {
            _device = device;
        ...
        private bool TryRequestItemUpgrade(MouseState mouseState)
        {
            ...
            if (!characterSlot.HasValue || !ItemUpgradeUI.CanUpgrade(characterSlot.Value, part, _runtimeServices.Assets))
            {
                return false;
            }
        ...
        private string ResolveItemDescription(int itemId, string fallbackCategory = null)
        {
            if (_runtimeServices.Catalog.TryGetItemName(itemId, out RuntimeItemName itemInfo))
            {
                return !string.IsNullOrWhiteSpace(itemInfo.Description)
        ...
        private IReadOnlyList<TooltipSection> BuildPetTooltipSections(CompanionEquipItem item, string petContextLine)
        {
        ...
        private string BuildSupportedPetsLine(CompanionEquipItem item)
        {
            ...
            {
                if (_runtimeServices.Catalog.TryGetItemName(petItemId, out RuntimeItemName petInfo)
                    && !string.IsNullOrWhiteSpace(petInfo.Name))
                {
                    petNames.Add(petInfo.Name);
                }
                else
                {
