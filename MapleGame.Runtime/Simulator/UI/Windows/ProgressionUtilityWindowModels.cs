...
namespace HaCreator.MapSimulator.UI
{
    ...
    internal static class CollectionBookSnapshotFactory
    {
            ...
            Func<string, int, float> measureTextWidth = null)
        {
            ...
            pages.AddRange(CreateRecipePages(catalog, progression, measureTextWidth));
        ...
        private static IEnumerable<CollectionBookPageSnapshot> CreateRecipePages(IRuntimeAssetCatalog catalog, ItemMakerProgressionSnapshot progression, Func<string, int, float> measureTextWidth = null)
        {
            ...
            recipeEntries.AddRange(
                progression.DiscoveredRecipeEntries
                    ...
                    .Select(entry => CreateEntry("Recipe", ResolveItemName(catalog, entry.OutputItemId), BuildRecipeEntryDetail(entry), CollectionBookEntryTone.Normal)));
            recipeEntries.AddRange(
                progression.UnlockedHiddenRecipeEntries
                    .OrderBy(entry => entry.OutputItemId)
                    .ThenBy(entry => entry.RecipeKey, StringComparer.Ordinal)
                    .Select(entry => CreateEntry("Hidden", ResolveItemName(catalog, entry.OutputItemId), BuildRecipeEntryDetail(entry), CollectionBookEntryTone.Accent)));
        ...
        private static void AddCompactDetailClauseRecords(
            ...
            Func<string, int, float> measureTextWidth = null)
        {
            if (records == null || compactClauses == null || compactClauses.Count < 2)
            {
                return;
            }
        ...
        private static int GetCompactDetailClauseBottom(
            IReadOnlyList<string> compactClauses,
            int leftStyleIndex,
            int rightStyleIndex,
            int top,
            ...
            Func<string, int, float> measureTextWidth = null)
        {
            if (compactClauses == null || compactClauses.Count < 2)
            {
                return fallbackBottom;
            }
        ...
        private static string ResolveItemName(IRuntimeAssetCatalog catalog, int itemId)
        {
            return catalog.TryGetItemName(itemId, out RuntimeItemName itemInfo)
