...
namespace HaCreator.MapSimulator.UI
{
    ...
    public sealed class ItemUpgradeUI : UIWindowBase
    {
        ...
        private readonly IRuntimeAssetCatalog _catalog;
        private readonly Random _random = new Random();
        private readonly Dictionary<EquipSlot, UpgradeState> _upgradeStates = new Dictionary<EquipSlot, UpgradeState>();
        private sealed class MetadataCaches
        {
        internal readonly Dictionary<int, IReadOnlyCollection<int>> ConsumableRequirementCache = new Dictionary<int, IReadOnlyCollection<int>>();
        internal readonly Dictionary<int, string> ConsumableOwnerPathCache = new Dictionary<int, string>();
        internal readonly Dictionary<int, EnhancementConsumableDefinition> DynamicConsumableDefinitionCache = new Dictionary<int, EnhancementConsumableDefinition>();
        internal readonly Dictionary<int, int> EquipUpgradeSlotCountCache = new Dictionary<int, int>();
        internal readonly Dictionary<int, VegaModifierProfile> VegaModifierProfileCache = new Dictionary<int, VegaModifierProfile>();
        internal readonly Dictionary<int, VegaCompatibleScrollProfile> VegaCompatibleScrollProfileCache = new Dictionary<int, VegaCompatibleScrollProfile>();
        internal readonly Dictionary<int, IReadOnlyCollection<EquipSlot>> ScrollTargetSlotCache = new Dictionary<int, IReadOnlyCollection<EquipSlot>>();
        }
        private static readonly System.Runtime.CompilerServices.ConditionalWeakTable<object, MetadataCaches> MetadataByOwner = new();
        private static MetadataCaches GetMetadataCaches(object owner) => MetadataByOwner.GetValue(owner ?? throw new ArgumentNullException(nameof(owner)), _ => new MetadataCaches());
        ...
        private IRuntimeAssetSource _runtimeAssets => _catalog.Assets;

        public ItemUpgradeUI(IDXObject frame, IRuntimeAssetCatalog catalog)
            : base(frame)
        {
        ...
        public static bool IsItemUpgradeConsumeCashConsumable(IRuntimeAssetCatalog catalog,
            int itemId,
            IRuntimeAssetSource runtimeAssets)
        {
            ArgumentNullException.ThrowIfNull(runtimeAssets);
            return !IsVegaSpellConsumable(itemId) &&
                   TryGetConsumableDefinition(catalog, itemId, out EnhancementConsumableDefinition definition, runtimeAssets) &&
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            return part != null
                   && !part.IsCash
                   && ResolveDefaultSlotCount(slot, part, runtimeAssets) > 0;
        }

        public void PrepareConsumableSelection(int itemId)
        {
            ClearPresentationState();
            if (!TryGetConsumableDefinition(_catalog, itemId, out EnhancementConsumableDefinition definition, _runtimeAssets))
            {
                return;
            }
        ...
        public int ResolveProjectedRemainingUpgradeSlotCountAfterRecovery(EquipSlot slot)
        {
            IReadOnlyList<KeyValuePair<EquipSlot, CharacterPart>> candidates = GetCandidates();
            for (int i = 0; i < candidates.Count; i++)
            {
                KeyValuePair<EquipSlot, CharacterPart> candidate = candidates[i];
                if (candidate.Key != slot)
                {
                    continue;
                }

                UpgradeState state = GetOrCreateState(candidate.Key, candidate.Value);
                ...
                return Math.Clamp(projectedRemainingSlots, 0, projectedTotalSlots);
            }
            ...
            bool success)
        {
            ...
            if (!TryGetConsumableDefinition(_catalog, validationResult.ConsumableItemId, out EnhancementConsumableDefinition consumableDefinition, _runtimeAssets))
            {
            ...
            {
                if (!TryGetConsumableDefinition(_catalog, validationResult.ModifierItemId, out EnhancementConsumableDefinition modifierDefinition, _runtimeAssets))
                {
            ...
            bool previewOnly)
        {
            ...
            if (!TryGetConsumableCompatibilityBlockReason(_catalog, consumable, selectedPart, out string compatibilityBlockReason, _runtimeAssets))
            {
        ...
        private float ResolveSuccessRate(EnhancementConsumable consumable, UpgradeState state, EnhancementConsumable modifier)
        {
            ...
            if (modifier != null &&
                TryResolveVegaModifierProfile(_catalog, modifier.ItemId, out VegaModifierProfile modifierProfile) &&
                TryResolveVegaCompatibleScrollProfile(_catalog, consumable.ItemId, out VegaCompatibleScrollProfile scrollProfile) &&
            ...
            {
                return ApplyModifierSuccessRate(_catalog, consumable.Definition.FlatSuccessRate, consumable, modifier);
            }
            ...
            return ApplyModifierSuccessRate(_catalog, baseRate, consumable, modifier);
        }

        private static float ApplyModifierSuccessRate(IRuntimeAssetCatalog catalog, float baseRate, EnhancementConsumable consumable, EnhancementConsumable modifier)
        {
            baseRate = MathHelper.Clamp(baseRate, 0f, 1.0f);
            if (modifier == null || !IsModifierCompatible(catalog, modifier, consumable))
            {
                return baseRate;
            }
        ...
        private void UpdateButtonStates()
        {
            ...
            bool canApply = consumable != null &&
                            TryGetConsumableCompatibilityBlockReason(_catalog, consumable, selection.Value, out _, runtimeAssets: _runtimeAssets) &&
                            modifierReady &&
                            consumable switch
                            {
        ...
        private VisualThemeKind ResolveCurrentVisualThemeKind()
        {
            if (_preferredConsumableItemId.HasValue &&
                TryGetConsumableDefinition(_catalog, _preferredConsumableItemId.Value, out EnhancementConsumableDefinition preferredDefinition, _runtimeAssets) &&
        ...
        {
            string ownerPath = ResolveConsumableOwnerPath(_catalog, itemId, _runtimeAssets);
        ...
        private string BuildReadyStatusMessage()
        {
            ...
            if (_preferredModifierItemId.HasValue &&
                TryGetConsumableDefinition(_catalog, _preferredModifierItemId.Value, out EnhancementConsumableDefinition modifierDefinition, _runtimeAssets) &&
            ...
            if (_preferredConsumableItemId.HasValue &&
                TryGetConsumableDefinition(_catalog, _preferredConsumableItemId.Value, out EnhancementConsumableDefinition consumableDefinition, _runtimeAssets) &&
        ...
        private IReadOnlyList<KeyValuePair<EquipSlot, CharacterPart>> GetCandidates()
        {
            ...
            return _characterBuild.Equipment
                .Where(entry => entry.Value != null
                    ...
                    && CanUpgrade(entry.Key, entry.Value, _runtimeAssets))
        ...
        private EnhancementConsumable ResolveConsumable(UpgradeState state, CharacterPart selectedPart)
        {
            ...
            if (_preferredConsumableItemId.HasValue)
            {
                ...
                if (GetConsumableCount(preferredItemId) > 0 &&
                    TryGetConsumableDefinition(_catalog, preferredItemId, out EnhancementConsumableDefinition preferredDefinition, _runtimeAssets) &&
                    preferredDefinition.EffectType != ConsumableEffectType.Modifier &&
                    IsConsumableCompatibleWithItem(preferredDefinition, selectedPart?.ItemId ?? 0, _runtimeAssets) &&
                    (preparedModifierConsumable == null || IsModifierCompatible(_catalog, preparedModifierConsumable, new EnhancementConsumable(preferredDefinition))))
                {
        ...
        public bool TryGetModifierPreview(EquipSlot slot, int modifierItemId, out ModifierPreview preview)
        {
            ...
            if (_characterBuild?.Equipment == null ||
                ...
                !CanUpgrade(slot, part, _runtimeAssets) ||
                !TryGetConsumableDefinition(_catalog, modifierItemId, out EnhancementConsumableDefinition modifierDefinition, _runtimeAssets) ||
                modifierDefinition.EffectType != ConsumableEffectType.Modifier)
            {
                return false;
            }
        ...
        public bool TryGetVegaRequestPreview(EquipSlot slot, int modifierItemId, out VegaRequestPreview preview)
        {
            ...
            EnhancementConsumable modifier = new EnhancementConsumable(modifierDefinition);
            if (!IsModifierCompatible(_catalog, modifier, consumable))
            {
                return false;
            }
        ...
        private EnhancementConsumable ResolveModifier(EnhancementConsumable consumable)
        {
            ...
            if (GetConsumableCount(modifierItemId) <= 0 ||
                !TryGetConsumableDefinition(_catalog, modifierItemId, out EnhancementConsumableDefinition definition, _runtimeAssets) ||
            ...
            return IsModifierCompatible(_catalog, modifier, consumable)
                ? modifier
                : null;
        }

        private static bool IsModifierCompatible(IRuntimeAssetCatalog catalog, EnhancementConsumable modifier, EnhancementConsumable consumable)
        {
            ...
            if (TryResolveVegaModifierProfile(catalog, modifier.ItemId, out VegaModifierProfile modifierProfile))
            {
                return TryResolveVegaCompatibleScrollProfile(catalog, consumable.ItemId, out VegaCompatibleScrollProfile scrollProfile)
                    && AreRatesEquivalent(scrollProfile.BaseSuccessRate, modifierProfile.RequiredBaseSuccessRate);
            }
        ...
        private EnhancementConsumable ResolveConsumableForModifier(UpgradeState state, CharacterPart selectedPart, EnhancementConsumableDefinition modifierDefinition)
        {
            if (_inventory != null)
            {
                ...
                foreach (InventoryType inventoryType in new[] { InventoryType.USE, InventoryType.CASH })
                {
                    ...
                    {
                        if (entry.Value <= 0 ||
                            !TryGetConsumableDefinition(_catalog, entry.Key, out EnhancementConsumableDefinition candidateDefinition, _runtimeAssets) ||
                        ...
                        if (!IsModifierCompatible(_catalog, modifier, candidate) ||
                            !IsConsumableCompatibleWithItem(candidateDefinition, selectedPart?.ItemId ?? 0, _runtimeAssets) ||
                            !TryGetConsumableCompatibilityBlockReason(_catalog, candidate, selectedPart, out _, _runtimeAssets))
                        {
                            continue;
                        }
            ...
            if (TryResolveVegaModifierProfile(_catalog, modifierDefinition.ItemId, out _))
            {
                return null;
            }
        ...
        {
            if (!_preferredModifierItemId.HasValue ||
                !TryGetConsumableDefinition(_catalog, _preferredModifierItemId.Value, out EnhancementConsumableDefinition definition, _runtimeAssets))
            {
                return consumable != null
            ...
            {
                ModifierBehavior.VegaTenPercent when TryResolveVegaModifierProfile(_catalog, definition.ItemId, out VegaModifierProfile tenProfile)
                    => $"a Vega-enabled {(int)Math.Round(tenProfile.RequiredBaseSuccessRate * 100f)}% scroll",
                ModifierBehavior.VegaSixtyPercent when TryResolveVegaModifierProfile(_catalog, definition.ItemId, out VegaModifierProfile sixtyProfile)
        ...
        {
            if (_inventory == null || !TryGetConsumableDefinition(_catalog, itemId, out EnhancementConsumableDefinition definition, _runtimeAssets))
            {
                return 0;
            }
        ...
        private string ResolveConsumableName(int itemId)
        {
            return TryGetConsumableDefinition(_catalog, itemId, out EnhancementConsumableDefinition definition, _runtimeAssets)
                   && !string.IsNullOrWhiteSpace(definition.Name)
                ? definition.Name
                : ResolveCachedItemNameOrFallback(_catalog, itemId);
        }

        private static string ResolveCachedItemNameOrFallback(IRuntimeAssetCatalog catalog, int itemId)
        {
            return $"Item #{itemId}";
        }

        private static string ResolveCachedItemDescription(IRuntimeAssetCatalog catalog, int itemId)
        {
            return InventoryItemMetadataResolver.TryResolveItemDescription(itemId, catalog, out string description)
                ? description
                : string.Empty;
        }
        ...
        private UpgradeState GetOrCreateState(EquipSlot slot, CharacterPart part)
        {
            ...
            {
                int defaultSlotCount = ResolveDefaultSlotCount(slot, part, _runtimeAssets);
        ...
        private void SyncStateToPart(CharacterPart part, UpgradeState state)
        {
            ...
            if (state.HasPotential)
            {
                ...
                part.ItemOptionIds = new List<int>(part.ItemOptionIds ?? new List<int>());
            }
            else
            {
        ...
        private static int ResolveDefaultSlotCount(
            EquipSlot slot,
            CharacterPart part,
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            int? authoredSlotCount = ResolveAuthoredUpgradeSlotCount(part, runtimeAssets);
        ...
        private static int? ResolveAuthoredUpgradeSlotCount(
            CharacterPart part,
            IRuntimeAssetSource runtimeAssets)
        {
            if (part?.ItemId <= 0)
            ...
            if (GetMetadataCaches(runtimeAssets).EquipUpgradeSlotCountCache.TryGetValue(part.ItemId, out int cachedCount))
            {
            ...
            GetMetadataCaches(runtimeAssets).EquipUpgradeSlotCountCache[part.ItemId] = resolvedCount;
        ...
        private EnhancementConsumable GetFirstAvailableConsumable(UpgradeState state, CharacterPart selectedPart, params int[] itemIds)
        {
            for (int i = 0; i < itemIds.Length; i++)
            {
                ...
                if (GetConsumableCount(itemId) <= 0 ||
                    !TryGetConsumableDefinition(_catalog, itemId, out EnhancementConsumableDefinition definition, _runtimeAssets) ||
                    ...
                    !IsConsumableCompatibleWithItem(definition, selectedPart?.ItemId ?? 0, _runtimeAssets))
                {
                    continue;
                }
        ...
        private static bool TryGetConsumableDefinition(IRuntimeAssetCatalog catalog, int itemId, out EnhancementConsumableDefinition definition, IRuntimeAssetSource runtimeAssets)
        {
            ArgumentNullException.ThrowIfNull(runtimeAssets);
            ...
            if (GetMetadataCaches(catalog).DynamicConsumableDefinitionCache.TryGetValue(itemId, out definition))
            {
                return true;
            }

            if (TryCreateDynamicConsumableDefinition(catalog, itemId, out definition, runtimeAssets))
            {
                GetMetadataCaches(catalog).DynamicConsumableDefinitionCache[itemId] = definition;
                return true;
            }
        ...
        private static bool IsConsumableCompatibleWithItem(EnhancementConsumableDefinition definition, int equipItemId, IRuntimeAssetSource runtimeAssets)
        {
            ...
            IReadOnlyCollection<int> requiredItems = GetRequiredEquipItemIds(definition, runtimeAssets);
            return requiredItems.Count == 0 || requiredItems.Contains(equipItemId);
        }

        private static IReadOnlyCollection<int> GetRequiredEquipItemIds(EnhancementConsumableDefinition definition, IRuntimeAssetSource runtimeAssets)
        {
            if (GetMetadataCaches(runtimeAssets).ConsumableRequirementCache.TryGetValue(definition.ItemId, out IReadOnlyCollection<int> cached))
            {
                return cached;
            }
            ...
            if (definition.InventoryType == InventoryType.CASH)
            {
                ...
                WzImage image = runtimeAssets.FindImage("Item", imagePath);
                if (image != null)
                {
                    image.ParseImage();
            ...
            IReadOnlyCollection<int> result = requiredItems.Count > 0
                ...
                : Array.Empty<int>();
            GetMetadataCaches(runtimeAssets).ConsumableRequirementCache[definition.ItemId] = result;
            return result;
        }

        private static bool TryGetConsumableCompatibilityBlockReason(IRuntimeAssetCatalog catalog, EnhancementConsumable consumable, CharacterPart selectedPart, out string reason, IRuntimeAssetSource runtimeAssets)
        {
            ArgumentNullException.ThrowIfNull(runtimeAssets);
            reason = null;
            ...
            bool matchesRequiredItem = IsConsumableCompatibleWithItem(consumable.Definition, selectedPart.ItemId, runtimeAssets);
            bool matchesTargetSlot = !TryGetScrollTargetSlots(catalog, consumable.Definition.ItemId, out IReadOnlyCollection<EquipSlot> targetSlots) ||
                                     targetSlots.Count == 0 ||
            ...
            string requiredFamilyLabel = ResolveRequiredEquipFamilyLabel(catalog, consumable.Definition);
            ...
            if (!matchesTargetSlot &&
                TryGetScrollTargetSlots(catalog, consumable.Definition.ItemId, out targetSlots) &&
                targetSlots.Count > 0)
            {
                string targetFamilyLabel = ResolveScrollTargetFamilyLabel(catalog, consumable.Definition.ItemId);
        ...
        private static string ResolveRequiredEquipFamilyLabel(IRuntimeAssetCatalog catalog, EnhancementConsumableDefinition definition)
        {
            IReadOnlyCollection<int> requiredItems = GetRequiredEquipItemIds(definition, catalog.Assets);
            if (requiredItems.Count == 0)
            {
                return null;
            }
            ...
            string[] itemNames = requiredItems
                .Select(itemId => ResolveCachedItemNameOrFallback(catalog, itemId))
        ...
        private static string ResolveConsumableOwnerPath(IRuntimeAssetCatalog catalog, int itemId, IRuntimeAssetSource runtimeAssets)
        {
            ArgumentNullException.ThrowIfNull(runtimeAssets);
            if (GetMetadataCaches(catalog).ConsumableOwnerPathCache.TryGetValue(itemId, out string cachedPath))
            {
                return cachedPath;
            }

            string ownerPath = null;
            if (TryGetConsumableDefinition(catalog, itemId, out EnhancementConsumableDefinition definition, runtimeAssets) &&
                definition.InventoryType == InventoryType.CASH)
            {
            ...
            GetMetadataCaches(catalog).ConsumableOwnerPathCache[itemId] = ownerPath;
            return ownerPath;
        }
        ...
        private static bool TryCreateDynamicConsumableDefinition(IRuntimeAssetCatalog catalog, int itemId, out EnhancementConsumableDefinition definition, IRuntimeAssetSource runtimeAssets)
        {
            ArgumentNullException.ThrowIfNull(runtimeAssets);
            definition = default;
            if (IsKnownSlotRecoveryScrollId(itemId) ||
                IsWzAuthoredSlotRecoveryScrollText(
                    ResolveCachedItemNameOrFallback(catalog, itemId),
                    ResolveCachedItemDescription(catalog, itemId)))
            {
                return TryCreateWzConsumeDefinition(catalog, itemId, ConsumableEffectType.SlotRecovery, out definition, runtimeAssets: runtimeAssets);
            }

            if (InnocenceScrollIds.Contains(itemId))
            {
                return TryCreateWzConsumeDefinition(catalog, itemId, ConsumableEffectType.Reset, out definition, runtimeAssets: runtimeAssets);
            }

            if (itemId == PerfectInnocenceScrollId)
            {
                return TryCreateWzConsumeDefinition(catalog, itemId, ConsumableEffectType.Reset, out definition, runtimeAssets, InventoryType.CASH);
            }

            if (GoldenHammerIds.Contains(itemId))
            {
                return TryCreateWzHammerDefinition(catalog, itemId, InventoryType.USE, HammerBehavior.None, out definition, runtimeAssets);
            }

            if (itemId == ViciousHammerId)
            {
                return TryCreateWzHammerDefinition(catalog, itemId, InventoryType.CASH, HammerBehavior.Vicious, out definition, runtimeAssets);
            }

            if (TryCreateStringBackedEnhancementDefinition(catalog, itemId, out definition, runtimeAssets))
            {
                return true;
            }
        ...
        private static bool TryCreateStringBackedEnhancementDefinition(IRuntimeAssetCatalog catalog, int itemId, out EnhancementConsumableDefinition definition, IRuntimeAssetSource runtimeAssets)
        {
            ArgumentNullException.ThrowIfNull(runtimeAssets);
            definition = default;
            ...
            if (!TryResolveEnhancementScrollProfile(catalog, itemId, out EnhancementScrollProfile profile, runtimeAssets))
            {
                return false;
            }

            WzSubProperty info = ResolveEnhancementScrollInfo(itemId, runtimeAssets);
            string description = ResolveCachedItemDescription(catalog, itemId);
        ...
        private static WzSubProperty ResolveEnhancementScrollInfo(int itemId, IRuntimeAssetSource runtimeAssets)
        {
        ...
        private static bool TryResolveEnhancementScrollProfile(IRuntimeAssetCatalog catalog, int itemId, out EnhancementScrollProfile profile, IRuntimeAssetSource runtimeAssets)
        {
            ArgumentNullException.ThrowIfNull(runtimeAssets);
            profile = default;
            string itemName = ResolveCachedItemNameOrFallback(catalog, itemId);
            ...
            string description = ResolveCachedItemDescription(catalog, itemId);
            if (!TryResolveWzEnhancementScrollData(catalog, itemId, description, out float successRate, out float destroyChance, out AuthoredStatDeltaProfile statDeltaProfile, runtimeAssets))
            {
                return false;
            }
        ...
        private static bool TryResolveWzEnhancementScrollData(IRuntimeAssetCatalog catalog,
            int itemId,
            string description,
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            if (statDeltaProfile.IsEmpty &&
                !TryGetScrollTargetSlots(catalog, itemId, out IReadOnlyCollection<EquipSlot> targetSlots) &&
                !IsGenericEquipmentRandomStatScroll(info, ResolveCachedItemNameOrFallback(catalog, itemId), description))
            {
                return false;
            }
        ...
        private static bool TryCreateWzConsumeDefinition(IRuntimeAssetCatalog catalog,
            int itemId,
        ...
        private static bool TryCreateWzHammerDefinition(IRuntimeAssetCatalog catalog, int itemId, InventoryType inventoryType, HammerBehavior hammerBehavior, out EnhancementConsumableDefinition definition, IRuntimeAssetSource runtimeAssets)
        {
            ArgumentNullException.ThrowIfNull(runtimeAssets);
            definition = default;
            string category = inventoryType == InventoryType.CASH ? "Cash" : "Consume";
            ...
            int success = explicitSuccess.HasValue
                ...
                : ResolveHammerSuccessRateFromDescription(catalog, itemId);
            definition = new EnhancementConsumableDefinition(
                itemId,
                ResolveCachedItemNameOrFallback(catalog, itemId),
                1,
        ...
        private static int ResolveHammerSuccessRateFromDescription(IRuntimeAssetCatalog catalog, int itemId)
        {
            string description = ResolveCachedItemDescription(catalog, itemId);
            if (!string.IsNullOrWhiteSpace(description))
            {
        ...
        internal static bool TryResolveVegaModifierRatePreview(IRuntimeAssetCatalog catalog, int itemId, out int requiredBasePercent, out int modifiedPercent)
        {
            requiredBasePercent = 0;
            modifiedPercent = 0;

            if (!TryParseVegaModifierRateText(ResolveCachedItemDescription(catalog, itemId), out requiredBasePercent, out modifiedPercent))
            {
                return false;
            }
        ...
        private static bool TryResolveVegaModifierProfile(IRuntimeAssetCatalog catalog, int itemId, out VegaModifierProfile profile)
        {
            if (GetMetadataCaches(catalog).VegaModifierProfileCache.TryGetValue(itemId, out profile))
            {
                return profile.IsValid;
            }

            profile = default;
            if (TryResolveVegaModifierRatePreview(catalog, itemId, out int requiredPercent, out int modifiedPercent))
            {
                profile = new VegaModifierProfile(requiredPercent / 100f, modifiedPercent / 100f);
            }

            GetMetadataCaches(catalog).VegaModifierProfileCache[itemId] = profile;
            return profile.IsValid;
        }

        private static bool TryResolveVegaCompatibleScrollProfile(IRuntimeAssetCatalog catalog, int itemId, out VegaCompatibleScrollProfile profile)
        {
            if (GetMetadataCaches(catalog).VegaCompatibleScrollProfileCache.TryGetValue(itemId, out profile))
            {
                return profile.IsValid;
            }
            ...
            GetMetadataCaches(catalog).VegaCompatibleScrollProfileCache[itemId] = profile;
            return profile.IsValid;
        }

        private static bool TryGetScrollTargetSlots(IRuntimeAssetCatalog catalog, int itemId, out IReadOnlyCollection<EquipSlot> slots)
        {
            if (GetMetadataCaches(catalog).ScrollTargetSlotCache.TryGetValue(itemId, out slots))
            {
            ...
            GetMetadataCaches(catalog).ScrollTargetSlotCache[itemId] = slots;
            return slots.Count > 0;
        }

        private static string ResolveScrollTargetFamilyLabel(IRuntimeAssetCatalog catalog, int itemId)
        {
        ...
        private static void AddTargetSlotsFromText(string text, ISet<EquipSlot> targetSlots)
        {
            ...
            if (normalized.IndexOf("helmet", StringComparison.OrdinalIgnoreCase) >= 0 ||
                ...
                normalized.IndexOf("rudolphâ€™s horn", StringComparison.OrdinalIgnoreCase) >= 0)
            {
        ...
        private static bool TryResolveTargetFamilyLabelFromText(string text, out string label)
        {
            ...
            if (ContainsAny(normalized, "helmets", "helmet", "headwear", "hats", "hat", "circlet", "rudolph's horn", "rudolphâ€™s horn") ||
                ContainsWholeWord(normalized, "cap") ||
                ContainsWholeWord(normalized, "caps"))
            {
                label = "hats";
        ...
        internal static IReadOnlyCollection<int> GetRequiredEquipItemIdsForTests(IRuntimeAssetCatalog catalog, int consumableItemId)
        {
            return TryGetConsumableDefinition(catalog, consumableItemId, out EnhancementConsumableDefinition definition, catalog.Assets)
                ? GetRequiredEquipItemIds(definition, catalog.Assets)
                : Array.Empty<int>();
        }
        ...
        internal static IReadOnlyCollection<int> GetStarterEnhancementEquipItemIds(IRuntimeAssetCatalog catalog)
        {
            return GetRequiredEquipItemIdsForTests(catalog, MapleMiracleCubeId);
        }

        internal static string ResolveConsumableOwnerPathForTests(IRuntimeAssetCatalog catalog, int consumableItemId)
        {
            return ResolveConsumableOwnerPath(catalog, consumableItemId, catalog.Assets);
        }

        internal static string ResolveRequiredEquipFamilyLabelForTests(IRuntimeAssetCatalog catalog, int consumableItemId)
        {
            return TryGetConsumableDefinition(catalog, consumableItemId, out EnhancementConsumableDefinition definition, catalog.Assets)
                ? ResolveRequiredEquipFamilyLabel(catalog, definition)
                : null;
        }

        internal static string ResolveConsumableCompatibilityBlockReasonForTests(IRuntimeAssetCatalog catalog, int consumableItemId, int equipItemId, EquipSlot slot)
        {
            if (!TryGetConsumableDefinition(catalog, consumableItemId, out EnhancementConsumableDefinition definition, catalog.Assets))
            {
                return null;
            }
            ...
            return TryGetConsumableCompatibilityBlockReason(catalog, consumable, selectedPart, out string reason, catalog.Assets)
                ? null
                : reason;
        }
