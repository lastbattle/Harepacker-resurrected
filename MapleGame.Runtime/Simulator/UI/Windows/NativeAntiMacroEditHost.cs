...
namespace HaCreator.MapSimulator.UI
{
    internal sealed class NativeAntiMacroEditHost : IDisposable
    {
        ...
        internal static int ActiveHostCount
        {
            get { lock (HostMapLock) { return HostByHandle.Count; } }
        }
        ...
        {
            if (_disposed || parentHandle == IntPtr.Zero || !IsWindow(parentHandle))
            {
                return false;
            }
        ...
        {
            // Parent destruction can destroy the child before owner cleanup runs.
            // Retire the static root immediately; Dispose remains safe afterwards.
            if (msg == WmNcDestroy)
            {
                IntPtr originalWndProc = _originalWndProc;
                try { return CallWindowProc(originalWndProc, hWnd, msg, wParam, lParam); }
                finally
                {
                    lock (HostMapLock) { HostByHandle.Remove(hWnd); }
                    _editHandle = IntPtr.Zero;
                    _originalWndProc = IntPtr.Zero;
                    if (_fontHandle != IntPtr.Zero)
                    {
        ...
        internal static bool ShouldCancelImeCompositionOnFocusChange(bool hasFocus)
        {
            ...
            // Keep blur cleanup enabled in the hosted seam as well to avoid
            // stale IME state when focus leaves the simulator-owned EDIT host.
            return true;
        }
