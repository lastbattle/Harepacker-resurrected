...
namespace HaCreator.MapSimulator.UI
{
    ...
    public class InventoryUI : UIWindowBase, IInventoryRuntime
    {
            ...
            : base(frame)
        {
            ...
            if (device != null)
            {
                _debugTooltipTexture = GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));
        ...
        private void DrawHoveredSlotTooltip(SpriteBatch sprite)
        {
            ...
            InventoryItemTooltipMetadata metadata = InventoryItemMetadataResolver.ResolveTooltipMetadata(slot.ItemId, _runtimeServices.Catalog, _hoveredInventoryType);
            ...
            Texture2D infoEffectPreviewTexture = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures)
                ? null
                : ResolveInfoEffectPreviewTexture(slot.ItemId);
            Texture2D infoPathPreviewTexture = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures)
                ...
                ? null
                : ResolveInfoPathPreviewTexture(slot.ItemId);
            ...
            Texture2D[] mobPreviewTextures = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures) || infoEffectPreviewTexture != null || infoPathPreviewTexture != null || rootEffectPreviewTexture != null
                ? Array.Empty<Texture2D>()
                : ResolveMobPreviewTextures(slot.ItemId);
            Texture2D[] consumeItemPreviewTextures = HasDrawableBitmap(iconRewardTextures)
                ...
                ? Array.Empty<Texture2D>()
                : ResolveConsumeItemPreviewTextures(slot.ItemId);
            Texture2D npcPreviewTexture = HasDrawableBitmap(iconRewardTextures)
                ...
                ? null
                : ResolveNpcPreviewTexture(slot.ItemId);
            ...
            {
                int mobY = contentY + (int)Math.Ceiling(iconBlockHeight) + TOOLTIP_SECTION_GAP;
                DrawHorizontalBitmapStrip(sprite, mobPreviewTextures, backgroundRect, mobY);
            }
            else if (consumeItemPreviewHeight > 0f)
            {
                int consumeItemY = contentY + (int)Math.Ceiling(iconBlockHeight) + TOOLTIP_SECTION_GAP;
                DrawHorizontalBitmapStrip(sprite, consumeItemPreviewTextures, backgroundRect, consumeItemY);
            }
            else if (npcPreviewHeight > 0f)
            {
                int npcY = contentY + (int)Math.Ceiling(iconBlockHeight) + TOOLTIP_SECTION_GAP;
        ...
        private Texture2D ResolveTooltipIconTexture(int itemId)
        {
            ...
            Texture2D texture = null;
            if (InventoryItemMetadataResolver.TryResolveTooltipIconCanvas(itemId, _runtimeServices.Assets, out WzCanvasProperty iconCanvas))
            {
        ...
        {
            if (_runtimeServices.Catalog.TryGetItemName(itemId, out RuntimeItemName itemInfo)
