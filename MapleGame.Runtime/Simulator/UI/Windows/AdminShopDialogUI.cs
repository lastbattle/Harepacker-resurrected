...
namespace HaCreator.MapSimulator.UI
{
    ...
    public sealed class AdminShopDialogUI : UIWindowBase
    {
        ...
        private readonly IReadOnlyList<WishlistCategoryNode> s_wishlistCategoryTree;
        private readonly Dictionary<string, WishlistCategoryNode> s_wishlistCategoryNodesByKey;
        private readonly IReadOnlyDictionary<string, WishlistCategoryLeafDefinition> s_wishlistCategoryLeaves;
        private readonly object WishlistSearchIndexLock = new();
        private IReadOnlyDictionary<int, WishlistSearchIndexEntry> _wishlistSearchIndexByItemId;
        private readonly object WishlistScanBlockLock = new();
        private HashSet<int> _wishlistScanBlockedItemIds;
        ...
        private readonly object CommodityCacheLock = new();
        private Dictionary<int, AdminShopCommodityData> _bestCommodityByItemId;
        private Dictionary<int, AdminShopCommodityData> _commodityBySerialNumber;
        private Dictionary<int, List<AdminShopCommodityData>> _commoditiesByItemId;
        ...
        private readonly IReadOnlyList<WishlistPriceRange> _wishlistPriceRanges;
        ...
        private int _packetOwnedWishlistSearchRequestCounter;
        private int _packetOwnedWishlistPendingSearchRequestId = -1;
        ...
        private int _packetOwnedWishlistPendingSearchRemotePageCount = -1;
        ...
        public AdminShopDialogUI(
            ...
            IRuntimeDataServices services,
            GraphicsDevice device)
            : base(frame)
        {
            this.services = services ?? throw new ArgumentNullException(nameof(services));
            _wishlistPriceRanges = BuildWishlistPriceRanges();
            s_wishlistCategoryTree = BuildWishlistCategoryTree();
            s_wishlistCategoryNodesByKey = BuildWishlistCategoryNodeLookup(s_wishlistCategoryTree);
            s_wishlistCategoryLeaves = BuildWishlistCategoryLeaves();
            ...
            _device = device;
            _pixelTexture = GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));
        ...
        internal string ApplyPacketOwnedAdminShopOpenIgnoredByUniqueModelessOwner(
            string blockingOwner,
            int npcTemplateId,
            int commodityCount)
        {
            string ownerText = string.IsNullOrWhiteSpace(blockingOwner)
                ? "another unique-modeless owner"
                : blockingOwner;
            _packetOwnedAdminShopSession.SetLastOwnerState(
                $"Packet 367 open for NPC {Math.Max(0, npcTemplateId).ToString(CultureInfo.InvariantCulture)} with {Math.Max(0, commodityCount).ToString(CultureInfo.InvariantCulture)} row(s) was ignored because {ownerText} owned the unique-modeless slot.");
            _footerMessage = string.IsNullOrWhiteSpace(blockingOwner)
                ? "Packet 367 open was ignored because another unique-modeless owner was active."
                : $"Packet 367 open was ignored because {blockingOwner} owned the unique-modeless slot.";
            UpdateActionButtonStates();
            return _footerMessage;
        }

        internal string ApplyPacketOwnedAdminShopResultIgnoredAtOwnerGate(string blockingOwner)
        {
            if (string.IsNullOrWhiteSpace(blockingOwner))
            {
                _packetOwnedAdminShopSession.SetLastOwnerState("Packet 366 was ignored at the owner gate because no live CAdminShopDlg unique-modeless owner was visible.");
                _footerMessage = "Packet 366 was ignored at the owner gate because no live CAdminShopDlg unique-modeless owner was visible.";
            }
            else
            {
                _packetOwnedAdminShopSession.SetLastOwnerState($"Packet 366 was ignored at the owner gate because {blockingOwner} owned the unique-modeless slot.");
                _footerMessage = $"Packet 366 was ignored at the owner gate because {blockingOwner} owned the unique-modeless slot.";
            }
            ...
            string blockingOwner)
        {
            ...
            string ownerState = string.IsNullOrWhiteSpace(blockingOwner)
                ? keepSessionActive
                    ? "Packet 366 was ignored because no live CAdminShopDlg unique-modeless owner was present, and the packet-owned session stayed staged."
                    : "Packet 366 was ignored because no live CAdminShopDlg unique-modeless owner was present."
                : keepSessionActive
                    ? $"Packet 366 was ignored because {blockingOwner} owned the unique-modeless slot instead of CAdminShopDlg, and the packet-owned session stayed staged."
                    : $"Packet 366 was ignored because {blockingOwner} owned the unique-modeless slot instead of CAdminShopDlg.";
            _packetOwnedAdminShopSession.RecordResultIgnoredByOwner(
                subtype,
                resultCode,
                ownerState,
                trailingByteCount,
            ...
            out bool reopenRequested)
        {
            ...
            if (gateAction == AdminShopPacketOwnedResultGateAction.IgnoreUnsupportedSubtype)
            {
                ...
                ClearPendingPacketOwnedWishlistSearchRequest();
                ClearPendingPacketOwnedUserSellSnapshot();
                _packetOwnedAdminShopSession.SetLastOwnerState("Packet 366 subtype was ignored by the admin-shop owner after m_bShopRequestSent was cleared.");
                _footerMessage = message;
                UpdateActionButtonStates();
                return true;
            }
        ...
        public string ExecuteCashStageInventoryAction(string actionKey)
        {
            if (!string.Equals(actionKey, "BtExTrunk", StringComparison.Ordinal))
            {
                _footerMessage = $"Unknown cash-stage inventory action '{actionKey}'.";
                UpdateActionButtonStates();
                return _footerMessage;
            }
            ...
            {
                _footerMessage = "CCSWnd_Inventory::BtExTrunk is only valid while the Cash Shop owner family is active.";
                UpdateActionButtonStates();
                return _footerMessage;
            }
            ...
            AdminShopEntry storageExpansionEntry = _paneStates[AdminShopPane.Npc].Entries
                .FirstOrDefault(entry => entry?.IsStorageExpansion == true);
            if (storageExpansionEntry == null)
            {
                _footerMessage = "CCSWnd_Inventory::BtExTrunk could not find the dedicated storage-expansion commodity in the Cash Shop catalog.";
                UpdateActionButtonStates();
                return _footerMessage;
            }

            TryReselectPacketOwnedPendingEntry(storageExpansionEntry);
            if (!ReferenceEquals(GetSelectedEntry(), storageExpansionEntry))
            {
                _footerMessage = "CCSWnd_Inventory::BtExTrunk could not arm the dedicated storage-expansion owner row.";
                UpdateActionButtonStates();
                return _footerMessage;
            }
        ...
        private void CapturePacketOwnedWishlistSearchSnapshot(
            byte subtype,
            byte resultCode,
            bool hasResultCode,
            string trailingPayloadSignature,
            byte[] trailingPayload)
        {
            ...
            string normalizedSignature = string.IsNullOrWhiteSpace(trailingPayloadSignature)
                ? "none"
                : trailingPayloadSignature;
            bool changed = !ArePacketOwnedWishlistSnapshotsEquivalent(_packetOwnedWishlistSearchSnapshot, snapshot)
                || !string.Equals(_packetOwnedWishlistSearchSnapshotSignature, normalizedSignature, StringComparison.Ordinal);
            _packetOwnedWishlistSearchSnapshot = snapshot;
            _packetOwnedWishlistSearchSnapshotSignature = normalizedSignature;
            if (changed)
            {
        ...
        {
            bool hadSnapshot = _packetOwnedWishlistSearchSnapshot != null
        ...
        private AdminShopPacketOwnedWishlistSearchSnapshot ApplyPendingWishlistSearchContext(
            AdminShopPacketOwnedWishlistSearchSnapshot snapshot)
        {
            if (snapshot == null)
            {
            ...
            int localSearchRequestId = snapshot.LocalSearchRequestId;
            string query = snapshot.Query ?? string.Empty;
            if (string.IsNullOrWhiteSpace(query)
                && !string.IsNullOrWhiteSpace(_packetOwnedWishlistPendingSearchQuery))
            {
                query = _packetOwnedWishlistPendingSearchQuery;
                usedFallback = true;
            }

            string categoryKey = snapshot.CategoryKey ?? string.Empty;
            if (string.IsNullOrWhiteSpace(categoryKey)
                && !string.IsNullOrWhiteSpace(_packetOwnedWishlistPendingSearchCategoryKey))
            {
                categoryKey = _packetOwnedWishlistPendingSearchCategoryKey;
                usedFallback = true;
            }

            int priceRangeIndex = snapshot.PriceRangeIndex;
            if (priceRangeIndex < 0 && _packetOwnedWishlistPendingSearchPriceRangeIndex >= 0)
            {
                priceRangeIndex = _packetOwnedWishlistPendingSearchPriceRangeIndex;
                usedFallback = true;
            }

            if (localSearchRequestId < 0 && _packetOwnedWishlistPendingSearchRequestId >= 0)
            {
                localSearchRequestId = _packetOwnedWishlistPendingSearchRequestId;
                usedFallback = true;
            }
            ...
            return new AdminShopPacketOwnedWishlistSearchSnapshot
            {
                ServiceSessionId = snapshot.ServiceSessionId,
                SearchSessionId = snapshot.SearchSessionId,
                LocalSearchRequestId = localSearchRequestId,
                Query = query,
        ...
        private void ClearPendingPacketOwnedWishlistSearchRequest()
        {
            _packetOwnedWishlistPendingSearchRequestId = -1;
            ...
            _packetOwnedWishlistPendingSearchRemotePageCount = -1;
            _packetOwnedAdminShopSession.ClearPendingWishlistSearch();
        }

        private void AdvancePacketOwnedWishlistSearchStateToken()
        {
            _packetOwnedWishlistSearchStateToken = _packetOwnedWishlistSearchStateToken == int.MaxValue
                ? 1
                : _packetOwnedWishlistSearchStateToken + 1;
        }
        ...
        private bool IsPacketOwnedWishlistResultSessionCurrent(WishlistSearchResult packetResult)
        {
            ...
            if (packetResult.PacketSearchSessionId >= 0
                && (snapshot.SearchSessionId >= 0 || snapshot.LocalSearchRequestId >= 0)
                && packetResult.PacketSearchSessionId != (snapshot.SearchSessionId >= 0 ? snapshot.SearchSessionId : snapshot.LocalSearchRequestId))
            {
                return false;
            }
        ...
        private bool TryValidatePacketOwnedWishlistResultSession(WishlistSearchResult packetResult, out string message)
        {
            ...
            AdminShopPacketOwnedWishlistSearchSnapshot snapshot = _packetOwnedWishlistSearchSnapshot;
            int liveSearchSessionId = snapshot?.SearchSessionId >= 0
                ? snapshot.SearchSessionId
        ...
        public string GetWishlistSearchServiceStateSignature()
        {
            ...
            return _packetOwnedAdminShopSession.HasObservableState
                ? string.Concat(
                    catalogSignature,
                    "|",
                    _packetOwnedAdminShopSession.BuildWishlistSearchSessionSignature(),
                    "|wlsr:",
                    _packetOwnedWishlistSearchStateToken.ToString(CultureInfo.InvariantCulture),
                    ":",
                    _packetOwnedWishlistSearchSnapshotSignature)
                : catalogSignature;
        }

        public string GetWishlistSearchServiceStateSummary()
        {
            string contractSummary = _packetOwnedAdminShopSession.BuildWishlistSearchStateSummary();
            if (_packetOwnedWishlistSearchSnapshot == null)
            {
                return contractSummary;
            }

            string wishlistSummary = BuildPacketOwnedWishlistSearchSnapshotSummary();
            return string.IsNullOrWhiteSpace(contractSummary)
                ? wishlistSummary
                : string.Concat(contractSummary, " | ", wishlistSummary);
        }

        public IReadOnlyList<string> GetWishlistSearchServiceStateDetailLines()
        {
            List<string> details = _packetOwnedAdminShopSession.BuildWishlistSearchStateDetailLines().ToList();
            if (_packetOwnedWishlistSearchSnapshot != null)
            {
                details.Add(BuildPacketOwnedWishlistSearchSnapshotSummary());
            }
        ...
        public int GetPacketOwnedWishlistSearchRemotePageIndex(string query, string categoryKey, int priceRangeIndex)
        {
            ...
            if (_packetOwnedAdminShopSession.IsActive
                ...
                && MatchesPacketOwnedWishlistSearchSnapshot(snapshot, query, categoryKey, priceRangeIndex))
            {
                return snapshot.RemotePageIndex;
            }

            return MatchesPendingPacketOwnedWishlistSearchRequest(query, categoryKey, priceRangeIndex)
                ? _packetOwnedWishlistPendingSearchRemotePageIndex
                : -1;
        }
        ...
        public int GetPacketOwnedWishlistSearchRemotePageCount(string query, string categoryKey, int priceRangeIndex)
        {
            ...
            if (_packetOwnedAdminShopSession.IsActive
                ...
                && MatchesPacketOwnedWishlistSearchSnapshot(snapshot, query, categoryKey, priceRangeIndex))
            {
                return ResolvePacketOwnedWishlistRemotePageCount(snapshot);
            }
            ...
            out string message)
        {
            ...
            _packetOwnedWishlistPendingSearchRemotePageCount = remotePageCount;
            ClearPacketOwnedWishlistSearchSnapshot();
            _packetOwnedAdminShopSession.RecordPendingWishlistSearch(
                _packetOwnedWishlistPendingSearchRequestId,
                _packetOwnedWishlistPendingSearchQuery,
        ...
        private bool MatchesPendingPacketOwnedWishlistSearchRequest(string query, string categoryKey, int priceRangeIndex)
        {
            if (!_packetOwnedAdminShopSession.IsActive
                || _packetOwnedWishlistPendingSearchRequestId < 0
                || _packetOwnedWishlistPendingSearchRemotePageIndex < 0
                || _packetOwnedWishlistPendingSearchRemotePageCount <= 0)
            {
                return false;
            }

            string normalizedQuery = BuildClientWishlistSearchQuery(query ?? string.Empty);
            string pendingQuery = BuildClientWishlistSearchQuery(_packetOwnedWishlistPendingSearchQuery ?? string.Empty);
            if (string.IsNullOrWhiteSpace(normalizedQuery)
                || !string.Equals(normalizedQuery, pendingQuery, StringComparison.OrdinalIgnoreCase))
            {
                return false;
            }

            string normalizedCategoryKey = string.IsNullOrWhiteSpace(categoryKey) ? "all" : categoryKey.Trim();
            string pendingCategoryKey = string.IsNullOrWhiteSpace(_packetOwnedWishlistPendingSearchCategoryKey)
                ? "all"
                : _packetOwnedWishlistPendingSearchCategoryKey.Trim();
            return string.Equals(normalizedCategoryKey, pendingCategoryKey, StringComparison.OrdinalIgnoreCase)
                   && _packetOwnedWishlistPendingSearchPriceRangeIndex == Math.Max(-1, priceRangeIndex);
        }

        public bool TrySubmitWishlistSearch(string query, int categoryIndex, out string message)
        {
            StagePacketOwnedWishlistSearchRequest(query, GetWishlistCategoryKey(categoryIndex), 0);
        ...
        public static bool TryResolveBestCommoditySerialNumberForItem(IRuntimeAssetSource assets, int itemId, out int commoditySerialNumber, out long price)
        {
            ArgumentNullException.ThrowIfNull(assets);
            commoditySerialNumber = 0;
            price = 0;
            if (itemId <= 0) return false;
            WzImage image = assets.FindImage("Etc", "Commodity.img");
            image?.ParseImage();
            if (image == null) return false;
            AdminShopCommodityData best = null;
            foreach (WzImageProperty property in image.WzProperties)
            {
                if (property is not WzSubProperty entry
                    || !TryGetIntProperty(entry, "SN", out int serial)
                    || !TryGetIntProperty(entry, "ItemId", out int entryId)
                    || entryId != itemId
                    || !TryGetIntProperty(entry, "Price", out int entryPrice)) continue;
                AdminShopCommodityData candidate = new()
                {
                    SerialNumber = serial,
                    ItemId = entryId,
                    Price = Math.Max(0, entryPrice),
                    Count = TryGetIntProperty(entry, "Count", out int count) ? Math.Max(1, count) : 1,
                    Priority = TryGetIntProperty(entry, "Priority", out int priority) ? priority : 0,
                    PeriodDays = TryGetIntProperty(entry, "Period", out int days) ? days : 0,
                    Gender = TryGetIntProperty(entry, "Gender", out int gender) ? AdminShopPacketOwnedSellTemplateParity.NormalizeCommodityGender(gender) : 2,
                    OnSale = TryGetIntProperty(entry, "OnSale", out int onSale) && onSale != 0
                };
                if (best == null || IsPreferredCommodity(candidate, best)) best = candidate;
            }
            if (best == null) return false;
            commoditySerialNumber = Math.Max(0, best.SerialNumber);
            price = Math.Max(0L, best.Price);
            return commoditySerialNumber > 0;
        }
        public bool TryResolveBestCommoditySerialNumberForItem(int itemId, out int commoditySerialNumber, out long price)
        {
            commoditySerialNumber = 0;
            price = 0;
        ...
        public static bool TryResolveCommodityBySerialNumber(IRuntimeAssetSource assets, int commoditySerialNumber, out int itemId, out long price, out int count, out bool onSale)
        {
            ArgumentNullException.ThrowIfNull(assets);
            itemId = 0;
            price = 0;
            count = 0;
            onSale = false;
            if (commoditySerialNumber <= 0) return false;
            WzImage image = assets.FindImage("Etc", "Commodity.img");
            image?.ParseImage();
            if (image == null) return false;
            foreach (WzImageProperty property in image.WzProperties)
            {
                if (property is not WzSubProperty entry
                    || !TryGetIntProperty(entry, "SN", out int serial)
                    || serial != commoditySerialNumber
                    || !TryGetIntProperty(entry, "ItemId", out int entryId)
                    || !TryGetIntProperty(entry, "Price", out int entryPrice)) continue;
                itemId = Math.Max(0, entryId);
                price = Math.Max(0, entryPrice);
                count = TryGetIntProperty(entry, "Count", out int entryCount) ? Math.Max(1, entryCount) : 1;
                onSale = TryGetIntProperty(entry, "OnSale", out int entrySale) && entrySale != 0;
            }
            return itemId > 0;
        }
        public bool TryResolveCommodityBySerialNumber(int commoditySerialNumber, out int itemId, out long price, out int count, out bool onSale)
        {
            itemId = 0;
            price = 0;
            count = 0;
        ...
        public IReadOnlyList<CommodityPurchaseVariantSnapshot> GetCommodityPurchaseVariants(int commoditySerialNumber)
        {
        ...
        {
            Texture2D texture = GraphicsResourceCreationScope.Register(new Texture2D(device, PaneWidth, PaneRowHeight));
        ...
        private Texture2D LoadItemIconTexture(int itemId)
        {
            ...
            WzImage itemImage = services.Assets.FindImage(category, imagePath);
            if (itemImage == null)
            {
                return null;
            }
        ...
        private void ApplyCommodityMetadata(AdminShopEntry entry, AdminShopCommodityData commodity)
        {
            ...
            if (InventoryItemMetadataResolver.TryResolveMaxStackForItem(entry.RewardItemId, services.Assets, out int maxStackSize))
            {
        ...
        private bool TryBuildPacketOwnedWishlistSearchResults(
            ...
            out List<WishlistSearchResult> results,
            out string summary)
        {
            results = new List<WishlistSearchResult>();
            summary = string.Empty;
        ...
        private bool IsWishlistCategoryKeyWithinRequestedCategory(
            ...
            int fallbackItemId)
        {
            ...
            int searchSessionId = snapshot?.SearchSessionId >= 0
                ? snapshot.SearchSessionId
                : snapshot?.LocalSearchRequestId ?? -1;
            ...
            int priceRangeIndex)
        {
            ...
            string normalizedQuery = BuildClientWishlistSearchQuery(query ?? string.Empty);
            string normalizedSnapshotQuery = BuildClientWishlistSearchQuery(snapshot.Query ?? string.Empty);
            if (!string.IsNullOrWhiteSpace(normalizedSnapshotQuery)
                && !string.Equals(normalizedSnapshotQuery, normalizedQuery, StringComparison.OrdinalIgnoreCase))
            {
                return false;
            }
            ...
            return snapshot.PriceRangeIndex < 0
                   || snapshot.PriceRangeIndex == priceRangeIndex;
        }

        private static int ResolveWishlistResultItemId(AdminShopEntry entry)
        {
            if (entry == null)
            {
                return 0;
        ...
        private bool IsClientWishlistScannableEntry(AdminShopEntry entry)
        {
            if (entry == null)
            {
                return false;
        ...
        private bool IsClientWishlistScannableItem(int itemId)
        {
        ...
        private HashSet<int> GetWishlistScanBlockedItemIds()
        {
            ...
            {
                _wishlistScanBlockedItemIds ??= ShopScannerWindow.LoadScanBlockedItemIds(services.Assets);
                return _wishlistScanBlockedItemIds;
            }
        }

        private static bool ArePacketOwnedWishlistSnapshotsEquivalent(
            AdminShopPacketOwnedWishlistSearchSnapshot left,
            AdminShopPacketOwnedWishlistSearchSnapshot right)
        {
            ...
            if (left.ServiceSessionId != right.ServiceSessionId
            ...
            IReadOnlyList<int> leftItemIds = left.ItemIds ?? Array.Empty<int>();
            IReadOnlyList<int> rightItemIds = right.ItemIds ?? Array.Empty<int>();
            if (leftItemIds.Count != rightItemIds.Count)
            {
                return false;
            }

            for (int i = 0; i < leftItemIds.Count; i++)
            {
                if (leftItemIds[i] != rightItemIds[i])
                {
                    return false;
                }
            }
        ...
        private string BuildPacketOwnedWishlistSearchSnapshotSummary()
        {
            ...
            int effectiveSearchSessionId = snapshot.SearchSessionId >= 0
                ...
                : snapshot.LocalSearchRequestId;
            string sessionLabel = snapshot.ServiceSessionId >= 0 || effectiveSearchSessionId >= 0
                ? $"session {Math.Max(-1, snapshot.ServiceSessionId).ToString(CultureInfo.InvariantCulture)}/{Math.Max(-1, effectiveSearchSessionId).ToString(CultureInfo.InvariantCulture)}"
                : "session unresolved";
            string queryLabel = string.IsNullOrWhiteSpace(snapshot.Query) ? "query <none>" : $"query \"{snapshot.Query}\"";
            string categoryLabel = string.IsNullOrWhiteSpace(snapshot.CategoryKey) ? "category all" : $"category {snapshot.CategoryKey}";
            string priceBandLabel = snapshot.PriceRangeIndex < 0 ? "price-band any" : $"price-band {snapshot.PriceRangeIndex.ToString(CultureInfo.InvariantCulture)}";
        ...
        private int ScoreWishlistEntry(AdminShopEntry entry, string query)
        {
        ...
        private bool MatchesClientWishlistItemNameSearch(AdminShopEntry entry, string clientSearchQuery)
        {
        ...
        private string BuildClientWishlistItemNameText(AdminShopEntry entry)
        {
        ...
        private List<string> BuildClientWishlistItemNameTerms(AdminShopEntry entry)
        {
            if (entry == null)
            {
        ...
        private string BuildWishlistAggregateSearchText(AdminShopEntry entry, out IReadOnlyList<string> indexTerms)
        {
        ...
        private AdminShopEntry CreateInventoryBackedUserEntry(AdminShopEntry catalogEntry, InventorySlotData slot, int slotIndex)
        {
        ...
        private bool MatchesWishlistCategory(AdminShopEntry entry, string categoryKey)
        {
            if (entry == null)
            {
                return false;
        ...
        private IReadOnlyList<WishlistPriceRange> BuildWishlistPriceRanges()
        {
            ...
            WzImage cashShopImage = services.Assets.FindImage("UI", "CashShop.img");
        ...
        private bool TryGetWishlistSearchIndexEntry(AdminShopEntry entry, out WishlistSearchIndexEntry searchIndexEntry)
        {
        ...
        private int ResolveWishlistSearchClientListOrder(AdminShopEntry entry)
        {
        ...
        private IReadOnlyDictionary<int, WishlistSearchIndexEntry> GetWishlistSearchIndexByItemId()
        {
            ...
            lock (WishlistSearchIndexLock)
            {
                ...
                WzImage searchImage = services.Assets.FindImage("String", "CashItemSearch.img");
        ...
        private void EnsureCommodityCache()
        {
            ...
            lock (CommodityCacheLock)
            {
                ...
                WzImage commodityImage = services.Assets.FindImage("Etc", "Commodity.img");
        ...
        private bool TryGetBestCommodityForItem(int itemId, out AdminShopCommodityData commodity)
        {
            commodity = null;
            EnsureCommodityCache();
        ...
        private bool TryGetCommodityBySerialNumber(int serialNumber, out AdminShopCommodityData commodity)
        {
            commodity = null;
            EnsureCommodityCache();
        ...
        private bool TryResolvePacketOwnedCommodityMetadata(
        ...
        private bool TryValidateInventorySourceRequest(AdminShopEntry entry, int requestQuantity, out string message)
        {
            ...
            if (InventoryItemMetadataResolver.TryResolveTradeRestrictionFlags(entry.SourceItemId, services.Assets, out bool isCashItem, out bool isNotForSale, out bool isQuestItem)
        ...
        private string BuildTradeRestrictedSourceItemMessage(AdminShopEntry entry, bool isCashItem, bool isNotForSale, bool isQuestItem)
        {
        ...
        private string ResolveSourceItemLabel(AdminShopEntry entry, int? overrideQuantity = null)
        {
            int quantity = Math.Max(1, overrideQuantity ?? entry?.SourceItemQuantity ?? 1);
            int itemId = entry?.SourceItemId ?? 0;
            string itemName = InventoryItemMetadataResolver.TryResolveItemName(itemId, services.Catalog, out string resolvedName)
                ? resolvedName
        ...
        private int ResolveRewardMaxStack(AdminShopEntry entry)
        {
            ...
            if (entry.RewardItemId > 0
                && InventoryItemMetadataResolver.TryResolveMaxStackForItem(entry.RewardItemId, services.Assets, out int maxStackSize))
            {
                entry.RewardMaxStackSize = maxStackSize;
                return maxStackSize;
            }
        ...
        private AdminShopEntry CreateSyntheticCommodityEntry(AdminShopCommodityData commodity)
        {
            ...
            string title = InventoryItemMetadataResolver.TryResolveItemName(commodity.ItemId, services.Catalog, out string itemName)
                ? itemName
                : $"Commodity SN {commodity.SerialNumber}";
            string detail = InventoryItemMetadataResolver.TryResolveItemDescription(commodity.ItemId, services.Catalog, out string description)
                ? description
            ...
            int rewardMaxStackSize = InventoryItemMetadataResolver.TryResolveMaxStackForItem(commodity.ItemId, services.Assets, out int resolvedMaxStack)
                ? resolvedMaxStack
        ...
        private AdminShopEntry CreatePacketOwnedCommodityEntry(PacketOwnedAdminShopCommoditySnapshot commodity, int packetRowOrdinal = -1)
        {
            ...
            string title = InventoryItemMetadataResolver.TryResolveItemName(resolvedItemId, services.Catalog, out string itemName)
                ? itemName
                : $"Item {resolvedItemId.ToString(CultureInfo.InvariantCulture)}";
            string detail = InventoryItemMetadataResolver.TryResolveItemDescription(resolvedItemId, services.Catalog, out string description)
                ? description
            ...
            int rewardMaxStackSize = InventoryItemMetadataResolver.TryResolveMaxStackForItem(resolvedItemId, services.Assets, out int resolvedMaxStack)
                ? resolvedMaxStack
                : InventoryItemMetadataResolver.ResolveMaxStack(inventoryType);
