...
namespace HaCreator.MapSimulator.UI
{
    internal sealed class GuildMarkWindow : UIWindowBase
    {
        ...
        private readonly UILoaderResourceCache _resourceCache;
            ...
            : base(CreateFrame(backgroundFrames, device))
        {
            ...
            _pixel = GraphicsResourceCreationScope.Register(new Texture2D(_device, 1, 1));
        ...
        private void DrawGuildMarkPreview(SpriteBatch sprite)
        {
            ...
            Texture2D backgroundTexture = GuildMarkTextureCache.GetBackgroundTexture(_device, _runtimeAssets, _resourceCache, _snapshot.MarkBackground, _snapshot.MarkBackgroundColor);
            Texture2D markTexture = GuildMarkTextureCache.GetMarkTexture(_device, _runtimeAssets, _resourceCache, _snapshot.Mark, _snapshot.MarkColor);
        ...
        private static IDXObject CreateFrame(Texture2D[] frames, GraphicsDevice device)
        {
            Texture2D texture = frames != null && frames.Length > 0 && frames[0] != null
                ? frames[0]
                : GraphicsResourceCreationScope.Register(new Texture2D(device, 250, 343));
