...
namespace HaCreator.MapSimulator.UI
{
    ...
    {
        private readonly IRuntimeDataServices _runtimeServices;
        ...
        private static readonly Regex RichTextTokenRegex = new(
            @"(\{\{ITEMICON:\d+\}\}|\{\{UICANVAS:[^}]+\}\}|\{\{QUESTSURFACE:[^}]+\}\}|\{\{QUESTSTYLE:[^}]+\}\}|\{\{QUESTREF:[^}]+\}\}|\{\{QUESTFONT:[^}]+\}\}|\{\{QUESTFONTRESET\}\}|\{\{QUESTFONTSIZE:-?\d+(?:\.\d+)?\}\}|\{\{QUESTFONTSIZERESET\}\}|\r?\n|\s+)",
        ...
        public QuestDetailWindow(IDXObject frame, string windowName, IRuntimeDataServices runtimeServices)
            : base(frame)
        {
        ...
        {
            _pixel ??= GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));
        ...
        {
            IReadOnlyList<QuestDetailCtEntry> summaryEntries = GetSummaryCtEntries();
        ...
        {
            return GetSummaryCtEntries().Count > 0;
        }

        private float DrawCtEntry(SpriteBatch sprite, Rectangle clipRect, QuestDetailCtEntry entry, float y)
        {
            if (entry == null)
            {
                return y;
        ...
        private float DrawCtEntryBody(SpriteBatch sprite, Rectangle clipRect, QuestDetailCtEntry entry, float y)
        {
            ...
            switch (entry.Kind)
            {
                case QuestDetailCtEntryKind.SectionHeader:
                {
                    Texture2D headerTexture = ResolveCtEntryHeaderTexture(entry);
                    DrawSectionHeaderClipped(
                        sprite,
                        clipRect,
                        headerTexture,
                        ResolveCtEntryHeaderFallbackText(entry),
                        x,
                        ref y,
                        ClientDetailScale);
                    break;
                ...
                {
                    bool preserveClientLineLayout = UsesClientCtVerbatimLineLayout(entry);
                    y = DrawRichTextClipped(
                        sprite,
                        entry.Text,
                        new Vector2(x, y),
        ...
        private float AdvanceCtEntryBody(QuestDetailCtEntry entry, float y)
        {
            ...
            switch (entry.Kind)
            {
                ...
                {
                    Texture2D headerTexture = ResolveCtEntryHeaderTexture(entry);
                    y = AdvanceSectionHeader(headerTexture, y);
                    break;
                }
                case QuestDetailCtEntryKind.RichText:
                {
        ...
        {
            return entry != null && (entry.RowIndex.HasValue || entry.RowHeight > 0);
        }
        ...
        private Texture2D ResolveCtEntryHeaderTexture(QuestDetailCtEntry entry)
        {
            ...
            if (!string.IsNullOrWhiteSpace(entry.HeaderSurfaceKey))
            {
                return ResolveQuestSurfaceTexture(entry.HeaderSurfaceKey);
            }

            return entry.Palette switch
            {
                QuestDetailCtEntryPalette.Requirement => _requirementHeaderTexture,
                QuestDetailCtEntryPalette.Reward => _rewardHeaderTexture,
                _ => _summaryHeaderTexture
            };
        }

        private static string ResolveCtEntryHeaderFallbackText(QuestDetailCtEntry entry)
        {
            if (entry == null)
            {
                return string.Empty;
            ...
            if (!string.IsNullOrWhiteSpace(entry.HeaderFallbackText))
            {
                return entry.HeaderFallbackText;
            }

            return entry.Palette switch
            {
                QuestDetailCtEntryPalette.Requirement => "Requirements",
                QuestDetailCtEntryPalette.Reward => "Rewards",
                _ => "Summary"
            };
        }

        private static Color ResolveCtEntryPaletteColor(QuestDetailCtEntryPalette palette, bool rewardSection)
        {
            return palette switch
            {
                QuestDetailCtEntryPalette.Requirement => new Color(215, 228, 215),
                QuestDetailCtEntryPalette.Reward => new Color(232, 220, 176),
                QuestDetailCtEntryPalette.Hint => new Color(243, 227, 168),
        ...
        {
            if (_state?.LogCtEntries != null && _state.LogCtEntries.Count > 0)
            {
                return _state.LogCtEntries;
            }

            return BuildFallbackLogCtEntries();
        }

        private IReadOnlyList<QuestDetailCtEntry> GetSummaryCtEntries()
        {
            if (_state?.SummaryCtEntries != null && _state.SummaryCtEntries.Count > 0)
            {
                return _state.SummaryCtEntries;
            }

            return BuildFallbackSummaryCtEntries();
        }

        private IReadOnlyList<QuestDetailCtEntry> BuildFallbackLogCtEntries()
        {
            if (_state == null)
            {
            ...
            if (HasRequirementContent())
            {
                ...
                if (!string.IsNullOrWhiteSpace(_state.RequirementText))
                {
                    entries.Add(new QuestDetailCtEntry
                    {
                        ...
                        Text = _state.RequirementText,
                        Palette = QuestDetailCtEntryPalette.Requirement,
                        Source = QuestDetailInlineReferenceSource.RequirementText,
                        XOffset = (int)ClientTextArrayX,
                        VerticalGapAfter = _state.RequirementLines != null && _state.RequirementLines.Count > 0 ? (int)ConditionSectionBodyGap : 0
                    });
                }
            ...
            if (HasRewardContent())
            {
                ...
                if (!string.IsNullOrWhiteSpace(_state.RewardText))
                {
                    entries.Add(new QuestDetailCtEntry
                    {
                        Section = QuestDetailCtSection.Log,
                        Kind = QuestDetailCtEntryKind.RichText,
                        Text = _state.RewardText,
                        Palette = QuestDetailCtEntryPalette.Reward,
                        Source = QuestDetailInlineReferenceSource.RewardText,
                        XOffset = (int)ClientTextArrayX,
                        RewardSection = true,
                        VerticalGapAfter = _state.RewardLines != null && _state.RewardLines.Count > 0 ? (int)ConditionSectionBodyGap : 0
                    });
                }
        ...
        private IReadOnlyList<QuestDetailCtEntry> BuildFallbackSummaryCtEntries()
        {
            if (_state == null || (string.IsNullOrWhiteSpace(_state.SummaryText) && _state.TotalProgress <= 0))
            {
                return Array.Empty<QuestDetailCtEntry>();
            }
            ...
            if (!string.IsNullOrWhiteSpace(_state.SummaryText))
            {
                entries.Add(new QuestDetailCtEntry
                {
                    Section = QuestDetailCtSection.Summary,
                    Kind = QuestDetailCtEntryKind.RichText,
                    Text = _state.SummaryText,
                    Palette = QuestDetailCtEntryPalette.Summary,
                    Source = QuestDetailInlineReferenceSource.SummaryText,
                    XOffset = (int)ClientTextArrayX,
                    VerticalGapAfter = _state.TotalProgress > 0 ? 8 : 0
                });
            }
        ...
        private float DrawRichTextClipped(
            ...
            bool preserveClientLineLayout = false)
        {
            ...
            bool preserveClientLineLayout = false)
        {
            ...
            foreach (RichTextToken token in EnumerateRichTextTokens(text, scale))
            {
                ...
                {
                    if (!preserveClientLineLayout && !lineHasContent)
                    {
                        continue;
                    }

                    if (!preserveClientLineLayout && (currentX - lineStartX) + measuredToken.X > maxWidth)
                    {
                        currentY += lineHeight;
                        currentX = lineStartX;
                        lineHeight = baselineHeight;
                ...
                else if (!preserveClientLineLayout && lineHasContent && (currentX - lineStartX) + measuredToken.X > maxWidth)
                {
                    currentY += lineHeight;
                    currentX = lineStartX;
                    lineHeight = baselineHeight;
        ...
        private bool TryCreateMarkerToken(string value, out RichTextToken token)
        {
            ...
            if (TryParseQuestFontResetMarker(value))
            {
                token = RichTextToken.FontResetToken();
                return true;
            }
            ...
            if (TryParseQuestFontSizeResetMarker(value))
            {
                token = RichTextToken.FontSizeResetToken();
                return true;
            }
        ...
        private static bool TryParseQuestFontResetMarker(string value)
        {
            return string.Equals(value?.Trim(), "{{QUESTFONTRESET}}", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static bool TryParseQuestFontSizeResetMarker(string value)
        {
            return string.Equals(value?.Trim(), "{{QUESTFONTSIZERESET}}", StringComparison.OrdinalIgnoreCase);
        }
        ...
        internal static bool TryParseQuestFontResetMarkerForTesting(string value)
        {
            return TryParseQuestFontResetMarker(value);
        }
        ...
        internal static bool TryParseQuestFontSizeResetMarkerForTesting(string value)
        {
            return TryParseQuestFontSizeResetMarker(value);
        }
            ...
            bool preserveClientLineLayout = false)
        {
            ...
            LayoutRichText(
                ...
                (token, drawPosition, measuredToken, _) =>
                {
                    ...
                    if (bounds.Contains(mouseX, mouseY) && bounds.Intersects(clipRect))
                    {
                        QuestDetailInlineReference inlineReference = token.InlineReference.Value;
                        if (inlineReference.Source == QuestDetailInlineReferenceSource.Unknown)
                        {
                            inlineReference = new QuestDetailInlineReference(
                                inlineReference.Kind,
                                inlineReference.TargetId,
        ...
        private HoveredQuestItemInfo TryResolveHoveredConditionItem(int mouseX, int mouseY, Rectangle clipRect, IReadOnlyList<QuestLogLineSnapshot> lines, float x, ref float y, float maxWidth, bool rewardSection)
        {
            ...
            QuestDetailInlineReferenceSource source = rewardSection
                ? QuestDetailInlineReferenceSource.RewardLine
            ...
            foreach (QuestLogLineSnapshot line in lines.Where(line => line != null))
            {
                ...
                {
                    return CreateHoveredQuestItem(line.ItemId.Value, line.Text, line.ItemQuantity, source);
                }
        ...
        private QuestDetailInlineReference? TryResolveHoveredConditionReference(int mouseX, int mouseY, Rectangle clipRect, IReadOnlyList<QuestLogLineSnapshot> lines, float x, ref float y, float maxWidth, bool rewardSection)
        {
            if (lines == null || lines.Count == 0)
            {
                return null;
            ...
            {
                ConditionRowLayout layout = BuildConditionRowLayout(line, x, y, maxWidth, rewardSection);
                QuestDetailInlineReference? inlineReference = TryResolveHoveredRichTextReference(
                    mouseX,
                    mouseY,
                    clipRect,
                    line.Text,
        ...
        private bool TryResolveInlineUiCanvasProperty(string uiCanvasPath, out WzCanvasProperty canvasProperty)
        {
            canvasProperty = null;
            if (string.IsNullOrWhiteSpace(uiCanvasPath))
            {
            ...
            WzObject current = _runtimeServices.Assets.FindObject(pathSegments[0], pathSegments[1]);
            if (current == null)
            {
                return false;
            }
        ...
        private HoveredQuestItemInfo CreateHoveredQuestItem(
            int itemId,
            string lineText,
            int? quantity,
            QuestDetailInlineReferenceSource source = QuestDetailInlineReferenceSource.Unknown)
        ...
        private Texture2D ResolveItemIcon(int itemId)
        {
            ...
            MapleLib.WzLib.WzImage itemImage = _runtimeServices.Assets.FindImage(category, imagePath);
        ...
        private string ResolveItemName(int itemId)
        {
            ...
            return InventoryItemMetadataResolver.TryResolveItemName(itemId, _runtimeServices.Catalog, out string itemName)
                ? itemName
                : $"Item #{itemId}";
        }

        private string ResolveItemDescription(int itemId)
        {
            return itemId > 0 && InventoryItemMetadataResolver.TryResolveItemDescription(itemId, _runtimeServices.Catalog, out string description)
                ? description
                : string.Empty;
        }
        ...
        private void DrawHoveredItemTooltip(SpriteBatch sprite)
        {
            ...
            string quantityLine = _hoveredQuestItem.Quantity.GetValueOrDefault(0) > 0
                ...
                : string.Empty;
            int? maxStackSize = InventoryItemMetadataResolver.TryResolveMaxStackForItem(_hoveredQuestItem.ItemId, _runtimeServices.Assets, out int resolvedMaxStackSize)
        ...
        private readonly struct RichTextToken
        {
            ...
            public static RichTextToken FontResetToken()
            {
                return new RichTextToken(RichTextTokenKind.FontReset, null, null, null, 0, 0);
            }
            ...
            public static RichTextToken FontSizeResetToken()
            {
                return new RichTextToken(RichTextTokenKind.FontSizeReset, null, null, null, 0, 0);
            }
        }
