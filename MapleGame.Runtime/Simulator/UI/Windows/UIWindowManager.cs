...
namespace HaCreator.MapSimulator.UI
{
    ...
    /// </summary>
    public class UIWindowManager : IDisposable
    {
        private readonly HashSet<IDisposable> ownedGraphicsResources = new(ReferenceEqualityComparer.Instance);

        internal IDisposable BeginResourceCreationScope()
        {
            ObjectDisposedException.ThrowIf(_disposed, this);
            return HaSharedLibrary.Util.GraphicsResourceCreationScope.Begin(resource =>
            {
                lock (ownedGraphicsResources)
                {
                    if (_disposed) resource.Dispose();
                    else if (resource is not GraphicsResource graphicsResource || !graphicsResource.IsDisposed)
                    {
                        if (ownedGraphicsResources.Add(resource) && resource is GraphicsResource trackedResource)
                            trackedResource.Disposing += OnOwnedGraphicsResourceDisposing;
                    }
                }
            });
        }

        private void OnOwnedGraphicsResourceDisposing(object sender, EventArgs args)
        {
            if (sender is not GraphicsResource resource) return;
            lock (ownedGraphicsResources)
            {
                ownedGraphicsResources.Remove(resource);
                resource.Disposing -= OnOwnedGraphicsResourceDisposing;
            }
        }
        ...
        public void Dispose()
        {
            ...
            {
                SoftKeyboardWindow.BeforeShow = null;
                SoftKeyboardWindow.AfterDismiss = null;
            }
            IDisposable[] graphicsResources;
            lock (ownedGraphicsResources)
            {
                graphicsResources = new IDisposable[ownedGraphicsResources.Count];
                ownedGraphicsResources.CopyTo(graphicsResources);
                foreach (IDisposable resource in graphicsResources)
                    if (resource is GraphicsResource graphicsResource)
                        graphicsResource.Disposing -= OnOwnedGraphicsResourceDisposing;
                ownedGraphicsResources.Clear();
            }
            foreach (IDisposable resource in graphicsResources)
            {
            ...
            windows.Clear();
            windowsByName.Clear();
            lazyWindowRegistrars.Clear();
            BeforeShowWindow = null;
            BeforeToggleWindowOpen = null;
            _focusedWindow = null;
            _draggingWindow = null;
            _activeSoftKeyboardHost = null;
            _softKeyboardModalOwner = null;
            _softKeyboardPreviousFocusedWindow = null;
            if (errors.Count > 0) throw new AggregateException("UI window cleanup failed.", errors);
        }
        ...
        {
            using IDisposable resourceCreationScope = BeginResourceCreationScope();
        ...
        private bool EnsureWindowRegistered(string windowName)
        {
            ...
            using (BeginResourceCreationScope())
                registrar(this);
