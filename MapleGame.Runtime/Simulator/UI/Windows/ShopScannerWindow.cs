...
namespace HaCreator.MapSimulator.UI
{
    public sealed class ShopScannerWindow : UIWindowBase, ISoftKeyboardHost
    {
        ...
        private readonly object ScannerIndexLock = new();
        private IReadOnlyList<ScannerIndexEntry> _scannerIndex;
        ...
        private string ResolveScannerItemName(int itemId)
        {
            if (itemId <= 0)
            {
                return string.Empty;
            ...
            IReadOnlyDictionary<int, RuntimeItemName> cache = services.Catalog.GetItemNames();
            return cache != null && cache.TryGetValue(itemId, out RuntimeItemName value)
        ...
        private IReadOnlyList<ScannerIndexEntry> GetScannerIndex()
        {
            ...
            lock (ScannerIndexLock)
            {
                ...
                HashSet<int> scanBlockedItemIds = LoadScanBlockedItemIds(services.Assets);
                List<ScannerIndexEntry> entries = new();
                int clientListOrder = 0;
                foreach (KeyValuePair<int, RuntimeItemName> itemInfo in EnumerateItemNameCache())
                {
                    int itemId = itemInfo.Key;
                    string itemName = itemInfo.Value?.Name?.Trim();
        ...
        private IEnumerable<KeyValuePair<int, RuntimeItemName>> EnumerateItemNameCache()
        {
            IReadOnlyDictionary<int, RuntimeItemName> cache = services.Catalog.GetItemNames();
            ...
            foreach (KeyValuePair<int, RuntimeItemName> pair in cache.OrderBy(pair => pair.Key))
            {
                yield return pair;
            }
        }

        internal static HashSet<int> LoadScanBlockedItemIds(HaCreator.MapSimulator.Assets.IRuntimeAssetSource assets)
        {
            HashSet<int> blocked = new();
            WzImage scanBlockImage = assets.FindImage("Etc", "ScanBlock.img")
                ?? assets.FindImage("etc", "ScanBlock.img");
