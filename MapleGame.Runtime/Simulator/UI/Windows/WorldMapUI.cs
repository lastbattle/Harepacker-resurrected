...
namespace HaCreator.MapSimulator.UI
{
    public sealed class WorldMapUI : UIWindowBase, ISoftKeyboardHost
    {
        ...
        private const int CompositeControlWidth = ListStartX + ListWidth;
        private const int CompositeControlHeight = 529;
            ...
            : base(frame)
        {
            ...
            _searchInputBackgroundTexture = GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));
            _searchInputBackgroundTexture.SetData(new[] { Color.White });
            _searchInputOutlineTexture = GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));
            _searchInputOutlineTexture.SetData(new[] { Color.White });
            _caretTexture = GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));
        ...
        public void PlaceWithinViewport(int viewportWidth, int viewportHeight)
        {
            Rectangle compositeBounds = MeasureCompositeBounds();
            Position = ResolveViewportPosition(
                viewportWidth,
                viewportHeight,
        ...
        private Rectangle MeasureCompositeBounds()
        {
            int left = 0;
            int top = 0;
            int right = Math.Max(CompositeControlWidth, CurrentFrame?.Width ?? 0);
            int bottom = Math.Max(CompositeControlHeight, CurrentFrame?.Height ?? 0);

            if (TryResolveActiveSurface(out WorldMapSurfaceDefinition surface) && surface?.BaseTexture != null)
            {
                right = Math.Max(right, surface.BaseTexture.Width);
                bottom = Math.Max(bottom, surface.BaseTexture.Height);
            }

            if (_sidePanelTexture != null)
            {
                left = Math.Min(left, _sidePanelOffset.X);
                top = Math.Min(top, _sidePanelOffset.Y);
                right = Math.Max(right, _sidePanelOffset.X + _sidePanelTexture.Width);
                bottom = Math.Max(bottom, _sidePanelOffset.Y + _sidePanelTexture.Height);
            }
        ...
        internal static Point ResolveViewportPosition(
            int viewportWidth,
            int viewportHeight,
            int compositeWidth,
            int compositeHeight,
            ...
            int compositeTop = 0)
        {
            int availableWidth = Math.Max(0, viewportWidth);
            int availableHeight = Math.Max(0, viewportHeight);
            int width = Math.Max(1, compositeWidth);
            int height = Math.Max(1, compositeHeight);
            int visibleLeft = width <= availableWidth ? (availableWidth - width) / 2 : 0;
            int visibleTop = height <= availableHeight ? (availableHeight - height) / 2 : 0;
            return new Point(visibleLeft - compositeLeft, visibleTop - compositeTop);
        }
        ...
        public void ConfigureWorldMapDocuments(
            IEnumerable<global::HaCreator.MapSimulator.WorldMap.WorldMapDocument> documents,
            Func<global::HaCreator.MapSimulator.WorldMap.WorldMapCanvasRef, Texture2D> textureResolver)
        {
            ConfigureWorldMapSurfaces(global::HaCreator.MapSimulator.WorldMap.WorldMapRuntimeAdapter
        ...
        {
            Texture2D texture = GraphicsResourceCreationScope.Register(new Texture2D(device, ListWidth, RowHeight));
        ...
        protected override IEnumerable<Rectangle> GetAdditionalInteractiveBounds()
        {
            ...
            yield return new Rectangle(
                Position.X + compositeBounds.X,
                Position.Y + compositeBounds.Y,
                compositeBounds.Width,
                compositeBounds.Height);
