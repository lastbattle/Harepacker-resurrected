...
namespace HaCreator.MapSimulator.UI
{
    public sealed partial class TrunkUI
    {
        ...
        {
            InventoryItemTooltipMetadata metadata = InventoryItemMetadataResolver.ResolveTooltipMetadata(slot.ItemId, _runtimeServices.Catalog, inventoryType);
            ...
            Texture2D infoEffectPreviewTexture = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures)
                ? null
                : ResolveInfoEffectPreviewTexture(sprite.GraphicsDevice, slot.ItemId);
            Texture2D infoPathPreviewTexture = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures)
                ...
                ? null
                : ResolveInfoPathPreviewTexture(sprite.GraphicsDevice, slot.ItemId);
            Texture2D rootEffectPreviewTexture = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures)
                ...
                ? null
                : ResolveRootEffectPreviewTexture(sprite.GraphicsDevice, slot.ItemId);
            Texture2D[] mobPreviewTextures = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures) || infoEffectPreviewTexture != null || infoPathPreviewTexture != null || rootEffectPreviewTexture != null
                ? Array.Empty<Texture2D>()
                : ResolveMobPreviewTextures(sprite.GraphicsDevice, slot.ItemId);
            Texture2D[] consumeItemPreviewTextures = HasDrawableBitmap(iconRewardTextures)
                ...
                ? Array.Empty<Texture2D>()
                : ResolveConsumeItemPreviewTextures(sprite.GraphicsDevice, slot.ItemId);
            Texture2D npcPreviewTexture = HasDrawableBitmap(iconRewardTextures)
                ...
                ? null
                : ResolveNpcPreviewTexture(sprite.GraphicsDevice, slot.ItemId);
            ...
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TooltipBitmapGap : 0f) + infoEffectPreviewHeight;
            }
            else if (infoPathPreviewHeight > 0f)
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TooltipBitmapGap : 0f) + infoPathPreviewHeight;
            }
            else if (rootEffectPreviewHeight > 0f)
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TooltipBitmapGap : 0f) + rootEffectPreviewHeight;
            }
            else if (mobPreviewHeight > 0f)
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TooltipBitmapGap : 0f) + mobPreviewHeight;
            }
            else if (consumeItemPreviewHeight > 0f)
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TooltipBitmapGap : 0f) + consumeItemPreviewHeight;
            }
            else if (npcPreviewHeight > 0f)
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TooltipBitmapGap : 0f) + npcPreviewHeight;
            }
            ...
            if (sampleTexture != null)
            {
                ...
                DrawCenteredTooltipBitmap(
                    ...
                    backgroundRect.X + Math.Max(TooltipPadding, (backgroundRect.Width - (infoEffectPreviewTexture?.Width ?? 0)) / 2),
                    sampleY + sampleTexture.Height + TooltipBitmapGap,
                    infoEffectPreviewTexture?.Width ?? 0);
                DrawCenteredTooltipBitmap(
                    sprite,
                    rootEffectPreviewTexture,
                    backgroundRect.X + Math.Max(TooltipPadding, (backgroundRect.Width - (rootEffectPreviewTexture?.Width ?? 0)) / 2),
                    sampleY + sampleTexture.Height + TooltipBitmapGap,
                    rootEffectPreviewTexture?.Width ?? 0);
                DrawCenteredTooltipBitmap(
                    sprite,
                    infoPathPreviewTexture,
                    backgroundRect.X + Math.Max(TooltipPadding, (backgroundRect.Width - (infoPathPreviewTexture?.Width ?? 0)) / 2),
                    sampleY + sampleTexture.Height + TooltipBitmapGap,
                ...
                DrawHorizontalBitmapStrip(
                    sprite,
                    consumeItemPreviewTextures,
                    backgroundRect,
                    sampleY + sampleTexture.Height + TooltipBitmapGap);
                DrawCenteredTooltipBitmap(
                    sprite,
                    npcPreviewTexture,
                    backgroundRect.X + Math.Max(TooltipPadding, (backgroundRect.Width - (npcPreviewTexture?.Width ?? 0)) / 2),
                    sampleY + sampleTexture.Height + TooltipBitmapGap,
            ...
            {
                int effectY = contentY + (int)Math.Ceiling(iconBlockHeight) + TooltipSectionGap;
                int effectX = backgroundRect.X + Math.Max(TooltipPadding, (backgroundRect.Width - infoEffectPreviewTexture.Width) / 2);
                sprite.Draw(infoEffectPreviewTexture, new Vector2(effectX, effectY), Color.White);
            }
            else if (infoPathPreviewHeight > 0f)
            {
                int effectY = contentY + (int)Math.Ceiling(iconBlockHeight) + TooltipSectionGap;
                int effectX = backgroundRect.X + Math.Max(TooltipPadding, (backgroundRect.Width - infoPathPreviewTexture.Width) / 2);
                sprite.Draw(infoPathPreviewTexture, new Vector2(effectX, effectY), Color.White);
            }
            else if (rootEffectPreviewHeight > 0f)
            {
                int effectY = contentY + (int)Math.Ceiling(iconBlockHeight) + TooltipSectionGap;
                int effectX = backgroundRect.X + Math.Max(TooltipPadding, (backgroundRect.Width - rootEffectPreviewTexture.Width) / 2);
                sprite.Draw(rootEffectPreviewTexture, new Vector2(effectX, effectY), Color.White);
            }
            else if (mobPreviewHeight > 0f)
            {
                int mobY = contentY + (int)Math.Ceiling(iconBlockHeight) + TooltipSectionGap;
                DrawHorizontalBitmapStrip(sprite, mobPreviewTextures, backgroundRect, mobY);
            }
            else if (consumeItemPreviewHeight > 0f)
            {
                int consumeItemY = contentY + (int)Math.Ceiling(iconBlockHeight) + TooltipSectionGap;
                DrawHorizontalBitmapStrip(sprite, consumeItemPreviewTextures, backgroundRect, consumeItemY);
            }
            else if (npcPreviewHeight > 0f)
            {
                int npcY = contentY + (int)Math.Ceiling(iconBlockHeight) + TooltipSectionGap;
                int npcX = backgroundRect.X + Math.Max(TooltipPadding, (backgroundRect.Width - npcPreviewTexture.Width) / 2);
                sprite.Draw(npcPreviewTexture, new Vector2(npcX, npcY), Color.White);
            }
        }
        ...
        private Texture2D ResolveSlotItemTexture(GraphicsDevice device, InventorySlotData slot)
        {
            ...
            WzImage itemImage = _runtimeServices.Assets.FindImage(category, imagePath);
            if (itemImage == null)
            {
                return null;
            }
        ...
        private Texture2D ResolveInfoSampleTexture(GraphicsDevice device, int itemId)
        {
            ...
            if (InventoryItemMetadataResolver.TryResolveInfoCanvas(itemId, "sample", _runtimeServices.Assets, out WzCanvasProperty sampleCanvas))
            {
        ...
        private Texture2D[] ResolveRewardPreviewTextures(GraphicsDevice device, int itemId)
        {
            ...
            IReadOnlyList<InventoryRewardPreviewItem> rewardItems =
                InventoryItemMetadataResolver.ResolveRewardPreviewItems(itemId, _runtimeServices.Assets, 8);
        ...
        private Texture2D ResolveRootEffectPreviewTexture(GraphicsDevice device, int itemId)
        {
            if (itemId <= 0 || device == null)
            {
                return null;
            ...
            Texture2D texture = null;
            if (InventoryItemMetadataResolver.TryResolveRootEffectFirstCanvas(itemId, _runtimeServices.Assets, out WzCanvasProperty effectCanvas))
            {
                texture = effectCanvas.GetLinkedWzCanvasBitmap()?.ToTexture2DAndDispose(device);
            }
        ...
        private Texture2D ResolveInfoPathPreviewTexture(GraphicsDevice device, int itemId)
        {
            if (itemId <= 0 || device == null)
            {
                return null;
            ...
            Texture2D texture = null;
            if (InventoryItemMetadataResolver.TryResolveInfoPathFirstCanvas(itemId, _runtimeServices.Assets, out WzCanvasProperty pathCanvas))
            {
                texture = pathCanvas.GetLinkedWzCanvasBitmap()?.ToTexture2DAndDispose(device);
            }
        ...
        private Texture2D ResolveInfoEffectPreviewTexture(GraphicsDevice device, int itemId)
        {
            if (itemId <= 0 || device == null)
            {
                return null;
            ...
            Texture2D texture = null;
            if (InventoryItemMetadataResolver.TryResolveInfoEffectFirstCanvas(itemId, _runtimeServices.Assets, out WzCanvasProperty effectCanvas))
            {
                texture = effectCanvas.GetLinkedWzCanvasBitmap()?.ToTexture2DAndDispose(device);
            }
        ...
        private Texture2D[] ResolveMobPreviewTextures(GraphicsDevice device, int itemId)
        {
            if (itemId <= 0 || device == null)
            {
                return Array.Empty<Texture2D>();
            ...
            IReadOnlyList<InventoryMobPreviewItem> mobItems =
                InventoryItemMetadataResolver.ResolveMobPreviewItems(itemId, _runtimeServices.Assets, 8);
            ...
            for (int i = 0; i < mobItems.Count; i++)
            {
                ...
                Texture2D texture = mobCanvas.GetLinkedWzCanvasBitmap()?.ToTexture2DAndDispose(device);
                if (texture != null)
                {
                    textures.Add(texture);
                }
        ...
        private Texture2D[] ResolveConsumeItemPreviewTextures(GraphicsDevice device, int itemId)
        {
            if (itemId <= 0 || device == null)
            {
                return Array.Empty<Texture2D>();
            ...
            IReadOnlyList<ConsumeItemRequirementMetadata> consumeItems =
                InventoryItemMetadataResolver.ResolveConsumeItemRequirements(itemId, _runtimeServices.Assets);
        ...
        private Texture2D ResolveNpcPreviewTexture(GraphicsDevice device, int itemId)
        {
            if (itemId <= 0 || device == null)
            {
                return null;
            ...
            Texture2D texture = null;
            if (InventoryItemMetadataResolver.TryResolveNpcPreviewCanvas(itemId, _runtimeServices.Assets, out WzCanvasProperty npcCanvas))
            {
                texture = npcCanvas.GetLinkedWzCanvasBitmap()?.ToTexture2DAndDispose(device);
            }
