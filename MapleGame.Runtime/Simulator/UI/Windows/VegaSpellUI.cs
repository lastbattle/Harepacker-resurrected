...
namespace HaCreator.MapSimulator.UI
{
    public sealed class VegaSpellUI : UIWindowBase
    {
        ...
        public VegaSpellUI(IDXObject frame, GraphicsDevice device, IRuntimeDataServices runtimeServices)
            : base(frame)
        {
            _device = device;
        ...
        private Texture2D TryResolveItemIcon(int itemId)
        {
            ...
            if (!InventoryItemMetadataResolver.TryResolveTooltipIconCanvas(itemId, _runtimeServices.Assets, out var canvas) || canvas == null)
            {
        ...
        private IReadOnlyList<KeyValuePair<EquipSlot, CharacterPart>> GetCandidates()
        {
            ...
            return CharacterBuild.Equipment
                .Where(entry => entry.Value != null &&
                                ...
                                ItemUpgradeUI.CanUpgrade(entry.Key, entry.Value, _runtimeServices.Assets) &&
                                (!_modifierItemId.HasValue ||
                                 _itemUpgradeBackend == null ||
        ...
        {
            if (_modifierItemId.HasValue &&
            InventoryItemMetadataResolver.TryResolveItemName(_modifierItemId.Value, _runtimeServices.Catalog, out string itemName) &&
        ...
        private (int baseRate, int modifiedRate) ResolveDisplayedRates()
        {
            ...
            if (_modifierItemId.HasValue &&
                ItemUpgradeUI.TryResolveVegaModifierRatePreview(_runtimeServices.Catalog, _modifierItemId.Value, out int requiredBasePercent, out int modifiedPercent))
            {
