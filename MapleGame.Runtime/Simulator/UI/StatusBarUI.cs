...
*/
        ...
        /// <summary>
        /// Returns every GPU texture owned by this composite status bar. The
        /// session resource cache uses this when retiring a UI session.
        /// </summary>
        internal IEnumerable<Texture2D> EnumerateOwnedTextures()
        {
            ...
            foreach (UIObject button in uiButtons)
                foreach (Texture2D texture in button?.EnumerateTextures() ?? Array.Empty<Texture2D>())
                    yield return texture;
            foreach (Texture2D texture in _channelButton?.EnumerateTextures() ?? Array.Empty<Texture2D>())
                yield return texture;

            yield return _pixelTexture;
            yield return _hpGaugeTexture;
            yield return _mpGaugeTexture;
            yield return _expGaugeTexture;
            foreach (Texture2D texture in _digitTextures ?? Array.Empty<Texture2D>()) yield return texture;
            foreach (Texture2D texture in _levelDigitTextures ?? Array.Empty<Texture2D>()) yield return texture;
            yield return _slashTexture;
            yield return _percentTexture;
            yield return _bracketLeftTexture;
            yield return _bracketRightTexture;
            yield return _dotTexture;
            foreach (Texture2D texture in _buffIconTextures.Values) yield return texture;
            foreach (Texture2D texture in _temporaryStatViewShadowTextures.Values) yield return texture;
            foreach (StatusBarKeyDownBarTextures value in _keyDownBarTextures.Values)
            {
                yield return value?.Bar;
                yield return value?.Gauge;
                yield return value?.Graduation;
            }
            foreach (Texture2D texture in _cooldownMaskTextures ?? Array.Empty<Texture2D>()) yield return texture;
            yield return _temporaryStatViewTexture;
            foreach (Texture2D texture in _tooltipFrames ?? Array.Empty<Texture2D>()) yield return texture;
            foreach (Texture2D texture in _hpWarningAnimation?.Frames ?? Array.Empty<Texture2D>()) yield return texture;
            foreach (Texture2D texture in _mpWarningAnimation?.Frames ?? Array.Empty<Texture2D>()) yield return texture;
        }
        #endregion
    }
