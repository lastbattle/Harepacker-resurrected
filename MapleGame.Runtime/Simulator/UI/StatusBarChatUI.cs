...
namespace HaCreator.MapSimulator.UI
{
    ...
    public class StatusBarChatUI : BaseDXDrawableItem, IUIObjectEvents
    {
        ...
        public void SetPixelTexture(GraphicsDevice graphicsDevice)
        {
            ...
            {
                _pixelTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(graphicsDevice, 1, 1));
        ...
        internal IEnumerable<Texture2D> EnumerateOwnedTextures()
        {
            foreach (Texture2D texture in base.EnumerateTextures())
                yield return texture;
            foreach (UIObject button in uiButtons)
                foreach (Texture2D texture in button?.EnumerateTextures() ?? Array.Empty<Texture2D>())
                    yield return texture;
            foreach (UIObject button in _shortcutTooltipButtons.Values)
                foreach (Texture2D texture in button?.EnumerateTextures() ?? Array.Empty<Texture2D>())
                    yield return texture;
            foreach (ChatTargetLabelPlacement value in _chatTargetLabels.Values)
                yield return value?.Texture;

            yield return _pixelTexture;
            yield return _chatEnterTexture;
            yield return _whisperPickerSelectedTexture;
            yield return _whisperPickerRowTexture;
            yield return _whisperPickerDialogTopTexture;
            yield return _whisperPickerDialogCenterTexture;
            yield return _whisperPickerDialogBottomTexture;
            yield return _whisperPickerDialogBarTexture;
            yield return _whisperPickerDialogLineTexture;
            foreach (Texture2D texture in _abilityPointNotificationAnimation?.Frames ?? Array.Empty<Texture2D>()) yield return texture;
            foreach (Texture2D texture in _skillPointNotificationAnimation?.Frames ?? Array.Empty<Texture2D>()) yield return texture;

            foreach (Texture2D texture in EnumerateButtonVisuals(_whisperPickerComboVisuals)) yield return texture;
            foreach (Texture2D texture in EnumerateButtonVisuals(_whisperPickerComboToggleVisuals)) yield return texture;
            foreach (Texture2D texture in EnumerateButtonVisuals(_whisperPickerPrevButtonVisuals)) yield return texture;
            foreach (Texture2D texture in EnumerateButtonVisuals(_whisperPickerNextButtonVisuals)) yield return texture;
            foreach (Texture2D texture in EnumerateButtonVisuals(_whisperPickerOkButtonVisuals)) yield return texture;
            foreach (Texture2D texture in EnumerateButtonVisuals(_whisperPickerCloseButtonVisuals)) yield return texture;

            if (_whisperPickerComboDropdownVisuals != null)
            {
                foreach (WhisperPickerComboDropdownStateVisuals state in new[]
                {
                    _whisperPickerComboDropdownVisuals.Normal,
                    _whisperPickerComboDropdownVisuals.Hover,
                    _whisperPickerComboDropdownVisuals.Pressed,
                    _whisperPickerComboDropdownVisuals.Disabled,
                    _whisperPickerComboDropdownVisuals.Selected
                })
                {
                    yield return state?.Left;
                    yield return state?.Center;
                    yield return state?.Right;
                }
            }
        }

        private static IEnumerable<Texture2D> EnumerateButtonVisuals(WhisperPickerButtonVisuals visuals)
        {
            if (visuals == null)
                yield break;
            yield return visuals.Normal;
            yield return visuals.Hover;
            yield return visuals.Pressed;
            yield return visuals.Disabled;
            yield return visuals.KeyFocused;
        }
    }
}
