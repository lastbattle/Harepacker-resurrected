...
namespace HaCreator.MapSimulator.UI
{
    ...
    /// </summary>
    public class UIObject : UIObjectViewModelBase, IDisposable
    {
        ...
        private Texture2D _soundLifetimeTexture;
        ...
        internal IEnumerable<Texture2D> EnumerateTextures()
        {
            foreach (BaseDXDrawableItem state in new[] { _normalState, _disabledState, _pressedState, _mouseOverState })
            {
                if (state == null)
                    continue;

                foreach (Texture2D texture in state.EnumerateTextures())
        ...
        /// <returns></returns>
        private WzSoundResourceStreamer CreateLocalSoundEffectWithWzProperty(WzBinaryProperty wzSoundProperty)
        {
            if (_disposed || wzSoundProperty == null)
            {
                return null;
            }

            Texture2D texture = _normalState?.Frame0?.Texture;
            if (texture == null || texture.IsDisposed) return null;
            if (_soundLifetimeTexture == null)
            {
                // UI textures are retired by the owning generation/session. Tie native
                // fallback audio to that same lifetime, including failed map staging.
                _soundLifetimeTexture = texture;
                texture.Disposing += OnSoundLifetimeTextureDisposing;
            }
        ...
        private void OnSoundLifetimeTextureDisposing(object sender, EventArgs args)
        {
            try { Dispose(); }
            catch (Exception error)
            {
                // A sound-device failure must not abort the texture owner's teardown.
                System.Diagnostics.Debug.WriteLine($"UI fallback audio cleanup failed: {error}");
            }
        }
        ...
        public void Dispose()
        {
            ...
            if (_soundLifetimeTexture != null)
            {
                _soundLifetimeTexture.Disposing -= OnSoundLifetimeTextureDisposing;
                _soundLifetimeTexture = null;
            }
            WzSoundResourceStreamer click = _seMouseClick;
            WzSoundResourceStreamer hover = _seMouseOver;
            _seMouseClick = null;
            _seMouseOver = null;
            try { click?.Dispose(); }
        ...
        private bool TryPlayClientSoundEffect(
            ...
            string semanticName)
        {
            if (_disposed || soundProperty == null)
            {
                return false;
            }
