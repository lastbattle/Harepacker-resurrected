...
namespace HaCreator.MapSimulator.UI
{
    ...
    public class BossHPBarUI
    {
        ...
        public void Initialize(GraphicsDevice device, SpriteFont font)
        {
            ...
            _pixelTexture = HaSharedLibrary.Util.GraphicsResourceCreationScope.Register(new Texture2D(device, 1, 1));
        ...
        public void TrackBoss(MobItem boss, int currentTime)
        {
            ...
            {
                int mobId = 0;
                int.TryParse(boss.RuntimeLife?.Id, out mobId);

                var info = new BossDisplayInfo
                {
                    Boss = boss,
                    BossName = boss.RuntimeLife?.DisplayName ?? "Boss",
                    Level = boss.AI.Level,
                    MobId = mobId,
