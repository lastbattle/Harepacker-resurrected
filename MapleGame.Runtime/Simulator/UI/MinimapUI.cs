...
namespace HaCreator.MapSimulator.UI
{
    ...
    public class MinimapUI : BaseDXDrawableItem, IUIObjectEvents
    {
            ...
            int tickCount)
        {
            ...
            {
                if (npc?.RuntimeLife == null || !npc.IsVisible)
                    continue;
            ...
            int tickCount)
        {
            ...
            for (int i = _portalMarkers.Count - 1; i >= 0; i--)
            {
                ...
                if (portal?.Portal == null || !portal.IsVisible)
                    continue;

                Point minimapPoint = WorldToMinimap(portal.Portal.X, portal.Portal.Y);
        ...
        {
            return npc?.RuntimeLife != null
                && IsClientNpcHoverCandidateForTesting(
                    npc.RuntimeLife.HideName == true,
                    npc.RuntimeLife.Hide,
                    npc.PacketEnabled);
        }
        ...
        internal IEnumerable<Texture2D> EnumerateOwnedTextures()
        {
            ...
            foreach (BaseDXDrawableItem item in new[]
            {
                _pixelDot, _expandedFrame, _collapsedFrame, _userMarker, _npcMarker,
                _questStartNpcMarker, _questEndNpcMarker, _portalMarker, _npcListPanel
            })
            {
                foreach (Texture2D texture in item?.EnumerateTextures() ?? Array.Empty<Texture2D>())
                    yield return texture;
            }
            if (_directionMarkers != null)
            {
                foreach (BaseDXDrawableItem item in _directionMarkers.Values)
                    foreach (Texture2D texture in item?.EnumerateTextures() ?? Array.Empty<Texture2D>()) yield return texture;
            }
            if (_helperMarkers != null)
            {
                foreach (BaseDXDrawableItem item in _helperMarkers.Values)
                    foreach (Texture2D texture in item?.EnumerateTextures() ?? Array.Empty<Texture2D>()) yield return texture;
            }
            foreach (UIObject button in uiButtons)
                foreach (Texture2D texture in button?.EnumerateTextures() ?? Array.Empty<Texture2D>()) yield return texture;
            foreach (Texture2D texture in _tooltipFrames ?? Array.Empty<Texture2D>()) yield return texture;
            yield return _tooltipPixelTexture;
        }
        #endregion
    }
}
