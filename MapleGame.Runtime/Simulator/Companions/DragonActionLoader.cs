...
namespace HaCreator.MapSimulator.Companions
{
    internal sealed class DragonActionLoader
    {
        ...
        public DragonActionLoader(GraphicsDevice device, IRuntimeDataServices runtimeServices = null)
        {
        ...
        private WzImage FindDragonImage(int dragonJob)
        {
            string resolvedImagePath = ResolveDragonImagePath(dragonJob);
            return TryFindImageByResolvedPath(resolvedImagePath)
                   ?? _runtimeServices?.Assets?.FindImage("Skill", $"Dragon/{dragonJob}.img")
                   ?? _runtimeServices?.Assets?.FindImage("Skill", $"{dragonJob}.img");
        }
        ...
        private WzImage TryFindImageByResolvedPath(string resolvedImagePath)
        {
            ...
            return string.IsNullOrWhiteSpace(relativeImagePath)
                ? null
                : _runtimeServices?.Assets?.FindImage("Skill", relativeImagePath);
        }
