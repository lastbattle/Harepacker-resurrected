...
namespace HaCreator.MapSimulator.Companions
{
    ...
    internal sealed class PetLoader : IDisposable
    {
        ...
        private readonly HashSet<Texture2D> _ownedTextures = new(ReferenceEqualityComparer.Instance);
        private List<IDXObject> _clientMultiPetHangFrames;
        private bool _disposed;

        public PetLoader(GraphicsDevice device, IRuntimeDataServices runtimeServices = null)
        {
        ...
        private PetImageEntry ResolvePetTemplateImgEntry(int petItemId)
        {
            string petImagePath = FormatPetImagePath(petItemId);
            WzImage petImage = _runtimeServices?.Assets?.FindImage("Item", petImagePath);
            if (petImage == null)
            {
                return null;
            }
        ...
        private PetImageEntry ResolveBasicPetEffectImgEntry()
        {
            WzImage effectImage = _runtimeServices?.Assets?.FindImage("Effect", "PetEff.img");
            if (effectImage == null)
            {
                return null;
            }
        ...
        {
            WzImage stringImage = _runtimeServices?.Assets?.FindImage("String", "Pet.img");
            if (stringImage == null)
            {
                return null;
            }
        ...
        private Dictionary<string, string> LoadPetDialogStrings(int petItemId)
        {
            WzImage petDialogImage = _runtimeServices?.Assets?.FindImage("String", "PetDialog.img");
        ...
        private WzSubProperty ResolvePetWearRoot(int petWearItemId, int petItemId)
        {
            ...
            WzImage petWearImage = _runtimeServices?.Assets?.FindImage("Character", $"PetEquip/{petWearItemId:D8}.img");
            ...
            int? resolvedDelayOverride = null)
        {
            ...
            try
            {
                ...
                var texture = OwnTexture(bitmap.ToTexture2DAndDispose(_device));
                if (texture == null)
                {
                    return null;
                }
        ...
        private IDXObject ComposePetWearFrame(PetActionFrame baseFrame, PetActionFrame overlayFrame)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    Texture2D texture = OwnTexture(composedBitmap.ToTexture2DAndDispose(_device));
                    if (texture == null)
                    {
                        return baseFrame.Frame;
                    }
        ...
        private Texture2D OwnTexture(Texture2D texture)
        {
            if (texture != null)
            {
        ...
        public void Dispose()
        {
            ...
            _cache.Clear();
            _petImgEntryCache.Clear();
            _animationSetCache.Clear();
            _actionCache.Clear();
            _clientMultiPetHangFrames = null;

            if (errors.Count > 0)
            {
                throw new AggregateException("One or more pet graphics resources failed to dispose.", errors);
            }
        }
        ...
        internal static CompositingQuality ResolveNativeCanvasCopyCompositingQualityForTesting()
        {
            return ClientNativeCanvasCopy.CompositingQuality;
        }

        internal static SmoothingMode ResolveNativeCanvasCopySmoothingModeForTesting()
        {
