...
namespace HaCreator.MapSimulator.Companions
{
    ...
    public sealed class AndroidEquipmentController
    {
        ...
        internal AndroidEquipmentController(IRuntimeDataServices runtimeServices = null)
        {
            _runtimeServices = runtimeServices;
        }

        public IReadOnlyDictionary<AndroidEquipSlot, CompanionEquipItem> EquippedItems => _equippedItems;
        public bool HasOwnerState => CompanionEquipmentController.HasAndroidOwnerState(_ownerBuild, _runtimeServices);
            ...
            out string rejectReason)
        {
            ...
            if (!CompanionEquipmentController.TryValidateAndroidOwnerState(_ownerBuild, _runtimeServices, out rejectReason))
            {
                return false;
            }
    ...
    public sealed class CompanionEquipmentController
    {
        ...
        public CompanionEquipmentController(GraphicsDevice device, IRuntimeDataServices runtimeServices = null)
        {
            _runtimeServices = runtimeServices;
            var loader = new CompanionEquipmentLoader(device, runtimeServices);
            Pet = new PetEquipmentController(loader);
            Dragon = new DragonEquipmentController(loader);
            Android = new AndroidEquipmentController(runtimeServices);
        ...
        private static IReadOnlyList<AndroidHeartRequirement> ParseSupportedAndroidHeartRequirements(
            string description,
            IRuntimeDataServices runtimeServices = null)
        {
            ...
            Dictionary<string, IReadOnlyCollection<int>> heartCatalog = GetAndroidHeartItemIdsByNormalizedName(runtimeServices);
        ...
        {
            return TryValidateAndroidOwnerState(build, null, out rejectReason);
        }

        internal static bool TryValidateAndroidOwnerState(
            ...
            out string rejectReason)
        {
            ...
            return CanPowerAndroid(androidPart, heartPart, runtimeServices, out rejectReason);
        }
        ...
        internal static bool HasAndroidOwnerState(CharacterBuild build, IRuntimeDataServices runtimeServices)
        {
            return TryValidateAndroidOwnerState(build, runtimeServices, out _);
        }
        ...
        {
            return CanPowerAndroid(androidPart, heartPart, null, out rejectReason);
        }

        private static bool CanPowerAndroid(
            CharacterPart androidPart,
            CharacterPart heartPart,
            IRuntimeDataServices runtimeServices,
            out string rejectReason)
        {
            rejectReason = null;
            ...
            IReadOnlyList<AndroidHeartRequirement> supportedHearts = ParseSupportedAndroidHeartRequirements(androidPart.Description, runtimeServices);
        ...
        private static Dictionary<string, IReadOnlyCollection<int>> GetAndroidHeartItemIdsByNormalizedName(IRuntimeDataServices runtimeServices)
        {
            var catalog = new Dictionary<string, List<int>>(StringComparer.Ordinal);
                WzImage stringImage = runtimeServices?.Assets?.FindImage("String", "Eqp.img");
                if (stringImage != null)
                {
                ...
                var result = new Dictionary<string, IReadOnlyCollection<int>>(catalog.Count, StringComparer.Ordinal);
                foreach (KeyValuePair<string, List<int>> entry in catalog)
                {
                    result[entry.Key] = new ReadOnlyCollection<int>(entry.Value);
                }
    ...
    internal sealed class CompanionEquipmentLoader
    {
        ...
        public CompanionEquipmentLoader(GraphicsDevice device, IRuntimeDataServices runtimeServices = null)
        {
        ...
        public CompanionEquipItem LoadDragonEquipment(int itemId)
        {
            ...
            WzImage image = _runtimeServices?.Assets?.FindImage("Character", $"Dragon/{itemId:D8}.img");
            if (image == null)
            {
                return null;
            }
        ...
        public CompanionEquipItem LoadPetEquipment(int itemId)
        {
            ...
            WzImage image = _runtimeServices?.Assets?.FindImage("Character", $"PetEquip/{itemId:D8}.img");
            if (image == null)
            {
                return null;
            }
        ...
        public CompanionEquipItem LoadMechanicEquipment(int itemId)
        {
            ...
            WzImage image = _runtimeServices?.Assets?.FindImage("Character", $"Mechanic/{itemId:D8}.img");
            if (image == null)
            {
                return null;
            }
        ...
        {
            WzImage stringImage = _runtimeServices?.Assets?.FindImage("String", "Eqp.img");
            if (stringImage == null)
            {
                return null;
            }
        ...
        private string LoadCachedItemName(int itemId)
        {
            return _runtimeServices?.Catalog?.TryGetItemName(itemId, out RuntimeItemName itemInfo) == true
                ? itemInfo.Name
                : null;
        }

        private string LoadCachedItemDescription(int itemId)
        {
            return _runtimeServices?.Catalog?.TryGetItemName(itemId, out RuntimeItemName itemInfo) == true
        ...
        private string LoadCachedItemCategory(int itemId, string fallbackCategory)
        {
            return _runtimeServices?.Catalog?.TryGetItemName(itemId, out RuntimeItemName itemInfo) == true
