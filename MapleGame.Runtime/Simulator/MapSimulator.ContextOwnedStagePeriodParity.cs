...
namespace HaCreator.MapSimulator
{
    public partial class MapSimulator
    {
        ...
        {
            _contextOwnedStagePeriodRuntime.BindMap(_mapInfo?.id ?? 0);
        ...
        private string ApplyContextOwnedStagePeriodChange(PacketStagePeriodChangePacket packet, int currentTick)
        {
            ...
            int mapId = _mapInfo?.id ?? 0;
            if (mapId <= 0 || _runtimeMapDefinition == null)
            {
        ...
        private void ReloadContextOwnedStagePeriodBackLayers()
        {
            ...
            {
                TryAppendContextOwnedStageBackground(background, ResolveRuntimeAssetProperty(background.Asset));
            }
        ...
        private bool TryReloadContextOwnedStagePeriodStageBackLayers()
        {
            ...
            foreach (ContextOwnedStageBackImageEntry entry in _contextOwnedStageCurrentBackImages)
            {
                ...
                if (!TryResolveContextOwnedStageBackInfo(entry, out WzImageProperty sourceProperty, out RuntimeBackgroundKind backgroundKind))
                {
                    continue;
                }
                ...
                var backgroundDefinition = new RuntimeBackgroundDefinition(
                    default,
                    resolvedEntry.X,
                    resolvedEntry.Y,
        ...
        private bool TryResolveContextOwnedStageBackInfo(ContextOwnedStageBackImageEntry entry,
            out WzImageProperty property, out RuntimeBackgroundKind kind)
        {
            property = null;
            kind = default;
            if (entry == null || string.IsNullOrWhiteSpace(entry.BackgroundSet)
                || string.IsNullOrWhiteSpace(entry.Number)) return false;

            WzImage image = runtimeServices.Catalog.GetBackgroundSet(entry.BackgroundSet);
            foreach (RuntimeBackgroundKind candidate in ContextOwnedStageSystemCatalog.ResolveClientMakeBackInfoTypeLookupOrder(entry))
            {
                property = image?[candidate.ToPropertyString()]?[entry.Number];
                if (property == null && candidate == RuntimeBackgroundKind.Spine && entry.Number == "0"
                    && image?["spine"] is WzSubProperty spineRoot
                    && spineRoot.WzProperties.Any(value => value is WzStringProperty text && text.IsSpineAtlasResources))
                    property = spineRoot;
        ...
        private void TryAppendContextOwnedStageBackground(RuntimeBackgroundDefinition background, WzImageProperty sourceProperty)
        {
            ...
            if (bgItem != null)
            {
                if (_contextOwnedStageCurrentBackColorArgb.HasValue
                    && ShouldApplyContextOwnedStageBackData(_mapInfo?.id ?? 0))
                {
        ...
        private string DescribeContextOwnedStagePeriodStatus()
        {
            ...
            ContextOwnedStageActiveBackMutationTrace activeBackMutation = _contextOwnedStageLastCacheMutationTrace.ActiveBackMutation;
            string activeBackMutationText = activeBackMutation.AppliedBackColorArgb.HasValue
                ? $"0x{activeBackMutation.AppliedBackColorArgb.Value:X8}"
                : "none";
            ContextOwnedStageCleanStageMutationTrace cleanStageMutation = _contextOwnedStageLastCacheMutationTrace.CleanStageMutation;
            string cleanStagePreviousModeText = cleanStageMutation.PreviousMode.HasValue
                ? cleanStageMutation.PreviousMode.Value.ToString(CultureInfo.InvariantCulture)
                : "none";
            string cacheTraceSummary = $"cacheTrace theme='{_contextOwnedStageLastCacheMutationTrace.StageTheme}' mode={_contextOwnedStageLastCacheMutationTrace.Mode.ToString(CultureInfo.InvariantCulture)} cleanStageInvoked={cleanStageMutation.Invoked} cleanStageHadCachedMode={cleanStageMutation.HadCachedMode} cleanStagePreviousMode={cleanStagePreviousModeText} keywordMutations={_contextOwnedStageLastCacheMutationTrace.KeywordMutations.Count.ToString(CultureInfo.InvariantCulture)} questMutations={_contextOwnedStageLastCacheMutationTrace.QuestMutations.Count.ToString(CultureInfo.InvariantCulture)} stagePeriodReused={_contextOwnedStageLastCacheMutationTrace.StagePeriodMutation.Reused} activeBackCleared={activeBackMutation.ClearedCurrentStageBackImagePointer} activeBackAssigned={activeBackMutation.AssignedCurrentStageBackImagePointer} activeBackCount={activeBackMutation.ActiveStageBackImageCount.ToString(CultureInfo.InvariantCulture)} activeBackColor={activeBackMutationText}";
