...
namespace HaCreator.MapSimulator
{
    public partial class MapSimulator
    {
        ...
        private string ApplyClientGuildResultPayload(byte[] payload)
        {
            ...
            {
            _guildSkillRuntime.SetSkills(SkillDataLoader.LoadGuildSkills(uiResourceCache, _DxDeviceManager.GraphicsDevice, runtimeServices.Assets));
            }
            ...
            {
                skillRecordDetail = _guildSkillRuntime.ApplyPacketOwnedSkillRecord(packet.GuildSkillRecord.Value, packet.GuildId);
            }
            else
            {
        ...
        private string DispatchSocialListGuildDialogRequest(SocialListGuildDialogRequestPacket request)
        {
            ...
            try
            {
                ...
                string description = request.Kind switch
                {
                    ...
                    SocialListGuildDialogRequestKind.CreateGuildAgreement => $"Create-guild agreement response accepted={request.Accepted} party={Math.Max(0, request.PartyId)}",
