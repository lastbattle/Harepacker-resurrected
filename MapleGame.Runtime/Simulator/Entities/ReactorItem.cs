...
namespace HaCreator.MapSimulator.Entities
{
    ...
    public class ReactorItem : BaseDXDrawableItem
    {
        ...
        private readonly RuntimeReactor _runtimeReactor;
        ...
        public RuntimeReactor RuntimeReactor
        {
            get { return _runtimeReactor; }
            private set { }
        }
        ...
        public ReactorItem(RuntimeReactor runtimeReactor, List<IDXObject> frames)
            : base(frames, runtimeReactor.Flip)
        {
            _runtimeReactor = runtimeReactor;
            ...
            _originWorldX = runtimeReactor?.X ?? 0;
            _originWorldY = runtimeReactor?.Y ?? 0;
            _currentWorldX = _originWorldX;
            _currentWorldY = _originWorldY;
        }

        public ReactorItem(
            RuntimeReactor runtimeReactor,
            ...
            : base(GetDefaultFrames(stateFrames), runtimeReactor.Flip)
        {
            _runtimeReactor = runtimeReactor;
            _stateFrames = new Dictionary<int, IDXObject[]>();
            _stateHitDurations = LoadStateHitDurations(runtimeReactor);
            _stateIndexedHitDurations = LoadStateIndexedHitDurations(runtimeReactor);
            ...
            _stateRepeatModes = LoadStateRepeatModes(runtimeReactor);
            _stateEventTypes = LoadStateEventTypes(runtimeReactor);
            _stateTransitions = LoadStateTransitions(runtimeReactor);
            _lazySourceFrameCache = new Dictionary<WzImageProperty, IDXObject[]>();
            _lazySourceFrameLoader = lazySourceFrameLoader;
            _authoredStates = LoadAuthoredStates(runtimeReactor);
            _rootHitDuration = LoadRootHitDuration(runtimeReactor);
            _rootHitFrames = rootHitFrames?.Count > 0 ? rootHitFrames.ToArray() : Array.Empty<IDXObject>();
            _rootHitProperty = rootHitProperty;
            _templateLayerMode = LoadTemplateLayerMode(runtimeReactor);
            _templateMoveEnabled = LoadTemplateMoveEnabled(runtimeReactor);
            _originWorldX = runtimeReactor?.X ?? 0;
            _originWorldY = runtimeReactor?.Y ?? 0;
            _currentWorldX = _originWorldX;
            _currentWorldY = _originWorldY;
        ...
        public ReactorItem(RuntimeReactor runtimeReactor, IDXObject frame0)
            : base(frame0, runtimeReactor.Flip)
        {
            _runtimeReactor = runtimeReactor;
            _stateFrames = new Dictionary<int, IDXObject[]>();
            _stateHitDurations = new Dictionary<int, int>();
        ...
        {
            if (persistInstance && _runtimeReactor != null)
            {
                _runtimeReactor.X = x;
                _runtimeReactor.Y = y;
            }
        ...
        public void SetFlipState(bool isFlipped)
        {
            ...
            if (_runtimeReactor != null)
            {
                _runtimeReactor.Flip = isFlipped;
            }
        }
        ...
        public Rectangle GetCurrentBounds(int tickCount)
        {
            ...
            if (frame == null)
            {
                ...
                int fallbackWidth = Math.Max(1, LastFrameDrawn?.Width ?? 1);
                int fallbackHeight = Math.Max(1, LastFrameDrawn?.Height ?? 1);
        ...
        private static Dictionary<int, AuthoredStateTransition[]> LoadStateTransitions(RuntimeReactor runtimeReactor)
        {
            var transitions = new Dictionary<int, AuthoredStateTransition[]>();

            WzImage linkedImage = runtimeReactor?.TemplateImage;
            if (linkedImage == null)
                return transitions;
        ...
        private static Dictionary<int, int[]> LoadStateEventTypes(RuntimeReactor runtimeReactor)
        {
        ...
        private static HashSet<int> LoadAuthoredStates(RuntimeReactor runtimeReactor)
        {
        ...
        private static Dictionary<int, int> LoadStateHitDurations(RuntimeReactor runtimeReactor)
        {
        ...
        private static int LoadRootHitDuration(RuntimeReactor runtimeReactor)
        {
            WzImage linkedImage = runtimeReactor?.TemplateImage;
            if (linkedImage == null)
            {
        ...
        private static Dictionary<(int State, int ProperEventIndex), int> LoadStateIndexedHitDurations(RuntimeReactor runtimeReactor)
        {
            return LoadStateIndexedHitDurations(runtimeReactor?.TemplateImage);
        }
        ...
        private static Dictionary<int, bool> LoadStateRepeatModes(RuntimeReactor runtimeReactor)
        {
        ...
        private static int LoadTemplateLayerMode(RuntimeReactor runtimeReactor)
        {
            WzImage linkedImage = runtimeReactor?.TemplateImage;
            if (linkedImage == null)
            {
        ...
        private static bool LoadTemplateMoveEnabled(RuntimeReactor runtimeReactor)
        {
            WzImage linkedImage = runtimeReactor?.TemplateImage;
            if (linkedImage == null)
            {
