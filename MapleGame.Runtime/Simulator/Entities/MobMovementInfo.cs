...
namespace HaCreator.MapSimulator.Entities
{
    ...
    public class MobMovementInfo
    {
            ...
            bool? facingLeft = null)
        {
            ...
            RuntimeFoothold belowFoothold = FindBelow(X, Y - Math.Min(VelocityY, 0f) - 2f);
        ...
        private void UpdateKnockbackPhysics(int deltaTimeMs, float speedFactor)
        {
            ...
            {
                RuntimeFoothold belowFH = FindBelow(X, Y - VelocityY - 2);
                if (belowFH != null)
                {
        ...
        private void UpdateWalkingMovementForJumper(int deltaTimeMs, float speedFactor)
        {
            ...
            if (MoveDirection == MobMoveDirection.Left)
            {
                RuntimeFoothold wallL = FindWallL(X - 4, Y - 4);
            ...
            {
                RuntimeFoothold wallR = FindWallR(X + 4, Y - 4);
        ...
        /// </summary>
        private float CalculateYOnFoothold(RuntimeFoothold fh, float x)
        {
        ...
        private void UpdatePlatformBoundaries()
        {
            ...
            HashSet<RuntimeFoothold> visited = new HashSet<RuntimeFoothold>();
            Queue<RuntimeFoothold> toVisit = new Queue<RuntimeFoothold>();
            ...
            {
                RuntimeFoothold fh = toVisit.Dequeue();
                ...
                // Add connected footholds through both endpoints.
                foreach (RuntimeFoothold connectedFh in fh.FirstDot.ConnectedLines)
                {
                    if (!visited.Contains(connectedFh) && !connectedFh.IsWall)
                    {
                        toVisit.Enqueue(connectedFh);
                    }
                }

                foreach (RuntimeFoothold connectedFh in fh.SecondDot.ConnectedLines)
                {
                    if (!visited.Contains(connectedFh) && !connectedFh.IsWall)
                    {
                        toVisit.Enqueue(connectedFh);
        ...
        private RuntimeFoothold FindBelow(float x, float y)
        {
            if (_allFootholds == null)
                return null;
        ...
        private RuntimeFoothold FindWallR(float x, float y)
        {
            if (_allFootholds == null)
                return null;
        ...
        /// <param name="y">Y position to search from</param>
        /// <returns>The wall foothold, or null if none found</returns>
        private RuntimeFoothold FindWallL(float x, float y)
        {
            if (_allFootholds == null)
                return null;
        ...
        public void FindCurrentFoothold(IEnumerable<RuntimeFoothold> footholds)
        {
            ...
            RuntimeFoothold belowFH = FindBelow(X, expectedFootholdY - 5);
        ...
        private RuntimeFoothold FindNextConnectedFoothold(MobMoveDirection direction)
        {
            if (CurrentFoothold == null)
                return null;
            ...
            RuntimeFootholdAnchor edgeAnchor;
            if (direction == MobMoveDirection.Left)
            {
                // Walking left - get the left-most anchor
                edgeAnchor = (x1 < x2)
                    ? CurrentFoothold.FirstDot
                    : CurrentFoothold.SecondDot;
            }
            else
            {
            ...
            foreach (RuntimeFoothold connectedFh in edgeAnchor.ConnectedLines)
            {
                if (connectedFh != CurrentFoothold && !connectedFh.IsWall)
                {
        ...
        {
            RuntimeFoothold nextFh = FindNextConnectedFoothold(direction);
            if (nextFh != null)
            {
