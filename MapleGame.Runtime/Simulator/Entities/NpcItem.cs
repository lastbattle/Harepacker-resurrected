...
namespace HaCreator.MapSimulator.Entities
{
    public class NpcItem : BaseDXDrawableItem, IInteractiveEntity
    {
            ...
            int TickCount)
        {
            ...
            {
                positionOffsetX = _renderOverrideX - _runtimeLife.X;
                positionOffsetY = _renderOverrideY - _runtimeLife.Y;
            }
        ...
        public bool ContainsMapPoint(int mapX, int mapY)
        {
            ...
            int frameWidth = LastFrameDrawn?.Width ?? 0;
            int frameHeight = LastFrameDrawn?.Height ?? 0;
            int width = Math.Max(100, frameWidth + 40);
            int height = Math.Max(120, frameHeight);

            // Left edge calculation matches debug overlay: instance.X - (instance.Width - 20)
            int left = npcX - (frameWidth - 20);
            int right = left + width;
            int top = npcY - frameHeight;
            int bottom = npcY;
        ...
        internal static string ResolveActorLocalMapleTvMessageLineText(
            string line,
            int maxChars,
            IRuntimeAssetSource assets = null)
        {
