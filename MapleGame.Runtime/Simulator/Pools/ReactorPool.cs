...
namespace HaCreator.MapSimulator.Pools
{
    ...
    public class ReactorRuntimeData
    {
        ...
        public bool PacketStateEndTimeActive { get; set; }
        ...
        public bool PacketEnterFadeActive { get; set; }
    ...
    public class ReactorPool
    {
        ...
        public void Initialize(ReactorItem[] reactors, GraphicsDevice graphicsDevice = null, int currentTick = 0)
        {
            ...
            for (int i = 0; i < _reactors.Length; i++)
            {
                ...
                if (reactor?.RuntimeReactor == null)
                    continue;

                int poolId = _nextPoolId++;
                var instance = reactor.RuntimeReactor;
                ...
                var data = new ReactorRuntimeData
                {
                    ...
                    PacketStateEndTimeActive = false,
            ...
            int? currentTick = null)
        {
            ...
            for (int i = 0; i < GetReactorCount(); i++)
            {
                ...
                if (reactor?.RuntimeReactor == null || data == null)
                {
                    continue;
                }
        ...
        public void DestroyReactor(int index, int playerId, int currentTick)
        {
            ...
            RemoveReactorNameLookup(reactor?.RuntimeReactor?.Name, index);
        ...
        public void ResetReactor(int index, int currentTick)
        {
            ...
            data.PacketStateEndTimeActive = false;
            ...
            data.PacketMoveStartX = reactor?.RuntimeReactor?.X ?? 0;
            data.PacketMoveStartY = reactor?.RuntimeReactor?.Y ?? 0;
            data.PacketMoveTargetX = reactor?.RuntimeReactor?.X ?? 0;
            data.PacketMoveTargetY = reactor?.RuntimeReactor?.Y ?? 0;
            ...
            data.PacketEnterFadeActive = false;
        ...
        private ReactorInteractionMetadata ResolveInteractionMetadata(RuntimeReactor instance)
        {
            WzImage linkedReactorImage = instance?.TemplateImage;
            WzSubProperty infoProperty = linkedReactorImage?["info"] as WzSubProperty;
            ...
            int? requiredQuestId = TryGetOptionalInt(infoProperty?["quest"])
                ...
                ?? TryInferRequiredQuestIdFromStateEvents(linkedReactorImage);
            ...
            ReactorActivationType activationType = requiredItemId.HasValue
                ...
                : requiredQuestId.HasValue
                    ...
                    : activateByTouch
                        ...
                        : TryInferActivationTypeFromStateEvents(linkedReactorImage)
                            ?? reactorType switch
                            {
                                ...
                                _ => HasHitAnimation(linkedReactorImage) ? ReactorActivationType.Hit : ReactorActivationType.Touch
                            };
        ...
        public ReactorItem SpawnReactor(int spawnIndex, int currentTick)
        {
            ...
            if (newReactor != null)
            {
                ...
                {
                    ApplyInteractionMetadata(newReactor?.RuntimeReactor, data);
                    ...
                    data.PacketMoveStartTime = 0;
                    data.PacketMoveEndTime = 0;
                    data.PacketMoveStartX = newReactor.RuntimeReactor?.X ?? 0;
                    data.PacketMoveStartY = newReactor.RuntimeReactor?.Y ?? 0;
                    data.PacketMoveTargetX = newReactor.RuntimeReactor?.X ?? 0;
                    data.PacketMoveTargetY = newReactor.RuntimeReactor?.Y ?? 0;
                    data.PacketMoveUsesDefaultRelMove = false;
                    data.PacketMoveExplicitlyRequested = false;
                    data.PacketObservedMovePixelsPerMs = 0f;
                    data.PacketEnterFadeStartTime = 0;
                ...
                AddReactorNameLookup(newReactor?.RuntimeReactor?.Name, spawnIndex);
            ...
            out string message)
        {
            ...
            if (runtimeData != null)
            {
                ReactorInteractionMetadata interactionMetadata = ResolveInteractionMetadata(reactor.RuntimeReactor);
        ...
        private void ApplyPacketEnterName(int index, string name)
        {
            ...
            ReactorItem reactor = GetReactor(index);
            string previousName = reactor?.RuntimeReactor?.Name;
            ...
            if (reactor?.RuntimeReactor != null)
            {
                reactor.RuntimeReactor.Name = runtimeName;
            }
            ...
            out string message)
        {
            ...
            ApplyPacketReactorState(
                ...
                reactor.RuntimeReactor?.Flip ?? false,
                currentTick,
            ...
            data.PacketStateEndTimeActive = true;
        ...
        public bool TryMovePacketOwnedReactor(int packetObjectId, int x, int y, int currentTick, out string message)
        {
            ...
            bool usesDefaultRelMove = IsClientTickAfter(moveEndTime, currentTick);
            ...
            ConfigurePacketStateMovement(
                ...
                y,
                moveEndTime,
                allowDefaultRelMove: IsClientTickAfter(moveEndTime, currentTick));
            data.PacketMoveUsesDefaultRelMove = usesDefaultRelMove;
            data.PacketMoveExplicitlyRequested = IsClientTickAfter(data.PacketMoveEndTime, currentTick);

            if (IsClientTickAfter(data.PacketMoveEndTime, currentTick))
            {
                StartPacketStateMovement(reactor, data, currentTick);
            }
            else
            ...
            data.StateStartTime = currentTick;
            RefreshReactorLayerPlacement(reactor);
            message = IsClientTickAfter(moveEndTime, currentTick)
        ...
        public bool TryLeavePacketOwnedReactor(int packetObjectId, int state, int x, int y, int currentTick, out string message)
        {
            ...
            ApplyPacketReactorState(index, state, x, y, reactor.RuntimeReactor?.Flip ?? false, currentTick, applyAnimationState: false);
        ...
        public void Update(int currentTick, float deltaTime)
        {
            ...
            foreach (var kvp in _reactorData)
            {
                ...
                if (data.IsPacketOwned)
                {
                    ...
                    {
                        if (data.PacketHitStartTime > 0 && HasClientTickReached(currentTick, data.PacketHitStartTime))
                        {
                            data.PacketHitStartTime = 0;
                        ...
                        if (data.PacketHitStartTime == 0
                            && (data.PacketAnimationEndTime <= 0 || HasClientTickReached(currentTick, data.PacketAnimationEndTime)))
                        {
                            reactor?.ClearTransientAnimation();
                    ...
                    if (data.State == ReactorState.Activated
                        && data.PacketHitStartTime > 0
                        && HasClientTickPassed(currentTick, data.PacketHitStartTime))
                    {
                        data.PacketHitStartTime = 0;
                        int hitAnimationDuration = StartPacketHitAnimation(reactor, data, currentTick, _onReactorLayerSoundRequested);
                        if (hitAnimationDuration > 0)
                    ...
                    if (data.State == ReactorState.Activated
                        && data.PacketAnimationEndTime > 0
                        && HasClientTickReached(currentTick, data.PacketAnimationEndTime))
                    {
                        data.PacketAnimationEndTime = 0;
                        reactor?.ClearTransientAnimation();
        ...
        internal static ReactorActivationType ResolveActivationType(RuntimeReactor runtimeReactor)
        {
            return ResolveReactorType(runtimeReactor) switch
            {
                ReactorType.ActivatedByAnyHit => ReactorActivationType.Hit,
                ReactorType.ActivatedLeftHit => ReactorActivationType.Hit,
                ReactorType.ActivatedRightHit => ReactorActivationType.Hit,
                ...
                ReactorType.ActivatedbyItem => ReactorActivationType.Item,
                ReactorType.AnimationOnly => ReactorActivationType.None,
                _ => HasHitAnimation(runtimeReactor?.TemplateImage) ? ReactorActivationType.Hit : ReactorActivationType.Touch
            };
        }

        internal static ReactorType ResolveReactorType(RuntimeReactor runtimeReactor)
        {
            WzImage linkedReactorImage = runtimeReactor?.TemplateImage;
        ...
        private void ApplyInteractionMetadata(RuntimeReactor runtimeReactor, ReactorRuntimeData data)
        {
            if (runtimeReactor == null || data == null)
                return;

            ReactorInteractionMetadata interactionMetadata = ResolveInteractionMetadata(runtimeReactor);
        ...
        internal static bool HasClientTickReached(int currentTick, int targetTick)
        {
            return unchecked(currentTick - targetTick) >= 0;
        }

        internal static bool HasClientTickPassed(int currentTick, int targetTick)
        {
            return unchecked(currentTick - targetTick) > 0;
        }

        internal static bool IsClientTickAfter(int tick, int referenceTick)
        {
            return unchecked(tick - referenceTick) > 0;
        }

        internal static bool HasPacketStateEndTime(ReactorRuntimeData data)
        {
            return data?.PacketStateEndTimeActive == true
                || (data != null && data.PacketStateEndTime > 0);
        }
        ...
        {
            int duration = Math.Max(1, unchecked(moveEndTime - moveStartTime));
            float progress = MathHelper.Clamp(unchecked(currentTick - moveStartTime) / (float)duration, 0f, 1f);
        ...
        private void UpdatePacketStateMovement(ReactorItem reactor, ReactorRuntimeData data, int currentTick)
        {
            ...
            if (HasClientTickReached(currentTick, data.PacketMoveEndTime))
            {
            ...
            out int index)
        {
            ...
            for (int i = 0; i < GetReactorCount(); i++)
            {
                ...
                if (!IsSameReactorTemplate(reactor.RuntimeReactor.Id, reactorId)
                    || reactor.CurrentWorldX != x
                    || reactor.CurrentWorldY != y
                    || reactor.RuntimeReactor.Flip != flip)
                {
                    continue;
                }

                if (!string.IsNullOrWhiteSpace(name)
                    && !string.IsNullOrWhiteSpace(reactor.RuntimeReactor.Name)
                    && !string.Equals(reactor.RuntimeReactor.Name, name, StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }

                bool hasPacketName = !string.IsNullOrWhiteSpace(name);
                bool hasAuthoredName = !string.IsNullOrWhiteSpace(reactor.RuntimeReactor.Name);
            ...
            int remainingCurrentAnimationDuration)
        {
            ...
            if (IsClientTickAfter(currentAnimationEndTime, currentTick))
            {
        ...
        {
            int fadeDuration = unchecked(fadeEndTime - fadeStartTime);
            if (fadeDuration <= 0)
            {
                return 1f;
            }

            int elapsed = unchecked(currentTick - fadeStartTime);
            if (elapsed <= 0)
            {
                return 0f;
            }
        ...
        {
            string templateId = reactor?.RuntimeReactor?.Id;
        ...
        internal static bool ShouldApplyPendingPacketVisualStateOnStateEnd(ReactorRuntimeData data, int currentTick)
        {
            return data != null
                ...
                && HasPacketStateEndTime(data)
                && HasClientTickReached(currentTick, data.PacketStateEndTime)
                && data.State == ReactorState.Activated
                && data.PacketHitStartTime <= 0
                && data.PacketAnimationEndTime <= 0;
        }
        ...
        internal static bool ShouldDelayActivatedPacketHandoffUntilStateEnd(ReactorRuntimeData data, int currentTick)
        {
            return data != null
                ...
                && data.PacketPendingVisualState < 0
                && HasPacketStateEndTime(data)
                && IsClientTickAfter(data.PacketStateEndTime, currentTick);
        }
        ...
        private static void BeginPacketEnterFade(ReactorRuntimeData data, int currentTick)
        {
            ...
            data.PacketEnterFadeActive = true;
        ...
        private static void InvokeReactorLayerSound(Action<string, int, int> callback, ReactorItem reactor, string descriptor)
        {
            ...
            callback(
                descriptor,
                reactor?.CurrentWorldX ?? reactor?.RuntimeReactor?.X ?? 0,
                reactor?.CurrentWorldY ?? reactor?.RuntimeReactor?.Y ?? 0);
        }
