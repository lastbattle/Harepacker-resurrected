    ...
    public class PortalPool
    {
        ...
        public void Initialize(PortalItem[] portals)
        {
            ...
            for (int i = 0; i < _portals.Length; i++)
            {
                ...
                var instance = portal.Portal;

                // Add to name lookup
                if (!string.IsNullOrEmpty(instance.Name))
                {
                    _portalNameToIndex[instance.Name] = i;
                }

                // Initialize hidden portal state
                if (IsHiddenPortalType(instance.Type))
                {
                    _hiddenPortalStates[i] = new HiddenPortalState
                    {
                        PortalIndex = i,
                        PortalName = instance.Name,
                        IsRevealed = false,
                        RevealedTime = 0,
                        Alpha = 0f,
                        TargetAlpha = 0f
        ...
        public PortalItem FindStartPoint()
        {
            ...
            {
                if (portal?.Portal?.Type == PortalType.StartPoint)
                    return portal;
            }
        ...
        public PortalItem FindPortalAtPosition(float x, float y, float range = 40f)
        {
            ...
            foreach (var portal in _portals)
            {
                ...
                float dx = portal.Portal.X - x;
                float dy = portal.Portal.Y - y;
        ...
        public List<(PortalItem portal, int index)> FindPortal_Hidden(float playerX, float playerY, float range = 0)
        {
            ...
            foreach (var kvp in _hiddenPortalStates)
            {
                ...
                float dx = portal.Portal.X - playerX;
                float dy = portal.Portal.Y - playerY;
        ...
        public int GetPropPH(int portalIndex)
        {
            ...
            return portal.Portal.VerticalRange ?? 30; // Default portal height
        }
        ...
        public int GetPropPSH(int portalIndex)
        {
            ...
            return portal.Portal.VerticalImpact ?? 0;
        }
        ...
        public PortalProperties GetPortalProperties(int portalIndex)
        {
            ...
            {
                Height = instance.VerticalRange ?? 30,
                SpecialHeight = instance.VerticalImpact ?? 0,
                Vertical = instance.VerticalImpact ?? 0,
                HorizontalRange = instance.HorizontalRange ?? 40,
                VerticalRange = instance.VerticalRange ?? 30,
        ...
        public PortalCollisionResult? FindPortalCollision(float playerX, float playerY, int playerHeight = 60)
        {
            ...
            for (int i = 0; i < _portals.Length; i++)
            {
                ...
                // Get detection range
                int hRange = instance.HorizontalRange ?? 40;
                int vRange = instance.VerticalRange ?? 30;
        ...
        public PortalItem CheckCollisionPortal(float playerX, float playerY, int playerHeight = 60)
        {
            ...
            for (int i = 0; i < _portals.Length; i++)
            {
                ...
                var instance = portal.Portal;
                int hRange = instance.HorizontalRange ?? 30;
                int vRange = instance.VerticalRange ?? 20;
        ...
        public PortalPoolStats GetStats()
        {
            ...
            if (_portals != null)
            {
                foreach (var portal in _portals)
                {
                    if (portal?.Portal?.Type == PortalType.Visible ||
                        portal?.Portal?.Type == PortalType.Default)
                        visibleCount++;
                }
            }
