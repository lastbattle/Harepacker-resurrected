    ...
    public sealed class RemoteUserActorPool
    {
        ...
        public sealed class RemoteActiveEffectMotionBlurState
        {
                ...
                out int overlayRefCount)
            {
                ...
                {
                    IReadOnlyList<AnimationEffects.SecondaryMotionBlurLayerReferenceOperation> normalizedEmptySnapshotOperations =
                        AnimationEffects.SecondaryMotionBlurLayerReferenceOperation.NormalizeTraceOrder(operations);
                    SimulatedLayerReferenceOperations =
                        AnimationEffects.SecondaryMotionBlurLayerReferenceOperation.NormalizeTraceOrder(
                            (SimulatedLayerReferenceOperations ?? Array.Empty<AnimationEffects.SecondaryMotionBlurLayerReferenceOperation>())
                            .Concat(normalizedEmptySnapshotOperations));
                    return normalizedEmptySnapshotOperations;
                }
                ...
                IReadOnlyList<AnimationEffects.SecondaryMotionBlurLayerReferenceOperation> normalizedSnapshotOperations =
                    AnimationEffects.SecondaryMotionBlurLayerReferenceOperation.NormalizeTraceOrder(operations);
                SimulatedLayerReferenceOperations =
                    AnimationEffects.SecondaryMotionBlurLayerReferenceOperation.NormalizeTraceOrder(
                        (SimulatedLayerReferenceOperations ?? Array.Empty<AnimationEffects.SecondaryMotionBlurLayerReferenceOperation>())
        ...
        private UILoaderResourceCache _uiResourceCache;
        ...
        public void Initialize(
            ...
            UILoaderResourceCache uiResourceCache)
        {
            _loader = loader;
            _skillLoader = skillLoader;
            _assets = assets ?? throw new ArgumentNullException(nameof(assets));
            _uiResourceCache = uiResourceCache ?? throw new ArgumentNullException(nameof(uiResourceCache));
        ...
        public bool TryApplyTransientSkillUseAvatarEffect(
            ...
            out string message,
            bool? facingRightOverride = null,
            string clientLayerOwnerName = null)
        {
            message = null;
            ...
            string clientLayerOwnerName = null)
        {
            ...
            pendingEffects.Add(new PendingRemoteTransientSkillUseAvatarEffectState
            {
                ...
                ClientLayerOwnerName = clientLayerOwnerName
            });
        ...
        private void ApplyTransientSkillUseAvatarEffect(
            RemoteUserActor actor,
            int registrationKey,
            SkillAnimation overlayAnimation,
            SkillAnimation underFaceAnimation,
            ...
            string clientLayerOwnerName = null)
        {
            ...
            var state = new RemoteTransientSkillUseAvatarEffectState
            {
                RegistrationKey = registrationKey,
                OverlayAnimation = overlayAnimation,
                UnderFaceAnimation = underFaceAnimation,
            ...
            if (TryRestoreRemoteTransientSkillUseAvatarEffectOwnerCounter(
                    actor,
                    state,
                    currentTime,
                    out int restoredAnimationStartTime))
            {
                state.AnimationStartTime = restoredAnimationStartTime;
            }

            actor.TransientSkillUseAvatarEffects.Add(state);
            StoreRemoteTransientSkillUseAvatarEffectOwnerCounter(actor, state, currentTime);
        }
        ...
        private void ApplyPendingTransientSkillUseAvatarEffects(RemoteUserActor actor, int currentTime)
        {
            ...
            for (int i = 0; i < pendingEffects.Count; i++)
            {
                ...
                ApplyTransientSkillUseAvatarEffect(
                    ...
                    pendingEffect.ClientLayerOwnerName);
            }

            _pendingTransientSkillUseAvatarEffectsByCharacterId.Remove(actor.CharacterId);
        }

        private bool TryRestoreRemoteTransientSkillUseAvatarEffectOwnerCounter(
            RemoteUserActor actor,
            RemoteTransientSkillUseAvatarEffectState state,
            int currentTime,
            out int animationStartTime)
        {
            ...
            if (actor == null
                || state == null
                || string.IsNullOrWhiteSpace(state.ClientLayerOwnerName)
                || !TryResolveRemoteTransientSkillUseAvatarEffectOwnerCounterContext(
                    actor,
                    state,
            ...
            if (!TryRestoreRemoteAuxiliaryLayerOwnerCounter(
                    actor.CharacterId,
                    state.ClientLayerOwnerName,
            ...
            animationStartTime = currentTime - Math.Max(0, animationElapsedMs);
            return true;
        }

        private void StoreRemoteTransientSkillUseAvatarEffectOwnerCounter(
            RemoteUserActor actor,
            RemoteTransientSkillUseAvatarEffectState state,
            int currentTime)
        {
        ...
        private static bool TryResolveRemoteTransientSkillUseAvatarEffectOwnerCounterContext(
            RemoteUserActor actor,
            RemoteTransientSkillUseAvatarEffectState state,
            out string actionName,
            out bool facingRight)
        {
            actionName = null;
            facingRight = state?.FacingRightOverride ?? actor?.FacingRight ?? true;
            if (state == null
                || (state.OverlayAnimation?.Frames?.Count ?? 0) <= 0
                   && (state.UnderFaceAnimation?.Frames?.Count ?? 0) <= 0)
            {
                return false;
            }

            actionName = $"skillUse:{Math.Max(0, state.RegistrationKey)}";
            return true;
        }
            ...
            bool applyRelationshipRecords = true)
        {
            ...
            if (packet.AvatarLook != null)
            {
                ...
                SyncTemporaryStatPresentation(actor);
                RegisterMeleeAfterImage(actor, 0, actor.ActionName, currentTime, 10, 0);
                RefreshRemoteVisibleLayerAdmissionsAfterAvatarStateChangeForParity(actor);
            }
            ...
            bool autoEnterHold = false)
        {
            ...
            {
                SkillId = skillId,
                SkillName = PreparedSkillHudRules.ResolveDisplayName(skillId, skillName, _assets),
        ...
        internal bool TryBuildRemoteMobDamageInfoPresentation(
        ...
        internal bool TryResolveRemoteHitMobAttackEffectPath(RemoteUserHitPacket packet, out string effectPath)
        {
            effectPath = null;
        ...
        internal bool TryResolveRemoteHitMobAttackAttachToOwnerForParity(
            RemoteUserHitPacket packet,
        ...
        private bool TryResolveMobAttackHitAttachToOwnerForTemplate(
            int mobTemplateId,
            int attackNumber,
            out bool attachToOwner)
        {
        ...
        private bool TryResolveAuthoredMobAttackHitEffectPath(RemoteUserHitPacket packet, out string effectPath)
        {
            effectPath = null;
            int mobTemplateId = packet.MobTemplateId.GetValueOrDefault();
            int attackNumber = packet.AttackIndex + 1;
        ...
        private bool TryResolveMobAttackHitEffectPath(int mobTemplateId, int attackNumber, out string effectPath)
        {
            effectPath = null;
            WzImage mobImage = TryFindMobImage(mobTemplateId);
            if (mobImage == null)
        ...
        private bool TryResolveLinkedMobTemplateId(int mobTemplateId, out int linkedMobTemplateId)
        {
        ...
        private bool TryResolveMobMaxHp(int mobTemplateId, out int maxHp)
        {
            maxHp = 0;
            WzImage mobImage = TryFindMobImage(mobTemplateId);
            if (mobImage == null)
        ...
        private WzImage TryFindMobImage(int mobTemplateId)
        {
            if (mobTemplateId <= 0)
            {
                return null;
            ...
            WzImage mobImage = _assets.FindImage(
                "Mob",
        ...
        private bool HasRemoteAnimationDisplayerSpecialBranch(int skillId)
        {
            ...
            WzImage skillImage = _assets.FindImage("Skill", $"{skillId / 10000}.img");
        ...
        private bool TryApplyRemoteDragonOfficialSkillUseFollowUp(
            RemoteUserActor actor,
            int skillId,
        ...
        public RemoteGrenadePresentation BuildRemoteThrowGrenadePresentationForParity(
            int characterId,
            int skillId,
            int skillLevel,
            int ownerLevel,
        ...
        private int ResolveRemoteGrenadeMaxGaugeTimeMs(int skillId)
        {
            if (skillId != MonsterBombSkillId)
            {
                return 1000;
        ...
        private bool TryResolveRemoteDragonKeyDownBarAnchor(
            RemoteUserActor actor,
            RemotePreparedSkillState prepared,
            int currentTime,
            bool isHolding,
        ...
        internal bool TryResolveRemoteDragonCompanionRenderStateForParity(
            RemoteUserActor actor,
            int currentTime,
        ...
        private bool ReleaseRemoteDragonHudOverlayIfOwnerUnavailableForParity(RemoteUserActor actor)
        {
            RemotePreparedSkillState prepared = actor?.PreparedSkill;
            if (prepared == null
                || !PreparedSkillHudRules.IsDragonOverlaySkill(prepared.SkillId))
        ...
        internal bool ReleaseRemoteDragonHudOverlayIfOwnerUnavailableForTesting(RemoteUserActor actor)
        {
        ...
        private bool TryResolveRemoteDragonHudMetadata(int jobId, out RemoteDragonHudMetadata metadata)
        {
            metadata = default;
        ...
        internal bool TryResolveRemoteDragonHudMetadataForTesting(int jobId, out RemoteDragonHudMetadata metadata)
        {
            return TryResolveRemoteDragonHudMetadata(jobId, out metadata);
        }

        private WzImage FindRemoteDragonHudImage(int dragonJob)
        {
            if (dragonJob <= 0)
            {
                return null;
                ...
                && resolvedImagePath.StartsWith("Skill/", StringComparison.OrdinalIgnoreCase))
            {
                ...
                WzImage resolvedImage = _assets.FindImage("Skill", relativeImagePath);
            ...
            return _assets.FindImage("Skill", $"Dragon/{dragonJob}.img")
                ?? _assets.FindImage("Skill", $"{dragonJob}.img");
        }
        ...
        private void RefreshRemoteVisibleLayerAdmissionsAfterAvatarStateChangeForParity(RemoteUserActor actor)
        {
            if (actor == null)
            {
                return;
        ...
        private static void ClearRemoteTemporaryStatAvatarEffectStates(RemoteUserActor actor)
        {
            if (actor == null)
            {
                return;
            ...
            var releasedStates = new HashSet<RemoteTemporaryStatAvatarEffectState>();
            ReleaseRemoteTemporaryStatAvatarEffectReference(actor.TemporaryStatSoulArrowEffect, releasedStates);
            ReleaseRemoteTemporaryStatAvatarEffectReference(actor.TemporaryStatWeaponChargeEffect, releasedStates);
            ReleaseRemoteTemporaryStatAvatarEffectReference(actor.TemporaryStatAuraEffect, releasedStates);
            foreach (RemoteTemporaryStatAvatarEffectState tailState in actor.TemporaryStatAuraEffectTails)
            {
                ReleaseRemoteTemporaryStatAvatarEffectReference(tailState, releasedStates);
            }

            ReleaseRemoteTemporaryStatAvatarEffectReference(actor.TemporaryStatMoreWildEffect, releasedStates);
            ReleaseRemoteTemporaryStatAvatarEffectReference(actor.TemporaryStatBarrierEffect, releasedStates);
            ...
            actor.TemporaryStatWeaponChargeEffect = null;
            actor.TemporaryStatAuraEffect = null;
            actor.TemporaryStatAuraEffectTails.Clear();
            actor.TemporaryStatMoreWildEffect = null;
            actor.TemporaryStatBarrierEffect = null;
        ...
        private static void ReleaseRemoteTemporaryStatAvatarEffectReference(
            RemoteTemporaryStatAvatarEffectState state,
            ISet<RemoteTemporaryStatAvatarEffectState> releasedStates)
        {
            if (state == null
                || releasedStates == null
            ...
            state.ReleaseAffectedLayerReference();
        }
        ...
        private bool ReleaseRemoteDragonCompanionIfOwnerUnavailableForParity(RemoteUserActor actor)
        {
            if (actor == null)
            {
                return true;
        ...
        internal bool ReleaseRemoteDragonCompanionIfOwnerUnavailableForTesting(RemoteUserActor actor)
        {
        ...
        private bool TryResolveGuildMarkTextures(
            ...
            out Texture2D markTexture)
        {
            ...
            backgroundTexture = GuildMarkTextureCache.GetBackgroundTexture(
                device, _assets, _uiResourceCache, backgroundId, backgroundColor);
            markTexture = GuildMarkTextureCache.GetMarkTexture(
                device, _assets, _uiResourceCache, markId, markColor);
        ...
        internal void RefreshRemoteVisibleLayerAdmissionsAfterAvatarStateChangeForTesting(RemoteUserActor actor)
        {
            RefreshRemoteVisibleLayerAdmissionsAfterAvatarStateChangeForParity(actor);
        }
            ...
            bool reseedTimelineFromPacket)
        {
            ...
            {
                if (existingState != null && !existingState.IsTerminated)
                {
        ...
        private void UpdateTransientSkillUseAvatarEffects(RemoteUserActor actor, int currentTime)
        {
