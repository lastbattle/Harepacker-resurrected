...
namespace HaCreator.MapSimulator.Pools
{
    ...
    public class MobPool
    {
        ...
        {
            var instance = mob.RuntimeLife;
            var mobData = instance?.MobData;
        ...
        {
            return _activeMobs.Where(m => m.RuntimeLife?.Id == mobTypeId);
        }
        ...
        private MobItem SpawnMobAtPoint(MobSpawnPoint spawnPoint, Func<MobSpawnPoint, MobItem> mobFactory)
        {
            ...
            {
                var mobName = newMob.RuntimeLife?.DisplayName ?? "Boss";
        ...
        private MobItem FindNearestEncounterTarget(MobItem source, System.Predicate<MobItem> predicate, float maxDistance)
        {
            ...
            int? sourceTeam = source.RuntimeLife?.Team;
            ...
            for (int i = 0; i < _activeMobs.Count; i++)
            {
                ...
                int teamPriority = SpecialMobInteractionRules.ResolveEncounterTargetPriority(sourceTeam, candidate.RuntimeLife?.Team);
