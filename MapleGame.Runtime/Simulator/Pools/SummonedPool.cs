...
namespace HaCreator.MapSimulator.Pools
{
    ...
    internal readonly record struct PacketOwnedExpiryFindHitMobInRectResult(
        int[] TargetIds,
        int PoisonMatchCount)
    {
        public int PackedClientResult => (TargetIds?.Length ?? 0) | (PoisonMatchCount << 16);
    }
    ...
    public sealed class SummonedPool
    {
        ...
        private IRuntimeAssetSource _assets;
        ...
        public void Initialize(
            ...
            IRuntimeAssetSource assets = null)
        {
        ...
        public bool TryCreate(SummonedCreatePacket packet, int currentTime, out string message)
        {
            ...
            var state = new PacketOwnedSummonState
            {
                ...
                TeslaCoilState = packet.TeslaCoilState,
                TeslaTrianglePoints = PacketOwnedSummonUpdateRules.ResolvePacketOwnedTeslaCreateTrianglePoints(
                    packet.TeslaCoilState,
                    packet.TeslaTrianglePoints),
        ...
        private bool TryResolveBodyContactDamage(PacketOwnedSummonState state, int currentTime)
        {
            ...
            foreach (MobItem mob in _mobPool.ActiveMobs)
            {
                ...
                SummonDamageRuntimeRules.BodyContactDamageResult damageResult =
                    ResolveSummonBodyContactDamage(mob);
                ApplySummonBodyContactDamageMetadata(state.Summon, damageResult);
                ApplySummonBodyContactDirection(state.Summon, mob);
                ApplySummonDamage(state, damageResult.Damage, currentTime, useHitAnimationState: true);
                PlayPacketIncDecHpFeedback(state.Summon, damageResult.Damage, currentTime);
                return true;
            }
        ...
        internal static int[] ResolvePacketOwnedExpiryFindHitMobInRectTargetOrder(
            ...
            bool includeEscortMob = false)
        {
            return ResolvePacketOwnedExpiryFindHitMobInRectResult(
                    summon,
                    candidates,
                    maxTargets,
                    facingRight,
        ...
        internal static PacketOwnedExpiryFindHitMobInRectResult ResolvePacketOwnedExpiryFindHitMobInRectResult(
            ActiveSummon summon,
            IEnumerable<PacketOwnedExpiryTargetCandidate> candidates,
            int maxTargets,
            bool facingRight,
            ...
            int poisonStatusId = 0)
        {
            ...
            List<int> targetIds = new(maxTargets);
            int poisonMatchCount = 0;
            foreach (PacketOwnedExpiryTargetCandidate candidate in orderedCandidates)
            {
                ...
                if (candidate.MobObjectId <= 0
                    || targetIds.Contains(candidate.MobObjectId)
                ...
                targetIds.Add(candidate.MobObjectId);
                if (poisonStatusId != 0 && candidate.PoisonStatusId == poisonStatusId)
                {
                    poisonMatchCount++;
                }
            }

            return new PacketOwnedExpiryFindHitMobInRectResult(targetIds.ToArray(), poisonMatchCount);
        }
        ...
        {
            int? normalizedMobTeam = NormalizePacketOwnedExpiryClientStateValue(mobTeam);
            int? normalizedOwnerTeam = NormalizePacketOwnedExpiryClientStateValue(ownerTeam);
            int? normalizedMobPhase = NormalizePacketOwnedExpiryClientStateValue(mobPhase);
            int? normalizedOwnerPhase = NormalizePacketOwnedExpiryClientStateValue(ownerPhase);
            bool isOurTeam = hasTeamContext
                             && normalizedMobTeam.HasValue
        ...
        private static int? NormalizePacketOwnedExpiryClientStateValue(int? value)
        {
            ...
            PacketOwnedExpiryTargetCandidate candidate)
        {
            if (candidate.BodyHitboxes != null)
            {
                ...
                {
                    if (!IsClientNonEmptyPacketOwnedExpiryRect(bodyHitbox))
                    {
                        continue;
                    }
            ...
            if (IsClientNonEmptyPacketOwnedExpiryRect(candidate.Hitbox))
            {
        ...
        private static bool IsClientNonEmptyPacketOwnedExpiryRect(Rectangle rect)
        {
            ...
            IRuntimeAssetSource assets)
        {
            ...
            MobAttackData templateAttackData = ResolvePacketMobAttackData(resolvedMobTemplateId, attackAction, assets);
            ...
            MobAttackData attackData)
        {
            ...
            bool hitAttach = attackData.HitAttach;
            ...
            if (TryResolvePacketMobAttackGeneralEffectAttachMetadata(
                    ...
                    out bool? hitEffectFacingAttach))
            {
                if (!hasHitAttach && hitEffectAttach.HasValue)
                {
                    hasHitAttach = true;
                    hitAttach = hitEffectAttach.Value;
                }

                if (!hasFacingAttach && hitEffectFacingAttach.HasValue)
                {
            ...
            if (attackInfo == null)
            {
                ...
                if (hasHitAttach)
                {
                    metadata.HitAttach = hitAttach;
                    metadata.HasHitAttachMetadata = true;
                }

                if (hasFacingAttach)
                {
                    metadata.FacingAttach = facingAttach;
                    metadata.HasFacingAttachMetadata = true;
                    if (!hasHitAttach)
                    {
                        metadata.HitAttach = facingAttach;
                        metadata.HasHitAttachMetadata = true;
                    }
                }
            ...
            if (hasHitAttach)
            {
                clone.HitAttach = hitAttach;
                clone.HasHitAttachMetadata = true;
            }

            if (hasFacingAttach)
            {
                clone.FacingAttach = facingAttach;
                clone.HasFacingAttachMetadata = true;
                if (!hasHitAttach)
                {
                    clone.HitAttach = facingAttach;
                    clone.HasHitAttachMetadata = true;
                }
            }
            ...
            out bool? facingAttach)
        {
            ...
            foreach (string token in EnumeratePacketMobAttackGeneralEffectAttachMetadataTokens(effectPath))
            {
                if (TryReadPacketMobAttackGeneralEffectAttachMetadataToken(
                        token,
                        out string metadataName,
                        out bool metadataValue))
                {
                    if (IsPacketMobAttackGeneralEffectFacingAttachMetadataName(metadataName))
                    {
                        facingAttach ??= metadataValue;
                    }
                    else if (IsPacketMobAttackGeneralEffectHitAttachMetadataName(metadataName))
                    {
                        hitAttach ??= metadataValue;
                    }
                }
            }
            ...
            string effectPath)
        {
            ...
            foreach (string rawToken in SplitPacketMobAttackGeneralEffectTokens(normalizedEffectPath))
            {
                ...
                {
                    string normalizedToken = NormalizePacketMobAttackGeneralEffectPathTokenShell(expandedToken);
        ...
        private static bool TryReadPacketMobAttackGeneralEffectAttachMetadataToken(
            string token,
            out string metadataName,
            out bool metadataValue)
        {
            ...
            for (int i = 0; i < segments.Length - 1; i++)
            {
                string candidateName = segments[i];
                if (!IsPacketMobAttackGeneralEffectAttachMetadataName(candidateName))
                {
                    continue;
                }

                if (TryParsePacketMobAttackGeneralEffectAttachMetadataValue(segments[i + 1], out metadataValue))
                {
                    metadataName = candidateName;
                    return true;
                }
            }
        ...
        private static bool TryParsePacketMobAttackGeneralEffectAttachMetadataValue(
            string token,
            out bool value)
        {
            value = false;
            ...
            if (int.TryParse(normalizedToken, NumberStyles.Integer, CultureInfo.InvariantCulture, out int intValue))
            {
                value = intValue > 0;
                return true;
            }

            if (string.Equals(normalizedToken, "true", StringComparison.OrdinalIgnoreCase))
            {
                value = true;
                return true;
            }

            if (string.Equals(normalizedToken, "false", StringComparison.OrdinalIgnoreCase))
            {
                value = false;
                return true;
            }
        ...
        private static bool IsPacketMobAttackGeneralEffectAttachMetadataName(string name)
        {
            return IsPacketMobAttackGeneralEffectHitAttachMetadataName(name)
                   || IsPacketMobAttackGeneralEffectFacingAttachMetadataName(name);
        }

        private static bool IsPacketMobAttackGeneralEffectHitAttachMetadataName(string name)
        {
            return string.Equals(name, "attach", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(name, "bHitAttach", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(name, "hitAttach", StringComparison.OrdinalIgnoreCase);
        }

        private static bool IsPacketMobAttackGeneralEffectFacingAttachMetadataName(string name)
        {
            return string.Equals(name, "attachfacing", StringComparison.OrdinalIgnoreCase)
        ...
        private static string ResolvePacketMobAttackFallbackTemplateId(int? packetMobTemplateId, MobItem mob)
        {
            ...
            return mob?.RuntimeLife?.Id;
        }

        private static MobAttackData ResolvePacketMobAttackData(
            int mobTemplateId,
            string attackAction,
            IRuntimeAssetSource assets)
        {
            ...
            string mobInfoId = mobTemplateId.ToString(CultureInfo.InvariantCulture).TrimStart('0');
            if (mobInfoId.Length == 0)
            {
                mobInfoId = "0";
            }

            WzImage mobImage = assets?.FindImage("Mob", mobTemplateId.ToString("D7", CultureInfo.InvariantCulture) + ".img")
                ?? assets?.FindImage("Mob", mobInfoId + ".img");
            if (mobImage == null)
            {
                return null;
            }
            ...
            MobData mobData = MobData.Parse(mobImage, mobTemplateId);
            IReadOnlyList<MobAttackData> attackData = mobData?.AttackData;
            ...
            IRuntimeAssetSource assets)
        {
            ...
            return frames?.Count > 0
                ...
                {
                    Frames = frames,
                    SourceFrameIndex = ResolvePacketMobAttackGeneralEffectSourceFrameIndex(
                        attackData?.HitEffectPath,
                        mobTemplateId,
                        attackAction),
            ...
            IRuntimeAssetSource assets)
        {
            ...
            {
                WzImageProperty property = ResolvePacketMobAttackGeneralEffectProperty(candidates[i], assets);
                if (property == null)
                {
                    continue;
                }

                property = ResolvePacketMobAttackGeneralEffectRenderableProperty(property, "Mob", assets);
                if (property == null)
                {
                    continue;
                }
            ...
            IRuntimeAssetSource assets)
        {
            ...
            return TryBuildPacketMobAttackClientInfoHitPath(
                   ...
                   && ResolvePacketMobAttackGeneralEffectProperty(clientInfoHitPath, assets) != null
                ? clientInfoHitPath
                : null;
        }
            ...
            IRuntimeAssetSource assets = null)
        {
            ...
            for (int depth = 0; depth < 8 && current != null; depth++)
            {
                ...
                if (TryExtractPacketMobAttackSourcePathToken(current, out string sourcePathToken))
                {
                    ...
                    current = ResolvePacketMobAttackGeneralEffectProperty(normalizedSourcePath, assets);
                    continue;
                }
                ...
                if (current is WzImageProperty sourcePropertyContainer)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(directSourcePath))
                    {
                        ...
                        {
                            current = ResolvePacketMobAttackGeneralEffectProperty(normalizedDirectSourcePath, assets);
                            continue;
                        }
                    }
                    ...
                    {
                        current = ResolvePacketMobAttackGeneralEffectProperty(sequenceSourceRootPath, assets);
                        continue;
                    }
                }
        ...
        private static bool TryExtractPacketMobAttackGeneralEffectSourceSequenceFrameIndex(
            string normalizedSourcePath,
            out int frameIndex)
        {
            frameIndex = 0;
            ...
            return segments.Length >= 4
                   && int.TryParse(segments[^1], NumberStyles.Integer, CultureInfo.InvariantCulture, out frameIndex)
                   && frameIndex >= 0;
        }
        ...
        internal static int ResolvePacketMobAttackGeneralEffectSourceFrameIndexForTest(
            string effectPath,
            string mobTemplateId,
            string attackAction)
        {
            return ResolvePacketMobAttackGeneralEffectSourceFrameIndex(effectPath, mobTemplateId, attackAction);
        }

        private static int ResolvePacketMobAttackGeneralEffectSourceFrameIndex(
            string effectPath,
            string mobTemplateId,
            string attackAction)
        {
            ...
            if (TryResolvePacketMobAttackGeneralEffectSourceSequenceFrameIndex(
                    sourceSequencePathTokens,
                    "Mob",
                    out int sourceFrameIndex))
            {
            ...
            out int sourceFrameIndex)
        {
            ...
            for (int i = 0; i < sourcePathTokens.Count; i++)
            {
                ...
                previousNormalizedSourcePath = normalizedSourcePath;
                if (TryExtractPacketMobAttackGeneralEffectSourceSequenceFrameIndex(
                        normalizedSourcePath,
                        out sourceFrameIndex))
                {
            ...
            for (int i = 0; i < token.Length; i++)
            {
                ...
                if (current == '\\' && i < token.Length - 1)
                {
                    if (i < token.Length - 5
                        ...
                        && TryParsePacketMobAttackGeneralEffectUnicodeEscapedChar(
                            token,
                            i + 2,
                            out char unicodeDecoded)
                        && IsPacketMobAttackGeneralEffectPercentDecodedPathCharacter(unicodeDecoded))
                    {
                        normalizedBuilder.Append(unicodeDecoded == '\\' ? '/' : unicodeDecoded);
                        i += 5;
                        continue;
                    }
        ...
        private static bool TryParsePacketMobAttackGeneralEffectUnicodeEscapedChar(
            string token,
            int firstHexIndex,
            out char decoded)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (!TryParsePacketMobAttackGeneralEffectHexDigit(token[firstHexIndex + i], out int digit))
                {
                    return false;
                }
        ...
        private static WzImageProperty ResolvePacketMobAttackGeneralEffectProperty(
            string effectUol,
            IRuntimeAssetSource assets = null)
        {
            ...
            WzImage image = assets?.FindImage(segments[0], segments[1]);
            if (image == null)
            {
                return null;
            }
