...
namespace HaCreator.MapSimulator.Pools
{
    ...
    public sealed class AffectedAreaPool
    {
        ...
        private readonly IRuntimeAssetCatalog _runtimeCatalog;
        ...
        public AffectedAreaPool(
            SkillLoader skillLoader,
            MobSkillEffectLoader mobSkillEffectLoader,
            GraphicsDevice graphicsDevice = null,
            IRuntimeAssetSource runtimeAssets = null,
        ...
        {
            WzSubProperty itemProperty = LoadItemPropertyRuntime(createInfo.SkillId);
            ...
            int startTime = ResolveStartTime(currentTime, createInfo.StartDelayUnits);
            int expireTime = ResolveAreaBuffItemExpireTimeRuntime(startTime, itemProperty, createInfo.SkillId, animation);
            return new ActiveAffectedArea
            {
                ObjectId = createInfo.ObjectId,
                Type = createInfo.Type,
        ...
        private int ResolveAreaBuffItemExpireTimeRuntime(
            int startTime,
            WzSubProperty itemProperty,
            int itemId,
            SkillAnimation animation)
        {
            int durationMs = ResolveAreaBuffItemDurationMsRuntime(itemProperty, itemId, animation);
            return durationMs > 0 ? unchecked(startTime + durationMs) : 0;
        }

        private int ResolveAreaBuffItemDurationMsRuntime(
            WzSubProperty itemProperty,
            int itemId,
            SkillAnimation animation = null)
        {
            string itemDescription = itemId > 0
                && _runtimeCatalog != null
                && InventoryItemMetadataResolver.TryResolveItemDescription(itemId, _runtimeCatalog, out string resolvedDescription)
                ? resolvedDescription
                : null;
            return AreaBuffItemMetadataResolver.ResolveDurationMs(
                itemProperty,
                itemDescription,
                LoadLinkedAreaBuffItemPropertyRuntime,
                LoadLinkedAreaBuffItemDescriptionRuntime,
                LoadLinkedAreaBuffPathPropertyRuntime,
                animation);
        }
        ...
        private SkillAnimation LoadAreaBuffItemFogAnimation(int itemId, WzSubProperty itemProperty = null)
        {
            ...
            itemProperty ??= LoadItemPropertyRuntime(itemId);
        ...
        private WzSubProperty LoadItemPropertyRuntime(int itemId)
        {
            ...
            WzImage itemImage = _runtimeAssets?.FindImage(category, imagePath);
            if (itemImage == null)
            {
                return null;
            }
        ...
        private WzSubProperty LoadLinkedAreaBuffItemPropertyRuntime(string itemInfoPath)
        {
            ...
            {
                WzImage linkedImage = _runtimeAssets?.FindImage("Item", imagePath);
                if (linkedImage == null)
                {
                    return null;
                }
            ...
            return LoadItemPropertyRuntime(fallbackItemId);
        }

        private string LoadLinkedAreaBuffItemDescriptionRuntime(string itemInfoPath)
        {
            ...
            return _runtimeCatalog != null
                && InventoryItemMetadataResolver.TryResolveItemDescription(itemId, _runtimeCatalog, out string description)
                ? description
                : null;
        }

        private WzImageProperty LoadLinkedAreaBuffPathPropertyRuntime(string linkedPath)
        {
            ...
            WzImage image = _runtimeAssets?.FindImage(category, imagePath);
            if (image == null)
            {
                return null;
            }
