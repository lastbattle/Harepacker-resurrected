...
namespace HaCreator.MapSimulator
{
    public partial class MapSimulator
    {
        ...
        // CWvsContext::CWvsContext at 0x9e90f0 initializes these three slots
        // contiguously immediately before m_bPredictQuit / m_anLogoutGiftCommoditySN.
        ...
        private void RegisterPacketOwnedLogoutGiftWindow()
        {
            ...
            {
                using (uiWindowManager.BeginResourceCreationScope())
                {
            ...
            // Authored assets are cached by the session; only fallback textures
            // generated while configuring this window belong to its map generation.
            Texture2D frameTexture = LoadPacketOwnedLogoutGiftFrameTexture();
            LogoutGiftButtonSkin buttonSkin = LoadPacketOwnedLogoutGiftButtonSkin();
            using (uiWindowManager.BeginResourceCreationScope())
                window.ConfigureVisualAssets(frameTexture, buttonSkin);
        ...
        private LogoutGiftOwnerSnapshot BuildPacketOwnedLogoutGiftSnapshot()
        {
            ...
            for (int i = 0; i < PacketOwnedLogoutGiftEntryCount; i++)
            {
                ...
                if (commoditySerialNumber > 0
                    && AdminShopDialogUI.TryResolveCommodityBySerialNumber(runtimeServices.Assets, commoditySerialNumber, out itemId, out price, out count, out onSale))
                {
                    ...
                    {
                        InventoryItemMetadataResolver.TryResolveItemName(itemId, runtimeServices.Catalog, out itemName);
                    }
                }
        ...
        private bool TryApplyPacketOwnedLogoutGiftPayload(byte[] payload, out string message)
        {
            ...
            switch (refreshDisposition)
            {
                ...
                {
                    ApplyPacketOwnedLogoutGiftClientUpdateRefresh(window);
                    ...
                    string hiddenTrailingSuffix = HasPacketOwnedLogoutGiftOpaqueTail()
                        ...
                        : string.Empty;
                    message =
                        $"CWvsContext::OnLogoutGift called inherited `CWnd::Update` (`CUILogoutGift::Update(1)`) on the instantiated logout-gift singleton while the visible chooser remained closed behind the StringPool 0x{PacketOwnedLogoutGiftCompletionStringPoolId.ToString("X", CultureInfo.InvariantCulture)} completion dialog, so only enabled child controls are refreshed. The owner-local `m_aCommodityID` copy remains {FormatPacketOwnedLogoutGiftOwnerCommodityList()} because the client packet-432 handler does not re-run `CUILogoutGift::OnCreate` or copy the context cache.{BuildPacketOwnedLogoutGiftClientUpdateSuffix()}{hiddenTrailingSuffix}{hiddenPayloadSuffix}";
                    _lastPacketOwnedLogoutGiftSummary = message;
                    NotifyEventAlarmOwnerActivity("packet-owned logout gift");
                    return true;
                }
            ...
            string trailingSuffix = HasPacketOwnedLogoutGiftOpaqueTail()
                ...
                : string.Empty;
            message =
                $"CWvsContext::OnLogoutGift called inherited `CWnd::Update` (`CUILogoutGift::Update(1)`) on the active logout-gift owner after `CUILogoutGift::TryShowLogoutGiftDialog` had already surfaced it, so only enabled child controls are refreshed. The owner-local `m_aCommodityID` copy remains {FormatPacketOwnedLogoutGiftOwnerCommodityList()} because packet 432 does not re-copy `CWvsContext::m_anLogoutGiftCommoditySN[0..2]` into the owner.{BuildPacketOwnedLogoutGiftClientUpdateSuffix()}{trailingSuffix}{payloadSuffix}";
            _lastPacketOwnedLogoutGiftSummary = message;
            NotifyEventAlarmOwnerActivity("packet-owned logout gift");
            return true;
        }
        ...
        internal static PacketOwnedLogoutGiftUpdateEffect ResolvePacketOwnedLogoutGiftUpdateEffect(
            bool ownerSingletonPresent,
            bool hasWindowInstance)
        {
            if (!ownerSingletonPresent)
            {
                return PacketOwnedLogoutGiftUpdateEffect.None;
            }

            return hasWindowInstance
                ? PacketOwnedLogoutGiftUpdateEffect.EnabledChildControlUpdate
        ...
        private string BuildPacketOwnedLogoutGiftClientUpdateSuffix()
        {
            if (_packetOwnedLogoutGiftClientUpdateCount <= 0)
            {
                return string.Empty;
            }
        ...
        private Texture2D TryLoadPacketOwnedLogoutGiftFrameTexture(string imageName, string propertyPath)
        {
            ...
            WzImage uiWindowImage = runtimeServices.Assets.FindImage("UI", imageName.Trim());
        ...
        private bool TryResolvePacketOwnedLogoutGiftSpecialItemName(int itemId, out string itemName)
        {
            itemName = null;
        ...
        private bool TryResolvePacketOwnedLogoutGiftSpecialItemProperty(int itemId, out WzSubProperty itemProperty)
        {
            itemProperty = null;
            if (!IsPacketOwnedLogoutGiftSpecialItem(itemId))
            {
            ...
            WzImage specialImage = runtimeServices.Assets.FindImage("Item", $"Special/{itemId / 10000:D4}");
        ...
        private bool TryEnsurePacketOwnedLogoutGiftCompletionDialogOwner()
        {
            ...
            WzImage basicImage = runtimeServices.Assets.FindImage("UI", "Basic.img");
    ...
    public enum PacketOwnedLogoutGiftUpdateEffect
    {
        None = 0,
        NativeSingletonOnly = 1,
        EnabledChildControlUpdate = 2,
    }
