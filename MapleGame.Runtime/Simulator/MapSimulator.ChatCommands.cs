...

{
...

    {
        ...
        {
            const string sessionUsage = "Usage: /socialroom tradingroom [packet] session [status|opcodes|discover <remotePort> [processName|pid] [localPort]|history [count]|clearhistory|replay <historyIndex>|sendraw <hex>|start <listenPort> <serverHost> <serverPort> [inboundOpcode|table|autodetect]|startauto <listenPort> <remotePort> [inboundOpcode|table|autodetect] [processName|pid] [localPort]|stop]";
            string sessionAction = args.Length > actionIndex + 1 ? args[actionIndex + 1] : "status";
            switch (sessionAction.ToLowerInvariant())
            {
                case "status":
                ...
                case "start":
                    ...
                    ushort inboundOpcode = TradingRoomPacketTable.ResolveRecoveredInboundOpcode(0);
                    if (args.Length > actionIndex + 5
                        && !TryResolveTradingRoomInboundOpcodeToken(args[actionIndex + 5], out inboundOpcode))
                    {
                        return ChatCommandHandler.CommandResult.Error($"{sessionUsage} {TradingRoomPacketTable.DescribeRecoveredInboundOpcodeSet()}");
                    }
                    ...
                    _tradingRoomOfficialSessionBridgeConfiguredInboundOpcode = inboundOpcode;
                ...
                case "startauto":
                    ...
                    ushort autoInboundOpcode = TradingRoomPacketTable.ResolveRecoveredInboundOpcode(0);
                    int trailingIndex = actionIndex + 4;
                    if (args.Length > trailingIndex
                        && TryResolveTradingRoomInboundOpcodeToken(args[trailingIndex], out ushort parsedInboundOpcode))
                    {
                        autoInboundOpcode = parsedInboundOpcode;
                        trailingIndex++;
                    }
                    else if (args.Length > trailingIndex && IsTradingRoomInboundOpcodeToken(args[trailingIndex]))
                    {
                        return ChatCommandHandler.CommandResult.Error($"{sessionUsage} {TradingRoomPacketTable.DescribeRecoveredInboundOpcodeSet()}");
                    }
                    ...
                    _tradingRoomOfficialSessionBridgeConfiguredInboundOpcode = autoInboundOpcode;
        ...
        private static bool IsTradingRoomInboundOpcodeToken(string token)
        {
            ...
            return string.Equals(normalizedToken, "table", StringComparison.OrdinalIgnoreCase)
                ...
                || string.Equals(normalizedToken, "autodetect", StringComparison.OrdinalIgnoreCase);
        }

        private static bool TryResolveTradingRoomInboundOpcodeToken(string token, out ushort inboundOpcode)
        {
            if (string.Equals(token?.Trim(), "autodetect", StringComparison.OrdinalIgnoreCase))
            {
                inboundOpcode = TradingRoomOfficialSessionBridgeManager.AutoDetectInboundTradingRoomOpcode;
                return true;
            }

            return TradingRoomPacketTable.TryResolveRecoveredInboundOpcodeToken(token, out inboundOpcode);
        }
        ...
        private ChatCommandHandler.CommandResult HandleTransportSessionCommand(string[] args)
        {
            ...
            switch (sessionAction.ToLowerInvariant())
            {
                ...
                case "discover":
                    if (args.Length < 3
                        || !TransportationSessionCommandParsing.TryParseRemotePort(args[2], out int discoverRemotePort))
                    {
                        return ChatCommandHandler.CommandResult.Error(sessionUsage);
                    }
                    ...
                    if (args.Length >= 5)
                    {
                        if (!TransportationSessionCommandParsing.TryParseLocalPortFilter(args[4], out int parsedLocalPort))
                        {
                            return ChatCommandHandler.CommandResult.Error(sessionUsage);
                        }
                ...
                case "attach":
                    if (args.Length < 3
                        || !TransportationSessionCommandParsing.TryParseRemotePort(args[2], out int attachRemotePort))
                    {
                        return ChatCommandHandler.CommandResult.Error(sessionUsage);
                    }
                    ...
                    if (args.Length >= 5)
                    {
                        if (!TransportationSessionCommandParsing.TryParseLocalPortFilter(args[4], out int parsedAttachLocalPort))
                        {
                            return ChatCommandHandler.CommandResult.Error(sessionUsage);
                        }
                ...
                case "attachproxy":
                    if (args.Length < 4
                        || !TransportationSessionCommandParsing.TryParseProxyListenPort(args[2], out int attachProxyListenPort)
                        || !TransportationSessionCommandParsing.TryParseRemotePort(args[3], out int attachProxyRemotePort))
                    {
                        return ChatCommandHandler.CommandResult.Error(sessionUsage);
                    }
                    ...
                    if (args.Length >= 6)
                    {
                        if (!TransportationSessionCommandParsing.TryParseLocalPortFilter(args[5], out int parsedAttachProxyLocalPort))
                        {
                            return ChatCommandHandler.CommandResult.Error(sessionUsage);
                        }
                ...
                case "sendinit":
                {
                    ...
                    if (args.Length >= 3)
                    {
                        if (!TransportationSessionCommandParsing.TryParseFieldIdOverride(args[2], out int parsedFieldId))
                        {
                            return ChatCommandHandler.CommandResult.Error(sessionUsage);
                        }
                    ...
                    if (args.Length >= 4)
                    {
                        if (!TransportationSessionCommandParsing.TryParseShipKindOverride(args[3], out int parsedShipKind))
                        {
                            return ChatCommandHandler.CommandResult.Error(sessionUsage);
                        }
                ...
                case "start":
                    if (args.Length < 5
                        || !TransportationSessionCommandParsing.TryParseProxyListenPort(args[2], out int listenPort)
                        || !TransportationSessionCommandParsing.TryParseRemotePort(args[4], out int remotePort))
                    {
                        return ChatCommandHandler.CommandResult.Error(sessionUsage);
                    }
                ...
                case "startauto":
                    if (args.Length < 4
                        || !TransportationSessionCommandParsing.TryParseProxyListenPort(args[2], out int autoListenPort)
                        || !TransportationSessionCommandParsing.TryParseRemotePort(args[3], out int autoRemotePort))
                    {
                        return ChatCommandHandler.CommandResult.Error(sessionUsage);
                    }
                    ...
                    if (args.Length >= 6)
                    {
                        if (!TransportationSessionCommandParsing.TryParseLocalPortFilter(args[5], out int parsedAutoLocalPort))
                        {
                            return ChatCommandHandler.CommandResult.Error(sessionUsage);
                        }
        ...
        /// </summary>
        private string DispatchMapleTvConsumeCashItemUseRequest(int inventoryPosition, int itemId)
        {
        ...
        private void RegisterChatCommands()
        {
            ...
            _chat.CommandHandler.RegisterCommand(
                ...
                args =>
                {
                    bool transportActive = IsTransitVoyageWrapperMap(_mapInfo) && _transportField.HasRouteConfiguration;
                    if (args.Length == 0 || string.Equals(args[0], "status", StringComparison.OrdinalIgnoreCase))
                    {
                        return ChatCommandHandler.CommandResult.Info(
                            $"{_transportField.DescribeStatus()}{Environment.NewLine}{_lastTransportFieldInitRequestSummary}{Environment.NewLine}{_transportPacketInbox.LastStatus}{Environment.NewLine}{_transportOfficialSessionBridge.DescribeStatus()}");
                    ...
                    switch (args[0].ToLowerInvariant())
                    {
                        ...
                        case "request":
                                ...
                                || _monsterCarnivalOfficialSessionBridge.HasPassiveEstablishedSocketPair)
                            {
                                ...
                                {
                                    field.MarkPendingLocalRequest(requestTab, entryIndex);
                ...
                args =>
                {
                    ...
                    if (string.Equals(args[0], "session", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (string.Equals(args[1], "attachproxy", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            return _massacreOfficialSessionBridge.TryAttachEstablishedSessionAndStartProxy(
                                    attachProxyListenPort,
                                    attachProxyRemotePort,
                                    attachProxyProcessSelector,
                                    attachProxyLocalPortFilter,
                        ...
                        if (string.Equals(args[1], "recent", StringComparison.OrdinalIgnoreCase))
                        {
                            int recentCount = 10;
                            if (args.Length >= 3
                                && !MassacreSessionCommandParsing.TryParseRecentCount(args[2], out recentCount))
                            {
                                return ChatCommandHandler.CommandResult.Error(MassacreSessionCommandParsing.RecentUsage);
                            }
                            ...
                            return ChatCommandHandler.CommandResult.Info(_massacreOfficialSessionBridge.DescribeRecentInboundPackets(recentCount));
                        }
                        ...
                        if (string.Equals(args[1], "clearrecent", StringComparison.OrdinalIgnoreCase))
                        {
                            if (args.Length != 2)
                            {
                                return ChatCommandHandler.CommandResult.Error(MassacreSessionCommandParsing.SessionUsage);
                            }
            ...
            _chat.CommandHandler.RegisterCommand(
                ...
                args =>
                {
                    ...
                    {
                        if (!InventoryItemMetadataResolver.TryResolveClientItemCrc(itemId, runtimeServices.Catalog, out uint itemCrc) || itemCrc == 0)
                        {
                    ...
                    bool TrySendMerchantBuyRequest(int requestedIndex, out string packetMessage)
                    {
                        ...
                        if (!InventoryItemMetadataResolver.TryResolveClientItemCrc(selectedEntry.ItemId, runtimeServices.Catalog, out uint itemCrc) || itemCrc == 0)
                        {
                ...
                args =>
                {
                    ...
                    switch (args[0].ToLowerInvariant())
                    {
                        ...
                        case "packet":
                            if (args.Length < 2)
                            {
                                return ChatCommandHandler.CommandResult.Error("Usage: /family packet <clear|seed|name <familyName>|precept <text>|authority <local|session|readonly|privilegeonly|manageonly>|localchart|info|result|privilegelist|setprivilege <payloadhex=..|payloadb64=..>|remove <memberId>|upsert <memberId> <parentId|root> <level> <online|offline> <currentRep> <todayRep> <name>|<job>|<location>>");
                            }
                            ...
                            switch (args[1].ToLowerInvariant())
                            {
                                ...
                                case "localchart":
                                    if (args.Length < 3)
                                    {
                                        return ChatCommandHandler.CommandResult.Error("Usage: /family packet localchart <payloadhex=..|payloadb64=..>");
                                    }

                                    if (!FamilyPacketCodec.TryParsePayloadToken(args[2], out byte[] familyLocalChartPayload, out string familyLocalChartError))
                                    {
                                        return ChatCommandHandler.CommandResult.Error(familyLocalChartError);
                                    }

                                    return ChatCommandHandler.CommandResult.Ok(_familyChartRuntime.ApplyLocalChartPacketPayload(familyLocalChartPayload));
                                case "info":
                                    if (args.Length < 3)
                                    {
                                ...
                                case "result":
                                    ...
                                    return _familyChartRuntime.TryApplyResultPacketPayload(familyResultPayload, out string familyResultMessage)
                                        ? ChatCommandHandler.CommandResult.Ok(AppendFamilyPrivilegeTransferCompletion(familyResultMessage))
                        ...
                        case "packetraw":
                            if (args.Length < 3)
                            {
                                return ChatCommandHandler.CommandResult.Error("Usage: /family packetraw <localchart|info|result|privilegelist|setprivilege> <hex>");
                            }
                            ...
                            {
                                "localchart" => ChatCommandHandler.CommandResult.Ok(_familyChartRuntime.ApplyLocalChartPacketPayload(familyRawPayload)),
                                ...
                                "result" => _familyChartRuntime.TryApplyResultPacketPayload(familyRawPayload, out string familyRawResultMessage)
                                    ? ChatCommandHandler.CommandResult.Ok(AppendFamilyPrivilegeTransferCompletion(familyRawResultMessage))
                                ...
                                _ => ChatCommandHandler.CommandResult.Error("Usage: /family packetraw <localchart|info|result|privilegelist|setprivilege> <hex>")
                            };
                        case "packetrecv":
                            ...
                            return _familyChartRuntime.TryApplyClientPacketPayload(familyRecvOpcode, familyRecvPayload, out string familyRecvMessage)
                                ? ChatCommandHandler.CommandResult.Ok(AppendFamilyPrivilegeTransferCompletion(familyRecvMessage))
            ...
            _chat.CommandHandler.RegisterCommand(
                ...
                args =>
                {
                    ...
                    switch (args[0].ToLowerInvariant())
                    {
                        ...
                        case "claim":
                            return ChatCommandHandler.CommandResult.Ok(TryHandleMessengerClaimActionThroughClientSeam());
                        case "claimnotice":
                            return _messengerRuntime.TryOpenClaimDialogFromChat(out string claimNoticeMessage)
                                ? ChatCommandHandler.CommandResult.Ok(claimNoticeMessage)
                                : ChatCommandHandler.CommandResult.Error(claimNoticeMessage);
                        case "claimcategory":
                            return ChatCommandHandler.CommandResult.Ok(
                                _messengerRuntime.SelectClaimDialogCategory(args.Length >= 2 ? string.Join(" ", args.Skip(1)) : "chat"));
                        case "claimconfirm":
                            return ChatCommandHandler.CommandResult.Ok(TryHandleMessengerClaimActionThroughClientSeam());
                        case "claimcancel":
                            return ChatCommandHandler.CommandResult.Ok(_messengerRuntime.CancelClaimDialog());
                        case "leave":
...
"/localutilitypacket [status|start [port]|stop|packet <sitresult|teleport|questresult|openui|openuiwithoption|commodity|notice|chat|buffzone|eventsound|minigamesound|skillguide|antimacro|apspevent|follow|followfail|directionmode|standalone|damagemeter|passivemove|hpdec|skillcooltime|193|231|234|242|243|246|247|250|251|252|262|263|264|265|266|267|268|269|270|273|274|275|276|291|366|367|1011|1012|1013|1014|classcompetition|classcompetitionauth|questguide|deliveryquest|adminshopopen|adminshopresult> [payloadhex=..|payloadb64=..]|packetraw <type> [hex]|packetclientraw <hex>]",
            ...
            _chat.CommandHandler.RegisterCommand(
                ...
                args =>
                {
                    ...
                    switch (action)
                    {
                            ...
                            if (args.Length < 2 || !int.TryParse(args[1], out int itemId) || itemId < 0)
                            {
                                ...
                                return ChatCommandHandler.CommandResult.Ok(
                                    _mapleTvRuntime.SetItem(
                                        ...
                                        itemId > 0 && InventoryItemMetadataResolver.TryResolveItemDescription(itemId, runtimeServices.Catalog, out string itemDescription) ? itemDescription : null));
                        ...
                        case "sendrequest":
                        case "send":
                            if (args.Length < 2 || !int.TryParse(args[1], out int inventoryPosition))
                            {
                                return ChatCommandHandler.CommandResult.Error("Usage: /mapletv sendrequest <inventoryPosition> [itemId]");
                            }


                            int overrideMapleTvItemId = 0;
                            if (args.Length > 2
                                && (!int.TryParse(args[2], out overrideMapleTvItemId) || overrideMapleTvItemId < 0))
                            {
                                return ChatCommandHandler.CommandResult.Error("Usage: /mapletv sendrequest <inventoryPosition> [itemId]");
                            }


                            string sendRequestMessage = DispatchMapleTvConsumeCashItemUseRequest(inventoryPosition, overrideMapleTvItemId);
                            return sendRequestMessage.StartsWith("CUserLocal::ConsumeCashItem", StringComparison.Ordinal)
                                ? ChatCommandHandler.CommandResult.Ok(sendRequestMessage)
                                : ChatCommandHandler.CommandResult.Error(sendRequestMessage);
            ...
            _chat.CommandHandler.RegisterCommand(
                ...
                args =>
                {
                    if (_temporaryPortalField == null || _mapInfo == null)
                    {
                    ...
                    {
                        return ChatCommandHandler.CommandResult.Info(_temporaryPortalField.DescribeRemotePortalStatus(_mapInfo.id));
                    }
...

                    {
                        ...
                        return ChatCommandHandler.CommandResult.Ok($"Cleared packet-authored remote portals. {_temporaryPortalField.DescribeRemotePortalStatus(_mapInfo.id)}");
                ...
                args =>
                {
                    ...
                    switch (args[0].ToLowerInvariant())
                    {
                        ...
                        case "backeffect":
                            ...
                            return TryApplyPacketOwnedStageTransitionPacket(
                                144,
                                PacketStageTransitionRuntime.BuildBackEffectPayload(effect, _mapInfo?.id ?? 0, pageId, durationMs),
            ...
            _chat.CommandHandler.RegisterCommand(
                ...
                args =>
                {
                    if (_affectedAreaPool == null || _mapInfo == null)
                    {
            ...
            _chat.CommandHandler.RegisterCommand(
                ...
                args =>
                {
                    if (_dropPool == null || _mapInfo == null)
                    {
        ...
        private string DescribePetAutoConsumeHpSetting(int petSlotIndex, PetRuntime pet)
        {
            if (pet == null)
            {
            ...
            string itemName = InventoryItemMetadataResolver.TryResolveItemName(pet.AutoConsumeHpItemId, runtimeServices.Catalog, out string resolvedItemName)
