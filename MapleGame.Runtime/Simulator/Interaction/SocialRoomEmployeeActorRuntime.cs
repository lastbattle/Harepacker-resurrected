...
namespace HaCreator.MapSimulator.Interaction
{
    internal sealed class SocialRoomEmployeeActorRuntime
    {
        ...
        private const int MiniRoomLayoutBaselineBoardWidth = 156;
        private const int MiniRoomLayoutBaselineBoardHeight = 159;
        private const int MiniRoomLayoutBaselineSignboardOriginX = 21;
        private const int MiniRoomLayoutBaselineSignboardOriginY = 29;
        private const int MiniRoomLayoutLegacyIconX = ChatBalloonPresentationRules.MiniRoomShopIconX;
        private const int MiniRoomLayoutLegacyIconY = ChatBalloonPresentationRules.MiniRoomShopIconY;
        private const int MiniRoomLayoutLegacyPrivacyIconX = ChatBalloonPresentationRules.MiniRoomShopPrivacyIconX;
        private const int MiniRoomLayoutLegacyPrivacyIconY = ChatBalloonPresentationRules.MiniRoomShopPrivacyIconY;
        private const int MiniRoomLayoutLegacyCurrentCountX = ChatBalloonPresentationRules.MiniRoomShopCurrentCountX;
        private const int MiniRoomLayoutLegacyCurrentCountY = ChatBalloonPresentationRules.MiniRoomShopCurrentCountY;
        private const int MiniRoomLayoutLegacyMaxCountX = ChatBalloonPresentationRules.MiniRoomShopMaxCountX;
        private const int MiniRoomLayoutLegacyMaxCountY = ChatBalloonPresentationRules.MiniRoomShopMaxCountY;
        private const int MiniRoomLayoutLegacyStatusX = ChatBalloonPresentationRules.MiniRoomShopStatusX;
        private const int MiniRoomLayoutLegacyStatusY = ChatBalloonPresentationRules.MiniRoomShopStatusY;
        ...
        private readonly int _npcRx0Offset;
        private readonly int _npcRx1Offset;
        ...
        public SocialRoomEmployeeActorRuntime(IRuntimeDataServices runtimeServices, int npcRx0Offset = 20, int npcRx1Offset = 20)
        {
            _runtimeServices = runtimeServices ?? throw new ArgumentNullException(nameof(runtimeServices));
            _npcRx0Offset = npcRx0Offset;
            _npcRx1Offset = npcRx1Offset;
        }
        ...
        {
            if (snapshot == null || player == null || texturePool == null || device == null)
            {
                Clear();
                return;
            }
            ...
            Func<int, string> questRecordValueProvider)
        {
            ...
            NpcItem actor = snapshot.Template switch
            {
                SocialRoomFieldActorTemplate.CashEmployee when snapshot.TemplateId > 0
                    => CreateCashEmployeeActor(actorKey, snapshot.TemplateId, texturePool, device, userScreenScaleFactor),
                _ => CreateNpcActor(
        ...
        private NpcItem CreateNpcActor(
            ...
            Func<int, string> questRecordValueProvider)
        {
            _runtimeServices.Catalog.TryGetNpcName(npcId, out RuntimeNpcName npcName);
            var npcInstance = new RuntimeLife(new RuntimeLifeDefinition
            {
                Asset = new RuntimeAssetKey("Npc", npcId.PadLeft(7, '0') + ".img"),
                Id = npcId,
                DisplayName = npcName?.Name ?? npcId,
                Rx0Shift = _npcRx0Offset,
                Rx1Shift = _npcRx1Offset,
                YShift = 8
            }, functionDescription: npcName?.Function);
            ...
            float userScreenScaleFactor)
        {
            ...
            _runtimeServices.Catalog.TryGetNpcName(MerchantNpcId, out RuntimeNpcName merchantName);
            var npcInstance = new RuntimeLife(new RuntimeLifeDefinition
            {
                Asset = new RuntimeAssetKey("Npc", MerchantNpcId.PadLeft(7, '0') + ".img"),
                Id = MerchantNpcId,
                DisplayName = merchantName?.Name ?? MerchantNpcId,
                Rx0Shift = _npcRx0Offset,
                Rx1Shift = _npcRx1Offset,
                YShift = 8
            }, functionDescription: merchantName?.Function);
        ...
        private void SyncActorPosition(PlayerCharacter player, NpcItem actor, SocialRoomFieldActorSnapshot snapshot)
        {
            ...
            actor.RuntimeLife.Flip = ResolveActorFlip(snapshot, player, actorX);
        }
        ...
        private EmployeeImageEntry ResolveEmployeeImgEntry(int templateId)
        {
            ...
            WzImage itemImage = _runtimeServices.Assets.FindImage("Item", imagePath);
            if (itemImage == null)
            {
                return null;
            }
        ...
        private void EnsureMiniRoomBalloonAssets(GraphicsDevice device)
        {
            ...
            WzImage chatBalloonImage = _runtimeServices.Assets.FindImage("UI", "ChatBalloon.img");
            ...
            int mapCenterY)
        {
            ...
            if (assets?.IsLoaded == true)
            {
                ...
                int y = actorScreenY - _activeActor.Height - assets.Left.Height - NameTagVerticalOffset;
            ...
            Vector2 fallbackPosition = new(
                ...
                actorScreenY - _activeActor.Height - textSize.Y - NameTagVerticalOffset);
            ...
            int mapCenterY)
        {
            ...
            int boxY = actorScreenY - _activeActor.Height - boxHeight - SignVerticalOffset;
