...
namespace HaCreator.MapSimulator.Interaction
{
    ...
    {
        public IRuntimeDataServices Services { get; init; }
    ...
    internal static class NpcDialogueTextFormatter
    {
        ...
        public static string Format(string text, NpcDialogueFormattingContext context = null)
        {
            ...
            formatted = NpcRegex.Replace(formatted, match => ResolveNpcName(context, match.Groups[1].Value));
            formatted = ItemNameRegex.Replace(formatted, match => ResolveItemName(context, match.Groups[1].Value));
            formatted = MobNameRegex.Replace(formatted, match => ResolveMobName(context, match.Groups[1].Value));
            formatted = ItemNameAliasRegex.Replace(formatted, match => ResolveItemName(context, match.Groups[1].Value));
            formatted = MalformedItemNameRegex.Replace(formatted, match => ResolveItemName(context, match.Groups["id"].Value));
            formatted = QuestRecordOrNameRegex.Replace(formatted, match => ResolveQuestRecordOrNameText(match.Groups[1].Value, context));
            formatted = QuestNameRegex.Replace(formatted, match => ResolveQuestName(context, match.Groups[1].Value));
            formatted = CurrentQuestNameRegex.Replace(formatted, match => ResolveActiveQuestNameText(context));
            formatted = QuestReferenceNameRegex.Replace(formatted, match => ResolveQuestName(context, match.Groups[1].Value));
            ...
            formatted = SkillNameRegex.Replace(formatted, match => ResolveSkillName(context, match.Groups[1].Value));
            formatted = MapNameRegex.Replace(formatted, match => ResolveMapName(context, match.Groups[1].Value));
            ...
            formatted = SelectedMobRegex.Replace(formatted, match => ResolveSelectedMobText(context, match.Groups[1].Value));
            formatted = CurrentSelectedMobRegex.Replace(formatted, match => ResolveActiveSelectedMobText(context));
            formatted = QuestAmountRegex.Replace(formatted, match => ResolveQuestAmountText(context, match.Groups[1].Value));
        ...
        public static string FormatPreservingQuestDetailMarkers(string text, NpcDialogueFormattingContext context = null)
        {
            ...
            preservedMarkers = ItemIconRegex.Replace(
                ...
                match => int.TryParse(match.Value.TrimStart('#', 'i', 'I', 'v', 'V').TrimEnd('#', ':'), out int itemId) && itemId > 0
                    ? BuildItemIconMarker(itemId)
                    : string.Empty);
            preservedMarkers = QuestDetailNpcReferenceRegex.Replace(
                preservedMarkers,
                match => int.TryParse(match.Groups["id"].Value, out int npcId) && npcId > 0
                    ? BuildQuestInlineReferenceMarker("npc", npcId, ResolveNpcName(context, match.Groups["id"].Value))
                    : string.Empty);
            preservedMarkers = QuestDetailMapReferenceRegex.Replace(
                preservedMarkers,
                match => int.TryParse(match.Groups["id"].Value, out int mapId) && mapId > 0
                    ? BuildQuestInlineReferenceMarker("map", mapId, ResolveMapName(context, match.Groups["id"].Value))
                    : string.Empty);
            preservedMarkers = QuestDetailMobReferenceRegex.Replace(
                preservedMarkers,
                match => int.TryParse(match.Groups["id"].Value, out int mobId) && mobId > 0
                    ? BuildQuestInlineReferenceMarker("mob", mobId, ResolveMobName(context, match.Groups["id"].Value))
                    : string.Empty);
            preservedMarkers = QuestDetailItemReferenceRegex.Replace(
                preservedMarkers,
                match => int.TryParse(match.Groups["id"].Value, out int itemId) && itemId > 0
                    ? BuildQuestInlineReferenceMarker("item", itemId, ResolveItemName(context, match.Groups["id"].Value))
                    : string.Empty);
        ...
        public static string BuildQuestInlineReferenceMarker(string kind, int targetId, string label)
        {
            ...
            string normalizedLabel = (label ?? string.Empty)
                ...
                .Trim();
            return string.IsNullOrWhiteSpace(normalizedKind) || targetId <= 0
                ? normalizedLabel
                : $"{{{{QUESTREF:{normalizedKind}:{targetId}:{normalizedLabel}}}}}";
        }
        ...
        private static string ResolveNpcName(NpcDialogueFormattingContext context, string npcIdText)
        {
            return context?.Services?.Catalog != null && context.Services.Catalog.TryGetNpcName(npcIdText, out var info) &&
                   !string.IsNullOrWhiteSpace(info?.Name)
                ? info.Name
                : $"NPC #{npcIdText}";
        }

        private static string ResolveItemName(NpcDialogueFormattingContext context, string itemIdText)
        {
            return int.TryParse(itemIdText, out int itemId) &&
                   context?.Services?.Catalog != null && context.Services.Catalog.TryGetItemName(itemId, out var itemInfo) &&
                   !string.IsNullOrWhiteSpace(itemInfo?.Name)
                ? itemInfo.Name
                : $"Item #{itemIdText}";
        }

        private static string ResolveQuestName(NpcDialogueFormattingContext context, string questIdText)
        {
            return context?.Services?.Assets.FindImage("Quest", "QuestInfo.img")?[questIdText] is WzSubProperty questInfo &&
        ...
        private static string ResolveSkillName(NpcDialogueFormattingContext context, string skillIdText)
        {
            return context?.Services?.Catalog != null && context.Services.Catalog.TryGetSkillName(skillIdText, out var skillInfo) &&
                   !string.IsNullOrWhiteSpace(skillInfo?.Name)
                ? skillInfo.Name
                : $"Skill #{skillIdText}";
        }

        private static string ResolveMapName(NpcDialogueFormattingContext context, string mapIdText)
        {
            if (context?.Services?.Catalog == null)
            {
                return $"Map #{mapIdText}";
            }

            string normalizedMapId = mapIdText.Length == 9 ? mapIdText : mapIdText.PadLeft(9, '0');
            return context.Services.Catalog.TryGetMapName(normalizedMapId, out var mapInfo) &&
                   !string.IsNullOrWhiteSpace(mapInfo?.MapName)
                ? mapInfo.MapName
                : $"Map #{mapIdText}";
        }

        private static string ResolveMobName(NpcDialogueFormattingContext context, string mobIdText)
        {
            return context?.Services?.Catalog != null && context.Services.Catalog.TryGetMobName(mobIdText, out var mobName) &&
                   !string.IsNullOrWhiteSpace(mobName.Name)
                ? mobName.Name
                : $"Mob #{mobIdText}";
        }
        ...
        private static string ResolveActiveQuestNameText(NpcDialogueFormattingContext context)
        {
            ...
            return activeQuestId > 0
                ? ResolveQuestName(context, activeQuestId.ToString())
                : "current quest";
        }
        ...
        private static string ResolveSelectedMobText(NpcDialogueFormattingContext context, string questIdText)
        {
            if (TryGetQuestMobRequirement(context, questIdText, out int mobId, out _))
            {
                return ResolveMobName(context, mobId.ToString());
            }

            return HasSelectedMobFlag(context, questIdText)
                ? "the selected monster"
                : $"Mob #{questIdText}";
        }

        private static string ResolveQuestAmountText(NpcDialogueFormattingContext context, string questIdText)
        {
            if (TryGetQuestMobRequirement(context, questIdText, out _, out int mobCount))
            {
                return mobCount.ToString();
            }

            if (TryGetQuestItemRequirement(context, questIdText, out _, out int itemCount))
            {
        ...
        private static string ResolveQuestValueText(NpcDialogueFormattingContext context, string questIdText)
        {
            return HasSelectedMobFlag(context, questIdText)
                ? "the selected bonus amount"
                : ResolveQuestAmountText(context, questIdText);
        }
        ...
        private static string ResolveActiveSelectedMobText(NpcDialogueFormattingContext context)
        {
            int activeQuestId = context?.ActiveQuestId ?? 0;
            return activeQuestId > 0
                ? ResolveSelectedMobText(context, activeQuestId.ToString())
                : "the selected monster";
        }
        ...
        private static string ResolveActiveQuestAmountText(NpcDialogueFormattingContext context)
        {
            int activeQuestId = context?.ActiveQuestId ?? 0;
            return activeQuestId > 0
                ? ResolveQuestAmountText(context, activeQuestId.ToString())
                : "the listed amount";
        }
        ...
        private static string ResolveActiveQuestValueText(NpcDialogueFormattingContext context)
        {
            int activeQuestId = context?.ActiveQuestId ?? 0;
            return activeQuestId > 0
                ? ResolveQuestValueText(context, activeQuestId.ToString())
                : "the listed amount";
        }
        ...
        private static string ResolveQuestRecordOrNameText(string questIdText, NpcDialogueFormattingContext context)
        {
            ...
            return string.IsNullOrWhiteSpace(recordText) || string.Equals(recordText, "0", StringComparison.Ordinal)
                ? ResolveQuestName(context, questIdText)
                : recordText;
        }
        ...
        private static bool HasSelectedMobFlag(NpcDialogueFormattingContext context, string questIdText)
        {
            return context?.Services?.Assets.FindImage("Quest", "QuestInfo.img")?[questIdText] is WzSubProperty questInfo &&
        ...
        private static bool TryGetQuestMobRequirement(NpcDialogueFormattingContext context, string questIdText, out int mobId, out int count)
        {
            if (TryGetQuestRequirement(context, questIdText, "mob", out WzImageProperty requirementProperty) &&
        ...
        private static bool TryGetQuestItemRequirement(NpcDialogueFormattingContext context, string questIdText, out int itemId, out int count)
        {
            if (TryGetQuestRequirement(context, questIdText, "item", out WzImageProperty requirementProperty) &&
        ...
        private static bool TryGetQuestRequirement(NpcDialogueFormattingContext context, string questIdText, string requirementName, out WzImageProperty requirementProperty)
        {
            requirementProperty = null;
            if (context?.Services?.Assets.FindImage("Quest", "Check.img")?[questIdText] is not WzSubProperty questCheck)
            {
                return false;
            }
