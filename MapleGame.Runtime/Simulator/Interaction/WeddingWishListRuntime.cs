...
namespace HaCreator.MapSimulator.Interaction
{
    ...
    internal sealed class WeddingWishListRuntime
    {
        ...
        private IRuntimeAssetCatalog _runtimeCatalog;
        ...
        internal void SetRuntimeAssetCatalog(IRuntimeAssetCatalog runtimeCatalog)
        {
        ...
        private bool TryApplyClientGetItemsRefresh(IReadOnlyList<byte> payload, int startOffset, out int refreshedRows)
        {
            ...
            for (int tabIndex = 0; tabIndex < TabInventoryTypes.Length; tabIndex++)
            {
                ...
                if (!TryReadClientGetItemRows(reader, TabInventoryTypes[tabIndex], rowCount, _runtimeCatalog, out List<InventorySlotData> rows))
                {
                    return false;
                }
        ...
        private static bool TryReadClientGetItemRows(
            BinaryReader reader,
            InventoryType type,
            int rowCount,
            IRuntimeAssetCatalog runtimeCatalog,
            out List<InventorySlotData> rows)
        {
            ...
            if (!TryDecodeClientGetItemRows(remainingBytes, type, rowCount, rows, runtimeCatalog, out int consumedBytes))
            {
            ...
            out int consumedBytes)
        {
            ...
            for (int rowLength = 5; rowLength <= payload.Length - minimumTailBytes; rowLength++)
            {
                ...
                if (!TryDecodeClientGetItemRowPayload(rowPayload, type, runtimeCatalog, out InventorySlotData item))
                {
                    continue;
                }
                ...
                if (TryDecodeClientGetItemRows(tailPayload, type, rowsRemaining - 1, rows, runtimeCatalog, out int tailConsumedBytes))
                {
        ...
        private static bool TryDecodeClientGetItemRowPayload(
            byte[] payload,
            InventoryType type,
            IRuntimeAssetCatalog runtimeCatalog,
            out InventorySlotData item)
        {
            item = null;
            ...
            if (firstByte is 1 or 2 or 3 && TryReadClientGetItemSlotBody(reader, firstByte, type, runtimeCatalog, out item))
            {
                return true;
            }
            ...
            item = CreateFallbackSlot(itemId, type, runtimeCatalog);
            item.Quantity = quantity;
            return item.ItemId > 0;
        }

        private static bool TryReadClientGetItemSlotBody(
            BinaryReader reader,
            byte slotType,
            InventoryType type,
            IRuntimeAssetCatalog runtimeCatalog,
        ...
        private InventorySlotData CloneForDialog(InventorySlotData slot)
        {
            return CloneForDialog(slot, ResolveInventoryTypeForSlot(slot), slot?.Quantity ?? 1);
        }

        private InventorySlotData CloneForDialog(InventorySlotData slot, InventoryType type, int quantity)
        {
            if (slot == null)
            {
                return null;
            ...
            {
                clone.ItemName = _runtimeCatalog != null
                    && InventoryItemMetadataResolver.TryResolveItemName(clone.ItemId, _runtimeCatalog, out string itemName)
                    ? itemName
                    : $"Item {clone.ItemId}";
            }
            ...
            {
                string description = _runtimeCatalog != null
                    && InventoryItemMetadataResolver.TryResolveItemDescription(clone.ItemId, _runtimeCatalog, out string resolvedDescription)
                    ? resolvedDescription
                    : null;
        ...
        private InventorySlotData CreateFallbackSlot(int itemId, InventoryType type)
        {
            return CreateFallbackSlot(itemId, type, _runtimeCatalog);
        }

        private static InventorySlotData CreateFallbackSlot(int itemId, InventoryType type, IRuntimeAssetCatalog runtimeCatalog)
        {
            string itemName = runtimeCatalog != null
                && InventoryItemMetadataResolver.TryResolveItemName(itemId, runtimeCatalog, out string resolvedItemName)
                ? resolvedItemName
                : null;
            string description = runtimeCatalog != null
                && InventoryItemMetadataResolver.TryResolveItemDescription(itemId, runtimeCatalog, out string resolvedDescription)
                ? resolvedDescription
                : null;
