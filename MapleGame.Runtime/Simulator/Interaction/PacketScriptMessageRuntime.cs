...
namespace HaCreator.MapSimulator.Interaction
{
    internal sealed class PacketScriptMessageRuntime
    {
        ...
        private readonly NpcDialogueFormattingContext formattingContext;
        public PacketScriptMessageRuntime(IRuntimeDataServices services)
        {
            runtimeServices = services ?? throw new ArgumentNullException(nameof(services));
            formattingContext = new NpcDialogueFormattingContext { Services = services };
        }
        private const short OutboundScriptAnswerOpcode = 65;
        private const int AskNumberInputDigitLimit = 10;
        private readonly object NpcStringImageLock = new();
        private readonly Dictionary<int, string> NpcNameByIdWzFallbackCache = new();
        private WzImage _npcStringImage;
        ...
        private NpcInteractionEntry DecodeSay(
            BinaryReader reader,
            ...
            Func<int, NpcItem> findNpcById)
        {
            ...
            return CreateEntry(
                ...
                rawText,
                AppendMetadata(
                    NpcDialogueTextFormatter.Format(rawText, formattingContext),
        ...
        private NpcInteractionEntry DecodeSayImage(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
        ...
        private NpcInteractionEntry DecodeAskYesNo(BinaryReader reader, PacketScriptSpeaker speaker, byte param, bool isQuestPrompt)
        {
            string rawText = ReadMapleString(reader);
        ...
        private NpcInteractionEntry DecodeAskText(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
            string rawText = ReadMapleString(reader);
        ...
        private NpcInteractionEntry DecodeAskNumber(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
            string rawText = ReadMapleString(reader);
        ...
        private NpcInteractionEntry DecodeAskMenu(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
            string rawText = ReadMapleString(reader);
            ...
            List<NpcInteractionChoice> choices = selections
                .Select(selection =>
                {
                    string label = NpcDialogueTextFormatter.Format(selection.Label, formattingContext);
        ...
        private PacketScriptDecodeResult DecodeAskAvatar(BinaryReader reader, PacketScriptSpeaker speaker, byte param, bool isMembershopAvatar)
        {
            string rawText = ReadMapleString(reader);
            int count = reader.ReadByte();
        ...
        private PacketScriptDecodeResult DecodeAskQuiz(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
        ...
        private PacketScriptDecodeResult DecodeAskSpeedQuiz(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
            long startPosition = reader.BaseStream.Position;
        ...
        private PacketScriptDecodeResult DecodeAskPet(
            BinaryReader reader,
        ...
        private PacketScriptDecodeResult DecodeAskSlideMenu(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
            long startPosition = reader.BaseStream.Position;
        ...
        private NpcInteractionEntry DecodeAskBoxText(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
            string rawText = ReadMapleString(reader);
            string defaultText = ReadMapleString(reader);
        ...
        private bool TryDecodeAskQuizClientPacket(BinaryReader reader, PacketScriptSpeaker speaker, byte param, out PacketScriptDecodeResult result)
        {
            long startPosition = reader.BaseStream.Position;
            result = null;
            try
            {
                ...
                result = CreateDecodedResult(CreateEntry(
                    ...
                    AppendMetadata(
                        ...
                        NpcDialogueTextFormatter.Format(problemText, formattingContext),
        ...
        private NpcInteractionEntry DecodeAskQuizCompactPacket(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
            string rawText = ReadMapleString(reader);
            int defaultValue = reader.ReadInt32();
            int minValue = reader.ReadInt32();
        ...
        private bool TryDecodeAskSpeedQuizClientPacket(BinaryReader reader, PacketScriptSpeaker speaker, byte param, out PacketScriptDecodeResult result)
        {
            long startPosition = reader.BaseStream.Position;
            result = null;
            try
        ...
        private NpcInteractionEntry DecodeAskSpeedQuizCompactPacket(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
            string rawText = ReadMapleString(reader);
            string defaultText = ReadMapleString(reader);
            ...
            List<NpcInteractionChoice> choices = options
                .Select((option, index) => CreateNumericResponseChoice(
                    $"{index + 1}. {NpcDialogueTextFormatter.Format(option, formattingContext)}",
                    option,
                    index))
                .ToList();
        ...
        private bool TryDecodeAskPetClientPacket(
            BinaryReader reader,
            PacketScriptSpeaker speaker,
            byte param,
            bool isPetAll,
        ...
        private NpcInteractionEntry DecodeAskPetCompactPacket(BinaryReader reader, PacketScriptSpeaker speaker, byte param, bool isPetAll)
        {
            string rawText = ReadMapleString(reader);
            if (!TryReadByte(reader, out byte count))
            {
        ...
        private NpcInteractionEntry CreatePetSelectionEntry(
            PacketScriptSpeaker speaker,
            byte param,
            bool isPetAll,
            string rawText,
        ...
        private List<PacketScriptPetSelectionCandidate> FilterSelectablePets(
        ...
        private string BuildPetItemChoiceLabel(int itemId, int index)
        {
            return $"{index + 1}. {DescribePetItemOption(itemId, index)}";
        }

        private string DescribePetItemOption(int itemId, int index)
        {
            if (itemId <= 0)
            {
            ...
            if (runtimeServices.Catalog.TryGetItemName(itemId, out var itemInfo))
            {
        ...
        private bool TryDecodeAskSlideMenuClientPacket(BinaryReader reader, PacketScriptSpeaker speaker, byte param, out PacketScriptDecodeResult result)
        {
            long startPosition = reader.BaseStream.Position;
            result = null;
            try
        ...
        private NpcInteractionEntry DecodeAskSlideMenuCompactPacket(BinaryReader reader, PacketScriptSpeaker speaker, byte param)
        {
            string rawText = ReadMapleString(reader);
        ...
        private NpcInteractionEntry CreateSlideMenuEntry(
            PacketScriptSpeaker speaker,
            byte param,
            int slideMenuType,
            ...
            string decodedPrefix)
        {
            List<NpcInteractionChoice> choices = options
                .Select((option, index) => CreateNumericResponseChoice(
                    $"{index + 1}. {NpcDialogueTextFormatter.Format(option.Label, formattingContext)}",
            ...
            return CreateEntry(
                ...
                rawText,
                AppendMetadata(
                    string.IsNullOrWhiteSpace(rawText) ? "No slide-menu prompt string was decoded." : NpcDialogueTextFormatter.Format(rawText, formattingContext),
                    choices.Count == 0
        ...
        private PacketScriptDecodeResult CreateDecodedResult(
        ...
        private PacketScriptDecodeResult DecodeIgnoredUnsupportedMessage(BinaryReader reader, PacketScriptSpeaker speaker, int messageType, byte param)
        {
        ...
        private bool ValidatePromptSubmission(
        ...
        private string BuildSpeakerSubtitle(PacketScriptSpeaker speaker, string messageKind, byte param, int? mode = null)
        {
        ...
        private PacketScriptSpeaker ResolveSpeaker(
            int speakerTypeId,
            ...
            NpcItem activeNpc)
        {
            ...
            if (activeNpc != null)
            {
                int activeNpcId = int.TryParse(activeNpc.RuntimeLife?.Id, out int parsedNpcId) ? parsedNpcId : 0;
        ...
        private PacketScriptSpeaker CreateSpeakerFromNpc(NpcItem npc, int speakerTypeId, int templateId)
        {
            int npcId = int.TryParse(npc.RuntimeLife?.Id, out int parsedNpcId) ? parsedNpcId : 0;
            int authoritativeNpcId = npcId > 0 ? npcId : Math.Max(0, templateId);
            string displayName = npc.RuntimeLife?.DisplayName;
        ...
        private string ResolveNpcNameFromCache(int npcId)
        {
            string key = npcId.ToString();
            if (runtimeServices.Catalog.TryGetNpcName(key, out var info))
            {
                return info?.Name;
            }
        ...
        private string ResolveNpcNameFromStringWz(int npcId)
        {
            if (npcId <= 0)
            {
                return null;
            ...
            lock (NpcStringImageLock)
            {
                ...
                _npcStringImage ??= runtimeServices.Assets.FindImage("String", "Npc.img");
        ...
        private string BuildAvatarChoiceLabel(int itemId, int index)
        {
        ...
        private string DescribeAvatarOption(int itemId)
        {
            string itemName = ResolveItemName(itemId);
            return string.IsNullOrWhiteSpace(itemName)
                ? $"Item {itemId}"
        ...
        {
            return runtimeServices.Catalog.TryGetItemName(itemId, out var itemName)
                ? itemName.Name?.Trim()
                : null;
        }
        ...
        private IReadOnlyList<SlideMenuOption> ParseSlideMenuButtonInfo(string buttonInfo)
        {
        ...
        private string BuildPetSelectionEntryText(
            string rawText,
            string optionDetails,
            string resolutionDetails,
            bool isPetAll,
        ...
        private PacketScriptResponsePacket CreateAutoResponsePacket(
            int messageType,
            byte param,
            PacketScriptSpeaker speaker,
            string submittedValue,
