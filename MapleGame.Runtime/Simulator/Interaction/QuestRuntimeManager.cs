...
namespace HaCreator.MapSimulator.Interaction
{
    ...
    internal sealed class QuestRuntimeManager
    {
        ...
        public QuestRuntimeManager(IRuntimeDataServices services)
        {
        ...
        public void RecordMobKill(RuntimeLife runtimeLife)
        {
            if (runtimeLife == null || !int.TryParse(runtimeLife.Id, out int mobId))
            {
                return;
            }
        ...
        {
            string npcName = npc?.RuntimeLife?.DisplayName;
            int npcId = GetNpcId(npc?.RuntimeLife);
            ...
            var entries = new List<NpcInteractionEntry>
            {
                new NpcInteractionEntry
                {
                    ...
                    Title = "Talk",
                    Subtitle = npc?.RuntimeLife?.FunctionDescription ?? string.Empty,
        ...
        private void AppendActionMetadataRewardLines(
            ICollection<QuestLogLineSnapshot> lines,
            QuestActionBundle actions,
            CharacterBuild build)
        {
            ...
            foreach ((int groupKey, List<QuestRewardItem> groupRewards) in GetFilteredChoiceRewardGroups(rewards, build))
            {
                ...
                pendingGroups.Add(new QuestRewardChoiceGroup
                {
                    ...
                    Options = groupRewards
                        .Select(reward => new QuestRewardChoiceOption
                        {
                            ItemId = reward.ItemId,
        ...
        private QuestRewardRaiseOwnerContext ResolveQuestRewardRaiseOwnerContext(int questId)
        {
            return InventoryItemMetadataResolver.TryResolveRaiseOwnerContextForQuest(
                questId, runtimeServices.Assets,
        ...
        private string BuildNpcText(QuestDefinition definition, QuestStateType state)
        {
            ...
            if (runtimeServices.Catalog.TryGetNpcName(key, out var npcInfo) &&
        ...
        private void AppendQuestDetailEquipNeedEligibilitySegments(
            IReadOnlyList<int> equipAllNeedItemIds,
            IReadOnlyList<int> equipSelectNeedItemIds,
            ICollection<string> segments)
        {
        ...
        private QuestWorldMapTarget BuildNpcWorldMapTarget(QuestDefinition definition, int? npcId, int mapId, string description)
        {
        ...
        private string ResolveMobName(int mobId)
        {
            string key = mobId.ToString();
            return runtimeServices.Catalog.TryGetMobName(key, out var mobName) &&
        ...
        {
            return runtimeServices.Catalog.TryGetItemName(itemId, out var itemInfo) &&
        ...
        private void EnsureDefinitionsLoaded()
        {
            ...
            foreach (WzSubProperty questInfo in (runtimeServices.Assets.FindImage("Quest", "QuestInfo.img")?.WzProperties ?? new List<WzImageProperty>()).OfType<WzSubProperty>())
            {
                if (!int.TryParse(questInfo.Name, out int questId))
                {
                    continue;
                }
        ...
        {
            WzSubProperty checkProp = runtimeServices.Assets.FindImage("Quest", "Check.img")?[questId.ToString()] as WzSubProperty;
            WzSubProperty actProp = runtimeServices.Assets.FindImage("Quest", "Act.img")?[questId.ToString()] as WzSubProperty;
            WzSubProperty sayProp = runtimeServices.Assets.FindImage("Quest", "Say.img")?[questId.ToString()] as WzSubProperty;
        ...
        private static int GetNpcId(RuntimeLife runtimeLife)
        {
            return runtimeLife != null && int.TryParse(runtimeLife.Id, out int npcId) ? npcId : 0;
        }
        ...
        internal string FormatEquipNeedItemList(IReadOnlyList<int> itemIds, CharacterBuild build)
        {
            if (itemIds == null || itemIds.Count == 0)
            {
                return string.Empty;
            ...
            IReadOnlyList<int> actionAllowedJobs = null)
        {
            ...
            foreach ((int groupKey, List<QuestRewardItem> groupRewards) in choiceGroups.OrderBy(pair => pair.Key))
            {
                ...
                if (groupRewards.Count > 1)
                {
                    messages?.Add($"Choice reward selection pending: {string.Join(", ", groupRewards.Select(reward => GetRewardItemDescription(reward, includeSelectionTag: false, includeFilters: true)))}");
                    continue;
                }
        ...
        private string BuildChoiceRewardDisplayText(
        ...
        {
            string key = mobId.ToString();
            return runtimeServices.Catalog.TryGetMobName(key, out var mobName) && !string.IsNullOrWhiteSpace(mobName.Name)
                ? mobName.Name
                : $"Mob #{mobId}";
        }
        ...
        private static NpcInteractionEntry CreateItemMakerEntry(NpcItem npc)
        {
            ...
            string subtitle = npc?.RuntimeLife?.FunctionDescription;
            return new NpcInteractionEntry
            {
                EntryId = -110,
        ...
        {
            string func = npc?.RuntimeLife?.FunctionDescription;
            ...
            string name = npc?.RuntimeLife?.DisplayName;
        ...
        private IReadOnlyList<string> BuildVisibleQuestActionMetadataLines(QuestActionBundle actions)
        {
            var lines = new List<string>();
            if (actions == null)
            {
        ...
        private string BuildNpcText(QuestDefinition definition)
        {
            var parts = new List<string>();
        ...
        private string ResolveNpcName(int npcId)
        {
            ...
            return runtimeServices.Catalog.TryGetNpcName(key, out var info) &&
