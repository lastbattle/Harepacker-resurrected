...
namespace HaCreator.MapSimulator.Interaction
{
    internal static class ClientCurseProcessParity
    {
        ...
        {
            if (ContainsBlockedContent(text, assets))
            {
            ...
            IRuntimeAssetSource assets = null)
        {
            ...
            if (!TryConvertBlockedContent(source, ignoreNewLine, assets, out string convertedText, out bool converted))
            {
                notice = GetInappropriateContentNotice();
                return false;
            }
        ...
        internal static string FilterTextForClientDisplay(
            string text,
            IRuntimeAssetSource assets = null)
        {
            return ContainsBlockedContent(text, assets)
                ? GetInappropriateContentNotice()
                : text ?? string.Empty;
        }

        private static bool ContainsBlockedContent(string text, IRuntimeAssetSource assets)
        {
            ...
            foreach (string blockedTerm in LoadBlockedTerms(assets))
            {
        ...
        private static bool TryConvertBlockedContent(
            string text,
            bool ignoreNewLine,
            IRuntimeAssetSource assets,
            out string convertedText,
            out bool converted)
        {
        ...
        private static HashSet<string> LoadBlockedTerms(IRuntimeAssetSource assets)
        {
            HashSet<string> blockedTerms = new(StringComparer.Ordinal);

            TryLoadTermsFromWz(blockedTerms, assets);
        ...
        private static void TryLoadTermsFromWz(
            HashSet<string> blockedTerms,
            IRuntimeAssetSource assets)
        {
            try
            {
                WzImage curseImage = assets?.FindImage("Etc", "Curse.img");
                if (curseImage == null)
                {
                    return;
                }
