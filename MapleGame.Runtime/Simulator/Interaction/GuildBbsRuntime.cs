...
namespace HaCreator.MapSimulator.Interaction
{
    ...
    internal sealed class GuildBbsRuntime
    {
        ...
        public GuildBbsRuntime(IRuntimeAssetSource assets = null)
        {
            _assets = assets;
            ...
            string linkedBoardAuthKeySourceLabel = null)
        {
            ...
            _linkedPermissionMask = linkedPermissionMask.HasValue
                ? NormalizePermissionMask(linkedPermissionMask.Value)
                : null;
        ...
        private bool TryProcessTitleForClientRegister(string title, out string processedTitle, out string notice)
        {
            return TryProcessTextForClientSubmission(title, ignoreNewLine: true, out processedTitle, out notice);
        }

        private bool TryProcessBodyForClientRegister(string body, out string processedBody, out string notice)
        {
            return TryProcessTextForClientSubmission(body, ignoreNewLine: true, out processedBody, out notice);
        }

        private bool TryProcessReplyForClientComment(string body, out string processedBody, out string notice)
        {
            return TryProcessTextForClientSubmission(body, ignoreNewLine: true, out processedBody, out notice);
        }

        private bool TryProcessTextForClientSubmission(
            string value,
            bool ignoreNewLine,
        ...
        private GuildBbsPermissionMask EffectivePermissionMask => NormalizePermissionMask(_packetPermissionMask ?? _linkedPermissionMask ?? _rolePermissionMask);
        ...
        private string AuthoritySourceLabel => _packetPermissionMask.HasValue
            ? "Packet"
