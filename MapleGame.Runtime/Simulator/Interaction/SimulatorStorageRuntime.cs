...
namespace HaCreator.MapSimulator.Interaction
{
    internal sealed class SimulatorStorageRuntime : IStorageRuntime
    {
        ...
        public SimulatorStorageRuntime(
            StorageAccountStore accountStore = null,
            string initialAccountLabel = null,
            string initialAccountKey = null,
            ISimulatorProfileStorage profileStorage = null,
            IRuntimeAssetCatalog runtimeCatalog = null)
        {
            _accountStore = accountStore ?? new StorageAccountStore(profileStorage: profileStorage);
        ...
        private void PopulateResolvedMetadata(InventorySlotData slot, InventoryType type)
        {
            if (slot == null)
            {
                return;
            ...
            if (string.IsNullOrWhiteSpace(slot.ItemName) &&
                ...
                InventoryItemMetadataResolver.TryResolveItemName(slot.ItemId, _runtimeCatalog, out string itemName) &&
            ...
            if (string.IsNullOrWhiteSpace(slot.Description) &&
                _runtimeCatalog != null &&
                InventoryItemMetadataResolver.TryResolveItemDescription(slot.ItemId, _runtimeCatalog, out string description) &&
