...
namespace HaCreator.MapSimulator.Effects
{
    ...
    public class WeddingField
    {
            ...
            UILoaderResourceCache uiResourceCache = null)
        {
            ...
            _uiResourceCache = uiResourceCache ?? new UILoaderResourceCache();
        ...
        private bool TryApplyRemoteRelationshipRecordAddPacket(int packetType, byte[] payload, out string errorMessage)
        {
            ...
            int resolvedOwnerCharacterId = relationshipRecord.CharacterId.GetValueOrDefault(characterId);
            ...
            if (!TryResolveParticipantForTemporaryStats(characterId, out WeddingRemoteParticipant participant, out _))
            {
                if (resolvedOwnerCharacterId > 0
                    && resolvedOwnerCharacterId != characterId
                    && TryResolveParticipantForTemporaryStats(resolvedOwnerCharacterId, out WeddingRemoteParticipant resolvedOwnerParticipant, out _))
                {
                    ApplyRelationshipRecordAdd(
                        resolvedOwnerParticipant,
                        packet.RelationshipType,
                        relationshipRecord,
                        packet.DispatchKey,
                        packet.PairLookupSerial);
        ...
        private static bool DoesPendingRelationshipRecordAddMatchRemove(
            ...
            long? itemSerial)
        {
            RemoteUserRelationshipRecord record = operation.RelationshipRecord;
            if (!record.IsActive)
            {
                return false;
            }
            ...
            {
                return record.CharacterId.GetValueOrDefault() == characterId.Value
                    || record.PairCharacterId.GetValueOrDefault() == characterId.Value;
            }

            if (itemSerial.HasValue)
            {
                long serial = itemSerial.Value;
                return (record.ItemSerial.HasValue && record.ItemSerial.Value == serial)
                    || (record.PairItemSerial.HasValue && record.PairItemSerial.Value == serial)
                    || (operation.RelationshipDispatchKey.Serial.HasValue && operation.RelationshipDispatchKey.Serial.Value == serial)
                    || (operation.RelationshipPairLookupSerial.HasValue && operation.RelationshipPairLookupSerial.Value == serial);
            }
        ...
        private void LoadCeremonyOverlay(GraphicsDevice device)
        {
            ...
            try
            {
                WzImage uiWindowImage = _runtimeServices?.Assets?.FindImage("UI", WeddingUiImageName);
        ...
        private List<IDXObject> LoadBlessFrames(GraphicsDevice device)
        {
            ...
            try
            {
                WzImage basicEffImage = _runtimeServices?.Assets?.FindImage("Effect", BlessEffectImageName);
        ...
        private void LoadCeremonyCelebrationAssets(GraphicsDevice device)
        {
            ...
            try
            {
                WzImage mapHelperImage = _runtimeServices?.Assets?.FindImage("Map", WeddingMapHelperImageName);
        ...
        private string ResolveWeddingDialogText(int npcId, int step)
        {
            ...
            try
            {
                WzImage npcImage = _runtimeServices?.Assets?.FindImage("String", "Npc.img");
                npcImage?.ParseImage();
        ...
        private void DrawParticipantLabels(
            SpriteBatch spriteBatch,
            SpriteFont font,
            WeddingRemoteParticipant participant,
            int screenX,
        ...
        private bool TryResolveParticipantGuildMarkTextures(
            GraphicsDevice device,
            WeddingRemoteParticipant participant,
            ...
            out Texture2D markTexture)
        {
            ...
            backgroundTexture = GuildMarkTextureCache.GetBackgroundTexture(device, _runtimeServices?.Assets, _uiResourceCache, backgroundId, backgroundColor);
            markTexture = GuildMarkTextureCache.GetMarkTexture(device, _runtimeServices?.Assets, _uiResourceCache, markId, markColor);
