...
namespace HaCreator.MapSimulator.Effects
{
    ...
    public class SpecialEffectFields
    {
        ...
        {
            _wedding.Initialize(device, requestBgmOverride, clearBgmOverride, weddingRemoteBuildFactory, weddingCharacterLoader, runtimeServices, uiResourceCache);
            _witchtower.Initialize(device, runtimeServices);
            _battlefield.Initialize(device, runtimeServices);
            _guildBoss.Initialize(device, runtimeServices);
            _dojo.Initialize(device, runtimeServices);
            _spaceGaga.Initialize(device, runtimeServices);
            _massacre.Initialize(device, runtimeServices);
            _cakePie.Initialize(device, runtimeServices);
        }
        ...
        public void ConfigureMap(MapInfo mapInfo, IEnumerable<RuntimePortal> portals = null)
        {
        ...
        public bool TryDispatchActiveWrapperPacket(int packetType, byte[] payload, int currentTimeMs, out string ownerName, out string message)
        {
            ...
            if (_wedding.HasPresentationOwner)
            {
                ownerName = _wedding.HasWeddingPacketOwner
                    ? "CField_Wedding::OnPacket"
                    : "CField_WeddingPhoto::OnPacket";
        ...
        {
            _wedding.SetBgmCallbacks(requestBgmOverride, clearBgmOverride);
        }
        #endregion
    }
}
