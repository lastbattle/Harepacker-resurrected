...
namespace HaCreator.MapSimulator.Effects
{
    ...
    public class DojoField
    {
        ...
        public void Configure(MapInfo mapInfo, IEnumerable<RuntimePortal> portals, bool hasNextFloorPortal = false)
        {
        ...
        private (int MapId, string PortalName) ResolveNextFloorDestinationFromPortals(IEnumerable<RuntimePortal> portals)
        {
            if (portals == null)
            {
                return (-1, string.Empty);
            ...
            RuntimePortal[] portalList = portals.Where(static portal => portal != null).ToArray();
            foreach (RuntimePortal portal in portalList)
            {
        ...
        private static (bool HasExitPortal, string PortalName, int TargetMapId) ResolveExitPortalContractFromPortals(IEnumerable<RuntimePortal> portals)
        {
            if (portals == null)
            {
            ...
            foreach (RuntimePortal portal in portals.Where(static portal => portal != null))
            {
        ...
        private bool TryResolvePortalContractsFromWzMap(
            int mapId,
            ...
            out int exitPortalTargetMapId)
        {
            ...
            if (mapId <= 0 || _runtimeServices?.Assets == null)
            {
                return false;
            }
        ...
        private static bool IsAuthoredDojoNextFloorPortal(int currentMapId, RuntimePortal portal, int targetMapId)
        {
            if (portal == null)
            {
                return false;
            ...
            out string resolvedTargetPortalName)
        {
            ...
            int nextStageCandidate = ResolveNextFloorMapIdCore(currentMapId, hasNextFloorPortal: true, static _ => true);
        ...
        private static string ResolveNextFloorPortalName(RuntimePortal portal)
        {
        ...
        private bool HasMapImage(int mapId)
        {
            if (mapId <= 0)
            {
                return false;
            }
            return FindMapImage(mapId) != null;
        }
        private bool HasPortalScript(int mapId, string scriptName)
        {
            if (mapId <= 0 || string.IsNullOrWhiteSpace(scriptName) || _runtimeServices?.Assets == null)
            {
                return false;
            }
            WzImage mapImage = FindMapImage(mapId);
        ...
        private WzImage ResolveLinkedMapImage(WzImage mapImage)
        {
            if (mapImage == null)
            {
                return null;
            ...
            if ((mapImage["info"]?["link"] as WzStringProperty)?.Value is not string linkedMapId
                ...
                || _runtimeServices?.Assets == null)
            {
                return mapImage;
            }
            return FindMapImage(linkedMapId) ?? mapImage;
        }

        private WzImage FindMapImage(int mapIdOrText)
        {
            return FindMapImage(mapIdOrText.ToString(CultureInfo.InvariantCulture));
        }

        private WzImage FindMapImage(string mapIdText)
        {
            if (_runtimeServices?.Assets == null || string.IsNullOrWhiteSpace(mapIdText))
                return null;
            string mapId = mapIdText.Trim();
            if (mapId.Length < 9)
                mapId = mapId.PadLeft(9, '0');
            if (mapId.Length < 1)
                return null;
            return _runtimeServices.Assets.FindImage("Map", $"Map/Map{mapId[0]}/{mapId}.img");
        }
        ...
        private void EnsureAssetsLoaded()
        {
            ...
            WzImage uiWindow = _runtimeServices?.Assets?.FindImage("UI", "UIWindow.img")
                ?? _runtimeServices?.Assets?.FindImage("UI", "UIWindow2.img");
