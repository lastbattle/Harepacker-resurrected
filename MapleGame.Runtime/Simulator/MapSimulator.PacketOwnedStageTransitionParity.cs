...
namespace HaCreator.MapSimulator
{
    public partial class MapSimulator
    {
        ...
        {
            _packetStageTransitionRuntime.BindMap(_mapInfo?.id ?? 0);
        ...
        private void RegisterPacketOwnedStageTransitionObject(
            BaseDXDrawableItem mapItem,
            RuntimeObjectDefinition definition,
            WzImageProperty objectProperty)
        {
            if (mapItem == null || definition == null)
                return;

            string objectName = definition.Name?.Trim();
            ...
            items.Add(mapItem);
            RuntimeObjectQuestDefinition[] rootAuthoredQuestInfo = ResolvePacketOwnedNamedObjectQuestInfo(objectProperty);
            _packetStageTransitionNamedObjectMetadata[mapItem] = new PacketOwnedNamedObjectStateMetadata(
                ...
                definition.Platform,
                definition.Dynamic,
                (byte)definition.Flow,
                definition.Rx,
                definition.Ry,
                definition.Cx,
                definition.Cy,
                (objectProperty?["sfx"] as WzStringProperty)?.Value?.Trim() ?? string.Empty,
                ...
                PacketOwnedNamedObjectMotionProfile.FromMapObject(
                    (byte)definition.Flow, definition.Rx, definition.Ry, definition.Cx, definition.Cy),
                ...
                ResolvePacketOwnedNamedObjectMetadataLanesForPacketParity(
                    definition.Dynamic, definition.Rotation, definition.Flow,
                    definition.Rx, definition.Ry, definition.Cx, definition.Cy,
                    definition.Quests.Count > 0 || rootAuthoredQuestInfo.Length > 0),
        ...
        {
            if (mapItem == null || definition == null || objectProperty == null ||
            ...
            foreach (int stateIndex in authoredStateIndexes.OrderBy(static state => state))
            {
                ...
                BaseDXDrawableItem branchItem = new(branchFrames, definition.Flip);
                ...
                RuntimeObjectQuestDefinition[] branchQuestInfo = ResolvePacketOwnedNamedObjectQuestInfo(branchProperty);
                QuestGatedMapObjectState? questState = BuildQuestGatedMapObjectState(
                    definition,
                    ResolvePacketOwnedNamedObjectBranchQuestInfo(rootAuthoredQuestInfo, branchQuestInfo));
        ...
        internal static RuntimeObjectQuestDefinition[] ResolvePacketOwnedNamedObjectBranchQuestInfo(
            IEnumerable<RuntimeObjectQuestDefinition> rootAuthoredQuestInfo,
            IEnumerable<RuntimeObjectQuestDefinition> branchAuthoredQuestInfo)
        {
        ...
        private static QuestGatedMapObjectState? BuildQuestGatedMapObjectState(
            RuntimeObjectDefinition definition,
            RuntimeObjectQuestDefinition[] authoredQuestInfo = null)
        {
            if (definition == null)
                return null;

            RuntimeObjectQuestDefinition[] mapQuestInfo = definition.Quests
                .Select(quest => new RuntimeObjectQuestDefinition(quest.QuestId, quest.State))
                .ToArray();
            RuntimeObjectQuestDefinition[] questInfo = MergePacketOwnedNamedObjectQuestInfo(mapQuestInfo, authoredQuestInfo);
            bool hiddenByMap = definition.Hide == true;
            string[] dynamicTags = ParseObjectTags(definition.Tags);
            return hiddenByMap || questInfo.Length > 0 || dynamicTags.Length > 0
                ? new QuestGatedMapObjectState(questInfo, dynamicTags, hiddenByMap)
                : null;
        }

        internal static RuntimeObjectQuestDefinition[] MergePacketOwnedNamedObjectQuestInfo(
            IEnumerable<RuntimeObjectQuestDefinition> mapQuestInfo,
            IEnumerable<RuntimeObjectQuestDefinition> authoredQuestInfo)
        {
            Dictionary<int, RuntimeObjectQuestDefinition> merged = new();
            if (authoredQuestInfo != null)
            {
                foreach (RuntimeObjectQuestDefinition quest in authoredQuestInfo)
                {
                    if (quest.QuestId > 0)
                    {
            ...
            {
                foreach (RuntimeObjectQuestDefinition quest in mapQuestInfo)
                {
                    if (quest.QuestId > 0)
                    {
                        merged[quest.QuestId] = quest;
        ...
        internal static RuntimeObjectQuestDefinition[] ResolvePacketOwnedNamedObjectQuestInfo(WzImageProperty objectProperty)
        {
            ...
            {
                return Array.Empty<RuntimeObjectQuestDefinition>();
            }

            List<RuntimeObjectQuestDefinition> questInfo = new();
            foreach (WzImageProperty child in questProperty.WzProperties)
            {
                ...
                questInfo.Add(new RuntimeObjectQuestDefinition(questId, (int)state));
            }

            return questInfo.Count == 0 ? Array.Empty<RuntimeObjectQuestDefinition>() : questInfo.ToArray();
        }
