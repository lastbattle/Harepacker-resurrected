    ...
    public partial class MapSimulator
    {
            ...
            out string message)
        {
            ...
            if (TryResolvePacketOwnedTeleportLivePortalTarget(
                ...
                out RuntimePortal liveRuntimePortal))
            {
                _lastPacketOwnedTeleportPortalIndex = livePortalIndex;
                RegisterPacketOwnedTeleportHandoff(liveRuntimePortal);
                string liveRegistrationMessage = ApplyPacketOwnedTeleportRegistrationSideEffects(liveRuntimePortal, currentTime);
                ApplySameMapTeleportPosition(liveRuntimePortal.X, liveRuntimePortal.Y);
                message = string.IsNullOrWhiteSpace(liveRegistrationMessage)
                    ? $"Applied packet-owned teleport result through live portal metadata index {livePortalIndex} ({liveRuntimePortal.pn} -> {liveRuntimePortal.tn})."
                    : $"Applied packet-owned teleport result through live portal metadata index {livePortalIndex} ({liveRuntimePortal.pn} -> {liveRuntimePortal.tn}). {liveRegistrationMessage}";
                return true;
            }

            if (allowCoordinatePortalResolution
                && TryResolvePacketOwnedTeleportPortalByPosition(targetX, targetY, out int portalIndex, out RuntimePortal portalInstance))
            {
        ...
        private bool TryApplyPacketOwnedTeleportResult(bool succeeded, int portalIndex, out string message)
        {
            ...
            RuntimePortal portalInstance = portal?.Portal;
        ...
        private bool TryFinalizePendingCrossMapTeleport(PendingCrossMapTeleportTarget target, out string message)
        {
            ...
            {
                if (_portalPool.GetPortal(portalIndex)?.Portal != null)
                {
        ...
        private bool TryResolvePacketOwnedTeleportLivePortalTarget(
            ...
            float targetY,
            out int portalIndex,
            out RuntimePortal portalInstance)
        {
            portalIndex = -1;
            portalInstance = null;
            ...
            portalInstance = _portalPool.GetPortal(portalIndex)?.Portal;
            return portalInstance != null;
        }

        private void RegisterPacketOwnedTeleportHandoff(RuntimePortal portalInstance)
        {
        ...
        private void RecordPacketOwnedTeleportPortalRequest(RuntimePortal sourcePortal)
        {
        ...
        private string ApplyPacketOwnedTeleportRegistrationSideEffects(RuntimePortal portalInstance, int currentTime)
        {
            if (portalInstance == null)
            {
                return null;
        ...
        private string ApplyPacketOwnedTeleportRegistrationSideEffects(float targetX, float targetY, int currentTime)
        {
            ...
            bool hasDynamicFoothold = IsPortalOwnedMovePathDynamicFootholdBranch(_mapInfo?.fieldType);
        ...
        private bool TryResolvePacketOwnedTeleportPortalByPosition(float targetX, float targetY, out int portalIndex, out RuntimePortal portalInstance)
        {
            ...
            out int portalIndex)
        {
            ...
            RuntimePortal portalInstance = portalPool.GetPortal(target.TargetPortalIndex)?.Portal;
            if (portalInstance == null)
            {
                return false;
            }
            ...
            out int portalIndex)
        {
            ...
            if (TryResolvePacketOwnedPortalRequestTargetByMapOnlyReturn(
                livePortals,
                target.SourceMapId,
                out RuntimePortal mapOnlyPortal))
            {
        ...
        private static bool TryResolvePacketOwnedTeleportPortalIndexByName(PortalPool portalPool, string portalName, out int portalIndex)
        {
            ...
            return portalPool.GetPortal(portalIndex)?.Portal != null;
        }
        ...
        private static bool TryCollectPacketOwnedTeleportLivePortalNames(
            ...
            out string[] livePortalNames)
        {
            livePortals = Array.Empty<RuntimePortal>();
            ...
            var portals = new List<RuntimePortal>();
            var names = new List<string>();
            for (int i = 0; i < portalPool.PortalCount; i++)
            {
                RuntimePortal portalInstance = portalPool.GetPortal(i)?.Portal;
                if (portalInstance == null)
                {
                    continue;
                }
            ...
            out int portalIndex)
        {
            ...
            {
                RuntimePortal instance = portalPool.GetPortal(candidatePortalIndex)?.Portal;
                if (instance == null)
                {
                    continue;
                }
            ...
            out RuntimePortal portalInstance)
        {
            ...
            for (int i = 0; i < portalPool.PortalCount; i++)
            {
                ...
                RuntimePortal instance = portal?.Portal;
                if (instance == null)
                {
                    continue;
                }
        ...
        internal static bool TryResolvePacketOwnedSourcePortalNameByTargetPortalName(
            IEnumerable<RuntimePortal> currentFieldPortals,
            int targetMapId,
            string targetPortalName,
            out string sourcePortalName)
        {
            ...
            RuntimePortal firstMatch = null;
            foreach (RuntimePortal portal in currentFieldPortals)
            {
                if (portal == null
        ...
        internal static bool TryResolvePacketOwnedSourcePortalNameByReplayedTargetFieldMetadata(
            IEnumerable<RuntimePortal> currentFieldPortals,
            IEnumerable<RuntimePortal> targetFieldPortals,
            int currentMapId,
            int targetMapId,
            float sourceX,
            float sourceY,
            ...
            out string[] matchedSourcePortalNames)
        {
            ...
            foreach (string coordinateSourcePortalName in coordinateSourcePortalNames)
            {
                ...
                foreach (RuntimePortal sourcePortal in currentFieldPortals)
                {
                    if (sourcePortal == null
        ...
        private bool TryResolvePacketOwnedTemporarySourcePortal(
            ...
            string targetPortalName,
            IEnumerable<string> targetPortalNameCandidates,
            out RuntimePortal sourcePortal,
            out string sourcePortalName,
            out string[] sourcePortalNameCandidates)
        {
            ...
            IEnumerable<RuntimePortal> currentFieldPortals = _runtimePortals;
            int currentMapId = _mapInfo?.id ?? -1;
            IEnumerable<RuntimePortal> targetFieldPortals = targetMapId == currentMapId
                ? currentFieldPortals
                : LoadTargetPortals(targetMapId);
            ...
            out int targetPortalIndex)
        {
            ...
            IEnumerable<RuntimePortal> targetPortals = targetMapId == (_mapInfo?.id ?? -1)
                ? _runtimePortals
                : LoadTargetPortals(targetMapId);
            if (!TryResolvePacketOwnedPortalRequestTargetFromBoardPortals(
                targetPortals,
                sourcePortal.tn,
                out RuntimePortal resolvedTargetPortal,
            ...
            out int targetPortalIndex)
        {
            ...
            int currentMapId = _mapInfo?.id ?? -1;
            IEnumerable<RuntimePortal> targetPortals = null;
            if (sourcePortal.tm > 0
                && sourcePortal.tm != MapConstants.MaxMap
                && sourcePortal.tm != currentMapId
                && _mapProvider != null)
            {
                targetPortals = LoadTargetPortals(sourcePortal.tm);
            ...
            out int targetPortalIndex)
        {
            ...
            IEnumerable<RuntimePortal> targetPortals = LoadTargetPortals(targetMapId);
        ...
        internal static bool TryResolvePacketOwnedTeleportPortalNameByLiveMetadata(
            IEnumerable<RuntimePortal> portals,
            string targetPortalName,
            IEnumerable<string> targetPortalNameCandidates,
            float targetX,
            float targetY,
            out string resolvedPortalName)
        {
            ...
            RuntimePortal[] livePortals = portals
            ...
            if (!string.IsNullOrWhiteSpace(targetPortalName))
            {
                RuntimePortal namedPortal = livePortals.FirstOrDefault(
            ...
            if (TryResolvePacketOwnedTeleportPortalByPosition(
                ...
                targetX,
                targetY,
                out RuntimePortal positionedPortal))
            {
            ...
            out string[] livePortalNames)
        {
            ...
            foreach (string candidatePortalName in candidatePortalNames)
            {
                ...
                RuntimePortal matchingPortal = portals.FirstOrDefault(
            ...
            out string[] portalNames)
        {
            ...
            var names = new List<string>();
            foreach (RuntimePortal portal in portals)
            {
                if (portal == null
            ...
            if (!TryGetOrCreatePacketOwnedAnimationFrames(
                ...
                () => LoadPacketOwnedAnimationFrames(
                    ResolvePacketOwnedPropertyPath(
                        runtimeServices.Assets.FindImage("Effect", PacketOwnedTeleportGeneralEffectImageName),
        ...
        private bool TryBuildPacketOwnedTeleportPortalRequest(RuntimePortal sourcePortal, out byte[] payload, out string summary)
        {
            ...
            out string summary)
        {
            ...
            if (!string.IsNullOrWhiteSpace(sourcePortal.tn)
                && TryResolvePacketOwnedPortalRequestTargetFromBoardPortals(
                    _runtimePortals,
                    sourcePortal.tn,
                    out RuntimePortal currentFieldPortal))
            {
            ...
            IEnumerable<RuntimePortal> targetPortals = LoadTargetPortals(sourcePortal.tm);
            if (!string.IsNullOrWhiteSpace(sourcePortal.tn)
                && TryResolvePacketOwnedPortalRequestTargetFromBoardPortals(
                    targetPortals,
                    sourcePortal.tn,
                    out RuntimePortal crossMapPortal))
            {
                ...
                out string[] reciprocalCandidateNames))
            {
                if (TryResolvePacketOwnedTeleportUniqueCandidatePortalName(
                    ...
                    && TryResolvePacketOwnedPortalRequestTargetFromBoardPortals(
                        ...
                        out RuntimePortal reciprocalPortal))
                {
        ...
        private static int GetPacketOwnedPortalListIndex(IEnumerable<RuntimePortal> portals, RuntimePortal targetPortal)
        {
        ...
        private static int GetPacketOwnedPortalListIndex(IEnumerable<RuntimePortal> portals, string targetPortalName)
        {
            if (portals == null || string.IsNullOrWhiteSpace(targetPortalName))
            {
                return -1;
