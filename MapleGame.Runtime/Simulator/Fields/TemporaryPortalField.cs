...
namespace HaCreator.MapSimulator.Fields
{
    internal sealed class TemporaryPortalField
    {
        ...
        public TemporaryPortalField(TexturePool texturePool, GraphicsDevice graphicsDevice, IRuntimeDataServices runtimeServices = null)
        {
            ...
            RemoteTownPortalResolvedDestination? destination)
        {
            ...
            if (!resolvedDestination.HasValue
                && TryResolveRemoteTownPortalCurrentMapReturnDestinationRuntime(currentMapId, out RemoteTownPortalResolvedDestination sourceMapFallbackDestination))
            {
            ...
            if (!resolvedDestination.HasValue
                && TryResolveRemoteTownPortalWzFallbackDestinationRuntime(currentMapId, preferredSourceMapId, out RemoteTownPortalResolvedDestination fallbackDestination))
            {
        ...
        private void ApplyRemoteOpenGateCreate(RemoteOpenGateCreatedPacket packet, int currentMapId, int currentTime)
        {
            ...
            RemoteOpenGateState? existingState = _remoteOpenGates.TryGetValue(key, out RemoteOpenGateState existingStateValue)
                ? FilterRemoteOpenGateStateByCurrentMap(existingStateValue, currentMapId)
                : null;
            ...
            {
                PromoteRemoteOpenGatePartnerAfterPartnerCreate(packet.OwnerCharacterId, packet.IsFirstSlot, currentMapId, currentTime);
            }
        ...
        private bool ApplyRemoteOpenGateRemove(RemoteOpenGateRemovedPacket packet, int currentMapId, int currentTime)
        {
            ...
            RestartRemoteOpenGatePartnerAfterPartnerLoss(packet.OwnerCharacterId, packet.IsFirstSlot, currentMapId, currentTime);
            SyncRemoteOpenGateVisuals();
            return true;
        }

        private void RestartRemoteOpenGatePartnerAfterPartnerLoss(uint ownerCharacterId, bool removedFirstSlot, int currentMapId, int currentTime)
        {
        ...
        private void PromoteRemoteOpenGatePartnerAfterPartnerCreate(uint ownerCharacterId, bool createdFirstSlot, int currentMapId, int currentTime)
        {
        ...
        private static RemoteOpenGateState? FilterRemoteOpenGateStateByCurrentMap(
            RemoteOpenGateState existingState,
            int currentMapId)
        {
            return existingState.MapId == currentMapId
                ? existingState
                : null;
        }
        ...
        private bool EnsureMysticDoorVisuals(out PortalVisualSet currentMapVisuals, out PortalVisualSet townVisuals, out PortalVisualSet frameVisuals)
        {
            ...
            if (!TryLoadMysticDoorSkillPropertyRuntime(out WzSubProperty skillProperty))
                return false;
        ...
        private void AdoptPendingRemoteTownPortalOwnerFieldObservations(uint ownerCharacterId, int townMapId)
        {
            ...
            {
                int? resolvedTownMapId = TryResolveRemoteTownPortalTownMapForSourceMapRuntime(sourceMapId, out int resolvedTownMap)
                    ? resolvedTownMap
                    : null;
        ...
        private bool TryResolveRemoteTownPortalCurrentMapReturnDestinationRuntime(
            int currentMapId,
            out RemoteTownPortalResolvedDestination destination)
        {
            destination = default;
            if (currentMapId <= 0
                || !TryResolveRemoteTownPortalTownMapForSourceMapRuntime(currentMapId, out int townMapId)
                || townMapId <= 0
                || townMapId == currentMapId
                || HasRemoteTownPortalPointInMapRuntime(currentMapId)
                || !TryResolveRemoteTownPortalTownDestinationFallbackPositionRuntime(townMapId, out float townX, out float townY))
            {
                return false;
            }
        ...
        private bool TryResolveRemoteTownPortalWzFallbackDestinationRuntime(
            int townMapId,
            int? preferredSourceMapId,
            out RemoteTownPortalResolvedDestination destination)
        {
            destination = default;
            if (!ShouldTryRemoteTownPortalTownSideWzFallback(IsRemoteTownPortalTownSideWzFallbackMapRuntime(townMapId)))
            {
                return false;
            }
            ...
            if (preferredSourceMapId.HasValue)
            {
                if (TryResolveRemoteTownPortalPreferredWzFallbackDestinationRuntime(
                        townMapId,
            ...
            if (!TryResolveUniqueRemoteTownPortalWzFallbackSourceMapRuntime(townMapId, out int sourceMapId)
                || !TryResolveRemoteTownPortalSourceFallbackPositionRuntime(sourceMapId, out float sourceX, out float sourceY))
            {
                return false;
            }
        ...
        private bool IsRemoteTownPortalTownSideWzFallbackMapRuntime(int mapId)
        {
            return IsRemoteTownPortalTownMapRuntime(mapId)
                   || HasRemoteTownPortalPointInMapRuntime(mapId);
        }

        private bool IsRemoteTownPortalTownMapRuntime(int mapId)
        {
            if (mapId <= 0)
            {
                return false;
            ...
            WzImage mapImage = TryGetMapImageForRemoteTownPortalMetadataLookupRuntime(mapId);
            if (mapImage == null)
            {
                return false;
            }
        ...
        private bool TryResolveRemoteTownPortalPreferredWzFallbackDestinationRuntime(
            int townMapId,
            int preferredSourceMapId,
            out RemoteTownPortalResolvedDestination destination,
            out bool preferredSourceValidated)
        {
            ...
            if (preferredSourceMapId <= 0
                ...
                || !TryResolveRemoteTownPortalTownMapForSourceMapRuntime(preferredSourceMapId, out int configuredTownMapId)
            ...
            if (!TryResolveRemoteTownPortalSourceFallbackPositionRuntime(preferredSourceMapId, out float sourceX, out float sourceY))
            {
                return false;
            }
        ...
        private bool TryResolveUniqueRemoteTownPortalWzFallbackSourceMapRuntime(int townMapId, out int sourceMapId)
        {
            sourceMapId = -1;
            if (townMapId <= 0 || _runtimeServices?.Catalog == null)
            {
                return false;
            }

            int matchCount = 0;
            foreach (string mapIdKey in _runtimeServices.Catalog.GetMapNames()?.Keys ?? Array.Empty<string>())
            {
                if (!int.TryParse(mapIdKey, out int candidateSourceMapId)
                    || candidateSourceMapId <= 0
                    || candidateSourceMapId == townMapId
                    || !TryResolveRemoteTownPortalTownMapForSourceMapRuntime(candidateSourceMapId, out int candidateTownMapId)
                    || candidateTownMapId != townMapId)
                {
                    continue;
                }
        ...
        private bool TryResolveRemoteTownPortalSourceFallbackPositionRuntime(int sourceMapId, out float sourceX, out float sourceY)
        {
            sourceX = 0f;
            sourceY = 0f;
            WzImage mapImage = TryGetMapImageForRemoteTownPortalMetadataLookupRuntime(sourceMapId);
            if (mapImage == null)
            {
                return false;
            }
        ...
        private bool TryResolveRemoteTownPortalTownDestinationFallbackPositionRuntime(int townMapId, out float townX, out float townY)
        {
            townX = 0f;
            townY = 0f;
            WzImage townMapImage = TryGetMapImageForRemoteTownPortalMetadataLookupRuntime(townMapId);
        ...
        private bool HasRemoteTownPortalPointInMapRuntime(int mapId)
        {
            WzImage mapImage = TryGetMapImageForRemoteTownPortalMetadataLookupRuntime(mapId);
            if (mapImage == null)
            {
            ...
            try
            {
                ...
                return portalParent.WzProperties.OfType<WzSubProperty>()
                    .Any(portal => IsRemoteTownPortalTownPointPortal(
        ...
        private bool TryResolveRemoteTownPortalTownMapForSourceMapRuntime(int sourceMapId, out int townMapId)
        {
            townMapId = -1;
            if (sourceMapId <= 0)
            {
        ...
        private WzImage TryGetMapImageForRemoteTownPortalMetadataLookupRuntime(int mapId)
        {
            if (mapId <= 0)
            {
                return null;
            ...
            return _runtimeServices?.Assets?.FindImage("Map", $"Map/Map{folderNum}/{mapIdKey}.img");
        }
            ...
            out PortalVisualSet removalVisuals)
        {
            ...
            WzImage skillImage = _runtimeServices?.Assets?.FindImage("Skill", "3510.img");
        ...
        private bool TryLoadMysticDoorSkillPropertyRuntime(out WzSubProperty skillProperty)
        {
            skillProperty = null;

            foreach ((string imageName, string skillId) in new[] { ("231.img", "2311002"), ("000.img", "0008001") })
            {
                WzImage skillImage = _runtimeServices?.Assets?.FindImage("Skill", imageName);
        ...
        internal static (byte ResolvedState, RemoteOpenGateVisualPhase Phase, int PhaseStartedAt) ResolveRemoteOpenGateCreateLifecycleForCurrentMapForTesting(
            byte packetState,
            bool hasExistingState,
            byte existingState,
            int existingMapId,
        ...
        {
            RemoteOpenGateState? state = hasExistingState
                ? FilterRemoteOpenGateStateByCurrentMap(
                    new RemoteOpenGateState(
                        OwnerCharacterId: 1,
                        State: existingState,
                        MapId: existingMapId,
        ...
        internal static RemoteOpenGateVisualPhase ResolveRemoteOpenGatePartnerPromotionForCurrentMapForTesting(
            RemoteOpenGateVisualPhase partnerPhase,
            int partnerMapId,
            int currentMapId)
        {
            RemoteOpenGateState partner = new(
                ...
                Phase: partnerPhase,
                PhaseStartedAt: 0);
            return partner.MapId == currentMapId
                ? ResolveRemoteOpenGatePartnerPromotion(partner, currentTime: 1000).Phase
                : partner.Phase;
        }
