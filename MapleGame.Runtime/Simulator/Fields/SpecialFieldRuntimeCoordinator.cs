...
namespace HaCreator.MapSimulator.Fields
{
    ...
    public sealed class SpecialFieldRuntimeCoordinator
    {
        ...
        private bool _trackCreatedGraphicsResources;
        private bool _graphicsResourcesRetired;
        private Func<int> _cookieHousePointProvider;
        private MapInfo _boundMapInfo;
        private bool _ownsBoundMapInfo;
        ...
        public void Initialize(
            ...
            UILoaderResourceCache uiResourceCache = null,
            bool ownCreatedGraphicsResources = false)
        {
            _trackCreatedGraphicsResources = ownCreatedGraphicsResources;
            TrackCreatedGraphicsResources(() =>
            {
                _specialEffects.Initialize(graphicsDevice, requestBgmOverride, clearBgmOverride, weddingRemoteBuildFactory, weddingCharacterLoader, runtimeServices, uiResourceCache);
                _minigames.Initialize(graphicsDevice, soundManager, ariantArenaRemoteBuildFactory, runtimeServices);
                _cookieHouse.SetRuntimeDataServices(runtimeServices);
                _partyRaid.SetRuntimeDataServices(runtimeServices);
                _partyRaid.Initialize(graphicsDevice);
            });
        }

        public void BindMap(RuntimeMapDefinition mapDefinition)
        {
            MapInfo mapInfo = mapDefinition?.CreateMapInfo();
            BindMap(mapInfo, mapDefinition, ownsMapInfo: true);
        }

        internal void BindMap(MapInfo mapInfo)
        {
            BindMap(mapInfo, mapDefinition: null, ownsMapInfo: false);
        }

        private void BindMap(MapInfo mapInfo, RuntimeMapDefinition mapDefinition, bool ownsMapInfo)
        {
            TrackCreatedGraphicsResources(() => BindMapCore(mapInfo, mapDefinition, ownsMapInfo));
        }

        private void BindMapCore(MapInfo mapInfo, RuntimeMapDefinition mapDefinition, bool ownsMapInfo)
        {
            Reset();
            _boundMapInfo = mapInfo;
            _ownsBoundMapInfo = ownsMapInfo;
            if (mapInfo == null)
            {
                return;
            }
            ...
            IReadOnlyList<RuntimePortal> portals = mapDefinition?.Portals
                .Select(portal => new RuntimePortal(portal))
                .ToArray() ?? Array.Empty<RuntimePortal>();
            _specialEffects.ConfigureMap(mapInfo, portals);
            _minigames.BindMap(mapDefinition, mapInfo);
        ...
        {
            ActiveArea = null;
            if (_ownsBoundMapInfo)
            {
                _boundMapInfo?.Image?.Dispose();
            }
            _boundMapInfo = null;
            _ownsBoundMapInfo = false;
        ...
        internal void SetBgmCallbacks(Action<string> requestBgmOverride, Action clearBgmOverride)
        {
            _specialEffects.SetBgmCallbacks(requestBgmOverride, clearBgmOverride);
        }

        internal void Retire()
        {
            SetBgmCallbacks(null, null);
            ...
            {
                _graphicsResourcesRetired = true;
                ownedGraphicsResources = _ownedGraphicsResources.ToArray();
            ...
            {
                throw new AggregateException("One or more special-field resources failed to retire.", cleanupErrors);
            }
        }

        private void TrackCreatedGraphicsResources(Action action)
        {
            if (!_trackCreatedGraphicsResources)
            {
                action();
                return;
            }

            using IDisposable scope = GraphicsResourceCreationScope.Begin(OwnCreatedGraphicsResource);
            action();
        }

        private void OwnCreatedGraphicsResource(IDisposable resource)
        {
            bool disposeImmediately;
            lock (_ownedGraphicsResourcesLock)
            {
                disposeImmediately = _graphicsResourcesRetired;
