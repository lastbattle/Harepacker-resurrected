...
namespace HaCreator.MapSimulator.Fields
{
    public static class FieldInteractionRestrictionEvaluator
    {
            ...
            IRuntimeAssetCatalog runtimeCatalog = null)
        {
            ...
            if (FieldLimitType.Unable_To_Use_Wedding_Invitation_Item.Check(fieldLimit) && IsWeddingInvitationItem(inventoryType, itemId, itemName, itemDescription, runtimeCatalog))
            {
            ...
            if (FieldLimitType.Unable_To_Use_Pet.Check(fieldLimit) && IsPetInteractionItem(inventoryType, itemId, itemName, itemDescription, runtimeAssets))
            {
            ...
            if (FieldLimitType.Unable_To_Summon_NPC.Check(fieldLimit) && IsNpcSummonItem(itemId, itemName, itemDescription, runtimeAssets))
            {
        ...
        private static bool IsWeddingInvitationItem(
            InventoryType inventoryType,
            int itemId,
            string itemName,
            string itemDescription,
        ...
        private static bool IsNpcSummonItem(
            int itemId,
            string itemName,
            string itemDescription,
            IRuntimeAssetSource runtimeAssets)
        {
            bool hasNpcReference = runtimeAssets != null
                                   && InventoryItemMetadataResolver.TryResolveNpcReference(itemId, runtimeAssets, out int npcId)
                                   && npcId > 0;
            bool hasSummonScript = runtimeAssets != null
                                   && InventoryItemMetadataResolver.TryResolveSpecScripts(itemId, runtimeAssets, out IReadOnlyList<string> scriptNames)
        ...
        private static bool IsPetInteractionItem(
            InventoryType inventoryType,
            int itemId,
            string itemName,
            string itemDescription,
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            if (runtimeAssets != null && InventoryItemMetadataResolver.IsPetFoodItem(itemId, runtimeAssets))
            {
                return true;
            }
