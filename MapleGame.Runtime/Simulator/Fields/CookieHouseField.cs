    ...
    public sealed class CookieHouseField
    {
        ...
        private void EnsureAssetsLoaded(GraphicsDevice graphicsDevice)
        {
            ...
            if (!TryLoadClientBackgroundCanvas())
            {
                ...
                {
                    WzImage uiWindow = _runtimeServices?.Assets?.FindImage("UI", imageName);
        ...
        private bool TryResolveClientMapProperty(string resourcePath, out WzImageProperty property, out string resolvedSourcePath)
        {
            property = null;
            ...
            {
                WzImage image = _runtimeServices?.Assets?.FindImage(ClientMapAssetCategory, candidateImageName);
                if (image == null)
                {
                    continue;
                }
        ...
        private IEnumerable<WzImage> EnumerateUiImages()
        {
            var seenNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (string imageName in PreferredUiWindowImages)
            {
                WzImage preferredImage = _runtimeServices?.Assets?.FindImage("UI", imageName);
            ...
            foreach (WzDirectory directory in _runtimeServices?.Assets?.GetDirectories("UI") ?? Array.Empty<WzDirectory>())
            {
                if (directory == null)
                {
                    continue;
