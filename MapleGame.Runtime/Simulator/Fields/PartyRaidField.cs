...
namespace HaCreator.MapSimulator.Fields
{
    ...
    public sealed class PartyRaidField
    {
        ...
        public void SetRuntimeDataServices(IRuntimeDataServices runtimeServices) => _runtimeServices = runtimeServices;
        ...
        private void EnsureAssetsLoaded(GraphicsDevice graphicsDevice)
        {
            ...
            WzImage uiWindow = _runtimeServices?.Assets?.FindImage("UI", "UIWindow.img") ?? _runtimeServices?.Assets?.FindImage("UI", "UIWindow2.img");
            WzImage effectImage = _runtimeServices?.Assets?.FindImage("Map", "Effect.img");
            WzImage mapEtcImage = _runtimeServices?.Assets?.FindImage("Map", "Obj/etc.img")
                ?? _runtimeServices?.Assets?.FindImage("Map", "etc.img");
            uiWindow?.ParseImage();
        ...
        private bool TryResolveBossGaugeCapacity(int mapId, out int gaugeCapacity)
        {
        ...
        private bool TryResolveMobMaxHp(string mobIdText, out int maxHp)
        {
            maxHp = 0;
        ...
        private WzImage FindResolvedMapImage(int mapId)
        {
            ...
            {
                string mapIdText = currentMapId.ToString("D9", CultureInfo.InvariantCulture);
                WzImage mapImage = _runtimeServices.Assets.FindImage("Map", $"Map/Map{mapIdText[0]}/{mapIdText}.img");
                if (mapImage == null)
                {
                    return null;
                }
        ...
        private WzImage FindResolvedMobImage(int mobId)
        {
            if (mobId <= 0)
            {
                return null;
            ...
            {
                WzImage mobImage = _runtimeServices?.Assets?.FindImage("Mob", WzInfoTools.AddLeadingZeros(currentMobId.ToString(CultureInfo.InvariantCulture), 7) + ".img");
                if (mobImage == null)
                {
                    return null;
                }
