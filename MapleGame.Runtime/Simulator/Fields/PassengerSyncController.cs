...
namespace HaCreator.MapSimulator.Fields
{
    ...
    public sealed class PassengerSyncController
    {
        ...
        private readonly Dictionary<int, RuntimeFoothold> _dynamicPlatformFootholds = new();
        private RuntimeFoothold _transportDeckFoothold;
        ...
        private bool TryAttachPlayerToDynamicPlatform(PlayerCharacter player, DynamicFootholdSystem dynamicFootholds)
        {
            ...
            RuntimeFoothold foothold = GetOrCreateSyntheticFoothold(_dynamicPlatformFootholds, SyntheticFootholdBaseId - platform.Id, platform.X, platform.X + platform.Width, platform.Y);
        ...
        private static bool TryResolveDynamicPlatform(
            ...
            RuntimeFoothold currentFoothold,
            float x,
            float y,
            float tolerance,
        ...
        private static bool TryGetDynamicPlatformId(RuntimeFoothold foothold, out int platformId)
        {
            platformId = -1;
            if (foothold == null)
            {
        ...
        private static RuntimeFoothold GetOrCreateSyntheticFoothold(
            Dictionary<int, RuntimeFoothold> cache,
            int cacheKey,
            float leftX,
            float rightX,
            float y,
            RuntimeFoothold existing = null)
        {
            RuntimeFoothold foothold = existing;
        ...
        private static RuntimeFoothold CreateSyntheticFoothold(int cacheKey, float leftX, float rightX, float y)
        {
            var first = new RuntimeFootholdAnchor(
                (int)MathF.Round(leftX), (int)MathF.Round(y));
            var second = new RuntimeFootholdAnchor(
                (int)MathF.Round(rightX), (int)MathF.Round(y));
            return new RuntimeFoothold(cacheKey, first, second);
        }

        private static void MoveSyntheticAnchor(RuntimeFootholdAnchor dot, float x, float y)
        {
            int roundedX = (int)MathF.Round(x);
            int roundedY = (int)MathF.Round(y);
            dot.SetPosition(roundedX, roundedY);
        }
