...
namespace HaCreator.MapSimulator
{
    public partial class MapSimulator
    {
        ...
        private PacketNpcPoolApplyResult ApplyPacketNpcLimitedDisableInfo(PacketNpcLimitedDisableInfoPacket packet, int currentTick)
        {
            ...
            {
                string templateId = NormalizeNpcTemplateId(npc?.RuntimeLife?.Id);
        ...
        private PacketNpcPoolApplyResult ApplyPacketNpcImitateData(IReadOnlyList<PacketNpcImitateEntry> entries, int currentTick)
        {
            ...
            foreach (PacketNpcImitateEntry entry in entries ?? Array.Empty<PacketNpcImitateEntry>())
            {
                ...
                foreach (NpcItem npc in EnumerateLiveNpcs())
                {
                    if (!string.Equals(NormalizeNpcTemplateId(npc?.RuntimeLife?.Id), templateId, StringComparison.Ordinal))
                    {
                        continue;
                    }
        ...
        {
            if (_runtimeMapDefinition == null || _texturePool == null || GraphicsDevice == null)
            {
                return null;
            }

            string templateId = packet.TemplateId.ToString(CultureInfo.InvariantCulture);
            runtimeServices.Catalog.TryGetNpcName(templateId, out RuntimeNpcName npcName);
            int rx0Shift = Math.Max(0, packet.X - packet.Rx0);
            int rx1Shift = Math.Max(0, packet.Rx1 - packet.X);
            var instance = new RuntimeLife(new RuntimeLifeDefinition
            {
                Asset = new RuntimeAssetKey("Npc", templateId.PadLeft(7, '0') + ".img"),
                Id = templateId,
                DisplayName = npcName?.Name ?? templateId,
                X = packet.X,
                Y = packet.Y,
                Rx0Shift = rx0Shift,
                Rx1Shift = rx1Shift,
            ...
            NpcItem npc = LifeLoader.CreateNpcFromProperty(
                ...
                localPlayerGender: _playerManager?.Player?.Build?.Gender,
                hasQuestCheckContext: true,
                questStateProvider: _questRuntime.GetCurrentState,
                questRecordValueProvider: questId => _questRuntime.TryGetQuestRecordValue(questId, out string value) ? value : string.Empty);
            npc?.ApplyPacketInit(
                packet.ObjectId,
                packet.X,
                packet.Y,
        ...
        private void RefreshNpcLookup()
        {
            ...
            foreach (NpcItem npc in _npcsArray)
            {
                ...
                int objectId = npc.PacketObjectId >= 0
                    ? npc.PacketObjectId
                    : TryParseNpcTemplateId(npc.RuntimeLife?.Id, out int templateId) ? templateId : -1;
                if (objectId >= 0)
                {
