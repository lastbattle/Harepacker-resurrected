...
namespace HaCreator.MapSimulator
{
    ...
    public partial class MapSimulator
    {
        ...
        {
            message = null;
            if (!IsLimitedViewWrapperMap(_mapInfo))
            {
        ...
        private void DrawKillCountWrapperOverlay()
        {
            if (_killCountWrapperValue is not int killCount
                || !IsKillCountWrapperMap(_mapInfo)
                || _spriteBatch == null
        ...
        private void DrawEscortResultFailOverlay(int currentTick)
        {
            if (_escortFailOverlayUntilTick == int.MinValue
                ...
                || !IsEscortResultWrapperMap(_mapInfo)
                || _spriteBatch == null
        ...
        private WzImage ResolveDynamicFootholdLinkedContractImage(int linkedMapId)
        {
            if (linkedMapId <= 0)
            {
                return null;
            ...
            return TryGetMapImageForMetadataLookup(linkedMapId);
        }
        ...
        private void ApplyTutorialFieldAppearance(MapInfo mapInfo)
        {
            ...
            if (!TryBuildTutorialAppearanceContract(mapInfo, build.Gender, out TutorialAppearanceContract appearanceContract, TryGetMapImageForMetadataLookup))
            {
                return;
            }
        ...
        {
            message = null;
            if (!TryBuildTutorialWrapperContract(_mapInfo, out TutorialWrapperContract contract)
            ...
            _tutorialFieldSpecificAppearanceMapId = _mapInfo?.id ?? _tutorialFieldSpecificAppearanceMapId;
            ...
            ApplyTutorialFieldAppearance(_mapInfo);
            message =
        ...
        {
            _activeWeddingPhotoSceneContract = TryBuildWeddingPhotoSceneContract(mapInfo, out WeddingPhotoSceneContract contract, TryGetMapImageForMetadataLookup)
                ? contract
                : null;
        ...
        private void EnsureClientOwnedLimitedViewMetadataLoaded()
        {
            ...
            WzImage mapEffectImage = runtimeServices.Assets.FindImage("Effect", "MapEff.img");
            ...
            if (TryBuildTutorialWrapperContract(mapInfo, out TutorialWrapperContract tutorialContract))
            {
                if (tutorialContract.Kind == TutorialWrapperKind.AranTutorial)
                {
                    TutorialAppearanceContract appearanceContract = TryBuildTutorialAppearanceContract(
                        ...
                        TryGetMapImageForMetadataLookup)
