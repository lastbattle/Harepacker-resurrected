...
namespace HaCreator.MapSimulator.Character
{
    ...
    public class CharacterAssembler
    {
            ...
            PortableChairLayer sourcePortableChairLayer = null)
        {
            ...
            if (frame.HasSubParts)
            {
                foreach (var subPart in frame.SubParts)
                {
                    ...
                    int zIndex = zOverride ?? ZMapReference.GetZIndex(subPart.Z, _build.RuntimeAssets);
            ...
            else if (frame.Texture != null)
            {
                ...
                int zIndex = zOverride ?? ZMapReference.GetZIndex(frame.Z, _build.RuntimeAssets);
                parts.Add(new AssembledPart
                {
        ...
        private AvatarRenderLayer ResolveAvatarRenderLayer(CharacterPartType partType, string zLayer, int zIndex)
        {
        ...
        private bool TryResolveAvatarRenderLayerFromZLayer(string zLayer, int zIndex, out AvatarRenderLayer renderLayer)
        {
            renderLayer = default;
        ...
        private AvatarRenderLayer ResolveAvatarRenderLayerFromZIndex(int zIndex, bool preferUnderCharacter)
        {
            int headZ = ZMapReference.GetZIndex("head", _build.RuntimeAssets);
            if (zIndex < headZ)
            {
            ...
            int faceZ = ZMapReference.GetZIndex("face", _build.RuntimeAssets);
            if (zIndex < faceZ)
            {
        ...
        private IReadOnlyList<string> GetVisibilityTokens(CharacterPart sourcePart, string zLayer)
        {
            IReadOnlyList<string> layerTokens = ZMapReference.GetSlotTokens(zLayer, _build.RuntimeAssets);
        ...
        private int GetVisibilityPriority(CharacterPart sourcePart, string zLayer, int zIndex)
        {
            int priority = ZMapReference.GetSlotPriority(sourcePart?.ISlot, _build.RuntimeAssets);
            if (priority != int.MinValue)
            {
                return priority;
            }

            priority = ZMapReference.GetSlotPriority(GetVisibilityTokens(sourcePart, zLayer), _build.RuntimeAssets);
