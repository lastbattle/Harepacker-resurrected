...
namespace HaCreator.MapSimulator.Character
{
    ...
    public class PlayerCharacter
    {
            ...
            IRuntimeAssetSource runtimeAssets)
        {
            ...
            _runtimeAssets = runtimeAssets ?? throw new ArgumentNullException(nameof(runtimeAssets));
        ...
        public void ApplyPacketOwnedPassiveMove(PassivePositionSnapshot snapshot, int currentTime, RuntimeFoothold foothold = null)
        {
        ...
        private void ApplyPacketOwnedPassiveMoveFootholdState(PassivePositionSnapshot snapshot, RuntimeFoothold foothold)
        {
            if (foothold != null)
            {
            ...
            Physics.SetLadderOrRopeLookup(findLadder == null
                ...
                : (x, y, range) =>
                {
                    ...
                    return ladder.HasValue
                        ? new RuntimeLadderOrRope(ladder.Value.x, ladder.Value.top, ladder.Value.bottom, ladder.Value.isLadder)
                        : null;
                });
        }
        ...
        private void TryGrabLadder()
        {
            ...
            bool foundLadder = Physics.TryGetLadderOrRope(X, Y, horizontalSearchRange, out RuntimeLadderOrRope ladder);
            if (!foundLadder)
            {
        ...
        private void TryGrabLadderDown()
        {
            ...
            if (Physics.TryGetLadderOrRope(X, Y + 15, 15f, out RuntimeLadderOrRope ladder))
            {
        ...
        private void ExitLadderAtTop()
        {
            ...
            RuntimeFoothold exitFoothold = _findFoothold?.Invoke(Physics.LadderX, (float)exitY, 24f);
        ...
        private void CheckFootholdLanding()
        {
            ...
            if (fh != null)
            {
                ...
                void CompleteLanding(RuntimeFoothold footholdToLandOn)
                {
        ...
        /// </summary>
        private double CalculateYOnFoothold(RuntimeFoothold fh, double x)
        {
        ...
        private bool TryRegrabLadderWhileHoldingUp()
        {
            ...
            bool foundLadder = Physics.TryGetLadderOrRope(X, Y, regrabHorizontalTolerance, out RuntimeLadderOrRope ladder);
            if (!foundLadder &&
            ...
            {
                ladder = new RuntimeLadderOrRope(_pendingLadderX, _pendingLadderTop, _pendingLadderBottom, _pendingLadderIsLadder);
                foundLadder = true;
            }
        ...
        private const string DebugPixelTexturePoolKey = "PlayerCharacter/DebugPixel";

        private Texture2D GetPixelTexture(GraphicsDevice device)
        {
            Texture2D pixelTexture = _texturePool?.GetTexture(DebugPixelTexturePoolKey);
            if (pixelTexture == null || pixelTexture.IsDisposed)
            {
                pixelTexture = new Texture2D(device, 1, 1);
                pixelTexture.SetData(new[] { Color.White });
                _texturePool?.AddTextureToPool(DebugPixelTexturePoolKey, pixelTexture);
            }
            return pixelTexture;
        }
            ...
            Func<float, float, float, RuntimeFoothold> findFoothold)
        {
            ...
            RuntimeFoothold foothold = findFoothold(partnerX, partnerY, searchRange);
            if (foothold == null)
            {
                return false;
            }
        ...
        private static float CalculateYOnFoothold(RuntimeFoothold foothold, float x)
        {
            if (foothold == null)
            {
                return float.NaN;
        ...
        private SkillAnimation LoadPacketOwnedEmotionEffectAnimationCore(string effectUol)
        {
            ...
            WzImage image = _runtimeAssets.FindImage(category, imageName);
            if (image == null)
            {
                return null;
            }
