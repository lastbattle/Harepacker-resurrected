...
namespace HaCreator.MapSimulator.Character.Skills
{
    ...
    public class SkillLoader
    {
        ...
        private readonly IRuntimeAssetSource _assets;
        ...
        public SkillAnimation LoadClientDefaultHitAnimation(string branchName)
        {
            ...
            WzImage image = _assets.FindImage("Character", "Afterimage/hit.img");
            if (image == null)
            {
        ...
        public SkillAnimation LoadClientCashBulletHitAnimation(int itemId)
        {
            ...
            WzImage image = _assets.FindImage("Item", imagePath);
            if (image == null)
            {
        ...
        public string EnsureRepeatSoundRegistered(SkillData skill, SoundManager soundManager)
        {
            ...
            {
                if (TryRegisterSkillSound(
                        soundManager,
                        skillSoundNode[preferredName],
                        skill.SkillId,
                        preferredName,
                        out string soundKey,
                        out WzBinaryProperty soundBinary))
                {
                    skill.RepeatSoundKey = soundKey;
                    skill.RepeatSoundProperty = soundBinary;
                    return soundKey;
                }
            }
        ...
        private void LoadSkillStrings(SkillData skill)
        {
            ...
            _skillStringImage ??= _assets.FindImage("String", "Skill.img");
        ...
        private MeleeAfterImageCatalog GetOrLoadCharacterAfterImageCatalog(string weaponTypeKey, int masteryIndex)
        {
            ...
            WzImage image = _assets.FindImage("Character", $"Afterimage/{weaponTypeKey}");
            if (image == null)
            {
            ...
            IReadOnlySet<string> supportedRawActionNames)
        {
            ...
            WzImage actionImage = _assets.FindImage("Character", "0000/00002000");
                ...
                foreach (WzImageProperty valueNode in tableEntry.WzProperties)
                {
                    ...
                    if (IsClientSummonedUolRowSetFieldName(valueNode.Name))
                    {
                        foreach (IReadOnlyList<string> rowValues in ReadClientSummonedUolRowSetValues(valueNode, preferredDelimiter, headers))
                        {
                            ...
                            {
                                yield return (headers, rowValues);
                            }
                        }
            ...
            if (node.WzProperties != null && node.WzProperties.Count > 0)
            {
                ...
                if (indexedChildren.Count > 0)
                {
                    ...
                    foreach (WzImageProperty child in indexedChildren.Values)
                    {
                        ...
                        hasDelimitedRowText |= DetectClientSummonedUolListDelimiter(value) != '\0';
                    }
                    ...
                    if (indexedValues.Count > 0)
                    {
                        ...
                        foreach (string indexedValue in indexedValues)
                        {
                            IReadOnlyList<string> row = SplitClientSummonedUolDelimitedList(indexedValue, preferredDelimiter);
                            if (row.Count > 0)
                            {
                                yield return row;
                            }
            ...
            IReadOnlyList<string> headers = null)
        {
            ...
            {
                foreach (WzImageProperty rowNode in node.WzProperties)
                {
                    if (rowNode == null)
                    {
                        continue;
        ...
        private static IEnumerable<string> SplitClientSummonedUolRowSetText(string text)
        {
            ...
            foreach (string line in normalizedText
            ...
            char preferredDelimiter)
        {
            ...
            return ReadClientSummonedUolDelimitedOrIndexedText(text, preferredDelimiter);
        }

        private static IReadOnlyList<string> ReadClientSummonedUolDelimitedOrIndexedText(
            string text,
            char preferredDelimiter)
        {
            if (string.IsNullOrWhiteSpace(text))
            ...
            return SplitClientSummonedUolDelimitedList(normalizedText, delimiter);
        }

        private static IReadOnlyList<string> SplitClientSummonedUolDelimitedList(
            string text,
            char preferredDelimiter)
        {
            string normalizedText = NormalizeClientSummonedUolEncodedPathSyntax(text).Trim();
            ...
            if (delimiter == '\0')
            {
                string value = TrimClientSummonedUolRecordTextFieldToken(normalizedText);
        ...
        private static bool IsClientSummonedUolValueListFieldName(string name)
        {
            ...
            return normalizedName.Equals("value", StringComparison.Ordinal)
                   ...
                   || normalizedName.Equals("entry", StringComparison.Ordinal)
        ...
        private static bool IsClientSummonedUolTableEntryValueName(string name)
        {
            ...
            return leafName.Equals("0", StringComparison.OrdinalIgnoreCase)
                   ...
                   || leafName.Equals("pathData", StringComparison.OrdinalIgnoreCase)
                   ...
                   || leafName.Equals("sourceValue", StringComparison.OrdinalIgnoreCase)
                   || leafName.Equals("sourceData", StringComparison.OrdinalIgnoreCase)
                   || leafName.Equals("srcPath", StringComparison.OrdinalIgnoreCase)
                   || leafName.Equals("srcValue", StringComparison.OrdinalIgnoreCase)
                   || leafName.Equals("asset", StringComparison.OrdinalIgnoreCase)
                   || leafName.Equals("assetPath", StringComparison.OrdinalIgnoreCase)
                   || leafName.Equals("assetValue", StringComparison.OrdinalIgnoreCase)
                   ...
                   || leafName.Equals("clientValue", StringComparison.OrdinalIgnoreCase)
        ...
        private SkillAnimation LoadItemBulletAnimationInternal(int itemId, int weaponItemId = 0, int weaponCode = 0)
        {
            ...
            WzImage itemImage = _assets.FindImage(category, imagePath);
            if (itemImage == null)
            {
                return null;
            }
        ...
        private IReadOnlyList<int> EnumerateAvailableSkillBookJobIds()
        {
            ...
            var result = new SortedSet<int>();
            CollectSkillBookJobIds(_assets.FindObject("Skill", string.Empty), result);

            if (result.Count == 0)
            {
                foreach (var directory in _assets.GetDirectories("Skill"))
                {
        ...
        private WzImage GetSkillImage(string imgName)
        {
            return (_skillWz?.WzDirectory?[imgName] as WzImage)
                   ?? _assets.FindImage("Skill", imgName);
        }

        private WzImageProperty GetSkillSoundNode(int skillId)
        {
            _skillSoundImage ??= _assets.FindImage("Sound", "Skill.img");
        ...
        private bool TryRegisterSkillSound(SkillData skill, SoundManager soundManager, WzImageProperty soundProperty, string soundName)
        {
            ...
            skill.CastSoundProperty = soundBinary;
            return true;
        }

        private static bool TryRegisterSkillSound(
            SoundManager soundManager,
            WzImageProperty soundProperty,
            int skillId,
            string soundName,
            out string soundKey,
            out WzBinaryProperty soundBinary)
        {
            soundKey = null;
            soundBinary = soundProperty as WzBinaryProperty
        ...
        private int ResolveMorphTemplateId(
            int skillId,
        ...
        private int ResolveFlagOnlyMorphAliasTemplateId(
            int skillId,
            WzImageProperty commonNode,
            WzImageProperty pvpCommonNode,
            WzImageProperty infoNode,
            IEnumerable<string> requestedActionNames)
        {
            ...
            return ResolveFlagOnlyMorphAliasTemplateId(
                ...
                requestedActionNames,
                (templateId, actionNames) => CharacterLoader.CanResolveMorphTemplate(templateId, actionNames, _assets));
        }
