...
namespace HaCreator.MapSimulator.Character
{
    ...
    public class CharacterBuild
    {
        ...
        private readonly object _itemOptionCacheLock = new();
        private readonly Dictionary<(int OptionId, int EquipLevel), ItemOptionStatBonus> _itemOptionStatBonusCache = new();
        ...
        /// <summary>
        /// Runtime asset source used for data-backed stat and character layout rules.
        /// Keeping this on the build lets cloned and remote builds retain the session
        /// they belong to without reaching through the editor's Program singleton.
        /// </summary>
        private IRuntimeAssetSource _runtimeAssets;
        public IRuntimeAssetSource RuntimeAssets
        {
            get => _runtimeAssets;
            set
            {
                if (ReferenceEquals(_runtimeAssets, value))
                {
                    return;
                }

                _runtimeAssets = value;
                lock (_itemOptionCacheLock)
                {
                    _itemOptionStatBonusCache.Clear();
                }
            }
        }
        ...
        private int SumPartItemOptionBonus(CharacterPart part, Func<ItemOptionStatBonus, int> selector)
        {
            ...
            {
                if (TryResolveItemOptionStatBonus(itemOptionId, equipLevel, RuntimeAssets, out ItemOptionStatBonus bonus))
                {
        ...
        private bool TryResolveItemOptionStatBonus(
            int itemOptionId,
            int equipLevel,
            IRuntimeAssetSource assets,
            out ItemOptionStatBonus bonus)
        {
            bonus = default;
            ...
            lock (_itemOptionCacheLock)
            {
                if (_itemOptionStatBonusCache.TryGetValue(cacheKey, out bonus))
                {
                    return !bonus.Equals(default(ItemOptionStatBonus));
                }
            }

            bonus = LoadItemOptionStatBonus(cacheKey.OptionId, cacheKey.EquipLevel, assets);
            if (!bonus.Equals(default(ItemOptionStatBonus)) || assets?.FindImage("Item", "ItemOption.img") != null)
            {
                lock (_itemOptionCacheLock)
                {
                    _itemOptionStatBonusCache[cacheKey] = bonus;
                }
            }
        ...
        private static ItemOptionStatBonus LoadItemOptionStatBonus(
            int itemOptionId,
            int equipLevel,
            IRuntimeAssetSource assets)
        {
            if (TryLoadWzItemOptionStatBonus(itemOptionId, equipLevel, assets, out ItemOptionStatBonus bonus))
            {
                return bonus;
            }
        ...
        private static bool TryLoadWzItemOptionStatBonus(
            int itemOptionId,
            int equipLevel,
            IRuntimeAssetSource assets,
            out ItemOptionStatBonus bonus)
        {
            bonus = default;
            WzImage itemOptionImage = assets?.FindImage("Item", "ItemOption.img");
        ...
        private AttackFormulaProfile ResolveAttackFormulaProfile()
        {
            ...
            return weaponCode switch
            {
                ...
                // WZ `Character/Weapon/01392000/info` keeps a hidden barehands owner
                // (`afterImage = barehands`, `sfx = barehands`, `attack = 8`) on the same
                // close-range pirate lane that client skill prepare validation admits as
                // `39|48` for knuckle-family checks.
                39 => new AttackFormulaProfile(false, 4.8f, TotalSTR, TotalDEX, 0.9f),
    ...
    public static class ZMapReference
    {
        ...
        private sealed record MapperSnapshot(
            string[] ZOrder,
            IReadOnlyDictionary<string, int> ZIndices,
            IReadOnlyDictionary<string, string[]> SlotMap);

        private static readonly MapperSnapshot DefaultMappers = new(
            DefaultZOrder,
            BuildZIndexMap(DefaultZOrder),
            new Dictionary<string, string[]>(StringComparer.Ordinal));
        private static readonly ConditionalWeakTable<IRuntimeAssetSource, MapperSnapshot> MappersBySource = new();

        public static IReadOnlyList<string> ZOrder
        {
            get
            {
                return DefaultMappers.ZOrder;
            }
        }

        public static int GetZIndex(string zLayer, IRuntimeAssetSource assets = null)
        {
            MapperSnapshot mappers = GetMappers(assets);
            if (!string.IsNullOrEmpty(zLayer) && mappers.ZIndices.TryGetValue(zLayer, out int index))
            {
                return index;
            }

            return mappers.ZOrder.Length > 0 ? mappers.ZOrder.Length / 2 : 50;
        }

        /// <summary>
        /// Check if z-layer exists in the order array
        /// </summary>
        public static bool HasZLayer(string zLayer, IRuntimeAssetSource assets = null)
        {
            MapperSnapshot mappers = GetMappers(assets);
            return !string.IsNullOrEmpty(zLayer) && mappers.ZIndices.ContainsKey(zLayer);
        }

        public static IReadOnlyList<string> GetSlotTokens(string zLayer, IRuntimeAssetSource assets = null)
        {
            MapperSnapshot mappers = GetMappers(assets);
            if (!string.IsNullOrEmpty(zLayer) && mappers.SlotMap.TryGetValue(zLayer, out string[] tokens))
            {
                return tokens;
            }
        ...
        public static int GetSlotPriority(string slotValue, IRuntimeAssetSource assets = null)
        {
            MapperSnapshot mappers = GetMappers(assets);
            ...
            {
                if (mappers.ZIndices.TryGetValue(token, out int priority))
                {
        ...
        public static int GetSlotPriority(IEnumerable<string> slotTokens, IRuntimeAssetSource assets = null)
        {
            MapperSnapshot mappers = GetMappers(assets);
            ...
            {
                if (!string.IsNullOrEmpty(token) && mappers.ZIndices.TryGetValue(token, out int priority))
                {
                    maxPriority = Math.Max(maxPriority, priority);
                }
            }
        ...
        public static void EnsureLoaded(IRuntimeAssetSource assets = null)
        {
            _ = GetMappers(assets);
        }

        private static MapperSnapshot GetMappers(IRuntimeAssetSource assets)
        {
            return assets == null
                ? DefaultMappers
                : MappersBySource.GetValue(assets, LoadMappers);
        }

        private static MapperSnapshot LoadMappers(IRuntimeAssetSource assets)
        {
            string[] zOrder = DefaultZOrder;
            IReadOnlyDictionary<string, int> zIndices = DefaultMappers.ZIndices;
            IReadOnlyDictionary<string, string[]> slotMap = DefaultMappers.SlotMap;

            try
            {
                var zMapImage = assets?.FindImage("base", "zmap.img");
                if (zMapImage != null)
                {
                    ...
                    if (loadedOrder.Count > 0)
                    {
                        ...
                        loadedOrder.Reverse();
                        zOrder = loadedOrder.ToArray();
                        zIndices = BuildZIndexMap(zOrder);
                    }
                }

                var sMapImage = assets?.FindImage("base", "smap.img");
                if (sMapImage != null)
                {
            ...
            return new MapperSnapshot(zOrder, zIndices, slotMap);
        }
