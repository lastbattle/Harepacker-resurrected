...
namespace HaCreator.MapSimulator.Character
{
    ...
    public class CharacterLoader
    {
        ...
        /// <summary>
        /// Get a character image by name, using WzFile if available or the session asset source.
        /// </summary>
        private WzImage GetCharacterImage(string imgName)
        {
            ...
            resolvedImage ??= _runtimeServices.Assets.FindImage("Character", imgName);
        ...
        public CharacterPart LoadMorph(int morphTemplateId)
        {
            ...
            WzImage morphImage = _runtimeServices.Assets.FindImage("Morph", imageName);
            ...
            ISet<int> activeTemplateIds)
        {
            ...
            try
            {
                ...
                foreach (int candidateTemplateId in EnumerateMorphTemplateCandidates(morphTemplateId, exactMorphImage, _runtimeServices.Assets))
                {
                    WzImage candidateImage = candidateTemplateId == morphTemplateId
                        ? exactMorphImage ?? _runtimeServices.Assets.FindImage("Morph", GetClientMorphImageName(candidateTemplateId))
                        : _runtimeServices.Assets.FindImage("Morph", GetClientMorphImageName(candidateTemplateId));
        ...
        internal static bool CanResolveMorphTemplate(int morphTemplateId, IRuntimeAssetSource assets)
        {
            return CanResolveMorphTemplate(morphTemplateId, Array.Empty<string>(), assets);
        }

        internal static bool CanResolveMorphTemplate(
            int morphTemplateId,
            ...
            IRuntimeAssetSource assets)
        {
            ...
            foreach (int candidateTemplateId in EnumerateMorphTemplateCandidates(morphTemplateId, exactMorphImage: null, assets))
            {
                ...
                WzImage candidateImage = assets?.FindImage("Morph", GetClientMorphImageName(candidateTemplateId));
                if (candidateImage == null)
                {
                    continue;
                }
        ...
        private static IEnumerable<int> EnumerateMorphTemplateCandidates(
            int morphTemplateId,
            WzImage exactMorphImage,
            IRuntimeAssetSource assets)
        {
            ...
            foreach (int rootCandidate in EnumerateMorphTemplateRootCandidates(morphTemplateId))
            {
                ...
                foreach (int candidate in EnumerateMorphLinkChain(rootCandidate, rootImage, seen, allowLinkInheritance, assets))
                {
            ...
            IRuntimeAssetSource assets)
        {
            ...
            WzImage morphImage = exactMorphImage ?? assets?.FindImage("Morph", GetClientMorphImageName(morphTemplateId));
        ...
        public ItemEffectAnimationSet LoadItemEffectAnimationSet(int itemId)
        {
            ...
            WzImage itemEffectImage = _runtimeServices.Assets.FindImage("Effect", "ItemEff.img");
        ...
        private WzSubProperty ResolveActiveEffectItemEffectProperty(int itemId)
        {
        ...
        private WzSubProperty ResolveNewYearCardEffectProperty(int itemId)
        {
            ...
            WzImage effectImage = _runtimeServices.Assets.FindImage("Effect", imageName);
            if (effectImage == null)
            {
                return null;
            }
        ...
        public RelationshipTextTagStyle LoadRelationshipTextTagStyle(RemoteRelationshipOverlayType relationshipType)
        {
            ...
            WzImage nameTagImage = _runtimeServices.Assets.FindImage("UI", "NameTag.img");
        ...
        public CarryItemEffectDefinition LoadCarryItemEffectDefinition()
        {
            ...
            WzImage characterEffectImage = _runtimeServices.Assets.FindImage("Effect", "CharacterEff.img");
        ...
        {
            WzImage setItemInfoImage = _runtimeServices.Assets.FindImage("Etc", "SetItemInfo.img");
            ...
            {
                WzImage linkedEffectImage = _runtimeServices.Assets.FindImage("Effect", imageName);
            ...
            WzImage setItemEffectImage = _runtimeServices.Assets.FindImage("Effect", "SetItemInfoEff.img");
        ...
        private string ResolvePortableChairName(int itemId)
        {
            return _runtimeServices.Catalog.TryGetItemName(itemId, out RuntimeItemName itemInfo)
                   && !string.IsNullOrWhiteSpace(itemInfo.Name)
                ? itemInfo.Name
                : $"Portable Chair {itemId}";
        }

        private string ResolvePortableChairDescription(int itemId)
        {
            return _runtimeServices.Catalog.TryGetItemName(itemId, out RuntimeItemName itemInfo)
        ...
        public FacePart LoadFace(int faceId)
        {
            ...
            // Resolve the image directly through the runtime asset facade.
            // Looking up the Character root first forces a detached clone of
            // the complete Character tree for IMG sources, which can be many
            // gigabytes in modern clients. The facade already knows how to
            // resolve nested image paths and only loads this face.
            if (imgNode == null)
            {
                imgNode = _runtimeServices.Assets.FindImage("Character", "Face/" + imgName);
            }
        ...
        public HairPart LoadHair(int hairId)
        {
            ...
            if (imgNode == null)
            {
                imgNode = _runtimeServices.Assets.FindImage("Character", $"Hair/{imgName}");
            }
        ...
        private CharacterFrame LoadWeaponFrame(WzCanvasProperty canvas, WzSubProperty parentFrame, string frameName, string actionName = null)
        {
            ...
            if (string.IsNullOrEmpty(zLayer) || !ZMapReference.HasZLayer(zLayer, _runtimeServices.Assets))
            {
        ...
        private void LoadEquipInfo(CharacterPart part, CharacterImageEntry imageEntry)
        {
            ...
            if (_runtimeServices.Catalog.TryGetItemName(part.ItemId, out RuntimeItemName itemInfo))
            {
                part.ItemCategory = itemInfo.Category;
                if (!string.IsNullOrWhiteSpace(itemInfo.Name))
                {
                    part.Name = itemInfo.Name;
                }

                part.Description = itemInfo.Description;
            }
        ...
        private WzImage GetEquipmentImage(string folder, int itemId)
        {
            ...
            WzImage resolvedImage = _runtimeServices.Assets.FindImage("Character", $"{folder}/{imgName}");
        ...
        private IDXObject LoadTexture(WzCanvasProperty canvas)
        {
            ...
            try
            {
                Texture2D texture = _texturePool.LoadCanvasTexture(canvas, _device);
                if (texture == null) return null;
        ...
        private string ResolveZLayer(string zLayer, string fallback)
        {
            if (!string.IsNullOrEmpty(zLayer) && ZMapReference.HasZLayer(zLayer, _runtimeServices.Assets))
            {
                return zLayer;
            ...
            if (!string.IsNullOrEmpty(fallback) && ZMapReference.HasZLayer(fallback, _runtimeServices.Assets))
            {
                return fallback;
            }
        ...
        {
            var candidates = new List<int>();
            WzImage makeCharInfoImage = _runtimeServices.Assets.FindImage("Etc", "MakeCharInfo.img");
        ...
        private void AttachEquipmentResolver(CharacterBuild build)
        {
            if (build != null)
            {
                build.RuntimeAssets = _runtimeServices.Assets;
