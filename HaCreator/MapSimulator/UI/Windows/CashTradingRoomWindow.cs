...
namespace HaCreator.MapSimulator.UI
{
    public sealed class CashTradingRoomWindow : UIWindowBase
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (Pressed(keyboardState, Keys.Enter) && CapturesKeyboardInput)
            {
                SubmitChatEntry();
            }
            ...
            int TickCount)
        {
            ...
            sprite.DrawString(_font, $"{localWallet.ToString("N0", CultureInfo.InvariantCulture)} meso", new Vector2(Position.X + 22, leftY + 16), moneyColor);
            ...
            sprite.DrawString(_font, $"Fonts: white / no-black / no-blue / gray / red / remain-gray", new Vector2(Position.X + 22, leftY + 86), mutedColor);
            ...
            sprite.DrawString(_font, $"Edit [{ChatEditX},{ChatEditY} {ChatEditWidth}x{ChatEditHeight}]  Focus {(_chatEditControl.HasFocus ? "on" : "off")}  Scroll [{ChatScrollX},{ChatScrollY} h{ChatScrollHeight} wheel {ChatWheelRange}]", new Vector2(Position.X + 22, stateY + 18), mutedColor);
        ...
        private static bool Pressed(KeyboardState keyboardState, Keys key)
        {
