...
namespace HaCreator.MapSimulator.Character.Skills
{
    ...
    public class SkillManager
    {
        ...
        private static readonly string[] ClientRandomMovingShootShoot1FamilyActionNames =
        {
            "shoot1"
        };
        private static readonly string[] ClientRandomMovingShootShoot2FamilyActionNames =
        {
            "shoot2"
        };
        ...
        public event Action SkillRecordsChanged;
        ...
        public void LoadSkillsForJob(int jobId)
        {
            ...
            NotifyHotkeysChanged(hotkeysChanged);
        ...
        public void SetSkillLevel(int skillId, int level)
        {
            ...
            if (level <= 0)
            {
                changed |= _skillLevels.Remove(skillId);
                changed |= _skillMasterLevels.Remove(skillId);
            }
            else
            {
                changed = !_skillLevels.TryGetValue(skillId, out int currentLevel) || currentLevel != level;
        ...
        private void BeginSkillRecordChangeBatch()
        {
            _skillRecordChangeBatchDepth++;
        }

        private void EndSkillRecordChangeBatch()
        {
        ...
        private void NotifySkillRecordsChanged()
        {
            if (_skillRecordChangeBatchDepth > 0)
            {
                _skillRecordChangedDuringBatch = true;
                return;
            }
        ...
        public bool TrySetHotkey(int slotIndex, int skillId)
        {
            ...
            bool stateChanged = !_skillHotkeys.TryGetValue(slotIndex, out int existingSkillId) || existingSkillId != skillId;
            stateChanged |= _macroHotkeys.Remove(slotIndex);
            stateChanged |= _itemHotkeys.Remove(slotIndex);
            _skillHotkeys[slotIndex] = skillId;
            NotifyHotkeysChanged(stateChanged);
            return true;
        }
        ...
        public bool TrySetMacroHotkey(int slotIndex, int macroIndex)
        {
            ...
            if (macroIndex < 0)
            {
                NotifyHotkeysChanged(_macroHotkeys.Remove(slotIndex));
                return true;
            }
            ...
            bool stateChanged = !_macroHotkeys.TryGetValue(slotIndex, out int existingMacroIndex) || existingMacroIndex != macroIndex;
            stateChanged |= _skillHotkeys.Remove(slotIndex);
            stateChanged |= _itemHotkeys.Remove(slotIndex);
        ...
        public bool TrySetItemHotkey(int slotIndex, int itemId, InventoryType inventoryType = InventoryType.NONE)
        {
            ...
            if (itemId <= 0)
            {
                NotifyHotkeysChanged(_itemHotkeys.Remove(slotIndex));
                return true;
            }
        ...
        {
            bool changed = _skillHotkeys.Count > 0;
            _skillHotkeys.Clear();
            NotifyHotkeysChanged(changed);
        ...
        {
            bool changed = _macroHotkeys.Count > 0;
            _macroHotkeys.Clear();
            NotifyHotkeysChanged(changed);
            if (hotkeys == null)
                return;
        ...
        {
            bool changed = _itemHotkeys.Count > 0;
            _itemHotkeys.Clear();
            NotifyHotkeysChanged(changed);
            if (hotkeys == null)
                return;
        ...
        public int RevalidateHotkeys(int startSlotIndex, int slotCount)
        {
            ...
            NotifyHotkeysChanged(removed > 0);
            return removed;
        }
        ...
        {
            bool changed = _skillHotkeys.Count > 0 || _macroHotkeys.Count > 0 || _itemHotkeys.Count > 0;
            _skillHotkeys.Clear();
            _macroHotkeys.Clear();
            _itemHotkeys.Clear();
            NotifyHotkeysChanged(changed);
        ...
        {
            bool stateChanged = !_itemHotkeys.TryGetValue(slotIndex, out ItemHotkeyBinding existingBinding) ||
                                existingBinding?.ItemId != itemId ||
                                existingBinding.InventoryType != inventoryType;
            stateChanged |= _skillHotkeys.Remove(slotIndex);
            stateChanged |= _macroHotkeys.Remove(slotIndex);
        ...
        private void NotifyHotkeysChanged(bool changed = true)
        {
            if (changed)
            {
        ...
        public bool RequestClientSkillCancel(int skillId, int currentTime, bool enforceFieldCancelRestrictions = false)
        {
            ...
            bool sideEffectsApplied = ApplyClientSkillCancelSideEffects(normalizedPacketSkillId, currentTime);
        ...
        internal static StatusBarBuffEntry ResolveVehicleDurabilityPassiveStatusBarEntryForParity(
            ...
            int startTime)
        {
            int currentValue = Math.Max(0, cooldownState.CurrentValue);
            int maxValue = Math.Max(currentValue, cooldownState.MaxValue);
            string durabilityText = maxValue > 0
                ? $"Durability: {currentValue}/{maxValue}"
                : $"Durability: {currentValue}";
            ...
            {
                SkillId = skillId,
                SkillName = skill?.Name ?? skillId.ToString(CultureInfo.InvariantCulture),
                Description = string.IsNullOrWhiteSpace(skill?.Description)
                    ? durabilityText
                    : $"{skill.Description}\n{durabilityText}",
                IconTexture = skill?.IconTexture ?? skill?.Icon?.Texture,
                StartTime = startTime,
                ...
                TemporaryStatDisplayNames = new[] { "Vehicle Durability" }
            };
        }
        ...
        internal void ApplyAuthoritativeSkillRecordSnapshot(IReadOnlyDictionary<int, int> skillLevelsBySkillId)
        {
            ...
            foreach (int activeSkillId in _skillLevels.Keys.ToList())
            {
                ...
                changed = true;
                changed |= _skillMasterLevels.Remove(activeSkillId);
            }
        ...
        private static bool IsClientOwnedVehicleToggleSkill(int skillId)
        {
            return skillId == 1004
                   ...
                   || skillId == 20011004
                   || skillId == 30001004
                   || skillId == WildHunterJaguarRiderSkillId
                   || skillId == 35001002
                   || skillId == BATTLESHIP_SKILL_ID;
        }
        ...
        private bool ApplyClientSkillCancelSideEffects(int skillId, int currentTime)
        {
            ...
            bool vehicleCancelRecorded = RecordVehicleValidCancel(skillId, currentTime);
            bool keydownCleanupApplied = CleanupKeydownPrepareAnimationOnCancel(skillId, currentTime, out bool dragonCleanupApplied);
        ...
        internal bool TryBuildPendingRepeatSkillModeEndEffectRequest(
            int skillId,
            int returnSkillId,
            int requestedAt,
        ...
        {
            request = default;
            error = "Repeat-skill mode-end request is not pending.";
            if (!HasPendingRepeatSkillModeEndRequest(skillId, returnSkillId, requestedAt))
            {
                return false;
            }

            int effectSkillId = ResolveRepeatSkillTimeoutEffectRequestSkillId(skillId);
            if (effectSkillId <= 0)
            {
                error = $"Repeat-skill {skillId} does not expose a packet-owned mode-end effect request.";
                return false;
            }
            ...
            if (skillLevel <= 0)
            {
                error = $"Repeat-skill {skillId} has no learned level for the packet-owned mode-end request.";
                return false;
            }

            return PacketOwnedMechanicRepeatSkillRuntime.TryCreateSkillEffectRequest(
                effectSkillId,
                skillLevel,
                sendLocal: true,
                out request,
            ...
            int? currentRawActionCodeOverride = null)
        {
            ...
            try
            {
                ...
                TryRequestExplicitSpecialSkillUseEffect(
                    skill,
                    currentTime,
                    attackOriginOverride ?? preferredTargetPositionOverride);
        ...
        private void ExecuteAttackPayload(
            ...
            string currentActionNameOverride = null,
            int? currentRawActionCodeOverride = null,
            bool requestSpecialEffect = true)
        {
            bool facingRight = facingRightOverride ?? _player.FacingRight;
        ...
        private void TryRequestExplicitSpecialSkillUseEffect(
            SkillData skill,
            int currentTime,
            Vector2? worldOrigin)
        {
        ...
        private bool HasAnimationDisplayerSpecialBranch(int skillId)
        {
            ...
            if (_animationDisplayerSpecialBranchCache.TryGetValue(skillId, out bool cached))
            {
                return cached;
            }
            ...
            bool hasSpecial = skillImage?["skill"]?[skillId.ToString("D7", CultureInfo.InvariantCulture)]?["special"] != null;
            _animationDisplayerSpecialBranchCache[skillId] = hasSpecial;
            return hasSpecial;
        }
        ...
        private int ResolveCurrentClientShootAttackVehicleId()
        {
            return ResolveCurrentClientShootAttackVehicleId(
                _player?.CurrentActionName,
                _activeSkillMount?.MountItemId ?? 0,
                _player?.ResolveMountedStateTamingMobPart()?.ItemId ?? 0,
                GetEquippedTamingMobPart()?.ItemId ?? 0,
            ...
            out int attackActionType)
        {
            ...
            // weapon `info/attack`. Preserve the recovered shoot rows as exact table
            // values and collapse the recovered melee-family rows only to the coarse
            // simulator targeting lane; the later random-action resolver still uses
            // weapon type to choose one-handed, two-handed, spear, polearm, knuckle, or
            // katara action families.
            return TryResolveMovingShootAttackActionTypeFromWeaponAttackMetadata(
                currentWeaponAttackActionType.Value,
                currentWeaponType,
                out attackActionType);
        }

        internal static bool TryResolveMovingShootAttackActionTypeFromWeaponAttackMetadata(
            int currentWeaponAttackActionType,
            string currentWeaponType,
            out int attackActionType)
        {
            ...
            switch (currentWeaponAttackActionType)
            {
                case 1:
                case 5:
                case 6:
                ...
                case 2 when normalizedWeaponType is "spear" or "polearm":
                case 10 when normalizedWeaponType == "katara":
                    attackActionType = MovingShootAttackActionTypeMelee;
                    return true;
            ...
            IReadOnlyCollection<string> publishedWeaponActionNames = null)
        {
            ...
            foreach ((string ActionName, int RawActionCode) candidate in EnumerateClientRandomMovingShootActionFamilyActions(
                         actionFamily,
                         publishedWeaponActionNames))
            {
                yield return candidate;
            ...
            IReadOnlyCollection<string> publishedWeaponActionNames = null)
        {
            string[] actionNames = actionFamily switch
            {
                ...
                ClientRandomMovingShootActionFamily.Shoot1 => ClientRandomMovingShootShoot1FamilyActionNames,
                ClientRandomMovingShootActionFamily.Shoot2 => ClientRandomMovingShootShoot2FamilyActionNames,
                ClientRandomMovingShootActionFamily.GunShoot => ClientRandomMovingShootShootFamilyActionNames,
        ...
        private static IReadOnlyCollection<string> ResolvePublishedMovingShootActionNames(WeaponPart weapon)
        {
            if (weapon == null)
            {
                return EmptyPublishedMovingShootActionNames;
            }

            var publishedActionNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            if (weapon.AvailableAnimations != null)
            {
                foreach (string actionName in weapon.AvailableAnimations)
                {
                    if (!string.IsNullOrWhiteSpace(actionName))
                    {
                        publishedActionNames.Add(actionName);
                    }
                }
            }
            ...
            {
                foreach (string actionName in weapon.Animations.Keys)
                {
                    if (!string.IsNullOrWhiteSpace(actionName))
                    {
                        publishedActionNames.Add(actionName);
            ...
            return publishedActionNames.Count > 0
        ...
        private static ClientRandomMovingShootActionFamily ResolveClientRandomMovingShootActionFamilyFromActionName(string normalizedActionName)
        {
            ...
            {
                if (string.Equals(normalizedActionName, "shoot1", StringComparison.OrdinalIgnoreCase))
                {
                    return ClientRandomMovingShootActionFamily.Shoot1;
                }

                if (string.Equals(normalizedActionName, "shoot2", StringComparison.OrdinalIgnoreCase))
                {
                    return ClientRandomMovingShootActionFamily.Shoot2;
                }

                if (string.Equals(normalizedActionName, "shootF", StringComparison.OrdinalIgnoreCase))
                {
                    return ClientRandomMovingShootActionFamily.GunShoot;
                }
            ...
            ClientRandomMovingShootActionFamily requiredFamily)
        {
            ...
            if (IsClientRandomMovingShootShootFamily(requiredFamily))
            {
                return requiredFamily == ClientRandomMovingShootActionFamily.GunShoot
                    ? IsClientRandomMovingShootShootFamily(candidateFamily)
        ...
        {
            return actionFamily is ClientRandomMovingShootActionFamily.Shoot
                or ClientRandomMovingShootActionFamily.Shoot1
        ...
        {
            return actionFamily is ClientRandomMovingShootActionFamily.Shoot1
                or ClientRandomMovingShootActionFamily.Shoot2
                or ClientRandomMovingShootActionFamily.GunShoot;
        }
        ...
        private static bool HasBoundJumpMovementProfile(
            ...
            string movementActionName = null)
        {
        ...
        private static bool UsesFlightBuffAvatarEffectFallback(SkillData skill)
        {
            return skill?.IsBuff == true
                   && HasClientOwnedAvatarEffectFallbackBranches(skill)
        ...
        internal static SkillMovementFamily ResolveMovementFamily(SkillData skill, string movementActionName)
        {
            ...
            if (HasBoundJumpMovementProfile(skill, candidateActions, movementActionName))
            {
        ...
        private void ApplyMobStatusFromSkillCore(SkillData skill, SkillLevelData levelData, MobAI mobAI, int currentTime, int fallbackDamage = 0)
        {
            ...
            string debuffToken = ResolveInferredMobDebuffToken(skill);
        ...
        internal static string ResolveInferredMobDebuffToken(SkillData skill)
        {
            ...
            {
                return skill.AffectedSkillEffect ?? string.Empty;
            }

            return string.IsNullOrWhiteSpace(skill.AffectedSkillEffect)
                ? skill.DebuffMessageToken
                : $"{skill.DebuffMessageToken}&&{skill.AffectedSkillEffect}";
        }
            ...
            bool forceCriticalForAttack = false)
        {
            ...
            for (int i = 0; i < bulletCount; i++)
            {
                ...
                proj.BulletAnimation = RegisterBulletAnimationOwner(skill, proj, spawnTime);
        ...
        private BulletAnimationPresentation RegisterBulletAnimationOwner(SkillData skill, ActiveProjectile projectile, int startTime)
        {
            if (skill == null || projectile?.Data == null)
            {
                return null;
            }

            bool presentationFacingRight = ResolveQueuedProjectilePresentationFacing(projectile);
            string ballUol = projectile.Data.ResolveGetBallLikeUolPath(
            ...
            return new BulletAnimationPresentation
            {
                Kind = kind,
                StartTime = startTime,
            ...
            string ballUol)
        {
            ...
            {
                return BulletAnimationOwnerKind.Magic;
            }

            return BulletAnimationOwnerKind.Normal;
        }
        ...
        internal static Vector2 ResolveBulletAnimationDestinationPoint(ActiveProjectile projectile)
        {
            ...
            if (projectile.PreferredTargetPosition.HasValue)
            {
                return projectile.PreferredTargetPosition.Value;
            }

            float lifetimeSeconds = Math.Max(1f, projectile.Data?.LifeTime ?? 0f) / 1000f;
            return new Vector2(
        ...
        private SkillAnimation ResolveSkillBallAnimation(SkillData skill, ActiveProjectile projectile)
        {
            return projectile?.Data?.ResolveGetBallLikeAnimation(
                projectile.SkillLevel,
                _player?.Level ?? 1,
                skill?.MaxLevel ?? 0);
        }
        ...
        private SkillAnimation ResolveBulletEffectAnimation(string effectUol)
        {
            return string.IsNullOrWhiteSpace(effectUol)
                ? null
                : _loader?.LoadSkillAnimationByNormalizedPath(effectUol);
        }
        ...
        {
            TryRequestDamageReactiveSpecialEffectOnDamage(currentTime);
        }

        private bool TryRequestDamageReactiveSpecialEffectOnDamage(int currentTime)
        {
            for (int i = _buffs.Count - 1; i >= 0; i--)
            {
                ...
                if (buff == null
                    ...
                    || !CanRequestDamageReactiveSpecialEffect(buff))
                {
                    continue;
                }
        ...
        private bool CanRequestDamageReactiveSpecialEffect(ActiveBuff buff)
        {
            return buff?.SkillData != null
                   && IsDamageReactiveSpecialSkillUseEffectCandidate(
                       buff.SkillData,
                       HasAnimationDisplayerSpecialBranch(buff.SkillId))
                   && ShouldTriggerDamageReactiveSpecialEffect(buff.LevelData?.Prop ?? 0);
        }

        internal static bool IsDamageReactiveSpecialSkillUseEffectCandidate(SkillData skill, bool hasSpecialBranch)
        {
        ...
        internal static bool ShouldTriggerDamageReactiveSpecialEffect(int prop, int? randomRollPercent = null)
        {
            if (prop <= 0)
            {
                return false;
            ...
            int roll = randomRollPercent ?? Random.Next(100);
            return roll < prop;
        }
        ...
        private bool TryTriggerExpiredSelfDestructSummon(ActiveSummon summon, int currentTime)
        {
            ...
            else if (TryBeginExplicitSelfDestructFallbackPlayback(summon, currentTime))
            {
                summon.LastAttackTime = currentTime;
            }
        ...
        private bool ApplySummonDamage(ActiveSummon summon, int damage, int currentTime, bool allowSelfDestructFinalAction)
        {
            ...
            if (summon.SkillData?.SelfDestructMinion == true && allowSelfDestructFinalAction)
            {
                ...
                {
                    TryBeginExplicitSelfDestructFallbackPlayback(summon, currentTime);
                }
        ...
        private bool ProcessSummonHealSupport(ActiveSummon summon, int currentTime)
        {
            ...
            bool recoveredStats = false;
            if (hpHeal > 0)
            {
                int previousHp = _player.HP;
                _player.Recover(hpHeal, 0);
                int newHp = _player.HP;
                recoveredStats = newHp > previousHp;
            }
            ...
            if (mpHeal > 0)
            {
                ...
                recoveredStats |= _player.MP > previousMp;
            }

            if (!ShouldCompleteHealingSupportAttempt(summon, recoveredStats))
            {
                return false;
            }
        ...
        private static int ResolveSupportSummonSuspendDurationMs(ActiveSummon summon)
        {
            ...
            bool preferHealFirst = SummonRuntimeRules.HasMinionAbilityToken(
                summon.SkillData.MinionAbility,
                "heal");
            int suspendDuration = SummonRuntimeRules.ResolveTrackedSuspendDurationMs(
                summon.SkillData,
                summon.AssistType,
                preferHealFirst,
        ...
        private bool TryBeginExplicitSelfDestructFallbackPlayback(ActiveSummon summon, int currentTime)
        {
            if (summon?.SkillData == null
            ...
            SetSummonActorState(
                summon,
                hasPrepareAnimation ? SummonActorState.Prepare : SummonActorState.Attack,
                currentTime);
            return true;
        }
        ...
        private bool ProcessFriendlySummonBuffSupport(ActiveSummon summon, int currentTime)
        {
            ...
            if (!SummonRuntimeRules.CanAffectLocalPlayerFromFriendlySupport(summon.SkillData, _player.State))
            {
                return false;
            }
        ...
        internal static bool ShouldCompleteHealingSupportAttempt(ActiveSummon summon, bool recoveredStats)
        {
            if (recoveredStats)
            {
                return true;
            ...
            // Client `CSummoned::TryDoingHealingRobot` sends action 13 and starts suspend/reheal
            // once the seated-range probe succeeds; it does not first verify that HP/MP will change.
        ...
        private static SkillData ResolveBuffAvatarEffectSkill(SkillData skill)
        {
            ...
            if (skill.HasPersistentAvatarEffect)
            {
                return skill;
            }
        ...
        private static bool HasClientOwnedAvatarEffectFallbackBranches(SkillData skill)
        {
            return HasClientOwnedAvatarEffectBranches(skill)
                   || skill?.RepeatEffect != null
                   || skill?.RepeatSecondaryEffect != null;
        }
            ...
            int currentTime = 0)
        {
            ...
            bool preferAuthoredExplicitFamilyTrayOwner = authoredExplicitFamilyOwnerTemporaryStat != null;
            ...
            return new StatusBarBuffEntry
            {
                ...
                SortOrder = ResolveStatusBarBuffSortOrder(
                    ...
                    preferAuthoredExplicitFamilyTrayOwner),
            ...
            BuffStatType stat)
        {
            ...
            int percent = ResolveDescriptionBackedBasicStatPercent(skill, levelData, stat);
            if (percent > 0)
            {
                return stat switch
                {
        ...
        private static int ResolveDescriptionBackedBasicStatPercent(
            SkillData skill,
            SkillLevelData levelData,
            BuffStatType stat)
        {
            ...
            return stat switch
            {
                BuffStatType.Strength => ResolveDescriptionBackedBasicStatPercentAlias(
                    skill,
                    levelData,
                    "all stats",
                    "all stat",
                    ...
                    "strength",
                    "str"),
                BuffStatType.Dexterity => ResolveDescriptionBackedBasicStatPercentAlias(
                    skill,
                    levelData,
                    "all stats",
                    "all stat",
                    ...
                    "dexterity",
                    "dex"),
                BuffStatType.Intelligence => ResolveDescriptionBackedBasicStatPercentAlias(
                    skill,
                    levelData,
                    "all stats",
                    "all stat",
                    ...
                    "intelligence",
                    "int"),
                BuffStatType.Luck => ResolveDescriptionBackedBasicStatPercentAlias(
                    skill,
                    levelData,
                    "all stats",
                    "all stat",
        ...
        private static int ResolveDescriptionBackedBasicStatPercentAlias(
            SkillData skill,
            SkillLevelData levelData,
            params string[] labelTokens)
        {
            return SkillLoader.ResolveDescriptionBackedPrimaryStatPercentAlias(skill, levelData, labelTokens);
        }
        ...
        private static IReadOnlyList<BuffTemporaryStatPresentation> GetBuffTemporaryStatPresentation(ActiveBuff buff)
        {
            ...
            if (levelData != null)
            {
                ...
                TrackIfMissing(temporaryStats, levelData.SpeedMax > 0, "Speed");
            ...
            IReadOnlyList<BuffTemporaryStatPresentation> temporaryStats)
        {
            ...
            foreach (BuffTemporaryStatPresentation temporaryStat in temporaryStats)
            {
                ...
                if (ShouldSuppressCompoundFamilyTooltipDisplayName(
                        temporaryStat?.Label,
            ...
            out string[] labels)
        {
            ...
            if (AuthoredPropertyTemporaryStatLabels.TryGetValue(propertyName, out labels) && labels != null && labels.Length > 0)
            {
                return true;
            }

            return TryResolveContextualTooltipTemporaryStatLabels(
                propertyName,
                activeTemporaryStatsByLabel,
                out labels);
        }

        private static bool TryResolveContextualTooltipTemporaryStatLabels(
            string propertyName,
            IReadOnlyDictionary<string, BuffTemporaryStatPresentation> activeTemporaryStatsByLabel,
            out string[] labels)
        {
            ...
            if (string.Equals(propertyName, "x", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (activeTemporaryStatsByLabel.ContainsKey(CriticalRateBuffLabel)
                    && activeTemporaryStatsByLabel.ContainsKey(SharpEyesBuffLabel))
                {
                    labels = new[] { CriticalRateBuffLabel };
                    return true;
                }

                if (activeTemporaryStatsByLabel.ContainsKey(AllStatsBuffLabel)
                    && activeTemporaryStatsByLabel.ContainsKey(MapleWarriorBuffLabel))
                {
                    labels = new[] { AllStatsBuffLabel };
                    return true;
                }
            }
            ...
            if ((string.Equals(propertyName, "y", StringComparison.OrdinalIgnoreCase)
                    ...
                    || string.Equals(propertyName, "u", StringComparison.OrdinalIgnoreCase))
            ...
            IReadOnlyDictionary<string, BuffTemporaryStatPresentation> activeTemporaryStatsByLabel)
        {
            ...
            if (string.Equals(label, HyperBodyBuffLabel, StringComparison.OrdinalIgnoreCase))
            {
                return activeTemporaryStatsByLabel.ContainsKey(MaxHpBuffLabel)
                    || activeTemporaryStatsByLabel.ContainsKey(MaxMpBuffLabel);
            }

            if (string.Equals(label, SharpEyesBuffLabel, StringComparison.OrdinalIgnoreCase))
            {
                return activeTemporaryStatsByLabel.ContainsKey(CriticalRateBuffLabel)
                    || activeTemporaryStatsByLabel.ContainsKey(CriticalDamageBuffLabel);
            }

            if (string.Equals(label, MapleWarriorBuffLabel, StringComparison.OrdinalIgnoreCase))
            {
                return activeTemporaryStatsByLabel.ContainsKey(AllStatsBuffLabel)
        ...
        private static ISet<string> BuildDirectBuffIconEligibleLabelSet(SkillLevelData levelData)
        {
            ...
            Track(
                ShouldUseDirectBuffIconFamily(
                    levelData,
                    levelData.Speed > 0 || levelData.SpeedPercent > 0 || levelData.SpeedMax > 0,
                    "speed", "speedMax", "psdSpeed", "indieSpeed"),
                "Speed");
            Track(
                ShouldUseDirectBuffIconFamily(
                    levelData,
        ...
        private static bool IsDirectBuffIconFamilyProperty(string propertyName)
        {
            return string.Equals(propertyName, "pad", StringComparison.OrdinalIgnoreCase)
                ...
                || string.Equals(propertyName, "speedMax", StringComparison.OrdinalIgnoreCase)
        ...
        private static void AppendPersistentAvatarEffectSignature(ref HashCode hash, SkillAnimation animation)
        {
            ...
            for (int i = 0; i < animation.Frames.Count; i++)
            {
                ...
                hash.Add(frame.ZoomStart);
                hash.Add(frame.ZoomEnd);
        ...
        {
            if (skill?.UsesHelperOnlyAffectedSkillPassiveData == true
            ...
            out int level)
        {
            ...
            if (skill?.UsesHelperOnlyAffectedSkillPassiveData != true || getSkillLevel == null)
            {
                return false;
            }
            ...
            int mapShiftX, int mapShiftY, int centerX, int centerY, int currentTime)
        {
            ...
            {
                anim = ResolveProjectilePresentationAnimation(proj);
        ...
        private SkillAnimation ResolveProjectilePresentationAnimation(ActiveProjectile projectile)
        {
            ...
            SkillAnimation resolvedAmmoVisual = ResolveProjectileVisualAnimation(projectile);
            if (resolvedAmmoVisual?.Frames?.Count > 0)
            {
                return resolvedAmmoVisual;
            }
            ...
            int currentTime)
        {
            ...
            foreach (ProjectileAfterimageLayer layer in afterimageLayers)
            {
                ...
                int screenX = (int)layer.Position.X - mapShiftX + centerX;
                int screenY = (int)layer.Position.Y - mapShiftY + centerY;
                layer.Frame.Texture.DrawBackground(
                    spriteBatch,
                    null,
                    null,
                    GetFrameDrawX(screenX, layer.Frame, layer.Flip),
                    screenY - layer.Frame.Origin.Y,
                    Color.White * layer.ResolveAlpha(currentTime),
                    layer.Flip,
                    null);
            }
        }
        ...
        {
            return skill?.UsesHelperOnlyAffectedSkillPassiveData == true
        ...
        public void Clear()
        {
            ...
            _currentCast = null;
            _skillLevels.Clear();
            bool hotkeysChanged = _skillHotkeys.Count > 0 || _macroHotkeys.Count > 0 || _itemHotkeys.Count > 0;
            _skillHotkeys.Clear();
            _macroHotkeys.Clear();
            _itemHotkeys.Clear();
            NotifyHotkeysChanged(hotkeysChanged);
