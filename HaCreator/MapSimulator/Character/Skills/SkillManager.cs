...
namespace HaCreator.MapSimulator.Character.Skills
{
    ...
    public class SkillManager
    {
        ...
        private string GetShootAttackRestrictionMessage(SkillData skill, SkillLevelData levelData = null)
        {
            ...
            return HasCompatibleShootAmmo(skillId, weaponCode, requiredAmmoCount)
                ? null
        ...
        private bool TryConsumeShootAmmo(SkillData skill, SkillLevelData levelData)
        {
            ...
            int weaponCode = GetEquippedWeaponCode();
            int requiredAmmoCount = ResolveRequiredShootAmmoCount(levelData);
            if (!TryResolveCompatibleShootAmmoSlot(skillId, weaponCode, requiredAmmoCount, out int slotIndex, out int itemId))
            {
                return false;
            }
        ...
        private bool HasCompatibleShootAmmo(int skillId, int weaponCode, int requiredAmmoCount)
        {
            if (_inventoryRuntime == null)
            {
                return true;
            ...
            return TryResolveCompatibleShootAmmoSlot(skillId, weaponCode, requiredAmmoCount, out _, out _);
        }
            ...
            out int itemId)
        {
            ...
            int preferredSlotIndex = FindCompatibleShootAmmoSlot(
                ...
                weaponCode,
                requiredAmmoCount,
                slot => slot?.IsActiveBullet == true && IsSkillCompatibleWithAmmoItem(skillId, slot.ItemId));
            ...
            slotIndex = FindCompatibleShootAmmoSlot(
                ...
                weaponCode,
                requiredAmmoCount,
                slot => IsSkillCompatibleWithAmmoItem(skillId, slot.ItemId));
            if (slotIndex < 0)
            {
                return false;
            }
        ...
        private static bool IsSkillCompatibleWithAmmoItem(int skillId, int itemId)
        {
            // Client `GetProperBulletPosition` includes a few skill-owned ammo one-offs (for example fire/ice pellets)
            // before falling back to the generic weapon-family scan. The simulator currently keeps the generic lane.
            return itemId > 0;
        }
            ...
            int shootRange0Override = 0)
        {
            ...
            {
                ProcessMagicAttack(skill, level, currentTime, facingRight, queueFollowUps, preferredTargetMobId, preferredTargetPositionOverride, attackOriginOverride);
                return;
            }

            if (skill.AttackType == SkillAttackType.Ranged)
            {
                ProcessNoProjectileRangedAttack(skill, level, currentTime, facingRight, queueFollowUps, preferredTargetMobId, preferredTargetPositionOverride, attackOriginOverride, shootRange0Override);
                return;
            }

            ProcessMeleeAttack(skill, level, currentTime, facingRight, queueFollowUps, preferredTargetMobId, preferredTargetPositionOverride, attackOriginOverride);
        }
        ...
        private int? ResolveDeferredPreferredTargetMobIdForMode(
            ...
            int shootRange0Override = 0)
        {
            Rectangle worldHitbox = GetWorldAttackHitbox(skill, level, levelData, mode, facingRight, attackOriginOverride, shootRange0Override);
            if (worldHitbox.Width <= 0 || worldHitbox.Height <= 0)
            {
                return null;
            }
        ...
        private static int ResolvePreparedSkillHudGaugeDuration(SkillData skill, int prepareDurationMs, int explicitGaugeDurationMs)
        {
            ...
            if (UsesReleaseTriggeredKeydownExecution(skill) && prepareDurationMs > 0)
            {
                return prepareDurationMs;
            }
        ...
        private static bool PreservesPrepareActionDuringHold(SkillData skill)
        {
            return skill?.SkillId == 4341002
                   ...
                   || skill?.SkillId == 22121000
                   || skill?.SkillId == 22151001;
        }
        ...
        private void ProcessMeleeAttack(
            ...
            Vector2? attackOriginOverride = null)
        {
            ProcessDirectionalAttack(skill, level, currentTime, AttackResolutionMode.Melee, facingRight, queueFollowUps, preferredTargetMobId, preferredTargetPosition, attackOriginOverride);
        }
        ...
        private void ProcessMagicAttack(
            ...
            Vector2? attackOriginOverride = null)
        {
            ProcessDirectionalAttack(skill, level, currentTime, AttackResolutionMode.Magic, facingRight, queueFollowUps, preferredTargetMobId, preferredTargetPosition, attackOriginOverride);
        }
        ...
        private void ProcessRangedAttack(
            ...
            int shootRange0Override = 0)
        {
            ProcessDirectionalAttack(skill, level, currentTime, AttackResolutionMode.Ranged, facingRight, queueFollowUps, preferredTargetMobId, preferredTargetPosition, attackOriginOverride, shootRange0Override);
        }
            ...
            int shootRange0Override = 0)
        {
            ...
            {
                ProcessRangedAttack(skill, level, currentTime, facingRight, queueFollowUps, preferredTargetMobId, preferredTargetPosition, attackOriginOverride, shootRange0Override);
                return;
            }
        ...
        private void ProcessDirectionalAttack(
            ...
            int shootRange0Override = 0)
        {
            TryProcessDirectionalAttack(skill, level, currentTime, mode, facingRight, queueFollowUps, preferredTargetMobId, preferredTargetPosition, attackOriginOverride, shootRange0Override);
        }
            ...
            int shootRange0Override = 0)
        {
            ...
            Rectangle worldHitbox = GetWorldAttackHitbox(skill, level, levelData, mode, facingRight, attackOrigin, shootRange0Override);
            ...
            List<MobItem> targets = ResolveTargetsInHitbox(worldHitbox, currentTime, maxTargets, mode, facingRight, preferredTargetMobId, preferredTargetPosition, attackOrigin);
            if (targets.Count == 0)
                return false;
        ...
        private Rectangle GetWorldAttackHitbox(SkillData skill, int level, SkillLevelData levelData, AttackResolutionMode mode, bool facingRight, Vector2? attackOriginOverride = null, int shootRange0Override = 0)
        {
        ...
        private void ApplyMobStatusFromSkillCore(SkillData skill, SkillLevelData levelData, MobAI mobAI, int currentTime, int fallbackDamage = 0)
        {
            ...
            ApplyExplicitMobDebuffMetadata(skill, levelData, mobAI, searchText, debuffToken, durationMs, currentTime, propPercent);
            ...
            int propPercent)
        {
                ...
                && IsMobDebuffSkill(skill, debuffToken, searchText))
            {
                ...
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.Speed, durationMs, currentTime, speedPercent, tickIntervalMs: 1000, propPercent);
            }
            ...
            Vector2? preferredTargetPositionOverride = null)
        {
            ...
            for (int i = 0; i < bulletCount; i++)
            {
                ...
                int? projectilePreferredTargetMobId = i < projectileTargetAssignments.Count
                    ...
                    : preferredTargetMobId;
                Vector2? projectilePreferredTargetPosition = ResolveDeferredPreferredTargetPosition(projectilePreferredTargetMobId, currentTime)
                    ?? preferredTargetPositionOverride;
                var proj = new ActiveProjectile
                {
                    Id = _nextProjectileId++,
            ...
            int currentTime)
        {
            ...
            for (int i = 0; i < teslaCoils.Count; i++)
            {
                ...
                int impactDelayMs = targetIndex >= 0 && targetImpactDelaysMs != null && targetIndex < targetImpactDelaysMs.Count
                    ...
                    : Math.Max(60, ResolveTeslaTargetImpactDelayMs(teslaCoil, target, currentTime));
                SpawnSummonProjectileVisual(
                    teslaCoil.SkillId,
                    teslaCoil.Level,
                    teslaCoil.LevelData,
        ...
        private bool TryProcessBeholderHealSupport(ActiveSummon summon, int currentTime)
        {
            ...
            summon.LastAttackAnimationStartTime = currentTime;
            summon.CurrentAnimationBranchName = "skill1";
            SetSummonActorState(summon, SummonActorState.Attack, currentTime);
            return true;
        }
        ...
        private bool TryProcessBeholderBuffSupport(ActiveSummon summon, int currentTime)
        {
            ...
            BeholderBuffCandidate? selectedBuff = SelectBeholderBuffCandidate(buffLevelData);
        ...
        private bool ProcessSummonHealSupport(ActiveSummon summon, int currentTime)
        {
            ...
            summon.CurrentAnimationBranchName = ResolveSupplementalSummonAnimationBranch(summon.SkillData, "heal", "support");
            if (summon.SkillData.HitEffect != null)
            {
                SpawnHitEffect(
        ...
        private bool TryProcessTeslaCoilAttack(ActiveSummon summon, int currentTime)
        {
            ...
            foreach (ActiveSummon teslaCoil in teslaCoils)
            {
                ...
                teslaCoil.CurrentAnimationBranchName = null;
        ...
        private bool ProcessSummonMobSupport(ActiveSummon summon, int currentTime)
        {
            ...
            summon.CurrentAnimationBranchName = ResolveSupplementalSummonAnimationBranch(summon.SkillData, "support", "heal");
            summon.LastAttackAnimationStartTime = currentTime;
            SetSummonActorState(summon, SummonActorState.Attack, currentTime);
            if (summon.SkillData.HitEffect != null)
            {
        ...
        private bool ProcessSummonAction(ActiveSummon summon, int currentTime)
        {
            ...
            summon.CurrentAnimationBranchName = ResolveSupplementalSummonAnimationBranch(summon.SkillData, "subsummon");
            summon.LastAttackAnimationStartTime = currentTime;
            SetSummonActorState(summon, SummonActorState.Attack, currentTime);
            if (summon.SkillData.HitEffect != null)
            {
        ...
        private BeholderBuffCandidate? SelectBeholderBuffCandidate(SkillLevelData buffLevelData)
        {
        ...
        private int GetMineMovementDirection()
        {
            ...
            int inputDirection = _player.HorizontalInputDirection;
            if (inputDirection == 0)
            {
                return 0;
            }
        ...
        public bool ApplyOrRefreshExternalAreaSupportBuff(int areaObjectId, SkillData sourceSkill, SkillLevelData levelData, int currentTime, int durationMs)
        {
            SkillLevelData projectedLevelData = RemoteAffectedAreaSupportResolver.CreateProjectedSupportBuffLevelData(levelData);
            ...
            if (existingBuff != null)
            {
                ...
                existingBuff.SkillData = BuildExternalAreaSupportBuffSkillData(sourceSkill, syntheticBuffId);
            ...
            var buff = new ActiveBuff
            {
                ...
                SkillData = BuildExternalAreaSupportBuffSkillData(sourceSkill, syntheticBuffId),
        ...
        public IReadOnlyList<StatusBarBuffEntry> GetStatusBarBuffEntries(int currentTime)
        {
            ...
            foreach (var buff in _buffs)
            {
                ...
                entries.Add(new StatusBarBuffEntry
                {
                    ...
                    SortOrder = ResolveStatusBarBuffSortOrder(primaryTemporaryStat, supplementalBuffIconEntry),
            ...
            BuffTemporaryStatPresentation primaryTemporaryStat)
        {
            ...
            return TryGetBuffIconCatalogEntry(GenericBuffIconKey, out BuffIconCatalogEntry genericEntry)
                ? genericEntry
                : null;
        }
        ...
        private static SkillData BuildExternalAreaSupportBuffSkillData(SkillData sourceSkill, int syntheticBuffId)
        {
            return new SkillData
            {
                SkillId = syntheticBuffId,
                Name = sourceSkill?.Name ?? $"Remote support area {Math.Abs(syntheticBuffId)}",
                Description = sourceSkill?.Description ?? string.Empty,
        ...
        private static SkillAnimation ResolveSummonAnimation(ActiveSummon summon, int currentTime, int elapsedTime, out int animationTime)
        {
            ...
            if (summon?.CurrentAnimationBranchName != null
            ...
            if (attackAnimation?.Frames.Count > 0 && summon != null)
            {
                ...
                if (attackElapsed >= 0 && attackSequenceDuration > 0 && attackElapsed < attackSequenceDuration)
                {
                    ...
                    animationTime = branchAnimation == null
