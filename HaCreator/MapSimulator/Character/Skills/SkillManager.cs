...
namespace HaCreator.MapSimulator.Character.Skills
{
    ...
    public class SkillManager
    {
        ...
        private void ClearPreparedSkillForForcedLocalRelease(int currentTime)
        {
            ...
            ClearSkillMount(_preparedSkill.SkillId);
            _preparedSkill = null;
        }
        ...
        private void RefreshQueuedProjectileFireTimeOrigin(ActiveProjectile projectile, int currentTime)
        {
            ...
            Vector2 refreshedOrigin = ResolveCurrentShootAttackOrigin(currentTime, projectile.FacingRight);
            ApplyQueuedProjectileFireTimeOrigin(projectile, refreshedOrigin);
        ...
        {
            return projectile?.IsQueuedFinalAttack == true
                   && skill?.IsAttack == true
                   && skill.AttackType == SkillAttackType.Ranged;
        }

        internal static void ApplyQueuedProjectileFireTimeOrigin(ActiveProjectile projectile, Vector2 refreshedOrigin)
        {
            if (projectile == null)
            {
                return;
        ...
        private void ProcessMeleeAttack(
            ...
            int? currentRawActionCodeOverride = null)
        {
            ProcessDirectionalAttack(skill, level, currentTime, AttackResolutionMode.Melee, facingRight, queueFollowUps, preferredTargetMobId, preferredTargetPosition, attackOriginOverride, 0, false, currentActionNameOverride, currentRawActionCodeOverride);
        }
        ...
        private void ProcessDirectionalAttack(
            ...
            int? currentRawActionCodeOverride = null)
        {
            TryProcessDirectionalAttack(skill, level, currentTime, mode, facingRight, queueFollowUps, preferredTargetMobId, preferredTargetPosition, attackOriginOverride, shootRange0Override, forceCriticalForAttack, currentActionNameOverride, currentRawActionCodeOverride);
        }
            ...
            int? currentRawActionCodeOverride = null)
        {
            ...
            if (hitbox.Width <= 0 || hitbox.Height <= 0)
            {
                if (mode == AttackResolutionMode.Melee
                    && TryGetMeleeAfterImageHitbox(skill, facingRight, out Rectangle afterImageHitbox, currentActionNameOverride, currentRawActionCodeOverride))
                {
        ...
        private void ApplyMobStatusFromSkillCore(SkillData skill, SkillLevelData levelData, MobAI mobAI, int currentTime, int fallbackDamage = 0)
        {
            ...
            if (MatchesMobStatusKeywords(searchText, "poison", "venom") || skill.Element == SkillElement.Poison)
            {
                ...
                TryApplyInferredMobStatus(mobAI, poisonEffect, durationMs, currentTime, dotValue, tickIntervalMs: 1000, allowStatusPackage);
            }
            ...
            if (MatchesMobStatusKeywords(searchText, "burn", "flame"))
            {
                ...
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.Burned, durationMs, currentTime, burnValue, tickIntervalMs: 1000, allowStatusPackage);
            }
        ...
        private static Vector2 ResolveSummonImpactDisplayPosition(
            SkillData skill,
            int targetOrder,
            MobItem mob,
            Vector2 summonCenter)
            ...
            int targetOrder)
        {
            ...
            for (int i = 0; i < attackCount; i++)
            {
                ...
                ShowSkillDamageNumber(mob, damage, isCritical, currentTime, i);
                Vector2 impactPosition = ResolveSummonImpactDisplayPosition(skill, targetOrder, mob, summonCenter);
                SpawnHitEffect(
                    skill.SkillId,
        ...
        private bool CanAutoDeployMine()
        {
            ...
            bool jaguarRideOwnershipActive = HasActiveClientOwnedVehicleTrackingOwner(WildHunterJaguarRiderSkillId)
                || (_activeSkillMount != null && IsWildHunterJaguarTamingMobItemId(_activeSkillMount.MountItemId))
                || (IsClientVehicleOwnershipGraceWindowActive(currentTime)
            ...
            return CanAutoDeployWildHunterMineWithoutVisibleMountedAction(
                ...
                ResolveWildHunterJaguarMountItemId());
        }
        ...
        public IReadOnlyList<StatusBarBuffEntry> GetStatusBarBuffEntries(int currentTime)
        {
            ...
            foreach (var buff in _buffs)
            {
                ...
                entries.Add(new StatusBarBuffEntry
                {
                    ...
                    SortOrder = ResolveStatusBarBuffSortOrder(temporaryStats, primaryTemporaryStat, supplementalBuffIconEntry),
