...
namespace HaCreator.MapSimulator.Character.Skills
{
    ...
    public class SkillManager
    {
        ...
        {
            public ClientSkillTimerExpiration(int skillId, string source, int expireTime)
            {
                SkillId = skillId;
                Source = source;
        ...
        private const int WildHunterSwallowHoldStunMs = 30000;
        ...
        private static readonly Dictionary<string, BuffTemporaryStatPresentation> TemporaryStatPresentationCatalog =
            new(StringComparer.OrdinalIgnoreCase)
            {
                ...
                // then inferred non-BuffIcon families after the WZ-backed catalog.
                ...
                [MaxHpBuffLabel] = CreateTemporaryStatPresentation(MaxHpBuffLabel, "Max HP", null, 100, 70),
                [MaxMpBuffLabel] = CreateTemporaryStatPresentation(MaxMpBuffLabel, "Max MP", null, 110, 71),
                [CriticalRateBuffLabel] = CreateTemporaryStatPresentation(CriticalRateBuffLabel, "Critical Rate", null, 120, 80),
                [BoosterBuffLabel] = CreateTemporaryStatPresentation(BoosterBuffLabel, "Booster", null, 130, 25),
                [StanceBuffLabel] = CreateTemporaryStatPresentation(StanceBuffLabel, "Stance", null, 140, 35),
                [InvincibleBuffLabel] = CreateTemporaryStatPresentation(InvincibleBuffLabel, "Invincible", null, 150, 10),
                [DamageReductionBuffLabel] = CreateTemporaryStatPresentation(DamageReductionBuffLabel, "Damage Reduction", null, 160, 20),
                [TransformBuffLabel] = CreateTemporaryStatPresentation(TransformBuffLabel, "Transform", null, 170, 15),
                ...
                [AuraBuffLabel] = CreateTemporaryStatPresentation(AuraBuffLabel, "Aura", null, 240, 18),
                [ShadowPartnerBuffLabel] = CreateTemporaryStatPresentation(ShadowPartnerBuffLabel, "Shadow Partner", null, 250, 28),
                [DebuffResistanceBuffLabel] = CreateTemporaryStatPresentation(DebuffResistanceBuffLabel, "Debuff Resistance", null, 260, 40),
                [RecoveryBuffLabel] = CreateTemporaryStatPresentation(RecoveryBuffLabel, "Recovery", null, 270, 85),
                [AllStatsBuffLabel] = CreateTemporaryStatPresentation(AllStatsBuffLabel, "All Stats", null, 280, 60),
                [HyperBodyBuffLabel] = CreateTemporaryStatPresentation(HyperBodyBuffLabel, "Hyper Body", null, 290, 50),
                [SharpEyesBuffLabel] = CreateTemporaryStatPresentation(SharpEyesBuffLabel, "Sharp Eyes", null, 300, 45),
                [SoulArrowBuffLabel] = CreateTemporaryStatPresentation(SoulArrowBuffLabel, "Soul Arrow", null, 310, 55),
                [DarkSightBuffLabel] = CreateTemporaryStatPresentation(DarkSightBuffLabel, "Dark Sight", null, 320, 12),
                [MagicGuardBuffLabel] = CreateTemporaryStatPresentation(MagicGuardBuffLabel, "Magic Guard", null, 330, 22),
                [BlessBuffLabel] = CreateTemporaryStatPresentation(BlessBuffLabel, "Bless", null, 340, 88),
                [MapleWarriorBuffLabel] = CreateTemporaryStatPresentation(MapleWarriorBuffLabel, "Maple Warrior", null, 350, 58),
                [DashBuffLabel] = CreateTemporaryStatPresentation(DashBuffLabel, "Dash", null, 360, 75)
            };

        private static readonly (string Label, string[] Fragments)[] TemporaryStatTextAliases =
        {
            ("PAD", new[] { "physical attack", "weapon attack", "attack power", "w.att", "w. att", "watk", "w atk", "pad:" }),
            ("PDD", new[] { "physical defense", "weapon defense", "weapon def", "w.def", "w. def", "wdef", "w def", "pdd:" }),
            ("MAD", new[] { "magic attack", "spell attack", "m.att", "m. att", "matk", "m atk", "mad:" }),
            ("MDD", new[] { "magic defense", "spell defense", "m.def", "m. def", "mdef", "m def", "mdd:" }),
            ("ACC", new[] { "accuracy", "acc", "acc:" }),
            ("EVA", new[] { "avoidability", "avoid", "eva", "eva:" }),
            (CraftBuffLabel, new[] { "craft", "incraft" }),
            ("Jump", new[] { "jump" }),
            ("Speed", new[] { "speed", "haste" }),
            (MaxHpBuffLabel, new[] { "max hp", "maximum hp", "maxhp" }),
            (MaxMpBuffLabel, new[] { "max mp", "maximum mp", "maxmp" }),
            ...
            (InvincibleBuffLabel, new[] { "invincible", "invincib", "immune to damage", "cannot be hit" }),
            (DamageReductionBuffLabel, new[] { "damage reduction", "reduce damage", "guard", "barrier", "shield", "meso guard", "combo barrier", "power guard", "magic guard", "achilles" }),
            ...
            (DropRateBuffLabel, new[] { "item drop", "drop rate", "drop +" }),
            (MesoRateBuffLabel, new[] { "meso up", "mesos obtained", "meso obtained", "meso +" }),
            (CriticalDamageBuffLabel, new[] { "critical damage", "crit damage", "crit dmg" }),
            ...
            (DebuffResistanceBuffLabel, new[] { "holy shield", "abnormal status", "status ailment", "resistance to status", "debuff immunity", "status immunity" }),
            (RecoveryBuffLabel, new[] { "hp recovery", "mp recovery", "recover hp", "recover mp", "regeneration", "regen", "heals hp", "heals mp" }),
            (AllStatsBuffLabel, new[] { "all stats", "all stat", "all players' stats", "all player's stats" }),
        ...
        public Action<int, int> OnClientSkillCancelRequested;
        public Action<int, int> OnClientSkillEffectRequested;
        public Action<int, int> OnRepeatSkillModeEndRequested;
        ...
        {
            return TryCastSkill(skillId, currentTime, ownerHotkeySlot: -1);
        }

        private bool TryCastSkill(int skillId, int currentTime, int ownerHotkeySlot)
        {
            ...
            {
                if (!CanInputSourceControlPreparedSkill(_preparedSkill, ownerHotkeySlot))
                {
                    return false;
                }
            ...
            // Start casting
            StartCast(skill, level, currentTime, ownerHotkeySlot);
            return true;
        }
        ...
        public bool TryCastHotkey(int keyIndex, int currentTime)
        {
            ...
            return TryCastSkill(skillId, currentTime, keyIndex);
        }
        ...
        public void ReleaseHotkeyIfActive(int keyIndex, int currentTime)
        {
            ...
            if (skillId > 0
                ...
                && _preparedSkill.OwnerHotkeySlot == keyIndex)
            {
                ReleasePreparedSkill(currentTime);
            }
        }
        ...
        public bool RequestClientSkillCancel(int skillId, int currentTime)
        {
            if (skillId <= 0)
            {
                return false;
            ...
            int cancelSkillId = ResolveClientCancelRequestSkillId(skillId);
            bool stateChanged = CancelClientOwnedSkillState(cancelSkillId, currentTime);
            bool sideEffectsApplied = ApplyClientSkillCancelSideEffects(cancelSkillId, currentTime);
            if (!stateChanged && !sideEffectsApplied)
            {
                return false;
            }

            OnClientSkillCancelRequested?.Invoke(cancelSkillId, skillId);
            return true;
        }
        ...
        private void StartCast(SkillData skill, int level, int currentTime, int ownerHotkeySlot = -1)
        {
            var levelData = skill.GetLevel(level);
            if (levelData == null)
                return;
            ...
            string prepareActionName = usesPreparedKeydownFlow ? GetPrepareActionName(skill) : null;
            ...
            if (usesPreparedKeydownFlow)
            {
                BeginPreparedSkill(skill, level, currentTime, ownerHotkeySlot);
                return;
            }
        ...
        private void UpdateMeleeAfterImageState(SkillData skill, string actionName, int currentTime)
        {
            ...
            if (_loader.TryResolveMeleeAfterImageAction(skill, weapon, actionName, _player.Level, masteryPercent, out MeleeAfterImageAction afterImageAction))
            {
        ...
        public void UpdateBasicMeleeAfterImageState(string actionName, int currentTime)
        {
            WeaponPart weapon = _player.Build?.GetWeapon();
            int masteryPercent = GetMastery(weapon);
            if (_loader.TryResolveMeleeAfterImageAction(null, weapon, actionName, _player.Level, masteryPercent, out MeleeAfterImageAction afterImageAction))
            {
        ...
        private bool ApplyClientSkillCancelSideEffects(int skillId, int currentTime)
        {
            bool sideEffectsApplied = false;
            sideEffectsApplied |= RecordVehicleValidCancel(skillId, currentTime);
            sideEffectsApplied |= CleanupKeydownPrepareAnimationOnCancel(skillId);
            return sideEffectsApplied;
        }
        ...
        private bool CleanupKeydownPrepareAnimationOnCancel(int skillId)
        {
            ...
            if (_currentCast != null
                && (_currentCast.SkillId == skillId
                    || ResolveClientCancelRequestSkillId(_currentCast.SkillId) == skillId))
            {
                _currentCast = null;
                clearedCurrentCast = true;
            }

            return clearedCurrentCast || _preparedSkill?.SkillId == skillId;
        }
        ...
        private bool CancelClientOwnedSkillState(int requestedSkillId, int currentTime)
        {
            ...
            if (IsSwallowCancelSkill(requestedSkillId) && _swallowState != null)
            {
                ClearSwallowState();
                stateChanged = true;
            }
        ...
        {
            int removedCount = _skillZones.RemoveAll(zone => DoesClientCancelMatchSkillId(zone?.SkillId ?? 0, requestedSkillId));
        ...
        private bool DoesPreparedSkillMatchClientCancelRequest(PreparedSkill prepared, int requestedSkillId)
        {
            ...
            return prepared.SkillId == requestedSkillId
                   || ResolveClientCancelRequestSkillId(prepared.SkillId) == requestedSkillId;
        }
        ...
        private bool DoesClientCancelMatchSkillId(int activeSkillId, int requestedSkillId)
        {
            ...
            return activeSkillId == requestedSkillId
                   || ResolveClientCancelRequestSkillId(activeSkillId) == requestedSkillId;
        }

        private int ResolveClientCancelRequestSkillId(int skillId)
        {
            SkillData skill = GetSkillData(skillId);
        ...
        private static bool IsSwallowCancelSkill(int skillId)
        {
            return skillId == WildHunterSwallowSkillId;
        }
        ...
        private void BeginPendingTankSiegeModeEndRequest(RepeatSkillSustainState sustain, int currentTime, bool emitTimeoutEffect)
        {
            ...
            OnRepeatSkillModeEndRequested?.Invoke(sustain.SkillId, sustain.ReturnSkillId);
            ...
            Vector2? attackOriginOverride = null)
        {
            ...
            {
                SpawnProjectile(skill, level, currentTime, facingRight, queueFollowUps, preferredTargetMobId, attackOriginOverride);
                return;
            }

            if (skill.AttackType == SkillAttackType.Magic)
            {
                ProcessMagicAttack(skill, level, currentTime, facingRight, queueFollowUps, preferredTargetMobId, attackOriginOverride);
                return;
            }

            if (skill.AttackType == SkillAttackType.Ranged)
            {
                ProcessRangedAttack(skill, level, currentTime, facingRight, queueFollowUps, preferredTargetMobId, attackOriginOverride);
                return;
            }

            ProcessMeleeAttack(skill, level, currentTime, facingRight, queueFollowUps, preferredTargetMobId, attackOriginOverride);
        }
        ...
        private bool TryExecuteWildHunterSwallowDigest(SkillData skill, int currentTime)
        {
            ...
            _swallowState.DigestCompleteTime = currentTime + SwallowDigestDurationMs;
            _swallowState.NextWriggleTime = currentTime + SwallowWriggleIntervalMs;
        ...
        private bool TryExecuteWildHunterSwallowAttack(SkillData skill, int level, SkillLevelData levelData, int currentTime)
        {
            ...
            HandleMobDeath(target, currentTime, MobDeathType.Swallowed);
            ClearSwallowState();
            ProcessRangedAttack(skill, level, currentTime, _player.FacingRight, queueFollowUps: false);
            return true;
        }
            ...
            int? preferredTargetMobId)
        {
            ...
            Vector2 attackOrigin = ResolveDeferredMovingShootOrigin(currentTime);
            preferredTargetMobId ??= ResolveDeferredPreferredTargetMobId(skill, level, currentTime, attackOrigin);
        ...
        private Vector2 ResolveDeferredMovingShootOrigin(int currentTime)
        {
            Point? shootPoint = _player?.TryGetCurrentBodyMapPoint("handMove", currentTime)
            ...
            Vector2? attackOriginOverride = null)
        {
            ...
            Rectangle worldHitbox = GetWorldAttackHitbox(skill, level, levelData, mode, _player.FacingRight, attackOriginOverride);
            if (worldHitbox.Width <= 0 || worldHitbox.Height <= 0)
            {
                return null;
            }

            return ResolveTargetsInHitbox(worldHitbox, currentTime, 1, mode, _player.FacingRight, null, attackOriginOverride)
                .FirstOrDefault()
                ?.PoolId;
        }
        ...
        private void BeginPreparedSkill(SkillData skill, int level, int currentTime, int ownerHotkeySlot = -1)
        {
            var levelData = skill.GetLevel(level);
            ...
            _preparedSkill = new PreparedSkill
            {
                ...
                SkillData = skill,
                LevelData = levelData,
                IsKeydownSkill = skill.IsKeydownSkill || usesReleaseTriggeredKeydown
            };
        ...
        private static bool CanInputSourceControlPreparedSkill(PreparedSkill prepared, int ownerHotkeySlot)
        {
            if (prepared == null)
            {
                return false;
            ...
            return ownerHotkeySlot == activeOwnerHotkeySlot;
        }
        ...
        private int GetPrepareDuration(SkillData skill, SkillLevelData levelData)
        {
            ...
            {
                int clientGaugeDurationMs = ResolveKeyDownHudProfile(skill?.SkillId ?? 0).GaugeDurationMs;
                if (clientGaugeDurationMs > 0)
                    return clientGaugeDurationMs;
            }
        ...
        private static KeyDownHudProfile ResolveKeyDownHudProfile(int skillId)
        {
            return skillId switch
            {
                ...
                3121004 => new KeyDownHudProfile(true, "KeyDownBar", 2000),
                3221001 => new KeyDownHudProfile(true, "KeyDownBar", 900),
                4341003 => new KeyDownHudProfile(true, "KeyDownBar", 1200),
                5201002 => new KeyDownHudProfile(true, "KeyDownBar", 1000),
                13111002 => new KeyDownHudProfile(true, "KeyDownBar", 1000),
                22121000 => new KeyDownHudProfile(true, "KeyDownBar3", 500, PreparedSkillHudSurface.World, showText: false),
                22151001 => new KeyDownHudProfile(true, "KeyDownBar2", 500, PreparedSkillHudSurface.World, showText: false),
                33101005 => new KeyDownHudProfile(true, "KeyDownBar", 900),
                33121009 => new KeyDownHudProfile(true, "KeyDownBar", 2000),
                35001001 => new KeyDownHudProfile(true, "KeyDownBar", 2000),
                35101009 => new KeyDownHudProfile(true, "KeyDownBar", 2000),
        ...
        {
            return skill?.SkillId == WildHunterSwallowSkillId;
        }

        private static bool PreservesPrepareActionDuringHold(SkillData skill)
        {
            return skill?.SkillId == 4341002
                   || skill?.SkillId == 4341003;
        }

        private readonly struct KeyDownHudProfile
        {
            public KeyDownHudProfile(bool visible, string skinKey, int gaugeDurationMs, PreparedSkillHudSurface surface = PreparedSkillHudSurface.StatusBar, bool showText = true)
            {
                Visible = visible;
        ...
        private bool TryApplyClientOwnedAvatarEffect(SkillData skill, int currentTime)
        {
            ...
            {
                return _player.ApplyTransientUnderFaceSkillAvatarEffect(skill.SkillId, skill.Effect, skill.EffectSecondary, currentTime);
            }
        ...
        private static bool UsesTransientMovementAvatarEffect(SkillData skill)
        {
            ...
            return movementFamily == SkillMovementFamily.Rush;
        }
        ...
        {
            if (skill?.CasterMove == true && !string.IsNullOrWhiteSpace(skill.PrepareActionName))
            {
                return skill.PrepareActionName;
            }
        ...
        private static SkillMovementFamily ResolveMovementFamily(SkillData skill, string movementActionName)
        {
            ...
            if (ActionTextContains(movementActionName, "teleport") || SkillTextContains(skill, "teleport"))
            {
                return SkillMovementFamily.Teleport;
            }

            if (ActionTextContains(movementActionName, "backstep"))
            {
                return SkillMovementFamily.Backstep;
            }

            if (skill.ClientInfoType == 40
                || ActionTextContains(movementActionName, "doublejump")
                || ActionTextContains(movementActionName, "flash jump")
                || ActionTextContains(movementActionName, "archerdoublejump")
                || ActionTextContains(movementActionName, "backspin")
                || ActionTextContains(movementActionName, "assaulter")
                || ActionTextContains(movementActionName, "screw")
            ...
            if (ActionTextContains(movementActionName, "fly")
            ...
            if (ActionTextContains(movementActionName, "jump")
            ...
            return !string.IsNullOrWhiteSpace(movementActionName)
            ...
            int horizontalRange)
        {
            ...
            float currentVerticalVelocity = (float)(_player.Physics?.VelocityY ?? 0d);
            _player.Physics.SetVelocity(
                horizontalSpeed * moveDirection,
                Math.Min(currentVerticalVelocity, -upwardSpeed));
        }
        ...
        private SkillData FindTeleportMasterySkill(int teleportSkillId)
        {
            ...
            {
                if (skill?.UsesAffectedSkillBodyAttack != true || skill.AffectedSkillId != teleportSkillId)
                {
                    continue;
                }
            ...
            string startupActionName = null)
        {
            ...
            _player.ApplySkillAvatarTransform(skill.SkillId, usesTankStartup ? "tank_rbooster_pre" : "rbooster");
            _player.Physics.Jump();
        ...
        private void ProcessDeferredSkillPayloads(int currentTime)
        {
            while (_deferredSkillPayloads.Count > 0)
            {
                ...
                {
                    if (!CanStartRocketBooster(pending.Skill, currentTime))
                    {
                        CancelRocketBoosterState(playExitAction: false);
                        continue;
                    }

                    ExecuteRocketBoosterLaunch(pending.Skill, pending.Level, pending.FacingRight, pending.ActionName);
                    continue;
                }
        ...
        private void UpdateRocketBooster(int currentTime)
        {
            ...
            if (currentTime - _rocketBoosterState.LandingAttackTime >= _rocketBoosterState.RecoveryDurationMs)
            {
        ...
        private bool CanMaintainRocketBooster(SkillData skill, int currentTime)
        {
            ...
            if (!string.IsNullOrWhiteSpace(_player.GetSkillBlockingRestrictionMessage(currentTime)))
            {
                return false;
            }

            if (!string.IsNullOrWhiteSpace(_additionalStateRestrictionMessageProvider?.Invoke(currentTime)))
            {
                return false;
            }
        ...
        private int GetMovementActionDurationMs(SkillData skill)
        {
            ...
            string actionName = ResolveMovementActionName(skill);
            if (string.IsNullOrWhiteSpace(actionName))
            {
                return 0;
            }
        ...
        {
            return string.Equals(_player?.CurrentActionName, "tank_rbooster_pre", StringComparison.OrdinalIgnoreCase)
                ? "tank_rbooster_pre"
                : "rbooster_pre";
        }
        ...
        private static bool IsSwallowSkill(SkillData skill)
        {
            return skill?.IsSwallowSkill == true
                   ...
                   || SkillTextContains(skill, "swallow");
        }
        ...
        private void ProcessMeleeAttack(
            ...
            Vector2? attackOriginOverride = null)
        {
            ProcessDirectionalAttack(skill, level, currentTime, AttackResolutionMode.Melee, facingRight, queueFollowUps, preferredTargetMobId, attackOriginOverride);
        }
        ...
        private void ProcessMagicAttack(
            ...
            Vector2? attackOriginOverride = null)
        {
            ProcessDirectionalAttack(skill, level, currentTime, AttackResolutionMode.Magic, facingRight, queueFollowUps, preferredTargetMobId, attackOriginOverride);
        }
        ...
        private void ProcessRangedAttack(
            ...
            Vector2? attackOriginOverride = null)
        {
            ProcessDirectionalAttack(skill, level, currentTime, AttackResolutionMode.Ranged, facingRight, queueFollowUps, preferredTargetMobId, attackOriginOverride);
        }
            ...
            Vector2? attackOriginOverride)
        {
            ...
            Rectangle worldHitbox = GetWorldAttackHitbox(skill, level, levelData, mode, facingRight, attackOrigin);
            ...
            List<MobItem> targets = ResolveTargetsInHitbox(worldHitbox, currentTime, maxTargets, mode, facingRight, preferredTargetMobId, attackOrigin);
            if (targets.Count == 0)
                return;
        ...
        private Rectangle GetWorldAttackHitbox(SkillData skill, int level, SkillLevelData levelData, AttackResolutionMode mode, bool facingRight, Vector2? attackOriginOverride = null)
        {
            Rectangle hitbox = skill.GetAttackRange(level, facingRight);
            if (hitbox.Width <= 0 || hitbox.Height <= 0)
            {
                if (mode == AttackResolutionMode.Melee
                    && TryGetMeleeAfterImageHitbox(skill, facingRight, out Rectangle afterImageHitbox))
                {
                    hitbox = afterImageHitbox;
                }
            }
            ...
            if (hitbox.Width <= 0 || hitbox.Height <= 0)
            {
                hitbox = mode switch
                {
                    ...
                    AttackResolutionMode.Ranged or AttackResolutionMode.Projectile => GetDefaultRangedHitbox(skill, levelData, facingRight),
        ...
        private bool TryGetMeleeAfterImageHitbox(SkillData skill, bool facingRight, out Rectangle hitbox)
        {
        ...
        private void ApplyMobStatusFromSkill(SkillData skill, SkillLevelData levelData, MobItem mob, int currentTime)
        {
            ...
            {
                TryApplyInferredMobStatus(mob.AI, MobStatusEffect.Blind, durationMs, currentTime, 0, tickIntervalMs: 1000, propPercent);
            }
        ...
        private void ApplyStatDrivenMobDebuffs(
            SkillData skill,
            SkillLevelData levelData,
            MobAI mobAI,
            string searchText,
            ...
            int propPercent)
        {
            ...
            {
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.PADamage, durationMs, currentTime, -attackReduction, tickIntervalMs: 1000, propPercent);
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.MADamage, durationMs, currentTime, -attackReduction, tickIntervalMs: 1000, propPercent);
            }

            int physicalDefenseReduction = ResolvePhysicalDefenseDebuffMagnitude(levelData, searchText);
            if (physicalDefenseReduction > 0)
            {
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.PDamage, durationMs, currentTime, -physicalDefenseReduction, tickIntervalMs: 1000, propPercent);
            }

            int magicDefenseReduction = ResolveMagicDefenseDebuffMagnitude(levelData, searchText);
            if (magicDefenseReduction > 0)
            {
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.MDamage, durationMs, currentTime, -magicDefenseReduction, tickIntervalMs: 1000, propPercent);
            }

            int accuracyReduction = ResolveAccuracyDebuffMagnitude(levelData, searchText);
            if (accuracyReduction > 0)
            {
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.ACC, durationMs, currentTime, -accuracyReduction, tickIntervalMs: 1000, propPercent);
            }
        }
            ...
            int propPercent)
        {
            ...
            {
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.Hypnotize, durationMs, currentTime, 1, tickIntervalMs: 1000, propPercent);
            }

            if (debuffToken.IndexOf("polymorph", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.Doom, durationMs, currentTime, 1, tickIntervalMs: 1000, propPercent);
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.Seal, durationMs, currentTime, 1, tickIntervalMs: 1000, propPercent);
            }
            ...
            if (debuffToken.IndexOf("amplifyDamage", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                ...
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.Ambush, durationMs, currentTime, amplifiedDamage, tickIntervalMs: 1000, propPercent);
            }
            ...
            if (debuffToken.IndexOf("elementalWeaken", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                ...
                TryApplyInferredMobStatus(mobAI, MobStatusEffect.Neutralise, durationMs, currentTime, weakenValue, tickIntervalMs: 1000, propPercent);
            }
                ...
                debuffToken.IndexOf("incTargetMDP", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                    ...
                    debuffToken.IndexOf("incTargetReward", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    ...
                    TryApplyInferredMobStatus(mobAI, MobStatusEffect.Showdown, durationMs, currentTime, rewardBonusPercent, tickIntervalMs: 1000, propPercent);
                }
            }
        }
            ...
            Vector2? attackOriginOverride = null)
        {
            ...
            for (int i = 0; i < bulletCount; i++)
            {
                ...
                var proj = new ActiveProjectile
                {
                    ...
                    LevelData = levelData,
                    X = attackOrigin.X,
                    Y = attackOrigin.Y - 20, // Adjust to hand height
                    PreviousX = attackOrigin.X,
                    PreviousY = attackOrigin.Y - 20,
                    FacingRight = facingRight,
                    SpawnTime = spawnTime,
                    OwnerId = 0,
        ...
        private static List<int> BuildProjectileReleaseSchedule(SkillLevelData levelData, int bulletCount)
        {
        ...
        private Rectangle GetProjectileFrameBounds(ActiveProjectile proj, SkillFrame frame, float anchorX, float anchorY)
        {
            ...
            bool shouldFlip = proj.FacingRight ^ (frame?.Flip ?? false);
        ...
        {
            return skillId == SATELLITE_SKILL_ID
                || skillId == ENHANCED_SATELLITE_SKILL_ID
                || skillId == SATELLITE_2_SKILL_ID;
        }
            ...
            int? damagePercentOverride = null)
        {
            ...
            foreach (MobItem mob in OrderSummonTargetsByDistance(summon, candidateTargets, currentTime))
            {
            ...
            {
                ExecuteTime = currentTime + ResolveSummonAttackExecutionDelayMs(summon, resolvedTargets, currentTime),
                ...
                TargetMobIds = resolvedTargets
                    .Select(mob => mob?.PoolId ?? 0)
                    .Where(poolId => poolId > 0)
                    .ToArray()
            };

            if (queuedAttack.TargetMobIds.Length <= 0)
            {
                return false;
            }
        ...
        private int ResolveSummonAttackExecutionDelayMs(ActiveSummon summon, IReadOnlyCollection<MobItem> targets, int currentTime)
        {
            ...
            foreach (MobItem target in targets)
            {
                ...
                delayMs = Math.Max(delayMs, travelDelayMs);
            }
        ...
        private bool TryResolveSelfDestructSummon(ActiveSummon summon, int currentTime)
        {
            ...
            summon.PendingRemovalTime = currentTime + Math.Max(
                1,
                summon.SkillData.SummonAttackAnimation?.TotalDuration
        ...
        {
            return skill != null && IsSatelliteSummonSkill(skill.SkillId);
        }
        ...
        private static SummonAssistType ResolveSummonAssistType(SkillData skill)
        {
            ...
            if (UsesOwnerAttackTargetedSummon(skill))
            {
        ...
        private Rectangle GetSummonAttackBounds(ActiveSummon summon)
        {
            ...
            Vector2 summonPosition = GetSummonPosition(summon);
            Rectangle localHitbox = summon.SkillData.GetSummonAttackRange(summon.FacingRight);
        ...
        {
            return RequestClientSkillCancel(skillId, Environment.TickCount);
        }
        ...
        private void RemoveBuffAt(int index, int currentTime, bool playFinish)
        {
            ...
            foreach (int skillId in _activeAffectedSkillPassives
                         .Where(entry => GetSkillData(entry.Key)?.AffectedSkillId == buff.SkillId)
                         .Select(entry => entry.Key)
                         .ToList())
            {
        ...
        private bool TryHandleBattleMageAuraToggle(SkillData skill, int currentTime)
        {
            ...
            CancelActiveBuff(skill.SkillId, currentTime, playFinish: true);
            RequestClientSkillCancel(skill.SkillId, currentTime);
            return true;
        }
        ...
        private static SkillData ResolveBuffAvatarEffectSkill(SkillData skill)
        {
            ...
            {
                SkillId = skill.SkillId,
                AvatarOverlayEffect = UsesSwallowBuffAvatarEffectFallback(skill)
                    ? null
                    : CreateLoopingAvatarEffect(skill.Effect),
                AvatarUnderFaceEffect = UsesSwallowBuffAvatarEffectFallback(skill)
                    ? CreateLoopingAvatarEffect(skill.Effect)
                    : CreateLoopingAvatarEffect(skill.EffectSecondary),
                HideAvatarEffectOnLadderOrRope = UsesSwallowBuffAvatarEffectFallback(skill)
            };
        }
        ...
        public IReadOnlyList<StatusBarBuffEntry> GetStatusBarBuffEntries(int currentTime)
        {
            ...
            foreach (var buff in _buffs)
            {
                ...
                BuffTemporaryStatPresentation primaryTemporaryStat = ResolvePrimaryTemporaryStat(temporaryStats);
                bool preferTemporaryStatIcon = primaryTemporaryStat != null && !string.IsNullOrWhiteSpace(primaryTemporaryStat.IconKey);
                entries.Add(new StatusBarBuffEntry
                {
                    ...
                    SortOrder = primaryTemporaryStat?.SortOrder ?? GenericBuffSortOrder,
        ...
        public int GetBuffStat(BuffStatType stat)
        {
            ...
            foreach (var buff in _buffs)
            {
                ...
                total += stat switch
                {
                    ...
                    BuffStatType.MaxHP => ResolveEnhancedStatBonus(buff.SkillData, data.EnhancedMaxHP),
                    BuffStatType.MaxMP => ResolveEnhancedStatBonus(buff.SkillData, data.EnhancedMaxMP),
                    _ => 0
                };
            }
            return total;
        ...
        private static IReadOnlyList<BuffTemporaryStatPresentation> GetBuffTemporaryStatPresentation(ActiveBuff buff)
        {
            ...
            void Track(int value, string label)
            {
                ...
                if (TemporaryStatPresentationCatalog.TryGetValue(label, out BuffTemporaryStatPresentation presentation))
                {
            ...
            string skillDescription = skill?.Description ?? string.Empty;
            string actionName = skill?.ActionName ?? string.Empty;
            string prepareActionName = skill?.PrepareActionName ?? string.Empty;
            string keydownActionName = skill?.KeydownActionName ?? string.Empty;
            string keydownEndActionName = skill?.KeydownEndActionName ?? string.Empty;
            string triggerCondition = skill?.TriggerCondition ?? string.Empty;
            string affectedSkillEffect = skill?.AffectedSkillEffect ?? string.Empty;
        ...
        private static BuffTemporaryStatPresentation ResolvePrimaryTemporaryStat(IReadOnlyList<BuffTemporaryStatPresentation> temporaryStats)
        {
            ...
            return temporaryStats
                .OrderBy(stat => stat.PrimaryPriority)
            ...
            string label)
        {
            if (!condition
                || temporaryStats == null
                || !TemporaryStatPresentationCatalog.TryGetValue(label, out BuffTemporaryStatPresentation presentation)
            ...
            string combinedText)
        {
            ...
            TrackIfMissing(
                temporaryStats,
                isHyperBody && hasLevelData && (levelData.EnhancedMaxHP > 0 || levelData.X > 0),
                MaxHpBuffLabel);
            TrackIfMissing(
                temporaryStats,
                isHyperBody && hasLevelData && (levelData.EnhancedMaxMP > 0 || levelData.Y > 0),
                MaxMpBuffLabel);
            ...
            TrackIfMissing(
                temporaryStats,
                isMapleWarrior && hasLevelData && (levelData.X > 0 || levelData.Y > 0 || levelData.Z > 0),
                AllStatsBuffLabel);
            ...
            TrackIfMissing(
                temporaryStats,
                isSharpEyes && hasLevelData && levelData.Y > 0,
                CriticalDamageBuffLabel);

            bool isMagicGuard = ContainsAny(combinedText, "magic guard", "damage dealt to you affects your mp instead of your hp");
            ...
            TrackIfMissing(temporaryStats, ContainsAny(combinedText, "charge", "elemental charge", "energy charge"), ChargeBuffLabel);
            TrackIfMissing(temporaryStats, ContainsAny(combinedText, "aura", "dark aura", "blue aura", "yellow aura", "body boost"), AuraBuffLabel);
            TrackIfMissing(temporaryStats, ContainsAny(combinedText, "shadow partner", "mirror image", "shadow image", "clone"), ShadowPartnerBuffLabel);
            TrackIfMissing(temporaryStats, ContainsAny(combinedText, "holy shield", "abnormal status", "status ailment", "status immunity"), DebuffResistanceBuffLabel);
        ...
        public int GetMastery(WeaponPart weapon)
        {
            ...
            foreach (var skill in _availableSkills)
            {
                ...
                if (levelData?.Mastery > mastery
                    && SkillMasteryAppliesToWeapon(skill, weaponCode)
        ...
        private static bool SkillMasteryAppliesToWeapon(SkillData skill, int weaponCode)
        {
            if (skill == null)
                return false;
            ...
            bool matchesWeapon = normalizedName switch
            {
                string value when value.Contains("spell mastery") || value.Contains("magic mastery")
                    => weaponCode is 37 or 38,
                string value when value.Contains("sword mastery") || value.Contains("sword expert")
                    => weaponCode is 30 or 40,
                string value when value.Contains("axe mastery") || value.Contains("axe expert")
                    => weaponCode is 31 or 41,
                string value when value.Contains("blunt weapon mastery")
                    || value.Contains("blunt weapon expert")
                    || value.Contains("bw mastery")
                    || value.Contains("bw expert")
                    => weaponCode is 32 or 42,
                string value when value.Contains("spear mastery") || value.Contains("spear expert")
                    => weaponCode == 43,
                string value when value.Contains("polearm mastery") || value.Contains("polearm expert")
                    => weaponCode == 44,
                string value when value.Contains("bow mastery") || value.Contains("bow expert")
                    => weaponCode == 45,
                string value when value.Contains("crossbow mastery") || value.Contains("crossbow expert")
                    => weaponCode == 46,
                string value when value.Contains("dual bowguns mastery") || value.Contains("dual bowguns expert")
                    => weaponCode == 52,
                string value when value.Contains("claw mastery") || value.Contains("claw expert")
                    => weaponCode == 47,
                string value when value.Contains("dagger mastery") || value.Contains("dagger expert")
                    => weaponCode == 33,
                string value when value.Contains("katara mastery") || value.Contains("katara expert")
                    => weaponCode is 33 or 34,
                string value when value.Contains("cane mastery") || value.Contains("cane expert")
                    => weaponCode == 36,
                string value when value.Contains("knuckle mastery") || value.Contains("knuckle expert")
                    => weaponCode == 48,
                string value when value.Contains("gun mastery")
                    || value.Contains("gun expert")
                    || value.Contains("mechanic mastery")
                    || value.Contains("extreme mech")
                    => weaponCode == 49,
                string value when value.Contains("cannon mastery") || value.Contains("cannon expert")
                    => weaponCode == 53,
                _ => false
            };

            return weaponCode > 0 && matchesWeapon;
        }
        ...
        private int GetPassiveBonus(SkillData skill, SkillLevelData levelData, BuffStatType stat)
        {
            ...
            total += stat switch
            {
                ...
                BuffStatType.MaxHP => ResolveEnhancedStatBonus(skill, levelData.EnhancedMaxHP),
                BuffStatType.MaxMP => ResolveEnhancedStatBonus(skill, levelData.EnhancedMaxMP),
                _ => 0
            };
        ...
        private bool HasActiveWildHunterSwallow()
        {
            MobItem target = GetSwallowTarget();
        ...
        private void UpdateAffectedSkillPassives(int currentTime)
        {
            ...
            foreach (SkillData skill in _availableSkills)
            {
                ...
                int level = GetSkillLevel(skill.SkillId);
                if (level <= 0 || !HasActiveBuff(skill.AffectedSkillId))
                {
        ...
        private void UpdateClientSkillTimers(int currentTime)
        {
            ...
            OnClientSkillTimersExpiredBatch?.Invoke(orderedTimers
                .Select(timer => new ClientSkillTimerExpiration(timer.SkillId, timer.Source, timer.ExpireTime))
                .ToArray());
        ...
        {
            for (int i = _queuedSummonAttacks.Count - 1; i >= 0; i--)
            {
                QueuedSummonAttack queuedAttack = _queuedSummonAttacks[i];
                if (queuedAttack == null || queuedAttack.ExecuteTime > currentTime)
                {
                    continue;
                }

                _queuedSummonAttacks.RemoveAt(i);
                ResolveQueuedSummonAttack(queuedAttack, currentTime);
            }
        }
        ...
        private void ExecuteQueuedSparkAttack(QueuedSparkAttack queuedAttack, int currentTime)
        {
            ...
            int level = queuedAttack.Level > 0 ? queuedAttack.Level : GetSkillLevel(queuedAttack.SkillId);
            SkillLevelData levelData = skill?.GetLevel(level);
            if (skill == null || levelData == null || !IsAttackTriggeredChainBuffSkill(skill, level))
                return;

            if (!_buffs.Any(buff => buff.SkillId == queuedAttack.SkillId))
                return;

            List<MobItem> targets = ResolveSparkTargets(queuedAttack, levelData, currentTime);
            if (targets.Count == 0)
                return;
            ...
            foreach (MobItem mob in targets)
            {
                bool died = ApplySkillAttackToMob(
                    ...
                    mob,
                    currentTime,
                    Math.Max(1, levelData.AttackCount),
                    AttackResolutionMode.Projectile,
                    skill.HitEffect,
        ...
        private void ResolveQueuedSummonAttack(QueuedSummonAttack queuedAttack, int currentTime)
        {
        ...
        private void ExecuteQueuedSerialAttack(QueuedSerialAttack queuedAttack, int currentTime)
        {
            ...
            int level = queuedAttack.Level > 0 ? queuedAttack.Level : GetSkillLevel(queuedAttack.SkillId);
            SkillLevelData levelData = skill?.GetLevel(level);
            if (!ShouldUseQueuedSerialAttack(skill) || levelData == null)
                return;
        ...
        {
            return skill?.IsPassiveSkillData == true
                   && skill.AffectedSkillId > 0
        ...
        private List<MobItem> ResolveSparkTargets(QueuedSparkAttack queuedAttack, SkillLevelData levelData, int currentTime)
        {
            if (_mobPool == null || levelData == null || queuedAttack == null)
                return new List<MobItem>();

            int sourceMobId = queuedAttack.SourceMobId ?? 0;
            MobItem sourceMob = sourceMobId > 0 ? FindAttackableMobByPoolId(sourceMobId, currentTime) : null;
            Vector2 origin = sourceMob != null
                ? GetMobHitboxCenter(sourceMob, currentTime)
                : queuedAttack.SourcePosition;
            float range = Math.Max(40f, levelData.Range > 0 ? levelData.Range : 150f);
            int maxTargets = Math.Max(1, levelData.MobCount);
            List<MobItem> targets = new(maxTargets);
            HashSet<int> excludedMobIds = new();

            // The client stores the originating mob for Spark and replays the proc from that anchor.
            if (sourceMob != null)
            {
                targets.Add(sourceMob);
                excludedMobIds.Add(sourceMob.PoolId);
            }

            int remainingTargets = maxTargets - targets.Count;
            if (remainingTargets <= 0)
                return targets;

            targets.AddRange(ResolveSequentialChainTargets(
                origin,
                range,
                remainingTargets,
                excludedMobIds,
        ...
        private List<MobItem> ResolveQueuedSerialTargets(QueuedSerialAttack queuedAttack, SkillLevelData levelData, int currentTime)
        {
            ...
            MobItem preferredTarget = queuedAttack.PreferredTargetMobId.HasValue
                ...
                : null;
            Vector2 origin = preferredTarget != null
                ? GetMobHitboxCenter(preferredTarget, currentTime)
                : queuedAttack.PreferredTargetPosition;
        ...
        private Rectangle GetDefaultRangedHitbox(SkillData skill, SkillLevelData levelData, bool facingRight)
        {
            ...
            int offsetX = facingRight ? 25 : -(width + 25);
