    ...
    public sealed class MemoryGameOfficialSessionBridgeManager : IDisposable
    {
        ...
        public bool TryStartFromDiscovery(int listenPort, int remotePort, string processSelector, out string status)
        {
            ...
            if (candidates.Count == 0)
            {
                ...
                status = $"Memory Game official-session discovery found no established TCP session for {selectorLabel} on remote port {remotePort}.";
                LastStatus = status;
                return false;
            }
            ...
            if (candidates.Count > 1)
            {
                ...
                status = $"Memory Game official-session discovery found multiple candidates for {selectorLabel} on remote port {remotePort}: {matches}. Use /memorygame session discover to inspect them, or start manually.";
                LastStatus = status;
                return false;
            }
