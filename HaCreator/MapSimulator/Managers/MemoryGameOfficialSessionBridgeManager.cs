...
{
    /// <summary>
    /// Built-in MiniRoom bridge that proxies a live Maple session and peels
    /// opcode 373 Match Cards traffic into the existing Memory Game inbox seam.
    /// </summary>
    public sealed class MemoryGameOfficialSessionBridgeManager : IDisposable
    {
        public const int DefaultListenPort = 18488;
        public const int InboundMiniRoomOpcode = 373;
        public const int OutboundMiniRoomOpcode = 144;
        ...
        public void Stop()
        {
            ...
            {
                StopInternal(clearPending: true);
                LastStatus = "Memory Game official-session bridge stopped.";
            }
        }
        ...
        private static bool TryResolveProcessSelector(string selector, out int? owningProcessId, out string owningProcessName, out string error)
        {
            ...
            {
                error = "Memory Game official-session discovery requires a process name or pid when a selector is provided.";
                return false;
            }
            ...
            out string status)
        {
            ...
            {
                status = $"Memory Game official-session discovery found no established TCP session for {DescribeDiscoveryScope(owningProcessId, owningProcessName, remotePort, localPort)}.";
                candidate = default;
                return false;
            }
            ...
            if (filteredCandidates.Count > 1)
            {
                ...
                status = $"Memory Game official-session discovery found multiple candidates for {DescribeDiscoveryScope(owningProcessId, owningProcessName, remotePort, localPort)}: {matches}. Use /memorygame session discover to inspect them, or add a localPort filter.";
                candidate = default;
                return false;
            }
