...
namespace HaCreator.MapSimulator.Managers
{
    ...
    public sealed class SocialRoomEmployeeOfficialSessionBridgeManager : IDisposable
    {
        ...
        public SocialRoomEmployeeOfficialSessionBridgeManager(Func<MapleRoleSessionProxy> roleSessionProxyFactory = null)
        {
        ...
        public void Start(int listenPort, string remoteHost, int remotePort, SocialRoomKind? preferredKind = null)
        {
            lock (_sync)
            {
                ...
                try
                {
                    ...
                    LastStatus = $"Social-room employee official-session bridge listening on 127.0.0.1:{ListenPort} and proxying to {RemoteHost}:{RemotePort}{DescribePreferredKindSuffix(preferredKind)}. {proxyStatus}";
                }
                catch (Exception ex)
                {
                    StopInternal(clearPending: true);
        ...
        private void OnRoleSessionServerPacketReceived(object sender, MapleSessionPacketEventArgs e)
        {
            ...
            ReceivedCount++;
            LastStatus = $"Queued employee opcode {opcode} from live session {e.SourceEndpoint}{DescribePreferredKindSuffix(PreferredKind)}.";
        }
