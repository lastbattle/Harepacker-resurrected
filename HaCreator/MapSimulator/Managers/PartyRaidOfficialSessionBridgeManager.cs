...
namespace HaCreator.MapSimulator.Managers
{
    ...
    public sealed class PartyRaidOfficialSessionBridgeManager : IDisposable
    {
        ...
        public bool TryStart(int listenPort, string remoteHost, int remotePort, out string status)
        {
            lock (_sync)
            {
                ...
                try
                {
                    ...
                    LastStatus = $"Party Raid official-session bridge listening on 127.0.0.1:{ListenPort} and proxying to {RemoteHost}:{RemotePort}.";
                    status = LastStatus;
                    return true;
                }
                catch (Exception ex)
        ...
        private async Task ListenLoopAsync(CancellationToken cancellationToken)
        {
            ...
            {
                LastStatus = $"Party Raid official-session bridge error: {ex.Message}";
            }
        }
        ...
        private async Task AcceptClientAsync(TcpClient client, CancellationToken cancellationToken)
        {
            ...
            try
            {
                lock (_sync)
                {
                    if (_activePair != null)
                    {
                        LastStatus = "Rejected Party Raid official-session client because a live Maple session is already attached.";
                        client.Close();
                        return;
                    }
                }
                ...
                clientSession.OnClientDisconnected += _ => ClearActivePair(pair, $"Party Raid official-session client disconnected: {pair.ClientEndpoint}.");
                serverSession.OnPacketReceived += (packet, isInit) => HandleServerPacket(pair, packet, isInit);
                serverSession.OnClientDisconnected += _ => ClearActivePair(pair, $"Party Raid official-session server disconnected: {pair.RemoteEndpoint}.");
                ...
                LastStatus = $"Party Raid official-session bridge connected {pair.ClientEndpoint} -> {pair.RemoteEndpoint}. Waiting for Maple init packet.";
            ...
            catch (Exception ex)
            {
                client.Close();
                pair?.Close();
                LastStatus = $"Party Raid official-session bridge connect failed: {ex.Message}";
            }
        }
        ...
        private void HandleServerPacket(BridgePair pair, PacketReader packet, bool isInit)
        {
            try
            {
                ...
                if (isInit)
                {
                    ...
                    LastStatus = $"Party Raid official-session bridge initialized Maple crypto for {pair.ClientEndpoint} <-> {pair.RemoteEndpoint}.";
                ...
                if (!TryDecodePartyRaidOpcode(raw, out int opcode, out byte[] payload))
                {
                    return;
                }
            ...
            {
                ClearActivePair(pair, $"Party Raid official-session server handling failed: {ex.Message}");
            }
        }
        ...
        private void HandleClientPacket(BridgePair pair, PacketReader packet, bool isInit)
        {
            ...
            {
                ClearActivePair(pair, $"Party Raid official-session client handling failed: {ex.Message}");
            }
        }
