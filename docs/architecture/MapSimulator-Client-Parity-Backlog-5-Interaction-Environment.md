...
| Partial | Dynamic objects / quest layers | Field object drawables now retain WZ `quest` visibility metadata, keep WZ `hide=1` object layers suppressed until a matching quest state or a published object-tag toggle explicitly reveals them, resolve WZ object `tags` through a dedicated published object-state seam instead of the obstacle bucket, and ingest authored map-level tagged-object defaults from `publicTaggedObjectVisible` plus the client-data typo variant `pulbicTaggedObjectVisible`, including grouped multi-tag entries, but richer event-scripted sequencing and map/NPC script families that publish those states automatically are still absent | Quest-driven maps now respect both quest state and author-authored hidden-layer defaults instead of flattening hidden layers into unconditional visibility or unconditional suppression, tagged event-map objects can fall back to authored default states when runtime overrides are cleared, and generic object-state publication no longer has to masquerade as an obstacle toggle even though full scripted event-map sequencing is still incomplete | map/field load path, `MapSimulator.cs`, `QuestRuntimeManager.cs`, `FieldEffects.cs`, `FieldObjectTagStateDefaultsLoader.cs` |
...
| Partial | Field-specific restrictions and effects | Map entry now surfaces `timeLimit`, `decHP`, `protectItem`, `allowedItem`, ambient `rain` / `snow`, `Unable_To_Jump`, field-transfer, `Unable_To_Use_Rocket_Boost`, and Mystic Door field-limit metadata through simulator notices, matching `rain` / `snow` map-info flags automatically start the simulator's ambient weather layer on field entry, blocked skill casts raise explicit field-limit feedback for both Rocket Booster and Mystic Door map bans, jump-disabled maps now reject jump input with the same field-rule messaging seam, cross-map portal travel respects `Unable_To_Migrate`, `decHP` maps deal periodic environmental damage with a mist cue unless the inventory currently holds a listed `protectItem`, `timeLimit` maps warn before expiry then transfer to `returnMap` / `forcedReturn`, and `allowedItem` maps now reject non-listed inventory-backed item consumption plus setup-chair activation through the same field-rule notice path, but session, party, teleport-item, and broader field-script enforcement are still not modeled | Event and boss maps now expose the first real slice of client field-rule timing, movement locks, migration locks, hazard behavior, ambient field weather, `protectItem` hazard suppression, `allowedItem` item-use gating, and Mechanic rocket-mobility bans instead of silently ignoring those map-info properties, even though wider rule enforcement is still incomplete | `MapSimulator.cs`, `FieldRuleRuntime.cs`, `FieldEnvironmentEffectEvaluator.cs`, `FieldSkillRestrictionEvaluator.cs`, `FieldInteractionRestrictionEvaluator.cs`, field systems (`CField::IsFearEffectOn`, `CField::OffFearEffect`) |
