...
| Missing | Remote portal-pool parity | The simulator covers local `Mystic Door` and local `Open Portal: GX-9` ownership, but the client keeps packet-authored portal instances from other users on separate field pools that are not yet modeled in the backlog or runtime. IDA shows `CTownPortalPool::OnPacket` dispatching create/remove for town portals (`330/331`) and `COpenGatePool::OnPacket` dispatching create/remove for Open Gate (`332/333`), which is a distinct remote-field ownership surface rather than an extension of local collision handling or local skill cast state. | Without these remote portal pools, visible parity stops at self-authored portal utility. Other users' Mystic Doors and Open Gates cannot enter the field through their packet-owned create/remove path, which leaves remote support skills, party utility visibility, and future `CUserPool` integration around remote portal ownership incomplete. | field portal/runtime layer (`CTownPortalPool::OnPacket`, `COpenGatePool::OnPacket`, local reference seam: `CUserLocal::CheckPortal_Collision`) |
| Missing | Remote affected-area / mist parity | The simulator has local zone-backed skill logic such as invincible smoke fields, but there is still no backlog row for the client-owned field pool that inserts and removes packet-authored affected areas independently of the local skill runtime. IDA shows `CAffectedAreaPool::OnPacket` dispatching created and removed affected areas (`328/329`), which is a separate owner for remote mist, aura, poison-cloud, and similar field zones that are not reducible to the local caster's current skill state once they exist in the map. | This is a real field-ownership gap, not just a missing effect draw. Without `CAffectedAreaPool`-style parity, remote support zones and hostile mist-style areas cannot be represented through the same packet-owned create/remove seam the client uses, which weakens skill-field interaction, area visibility, and future remote-user parity across party play and mob-driven field effects. | field affected-area/runtime layer (`CAffectedAreaPool::OnPacket`, local reference seam: `SkillManager`, zone/effect draw path) |
| Partial | Field-specific restrictions and effects | Map entry now surfaces `timeLimit`, `decHP`, `protectItem`, `allowedItem`, ambient `rain` / `snow`, `Unable_To_Jump`, `Unable_To_Use_Teleport_Item`, field-transfer, `Unable_To_Use_Rocket_Boost`, Mystic Door field-limit metadata, and WZ-authored `onUserEnter` / `onFirstUserEnter` / `fieldScript` names through simulator notices, matching `rain` / `snow` map-info flags automatically start the simulator's ambient weather layer on field entry, blocked skill casts raise explicit field-limit feedback for both Rocket Booster and Mystic Door map bans, jump-disabled maps now reject jump input with the same field-rule messaging seam, cross-map portal travel respects `Unable_To_Migrate`, `decHP` maps deal periodic environmental damage with a mist cue unless the inventory currently holds a listed `protectItem`, `timeLimit` maps warn before expiry then transfer to `returnMap` / `forcedReturn`, `allowedItem` maps now reject non-listed inventory-backed item consumption plus setup-chair activation through the same field-rule notice path, cash teleport-rock items now route into the simulator's map-transfer window unless `Unable_To_Use_Teleport_Item` blocks them, WZ-authored numeric `consumeItemCoolTime` values now enforce a shared consume-item cooldown through the same inventory guard seam, and pending cross-map transfers now reject under-`lvLimit` characters plus maps marked `partyOnly` or `expeditionOnly` using the simulator's existing party-roster and expedition-search session state with explicit map-restriction notices, but actual field-script execution, server-authored admission state, and `moveLimit` still remain unmodeled or notice-only | Event and boss maps now expose the first real slice of client field-rule timing, movement locks, migration locks, hazard behavior, ambient field weather, `protectItem` hazard suppression, `allowedItem` item-use gating, teleport-item map bans, Mechanic rocket-mobility bans, numeric consume cooldowns, basic level, party, and expedition entry gating, and notice-only surfacing of entry-owned field scripts instead of silently ignoring those map-info properties, even though actual script execution, deeper server/session checks, and `moveLimit` still need further enforcement | `MapSimulator.cs`, `FieldRuleRuntime.cs`, `FieldEntryRestrictionEvaluator.cs`, `FieldEnvironmentEffectEvaluator.cs`, `FieldSkillRestrictionEvaluator.cs`, `FieldInteractionRestrictionEvaluator.cs`, `TeleportItemUsageEvaluator.cs`, `UnitTest_MapSimulator/FieldInteractionRestrictionEvaluatorTests.cs`, `UnitTest_MapSimulator/FieldRuleRuntimeTests.cs`, field systems (`CField::IsFearEffectOn`, `CField::OffFearEffect`) |
