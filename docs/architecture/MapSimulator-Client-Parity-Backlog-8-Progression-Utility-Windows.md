...
- Progression or utility window groundwork now has a shared seam: named owners for world map, map transfer, cash-service, channel-selection, crafting, enhancement, character-info, and quest-side utility windows are registered with `UIWindowManager`, while minimap and status-bar utility buttons now route into those owners instead of staying entirely dead-ended. The world/channel path now opens dedicated simulator-side `WorldSelect`, `ChannelSelect`, and timed `ChannelShift` surfaces backed by `UIWindow2.img/Channel` art, with the channel grid, centered placement, per-channel load feedback, login-step request gating, and delayed `CheckUserLimitResult` / `SelectWorldResult` style request flow now aligned to the client-owned selector layout instead of remaining a placeholder-only launch seam. The map-transfer owner now also keeps saved teleport destinations in a character-scoped destination book keyed to the active player or selected login-roster character instead of sharing one global saved list across every simulator avatar.
...
| Partial | Map transfer / navigation window parity | The simulator now routes minimap `BtMap` into a dedicated WZ-backed `MapTransfer` owner built from the `Teleport*` window art, constrains the destination surface to the client's five visible rows, caps saved teleport destinations to a client-shaped five-vs-ten slot budget based on the loaded `Teleport` vs `Teleport3` art variant, respects teleport-item field limits separately from generic migration checks, layers return-map plus portal-derived routes on top of the saved list while driving actual map changes through the existing pending-map seam instead of stopping at the minimap shell, and now keeps saved teleport destinations in a character-scoped destination book keyed to the active player or selected login-roster character instead of one simulator-global list, but it still does not mirror the client's full `CUIMapTransfer` behavior surface such as packet-backed teleport-rock persistence across sessions, the edit-target/manual entry path, richer world-map integration, or stricter client destination categories beyond the simulator's route synthesis | Navigation testing can now exercise a more client-shaped transfer and destination-selection surface above raw portal travel, and saved destinations no longer bleed across unrelated simulator characters, but the window still needs deeper character-data sourcing and button-flow parity before it matches the official flow | navigation/UI layer, minimap integration, destination-selection flow (`CUIMapTransfer::Draw`, `CUIMapTransfer::OnButtonClicked`, `CUIMapTransfer::OnCreate`) |
| Partial | Cash shop / admin shop dialog parity | The simulator now replaces the generic Cash Shop and MTS placeholders with a dedicated `AdminShopDialogUI` owner backed by `UIWindow2.img/Shop`, reuses the shop dialog frame plus `BtBuy` / `BtSell` / `BtExit` / `BtRecharge`, keeps Cash Shop and MTS inside the same button-driven surface, routes the status-bar launchers so only the requested service window stays open, exposes separate scrollable NPC-offer and user-listing panes with per-row selection and request submission messaging, now draws WZ-backed service and pane tab strips, mirrors the client-owned twin scrollbar layout called out by `CAdminShopDlg::OnCreate`, and replaces the wish-list footer prompt with a `FadeYesNo`-style confirmation modal before saving a wish-list entry, but it still does not mirror the full `CAdminShopDlg` behavior surface such as packet-backed shop sessions, live server-populated catalogs, or the richer server-driven catalog and trade state that fill those controls in the official client | The simulator can now exercise the client cash-service launch path through a dedicated dialog owner with concrete per-item browsing, client-shaped scrollbar and tab affordances, and a real wish-list confirmation step instead of a dead-end placeholder, while the remaining gap is the deeper live session and catalog fidelity inside that owner | cash-service UI layer, status-bar integration, shop-session flow (`CAdminShopDlg::OnCreate`, `CAdminShopDlg::Draw`, `CAdminShopDlg::OnButtonClicked`) |
| Partial | World and channel selector parity | The simulator now routes `BtChannel` into dedicated `WorldSelect`, `ChannelSelect`, and timed `ChannelShift` windows, uses `UIWindow2.img/Channel` world badges and channel art for the local selector stack, preserves a visible current world/channel choice for the simulator session, expands the world list to the full badge set, aligns the channel grid and window centering to the client `GetRect`/owner layout, simulates per-world channel availability plus busy/full load bars so invalid targets no longer behave like always-open buttons, and now mirrors the client-owned request shape more closely by disabling selector input outside the login `WorldSelect` step and resolving world/channel picks through delayed `CheckUserLimitResult` / `SelectWorldResult` style simulator responses instead of committing immediately, but it still lacks the client's packet-backed world population data, adult-channel rules, and real server-authored result codes or population updates | Channel switching is no longer a dead-end HUD affordance, and the remaining gap has narrowed to live server data, adult gating, and deeper packet fidelity rather than missing selector surfaces or always-generic channel entries | world-selection and channel-selection UI layer, status-bar integration (`CUIWorldSelect::OnCreate`, `CUIWorldSelect::OnButtonClicked`, `CUIChannelSelect::OnCreate`, `CUIChannelSelect::Draw`, `CUIChannelSelect::OnButtonClicked`, `CUIChannelShift::OnCreate`, `CUIChannelShift::Draw`) |
| Partial | Inventory window parity | Legacy and Big Bang inventory windows now render right-aligned meso text, per-slot item icons and stack counts, WZ-backed grade and quality markers, disabled-slot overlays, active-slot highlighting, and Big Bang tab or expanded layouts, while the simulator also feeds meso and picked-up items into the inventory surface at runtime, resolves equip pickup icons through `Character/*/*.img` like the client asset families, respects `info/slotMax`-style stack splitting and merge limits for stackable pickups, wires `BtGather` and `BtSort` into simulator-side stack compaction and ordering, and now shows shared client-style hover tooltips with cached item names, type labels, stack data, and descriptions across both legacy and Big Bang inventory layouts, but it still lacks fuller client-accurate slot-unlock progression plus drag/drop behavior | The simulator inventory is now a usable player-facing surface instead of a frame shell, so the remaining gap is narrowed to client-accurate inventory state rules and deeper interaction fidelity rather than basic draw ownership | `InventoryUI.cs`, `InventoryUIBigBang.cs`, `UIWindowLoader.cs`, inventory/runtime integration (`CUIItem::Draw`) |
| Partial | Item maker / crafting window parity | The simulator now replaces the `ItemMaker` placeholder with a dedicated WZ-backed `Maker` window owner, exposes recipe selection plus `BtStart` / `BtCancel` flow, animates the maker gauge during crafting, seeds a starter ore inventory plus meso and catalyst stock, switches across the client `Etc/ItemMake.img` bucket set (`0`, `1`, `2`, `4`, `8`, `16`) through an in-window page selector instead of stopping at ETC-only recipes, enforces per-recipe level, meso, catalyst, and `reqItem` requirements against the shared inventory runtime, and resolves weighted `randomReward` branches during completion, but it still lacks NPC launch parity, profession or mastery runtime gating beyond the current local maker-skill value, and the deeper `CUIItemMaker` dropdown or page-state behavior around the client selector surface | Production and crafting progression now has a broader client-backed recipe surface and tighter inventory validation instead of a dead-end placeholder, while the remaining gap is the deeper launch path and client-side gating or interaction rules that surround that loop | inventory/runtime integration, crafting UI layer (`CUIItemMaker::Draw`, `CUIItemMaker::OnButtonClicked`) |
| Partial | Item upgrade / enhancement window parity | The simulator now registers a dedicated `ItemUpgrade` owner instead of a placeholder, loads Gold Hammer or Vicious Hammer style client art from `UIWindow2.img` or `UIWindow.img`, binds the window to the live `CharacterBuild` plus inventory runtime, seeds starter enhancement scroll stock so the loop is immediately exercisable, resolves eligible items from WZ-backed `info/tuc` upgrade-slot data instead of a pure slot-type guess, consumes enhancement scrolls from the inventory surface, distinguishes failed-but-surviving attempts from curse-style item destruction, tracks per-item remaining upgrade slots and success or fail counts, and applies lightweight stat growth back into `CharacterBuild`, but it still lacks NPC or inventory launch seams and the broader family of enhancement variants such as cube, potential-stamp, and Vega-style result flows | Progression-sensitive item workflows now have a visible dedicated surface, live inventory consumption, and a more client-shaped result matrix instead of a mostly decorative seam, while the remaining gap is the deeper launch path and broader enhancement-family coverage that surround the official client flow | inventory/runtime integration, enhancement UI layer (`CUIItemUpgrade::Draw`, `CUIItemUpgrade::OnButtonClicked`) |
