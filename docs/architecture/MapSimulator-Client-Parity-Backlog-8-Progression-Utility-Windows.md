...
- Progression or utility window groundwork now has a shared seam: named placeholder owners for world map, map transfer, cash-service, channel-selection, crafting, enhancement, character-info, and quest-side utility windows are registered with `UIWindowManager`, while minimap and status-bar utility buttons now route into those owners instead of staying entirely dead-ended.
...
|--------|------|-----|----------------|--------------|
| Partial | Minimap window parity | The minimap already supports client-style frame loading, collapse/maximize toggles, drag bounds, and a live player dot derived from world position, and `BtBig` / `BtMap` now route into named world-map and map-transfer window owners instead of staying as no-op handlers, but the simulator still does not mirror client marker sets, map-name overlays, region/full-map transitions, or richer minimap interaction states | Navigation feedback now has the correct launch seam for broader minimap follow-up work, but it still falls short of the client's actual minimap tool rather than merely needing polish | `MinimapUI.cs`, minimap/window loader path |
