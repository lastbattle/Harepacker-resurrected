...
| Status  | Area                                | Gap | Why it matters | Primary seam |
|---------|-------------------------------------|-----|----------------|--------------|
| Partial | Mob action-loader / frame-build parity | The simulator keeps the explicit `CActionMan::LoadMobAction` seam and now also carries an explicit client action-slot owner path in `LifeLoader` instead of only keyed-by-authored-name loading. Mob actions are now resolved through a dedicated simulator `s_sMobAction` slot table for the core client action surface (`stand`/`move`/`fly`/`hit1`/`die1`/`regen`, `attack1..8`, `skill1..16`), cached by client slot where mapped, and then materialized in deterministic slot order before authored-name fallbacks are applied. Existing per-frame parity remains in place on this same seam: `delay`, `head`, `lt`, `rb`, `multiRect`, origin-backed visual bounds/frame metadata into `MobAnimationSet`, reverse-pass duplication for reverse-flagged actions, and lane-aware body-rectangle collision consumers in `MobItem`/combat targeting. Attack-support metadata/effects continue to key through the same resolved action-name seam so mapped slot actions stay consistent across frame, hit, projectile, warning, and extra-effect consumers. Focused seam coverage now exists in `UnitTest_MapSimulator/MobActionLoaderParityTests.cs` for the explicit client action-slot table contract (slot order, canonical name resolution, and unknown-action fallback). Remaining gap is narrower but still `Partial`: the full byte-accurate client `s_sMobAction` index map recovered from IDA (including any uncovered or version-specific slots beyond this implemented core surface) is not yet fully mirrored end-to-end, and rarer frame-owned fields or branch-specific consumers beyond the currently modeled origin/rect/head/reverse/multi-body seams may still exist in the native cache path. | Naming and partially implementing the `CActionMan` owner keeps mob animation parity split the same way as summons, NPCs, pets, and employees: runtime `CMob::*` logic can keep evolving independently while loader-owned frame table construction, cached reuse, authored rect math, per-lane body geometry, and reverse playback now have a concrete simulator seam instead of staying implicit inside generic mob loading. | `LifeLoader.cs`, `MobAnimationSet.cs`, `MobItem.cs`, `MobAttackSystem.cs`, `UnitTest_MapSimulator/MobActionLoaderParityTests.cs` (`CActionMan::LoadMobAction`, `CMob::DoAttack`, `CMob::OnNextAttack`, `CMob::ProcessAttack`, `CMob::GetAttackInfo`) |
