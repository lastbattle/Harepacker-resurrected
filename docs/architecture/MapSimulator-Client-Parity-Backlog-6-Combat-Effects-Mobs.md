...
| Status | Area | Gap | Why it matters | Primary seam |
|--------|------|-----|----------------|--------------|
| Partial | Damage-number and tremble animation-owner parity | The simulator already renders combat text and impact feedback, and this row now keeps the recovered visible owner explicit at the `CAnimationDisplayer` seam instead of letting the registration shape disappear into generic mob-runtime or HUD code. `CAnimationDisplayer::Effect_HP` still owns number-family selection, digit composition, critical-banner insertion, temporary canvas creation, and one-time layer registration, while `CAnimationDisplayer::Effect_Tremble` owns the shake gate, start/end timing, and reduction curve. WZ inspection still anchors the assets to `effect/BasicEff.img/NoCri0`, `NoCri1`, and `NoCri1/effect`, including the separate `NoCri1/effect` critical canvas (`62x57`, origin `41,70`) over the `NoCri0` digits and the client `CreateCanvas(lWidth, 57)` height. The managed simulator path now formats values through `MapleStoryStringPool` id `0x1A15`, keeps the red-only critical-owner split on both spawned and helper paths, preserves the client large-first/small-tail digit mix plus the separate critical banner over `NoCri0` digits, materializes the prepared payload into a temporary composite canvas texture once at spawn, and builds the shared one-time canvas-layer owner with the recovered `400ms` hold, `600ms` fade, `30px` rise, `250ms` critical delay, `lCenterTop - 47` placement, and `-30px` critical-banner Y offset. `BuildOneTimeLayerRegistration` now carries the prepared insert-descriptor array and prepared managed registration payload together with the recovered `CreateCanvas(lWidth, 57)` dimensions, client-shaped layer position write, animation-layer `InsertCanvas` call shape, one-time registration marker, and recovered layer-registration literals (`CreateLayer` canvas value `0`, initial layer option `0xC0050004`, layer priority `-1`, final layer option `0`), and `RegisterOneTimeCanvasLayer` now accepts that prepared registration payload verbatim from the damage-number seam instead of rebuilding the registration shape from scalar timing arguments lower down. The prepared damage-number visual also keeps the named temporary-canvas composition trace with the recovered canvas dimensions, per-digit source set and sprite, source origin and dimensions, and canvas offset for each client-shaped insertion while keeping the critical banner explicitly marked as a separate layer-owned overlay rather than part of the digit composite. The existing tremble gate, start/end timing, heavy-vs-normal duration split, and additional-time reduction behavior remain explicit owner-layer state from `CAnimationDisplayer::Effect_Tremble`. Focused seam coverage is now checked in on disk in `UnitTest_MapSimulator/CombatFeedbackAnimationOwnerParityTests.cs` for the red-only critical-owner split, large-first/small-tail digit layout, separate critical-banner composition, non-red helper-path suppression, recovered one-time layer placement, recovered layer-registration literals, explicit prepared-registration handoff, recovered `CreateCanvas` and digit-composition trace shape, one-time registration marking, and tremble gate plus duration or reduction rules. The remaining gap is now narrowly centered on native Gr2D execution: the simulator still does not instantiate the client's real COM `IWzGr2DLayer` object graph, execute the recovered writes and `InsertCanvas` calls against native layer objects, or reproduce byte-for-byte blend and composite output. | Without an explicit `CAnimationDisplayer` combat-feedback row, combat parity can look further along than it is once mob AI, hit resolution, HP bars, and generic damage text exist. Naming the owner keeps future work anchored to the client seam that actually builds and schedules damage-number and tremble presentation instead of burying the remaining visual differences inside unrelated mob-runtime rows. | combat feedback animation layer (`CAnimationDisplayer::Effect_HP`, `CAnimationDisplayer::Effect_Tremble`, simulator seams: `CombatEffects.cs`, `DamageNumberRenderer.cs`, `ScreenEffects.cs`, `MobAttackSystem.cs`, `MapSimulator.cs`, `UnitTest_MapSimulator/CombatFeedbackAnimationOwnerParityTests.cs`) |

### 5. Mob anger-gauge burst animation-owner discovered by follow-up IDA scan

| Status | Area | Gap | Why it matters | Primary seam |
|--------|------|-----|----------------|--------------|
| Partial | Mob anger-gauge burst animation-owner parity | The simulator now gives the visible full-charge burst its own owner seam instead of leaving it as an inline mob overlay. `LifeLoader` records the authored mob burst UOL through the client-backed StringPool ids for `Mob/%07d.img` and `AngerGaugeEffect`, `MobItem` now resolves the mob head anchor as the burst origin, and full-charge updates now register the burst through `AnimationEffects.AddOneTimeAttached` as a one-shot attached animation instead of reusing the generic per-frame mob draw path. The registration also keeps a client-shaped cooldown seam by deriving the replay interval from the authored burst frame delays, so a mob that remains fully charged can re-arm the burst without conflating that timing with the underlying `ChargeCount` / `AngerGauge` runtime logic. | Rage-boss parity is closer because the visible full-charge burst is now tracked as its own mob-to-animation owner with an explicit authored path, head-anchor origin, and one-time registration point, which keeps future work attached to the actual client seam rather than hiding it under generic mob overlays. The remaining gap is narrower: the simulator still approximates `CAnimationDisplayer::LoadLayer` / `GA_STOP` / `RegisterOneTimeAnimation` through attached DX frames and does not yet recreate the native COM layer parentage on the mob action layer, so overlay-parent ordering and the exact native full-charge cooldown source remain inferred rather than client-identical. | mob anger-gauge presentation layer (`CMob::AngerGaugeFullChargeEffect`, `CAnimationDisplayer::Effect_FullChargedAngerGauge`, `CAnimationDisplayer::LoadLayer`, `CAnimationDisplayer::RegisterOneTimeAnimation`) |
