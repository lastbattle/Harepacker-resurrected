...
| Status | Area | Gap | Why it matters | Primary seam |
|--------|------|-----|----------------|--------------|
| Partial | Damage-number and tremble animation-owner parity | The simulator already renders combat text and impact feedback, but this scan shows the client keeps the visible owner in `CAnimationDisplayer` rather than only inside the mob runtime or HUD code. `CAnimationDisplayer::Effect_HP` owns number-family selection, digit composition, critical-banner insertion, temporary canvas creation, and one-time layer registration, while `CAnimationDisplayer::Effect_Tremble` owns the shake gate, start/end timing, and reduction curve. This pass moved the seam materially closer to that owner again: the WZ-backed damage-number renderer still formats values through `MapleStoryStringPool` id `0x1A15`, keeps the red-only critical-owner split, preserves the client large-first/small-tail digit mix plus the separate `NoCri1` critical banner over `NoCri0` digits, and still materializes the prepared payload into a real temporary composite canvas texture once at spawn, but that payload no longer stays in the local damage-number draw loop. The prepared canvas, client `CreateCanvas(lWidth, 57)` height, `lCenterTop - 47` placement rule, `400ms` hold + `600ms` fade timeline, `30px` rise, and delayed critical-banner offset now flow through a shared one-time canvas-layer registration seam in `AnimationEffects`, so the visible owner sits materially closer to `CAnimationDisplayer::RegisterOneTimeAnimation` while the existing tremble gate, start/end timing, heavy-vs-normal duration split, and reduction behavior remain explicit owner-layer state. Focused seam coverage now exists in `UnitTest_MapSimulator/CombatFeedbackAnimationOwnerParityTests.cs` for one-time layer placement, timeline constants, and delayed critical-banner propagation, alongside the existing string-pool decimal format, red-only critical-owner split, mixed digit-layout rule, prepared canvas height and placement, critical-banner relative offset, and tremble gate plus end-tick lifecycle coverage already tracked for this row. The remaining gap is now narrower and mostly inside the final Gr2D details: the simulator registers a shared one-time canvas layer, but it still does not mirror the client's exact `IWzGr2DLayer::InsertCanvas` call order, layer flags, or blend/composite behavior byte-for-byte, and the delayed critical banner is still drawn by the simulator's canvas-layer helper rather than by the client's exact two-insert Gr2D sequence. | Without an explicit `CAnimationDisplayer` combat-feedback row, combat parity can look further along than it is once mob AI, hit resolution, HP bars, and generic damage text exist. Naming the owner keeps future work anchored to the client seam that actually builds and schedules damage-number and tremble presentation instead of burying the remaining visual differences inside unrelated mob-runtime rows. | combat feedback animation layer (`CAnimationDisplayer::Effect_HP`, `CAnimationDisplayer::Effect_Tremble`, simulator seams: `CombatEffects.cs`, `DamageNumberRenderer.cs`, `ScreenEffects.cs`, `MobAttackSystem.cs`, `MapSimulator.cs`) |
