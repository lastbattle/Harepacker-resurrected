...
| Status | Area | Gap | Why it matters | Primary seam |
|--------|------|-----|----------------|--------------|
| Partial | Damage-number and tremble animation-owner parity | The simulator already renders combat text and impact feedback, but this scan shows the client keeps the visible owner in `CAnimationDisplayer` rather than only inside the mob runtime or HUD code. `CAnimationDisplayer::Effect_HP` owns number-family selection, digit composition, critical-banner insertion, temporary canvas creation, and one-time layer registration, while `CAnimationDisplayer::Effect_Tremble` owns the shake gate, start/end timing, and reduction curve. This pass stays at that recovered owner seam and now actually ships the previously-described managed registration handoff instead of only rebuilding the trace inside the generic canvas-layer owner. WZ inspection still anchors the assets to `effect/BasicEff.img/NoCri0`, `NoCri1`, and `NoCri1/effect`, including the separate `NoCri1/effect` critical canvas (`62x57`, origin `41,70`) over the `NoCri0` digits and the client `CreateCanvas(lWidth, 57)` height. The simulator still formats values through `MapleStoryStringPool` id `0x1A15`, keeps the red-only critical-owner split, preserves the client large-first/small-tail digit mix plus the separate critical banner over `NoCri0` digits, materializes the prepared payload into a real temporary composite canvas texture once at spawn, and builds the shared one-time canvas-layer owner with the recovered `400ms` hold, `600ms` fade, `30px` rise, `250ms` critical delay, `lCenterTop - 47` placement, and `-30px` critical-banner Y offset. The managed owner now also keeps the recovered registration trace explicit one level higher at the damage-number seam instead of only in the generic canvas-layer owner: `BuildOneTimeLayerRegistration` carries the `CreateCanvas(lWidth, 57)` dimensions, client-shaped layer position write, animation-layer `InsertCanvas` call shape, the one-time registration marker, and the recovered layer-registration literals (`CreateLayer` canvas value `0`, initial layer option `0xC0050004`, layer priority `-1`, final layer option `0`) as part of the prepared registration payload, `RegisterOneTimeCanvasLayer` accepts that trace verbatim from the damage-number seam, and the direct `PrepareVisual(...)` helper now honors the same red-only critical-owner split as the spawned runtime path instead of letting non-red callers leak critical spacing through the helper surface. This run also keeps a named temporary-canvas composition trace on the prepared damage-number visual, carrying the recovered `CreateCanvas(lWidth, 57)` dimensions, per-digit source set and sprite, source origin and dimensions, and the canvas offset for each client-shaped digit insertion while keeping the critical banner explicitly marked as a separate layer-owned overlay rather than part of the digit composite. The existing tremble gate, start/end timing, heavy-vs-normal duration split, and reduction behavior remain explicit owner-layer state from `CAnimationDisplayer::Effect_Tremble`. Focused seam coverage is now checked in on disk in `UnitTest_MapSimulator/CombatFeedbackAnimationOwnerParityTests.cs` for the red-only critical-owner split, large-first/small-tail digit layout, separate critical-banner composition, non-red helper-path suppression, recovered one-time layer placement, recovered layer-registration literals, explicit recovered-registration handoff, recovered `CreateCanvas` and digit-composition trace shape, one-time registration marking, and the tremble gate plus duration or reduction rules. The remaining gap is still narrowly centered on native Gr2D execution: the simulator does not instantiate the client's real COM `IWzGr2DLayer` object graph, execute the recovered writes and `InsertCanvas` calls against native layer objects, or reproduce byte-for-byte blend and composite output. | Without an explicit `CAnimationDisplayer` combat-feedback row, combat parity can look further along than it is once mob AI, hit resolution, HP bars, and generic damage text exist. Naming the owner keeps future work anchored to the client seam that actually builds and schedules damage-number and tremble presentation instead of burying the remaining visual differences inside unrelated mob-runtime rows. | combat feedback animation layer (`CAnimationDisplayer::Effect_HP`, `CAnimationDisplayer::Effect_Tremble`, simulator seams: `CombatEffects.cs`, `DamageNumberRenderer.cs`, `ScreenEffects.cs`, `MobAttackSystem.cs`, `MapSimulator.cs`, `UnitTest_MapSimulator/CombatFeedbackAnimationOwnerParityTests.cs`) |

### 5. Mob anger-gauge burst animation-owner discovered by follow-up IDA scan

| Status | Area | Gap | Why it matters | Primary seam |
|--------|------|-----|----------------|--------------|
| Missing | Mob anger-gauge burst animation-owner parity | The current combat backlog already tracks Astaroth-style rage logic, `ChargeCount`, `AngerGauge`, and `attack/info/AngerAttack`, but it still does not give the visible full-charge burst its own owner. IDA now makes that owner concrete: `CMob::AngerGaugeFullChargeEffect` rate-limits the burst through `m_bFullChargeEffectStartTime + m_bFullChargeEffectTime`, formats the authored UOL from the mob template id via StringPool ids `0x3CE` and `0xC2F`, then passes that path to `CAnimationDisplayer::Effect_FullChargedAngerGauge` with the mob head vector as origin and the mob action layer as overlay parent. `Effect_FullChargedAngerGauge` then loads the authored layer, forces `GA_STOP`, and registers it as a one-time animation instead of leaving the burst on the generic mob runtime or the existing damage-number/tremble owner row. The simulator backlog therefore still lacks an explicit place to track the mob-owned anger-gauge burst composition, registration timing, and overlay-parent behavior separately from the already-implemented WZ charge logic. | Rage-boss parity can look more complete than it really is once the gauge value, gated attack, and WZ effect path exist, but the client still routes the visible full-charge burst through a dedicated `CMob` to `CAnimationDisplayer` owner seam. Naming that seam keeps future work anchored to the actual burst presentation owner instead of hiding it under generic special-mob logic or unrelated combat-feedback animation rows. | mob anger-gauge presentation layer (`CMob::AngerGaugeFullChargeEffect`, `CAnimationDisplayer::Effect_FullChargedAngerGauge`, `CAnimationDisplayer::LoadLayer`, `CAnimationDisplayer::RegisterOneTimeAnimation`) |
