...
| Partial | Item/skill tooltip behavior | Skill hover tooltips now reuse the client `UIWindow2.img/Skill/main/tip0..tip2` frames in the skill window, quick-slot skill hovers, status-bar buff tray, and status-bar cooldown tray; the status-bar buff/cooldown hovers now also follow the same icon-led client-style layout and select the correct left/center/right frame variant instead of stretching those textures as a composite strip, while the cooldown tray continues to surface the live skill description plus remaining cooldown time instead of forcing the player to infer entries from icon art alone, and the Big Bang equipment window now renders live equipped-item icons from the current `CharacterBuild` plus a framed hover tooltip that pulls String.wz category/description text and parsed equip `info` metadata for stat gains, upgrade slots, durability, `reqJob`, `reqPOP`, and slot-specific eligibility lines instead of stopping at slot-plus-item-id stubs; those Big Bang equip hovers now also load the client `UIWindow2.img/ToolTip/Equip` canvas set so recognized equipment categories, property labels, requirement labels, and job-filter badges render from WZ art inside the existing tooltip shell instead of remaining entirely font-drawn, and the same richer `ToolTip/Equip`-backed presentation now carries into the Big Bang dragon, mechanic, and android equipment-tab hovers when those companion items expose parsed `CharacterPart` metadata. Full client `ToolTip/Equip` framing, bitmap numeric/value composition, potential/additional-option sections, and broader cross-UI item-tooltip parity are still incomplete | The simulator now has a shared client-style tooltip surface across the main skill HUDs, the Big Bang equipment window no longer leaves equipped-item hovers blank or metadata-thin, and the companion equipment tabs no longer fall back to item-id-only hover stubs; equipment hovers in that window also consume the client `ToolTip/Equip` label and job-filter canvases rather than only approximating the body with plain text, but the remaining gap is still the rest of the client's richer equip-tooltip shell and numeric/value rendering plus cross-window item-tooltip parity outside this equipment owner | `SkillUIBigBang.cs`, `QuickSlotUI.cs`, `StatusBarUI.cs`, `EquipUIBigBang.cs`, `CharacterData.cs`, `CharacterLoader.cs`, `UIWindowLoader.cs`, `MapSimulator.cs` |
| Partial | Charge bars and richer skill HUD | The simulator still renders the client-timed prepared-skill gauge set for the supported status-bar branches, now keeps longer prepare windows from flipping to the generic `Ready` caption as soon as the short client gauge fill completes by surfacing a `Preparing <n> sec` state until the real hold or release point arrives, and still routes the Evan/dragon `22151001` / `22121000` family through a separate world-overlay pass instead of the bottom status-bar surface while keeping the client `KeyDownBar2` / `KeyDownBar3` WZ skins on that split path; that dragon overlay now anchors from the live assembled avatar frame top rather than a fixed `player.Y - 80` approximation before the same `bar/origin` offset is applied, and the generic skill-name plus ready-label text stays suppressed on the dragon-owned branch so it no longer reuses the status-bar caption treatment. The remaining gap is the rest of the client's branch-specific text or state variants and the exact dragon-layer ownership or placement details beyond the simulator's avatar-top approximation | Prepared/keydown skills now surface closer client timing and HUD feedback instead of a single generic bar, longer prepare branches no longer advertise `Ready` while their actual prepare timer is still running, sustained keydown branches expose both their live maintain label and remaining capped-hold bar state more clearly, and the Evan/dragon branch no longer falls back to the status-bar path or the older fixed-height overlay anchor; the simulator still lacks the rest of the client's richer skill-state UI and the true dragon-owned placement or layering path | `StatusBarUI.cs`, `UILoader.cs`, `SkillManager.cs`, `MapSimulator.cs`, `PlayerCharacter.cs` (`CUserLocal::DrawKeyDownBar`, `CDragon::DrawKeyDownBar`, `CUserLocal::Update`, `get_max_gauge_time`, `UI/Basic.img/KeyDownBar*/bar/origin`) |
