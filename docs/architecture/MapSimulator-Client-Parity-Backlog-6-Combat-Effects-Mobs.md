...
| Status | Area | Gap | Why it matters | Primary seam |
|--------|------|-----|----------------|--------------|
| Missing | Mob action-loader / frame-build parity | The combat backlog still names only `CMob::*` behavior owners even though IDA now confirms that the client gives mob animation construction to a separate `CActionMan` owner. `CActionMan::LoadMobAction` at `0x41f530` first checks a dedicated `(templateId, action)` cache, resolves the action node through the static `s_sMobAction` table on the mob IMG entry, enumerates each frame canvas, reads per-frame `delay`, `head`, `lt`, `rb`, and multi-body metadata, computes the cached frame rect and head anchor, and appends a reversed playback pass when the action publishes the client reverse flag. A repo search across `HaCreator/MapSimulator` still shows broad mob combat/runtime coverage but does not show a named parity row or simulator seam that is explicitly anchored to this separate loader owner the way backlog 2 already tracks `LoadSummonedAction`, backlog 7 tracks `LoadNpcAction`, and backlog 10 tracks `LoadPetAction` / `LoadEmployeeAction`. | Without this row, mob parity can look further along than it is because attack/runtime logic is already well covered while the client-owned loader that decides frame tables, delay fallback, rect math, cached reuse, and reverse playback remains undocumented. Naming this owner keeps future mob animation work anchored to the same loader-vs-runtime split the other backlog documents already use instead of folding frame-build differences into generic `CMob` behavior notes. | mob action-loader layer (`CActionMan::LoadMobAction`, `CMob::DoAttack`, `CMob::OnNextAttack`, `CMob::ProcessAttack`, `CMob::GetAttackInfo`) |
