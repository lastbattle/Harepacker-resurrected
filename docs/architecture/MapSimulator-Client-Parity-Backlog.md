...
| Partial | Skill UI layout | Row height, positions, hit boxes, SP placement, quick-slot assignment wiring, and a client-backed `Basic.img/VScr` scrollbar at the confirmed `153,93,155` placement are now present, with the Big Bang hover tooltip reusing the client `tip0..tip2` frames, the scrollbar supporting arrow clicks, track paging, thumb dragging, and wheel-over-scrollbar input, and the row list now drawing the client `Skill/main/recommend/0` banner against a visible-tab levelable-skill fallback instead of leaving the recommendation surface blank; full skill-up behavior and the client's skill-root/SP-threshold recommendation table are still missing | The window is usable and closer to the client than the old notes imply, but it is still not a full `CUISkill` clone | `SkillUI.cs`, `SkillUIBigBang.cs`, `UIWindowLoader.cs` (`CUISkill::Draw`, `CUISkill::OnCreate`, `CUISkill::GetRecommendSKill`, `UI/UIWindow2.img/Skill/main/recommend`) |
| Partial | Quick slot cooldown overlay | Quick-slot skill cooldowns now follow the client `DrawSkillCooltime` stepped `CoolTime[0..14]` mask progression with `CoolTime[15]` as the cleared state, and the simulator keeps a compact remaining-seconds badge in the slot corner instead of a centered debug-style timer, but the item-side `DrawConsumeItemCooltime` pass is still missing because the simulator has no inventory-backed consumable model | Skill cooldown feedback is visibly closer to the client instead of being a continuously interpolated overlay, but quick-slot cooldown parity is still incomplete for consumables and the broader tray behavior | `QuickSlotUI.cs` (`CUIStatusBar::CQuickSlot::Draw`, `CUIStatusBar::CQuickSlot::DrawSkillCooltime`, `CUIStatusBar::CQuickSlot::DrawConsumeItemCooltime`) |
...
| Partial | Full skill UI behavior | The Big Bang skill window now mirrors `CUISkill::GetSkillRootVisible` at the tab layer by only exposing the visible skill-book path for the current job, keeps keyboard-selected entries in view while accepting pane-hover wheel scrolling plus `Up` / `Down` / `PageUp` / `PageDown` / `Home` / `End` navigation, and now renders the client recommendation banner with a visible-tab levelable-skill fallback, but skill-up actions, guide flow, and the client's exact skill-root recommendation table are still missing | The simulator now respects the client's visible-root job path and has a less brittle list-navigation surface, but the rest of the `CUISkill` behavior remains incomplete | `SkillUI.cs`, `SkillUIBigBang.cs`, `UIWindowLoader.cs` (`CUISkill::Draw`, `CUISkill::GetSkillRootVisible`, `CUISkill::GetRecommendSKill`) |
| Partial | Item/skill tooltip behavior | Skill hover tooltips now reuse the client `UIWindow2.img/Skill/main/tip0..tip2` frames in the skill window, quick-slot skill hovers, and status-bar buff tray, including the side/up frame selection used when the tooltip is clamped near viewport edges, but item tooltips and broader cross-UI parity are still incomplete | The simulator now has a shared client-style skill-tooltip surface across the main skill HUDs instead of the skill window falling back to a flat debug panel, but it still lacks the rest of the client's tooltip behavior | `SkillUIBigBang.cs`, `QuickSlotUI.cs`, `StatusBarUI.cs`, `UILoader.cs`, `MapSimulator.cs` |
