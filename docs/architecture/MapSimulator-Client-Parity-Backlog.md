...
| Status | Area | Gap | Why it matters | Primary seam |
|--------|------|-----|----------------|--------------|
| Partial | Generic skill casting | Skills can load, cast, consume MP/HP, apply cooldowns, spawn projectiles, and show effects | Good simulator foundation, but still generic rather than client-specific | `SkillManager.cs` |
| Partial | Buff stat application | Buff logic applies only a subset of stats directly; the code explicitly leaves `MAD`, `MDD`, `ACC`, `EVA`, `Speed`, and `Jump` as future work | Active skills do not fully affect player behavior or numbers | `SkillManager.ApplyBuffStats` |
| Partial | Melee / ranged / magic resolution | The runtime distinguishes projectile vs non-projectile, but real client behavior is still collapsed into simple rectangles, basic projectile flight, and generic damage application | Skills look castable without reproducing actual hit logic | `SkillManager.ProcessMeleeAttack`, `CheckProjectileCollisions` (`CUserLocal::DoActiveSkill_MeleeAttack`, `CUserLocal::DoActiveSkill_ShootAttack`, `CUserLocal::DoActiveSkill_MagicAttack`) |
| Missing | `DoActiveSkill_*` family parity | No specific implementations for bound jump, combat step, melee skill families, magic skill families, summons, meso explosion, open gate, smoke shell, prepare/charge, or admin-only skill behavior | This is the largest gameplay parity hole on the player side | `SkillManager`, `PlayerCharacter`, `PlayerCombat` (`CUserLocal::DoActiveSkill_*` family) |
| Missing | Key-down / charge skills | No equivalent to the client's prepare/charge flow or charge UI bar | A major class of warrior, mage, and special skills cannot behave correctly | `SkillManager`, UI layer (`CUserLocal::DoActiveSkill_Prepare`, `CUserLocal::TryDoingPreparedSkill`, `CUserLocal::DrawKeyDownBar`) |
| Missing | Summon simulation | Summon skills can be classified from data, but there is no summon entity lifecycle with attack, follow, duration, or mob aggro behavior | Summon-heavy jobs remain non-parity | `SkillManager`, entity/pool layer |
