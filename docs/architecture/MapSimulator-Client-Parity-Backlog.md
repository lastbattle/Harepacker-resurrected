...
| Partial | Dynamic objects / quest layers | Field object drawables now retain WZ `quest` visibility metadata and re-evaluate it against the simulator quest runtime, but broader event-scripted object toggles are still absent | Quest-driven maps can now hide and reveal matching object layers instead of treating all field objects as permanently static, while wider event-map presentation parity remains incomplete | map/field load path, `MapSimulator.cs`, `QuestRuntimeManager.cs` |
