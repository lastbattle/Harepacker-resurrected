...
- `Summon aggro registration` ? Partial: active summons now register with the simulator's puppet/aggro system, resolve visible summon-body branches such as `summoned` and `stand` from `Skill.wz`, and force nearby mobs to chase summon positions, but summon-type-specific placement, follow, and aggro rules are still incomplete.
