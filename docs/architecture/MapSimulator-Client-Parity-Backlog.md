...
- `Skill UI layout` ? Partial: the Big Bang skill window now reuses the client-facing `UIWindow2.img/Skill/main/tip0..tip2` tooltip chrome, switches between right, left, and above hover placement as the cursor approaches screen edges, only exposes the skill-book tabs for the active job path instead of always drawing all five books, keeps the selected entry visible while the list responds to pane-hover wheel scrolling plus `Up` / `Down` / `PageUp` / `PageDown` / `Home` / `End` navigation, and now draws the client `Basic.img/VScr` scrollbar chrome at the client-confirmed `153,93,155` slot with arrow clicks, track paging, thumb dragging, and wheel-over-scrollbar support, but recommendation logic and skill-up behavior are still incomplete.
