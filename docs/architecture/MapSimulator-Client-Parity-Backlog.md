...
| Partial | Dynamic objects / quest layers | Field object drawables now retain WZ `quest` visibility metadata, preserve per-object `hide` flags from the map data, and re-evaluate those combined rules against the simulator quest runtime, but broader event-scripted object toggles are still absent | Quest-driven maps now respect both quest state and author-authored hidden-layer defaults instead of treating flagged object layers as permanently visible once loaded, while wider event-map presentation parity remains incomplete | map/field load path, `MapSimulator.cs`, `QuestRuntimeManager.cs` |
