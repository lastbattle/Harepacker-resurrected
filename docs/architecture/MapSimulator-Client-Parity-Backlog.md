...
| Partial | Client-managed avatar effect layers | The simulator now loads persistent avatar-bound skill layer branches from WZ (`special`, `special0`, `finish`, `finish0`, ladder-aware `back` / `back_finish`, and `repeat` on `suddenDeath` skills), attaches them to the player render path as over-character / under-face layers, switches the `More Wild` family onto its ladder override, and plays finish cleanup on buff expiry or cancellation, but client-owned non-buff layer producers such as double-jump, flying-wing, swallowing, and broader ride/oak-cask cleanup in `CUser::Update` still remain | Visible parity is closer for `More Wild`, `Final Cut`-style sudden-death buffs, and other skill-backed persistent overlays because those extra layers now follow the avatar and clean up on the same lifecycle instead of staying entirely absent, but the rest of the client's update-owned effect layer surface is still incomplete | `PlayerCharacter`, `SkillLoader`, `SkillManager`, `CharacterAssembler` (`CUser::Update`, `CUser::UpdateMoreWildEffect`, `Skill/3312.img/33121006/{special,special0,finish,finish0,back,back_finish}`, `Skill/434.img/4341002/repeat`) |
...
| Partial | Dynamic objects / quest layers | Field object drawables now retain WZ `quest` visibility metadata, preserve per-object `hide` flags for both quest-tagged and hide-only object layers, and re-evaluate those combined rules against the simulator quest runtime, but broader event-scripted object toggles are still absent | Quest-driven maps now respect both quest state and author-authored hidden-layer defaults instead of treating hide-only object layers as permanently visible once loaded, while wider event-map presentation parity remains incomplete | map/field load path, `MapSimulator.cs`, `QuestRuntimeManager.cs` |
...
| Partial | Field-specific restrictions and effects | Map entry now surfaces `timeLimit`, `decHP`, `protectItem`, `allowedItem`, field-transfer, and Mystic Door field-limit metadata through simulator notices, blocked skill casts raise explicit field-limit feedback, cross-map portal travel now respects `Unable_To_Migrate`, `decHP` maps deal periodic environmental damage with a mist cue, and `timeLimit` maps warn before expiry then transfer to `returnMap` / `forcedReturn`, but session, party, inventory, and broader field-script enforcement are still not modeled | Event and boss maps now expose the first real slice of client field-rule timing, migration locks, and hazard behavior instead of silently ignoring those map-info properties, even though wider rule enforcement is still incomplete | `MapSimulator.cs`, `FieldRuleRuntime.cs`, `FieldSkillRestrictionEvaluator.cs`, field systems (`CField::IsFearEffectOn`, `CField::OffFearEffect`) |
