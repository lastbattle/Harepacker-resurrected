...
| Partial | Reactor interaction parity | Local player movement now runs reactor touch checks through `ReactorPool`, and reactor visuals can load and swap across WZ-backed numeric states instead of collapsing to `0/0`, but skill reactors, moving reactors, and richer event-script handling are still incomplete | Environmental interaction now covers the basic client touch-reactor path, even though more specialized reactor branches are still missing | `ReactorPool.cs`, `MapSimulator.cs`, `EffectLoader.cs` (`CUserLocal::CheckReactor_Collision`) |
| Partial | Quest-alert, balloon, and idle social feedback loops | NPCs with available, in-progress, or completable quests now render the client-backed `QuestIcon` alert art above their heads based on the simulator quest runtime, but balloon lifetimes, emotion reset timing, and idle pet auto-speech are still not modeled | The simulator now signals nearby quest state in-world instead of hiding that feedback inside the dialogue overlay alone, while the broader social/balloon update loop still remains incomplete | `QuestRuntimeManager.cs`, `MapSimulator.cs`, `UIWindow*.img/QuestIcon` (`CUserLocal::Update`, `CUser::PetAutoSpeaking`) |
