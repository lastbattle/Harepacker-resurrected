...
- `Summon aggro registration` ? Partial: active summons now register with the simulator's puppet/aggro system, transition from one-shot `summoned` spawn branches into looping idle branches such as `stand` and `fly`, persist across map reconnects, and force nearby mobs to chase summon positions, but summon-type-specific placement, follow, and aggro rules are still incomplete.
...
- `Mount / vehicle / mechanic mode rendering` ? Partial: `CharacterAssembler` now keeps exact `Character/TamingMob` ride actions when they exist (for example `01902000` exposes `stand1`, `walk1`, `jump`, `ladder`, `rope`, and `fly`) and falls back to seated passenger-style frames when the variant only exposes `sit` (for example `01983000`), so mount assembly no longer assumes every taming-mob asset matches the body's action surface; skill-driven vehicle passenger and mechanic-mode activation paths are still missing.
- `Skill-specific avatar transforms` ? Partial: the runtime now applies mechanic-mode avatar transform profiles when v115 mechanic skills cast their `tank_pre`, `siege_pre`, and `tank_siegepre` actions (`35121005`, `35111004`, `35121013`), keeps their client-facing stand/walk/attack posture families active after the entry animation, and clears them on buff expiry or skill-state resets; exit/after branches and broader class-specific transform families such as Flamethrower are still not modeled.
...
| Partial | Mount / vehicle / mechanic mode rendering | `TamingMob` assembly now preserves exact ride frames for normal mounts and falls back to seated passenger variants when the asset only exposes `sit`, but the simulator still lacks skill-driven vehicle passenger and mechanic-mode activation/state routing | Mounted presentation no longer hard-fails when taming-mob assets do not share the body's full action surface, but broader special avatar modes are still incomplete | `PlayerCharacter`, `CharacterLoader`, `CharacterAssembler` (`Character/TamingMob/01902000`, `Character/TamingMob/01983000`, `CAvatar::MoveAction2RawAction`, `CUser::Update`) |
| Partial | Skill-specific avatar transforms | Mechanic-mode skills now activate dedicated avatar action families (`tank_*`, `siege_*`, `tank_siege*`) from their v115 WZ `action/0` nodes and keep those posture families active until buff expiry or runtime cleanup, but exit/after animations and other class-specific transform families are still missing | Mechanic jobs no longer drop back to generic standing and attack actions immediately after entering siege or tank mode, but broader transform parity is still incomplete | `SkillManager`, `PlayerCharacter.TriggerSkillAnimation`, `CharacterAssembler` (`Skill/3512.img/35121005`, `Skill/3511.img/35111004`, `Skill/3512.img/35121013`, `CAvatar::MoveAction2RawAction`) |
...
| Partial | Summon simulation | Summon-family skills now create a generic summon lifecycle with duration, rendering, periodic nearby attacks, deferred mob removal during summon attack resolution, and registration into the simulator's puppet/aggro system so nearby mobs can redirect toward summon positions, but they do not yet reproduce client follow/placement/aggro rules per summon type | Summon-heavy jobs are now usable, more runtime-stable, and no longer ignore the existing summon aggro seam, but they are still not client-parity | `SkillManager`, `MobPool.cs`, entity/pool layer |
...
| Partial | Skill UI layout | Row height, positions, hit boxes, SP placement, and quick-slot assignment wiring are present, including post-Big Bang skill-icon drag-and-drop into the quick-slot bar, but the window still lacks scrollbar, tooltip, recommendation, and skill-up parity | The window is usable and closer to the client than the old notes imply, but it is still not a full `CUISkill` clone | `SkillUI.cs`, `SkillUIBigBang.cs` (`CUISkill::Draw`) |
| Partial | Quick slot cooldown overlay | Cooldown masking and remaining-seconds countdown text now render on assigned quick-slot skills | Surface-level parity exists without the client's full cooldown tray and feedback behavior | `QuickSlotUI.cs` (`CUIStatusBar::CQuickSlot::Draw`) |
...
| Partial | Charge bars and richer skill HUD | The status bar now renders a WZ-backed key-down/charge gauge for prepared skills, but the client's richer branch-specific HUD selection and additional skill-state feedback are still incomplete | Prepare/keydown skills now have visible hold progress, but not the full client HUD surface | `StatusBarUI.cs`, `UILoader.cs`, `MapSimulator.cs` (`CUserLocal::DrawKeyDownBar`) |
...
5. The generic skill runtime exists, but several named `CUserLocal::DoActiveSkill_*` families are still only partially matched.
6. Buff icon / temporary-stat status-bar parity is only partially matched.
7. Mount, vehicle, mechanic, and summon presentation are still missing.
8. Charge/prepare skills and their UI are only partially matched.
