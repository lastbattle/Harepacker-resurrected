...
| Missing | Alternate VAC character-entry parity | The login backlog still assumes that every successful pre-field character handoff follows the normal `SelectCharacterResult` route, but `CLogin::OnPacket` has a separate packet type `9` owner in `CLogin::OnSelectCharacterByVACResult`. IDA shows that this path is not just a reused success callback: it consumes a leading result byte plus a secondary code byte, maps its own login-utility error family (`14`, `15`, `16`, `17`, `19`, `20`, `21`, `27`, `33`, `40`, `56`, `62`), returns to Title for several failure branches, opens the same security website handoff for result codes `14` and `15`, and treats success differently from the normal select-character row by issuing the connection from either the direct success branch (`result = 0`) or the alternate authenticated branch (`result = 12` with secondary code `11` or `13`, plus `result = 23`) before playing the entry sound. None of the current simulator login owners, packet codecs, or backlog rows name that alternate result handler or model its distinct connect-vs-error table. | If this path stays invisible, the simulator can look complete for character entry while still missing a real client-owned alternate bootstrap route that bypasses part of the ordinary roster-select result handling. Tracking it separately keeps future work anchored to the actual packet router and prevents VAC-only failures or handoffs from being misfiled under the standard character-select row. | alternate login packet-entry layer (`CLogin::OnPacket`, `CLogin::OnSelectCharacterByVACResult`, `CWvsContext::IssueConnect`, `CLoginUtilDlg::Error`) |
