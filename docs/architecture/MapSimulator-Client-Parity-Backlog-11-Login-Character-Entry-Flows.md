...
| Partial | Recommend world helper parity | The simulator now builds the login recommend-world helper from the client `Login.img/WorldSelect/alert` frame and button art, draws the selected world's WZ-backed `Login.img/WorldSelect/world/<id>` badge instead of a placeholder label, and wraps per-world helper text in the same 125px message lane the client uses. The login packet configuration seam can now also carry packet-authored recommend-world ordering plus per-world message strings into the helper so the window no longer has to rely solely on simulator-derived recommendation copy. Remaining work is narrower: the helper is still positioned by simulator window layout rather than a recovered `CUIRecommendWorld::GetRect` owner placement, and there is still no live socket-backed `RecommendWorldMessage` intake behind the configured packet data | Server recommendation is part of the client's account-entry UX, not just an incidental line of text on the world list. Wiring a dedicated helper into the login world step means recommendation and guided selection now exist as a distinct pre-field surface instead of being implied only through world-list status text | world-recommendation UI layer, world-selection assist flow (`CUIRecommendWorld::OnCreate`, `CUIRecommendWorld::Draw`, `CUIRecommendWorld::OnButtonClicked`; simulator seams: `HaCreator/MapSimulator/UI/Windows/RecommendWorldWindow.cs`, `HaCreator/MapSimulator/Loaders/UIWindowLoader.cs`, `HaCreator/MapSimulator/MapSimulator.cs`) |
