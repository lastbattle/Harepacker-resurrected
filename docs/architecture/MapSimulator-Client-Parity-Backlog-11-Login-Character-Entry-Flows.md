...
### 4. Additional alternate-entry packet handler discovered by targeted `CLogin::OnPacket` scan
...
| Partial | Dedicated create-character flow parity | The simulator now exposes a dedicated login-map create-character owner instead of routing `BtNew` straight into the shared utility prompt: WZ-backed `Login.img/NewChar` stage art (`charAlert`, `charJob`, `charSet`, `charName`, `jobSelect`, `avatarSel`, `dice`, `BtYes`, `BtNo`, `BtCheck`) is loaded into a separate `LoginCreateCharacterWindow`, `Etc/MakeCharInfo.img` starter pools are now consumed per race family (`000`, `1000`, `2000`, `2001`, `3000`) through the existing `CharacterLoader` seam, and the active flow keeps a client-shaped pre-field state machine for race selection, job selection, avatar selection, typed-name entry, duplicate-name validation, and packet-result create handling instead of collapsing all of that into the roster mutation seam. Client evidence from `CLogin::OnNewCharJobSel`, `CLogin::OnNewCharAvatarSelCanceled`, `CUINewCharNameSelect::OnButtonClicked`, `CUINewCharNameSelect::OnKey`, `CLogin::OnCheckDuplicatedIDResult`, and `CLogin::OnCreateNewCharacterResult` is now reflected in the active path as well: race or job confirm advances through dedicated create steps, avatar cancel backs out through the same flow instead of dropping to the roster shell, Enter on the name owner routes through the duplicate-id packet seam, duplicate-id success returns to avatar selection by recording the checked name instead of immediately mutating the roster, duplicate-id failure stays inside the create flow, and create-result success still mutates the authoritative roster only through `CreateNewCharacterResult`. The remaining gap is narrower and now mostly owner fidelity: the simulator still approximates the four client classes through one staged owner instead of separate `CUINewCharRaceSelect` / `CUINewCharJobSelect` / `CUINewCharAvatarSelect` / `CUINewCharNameSelect` implementations, race choice is still a simulator text-list overlay rather than the client's exact button layout from `CUINewCharRaceSelect::OnCreate`, avatar editing still approximates the client's per-row left/right control family and exact draw placements, and the generated non-packet create path still uses simulator-authored starter stats and beginner/job metadata rather than reproducing every race-specific request field that `SendNewCharPacket` encodes end to end | Character creation is part of the visible pre-field pipeline, not just a roster mutation side effect. The simulator now has a dedicated pre-field create flow for that pipeline, but the client's separate owner classes and some exact layout or request details still remain to be mirrored | create-character owner family, login packet follow-up path (`CUINewCharRaceSelect`, `CUINewCharJobSelect`, `CUINewCharAvatarSelect`, `CUINewCharNameSelect`, `CLogin::OnNewCharJobSel`, `CLogin::OnNewCharAvatarSelCanceled`, `CUINewCharNameSelect::OnButtonClicked`, `CUINewCharNameSelect::OnKey`, `CLogin::OnCheckDuplicatedIDResult`, `CLogin::OnCreateNewCharacterResult`; simulator seams: `HaCreator/MapSimulator/UI/Windows/LoginCreateCharacterWindow.cs`, `HaCreator/MapSimulator/Managers/LoginCreateCharacterFlowState.cs`, `HaCreator/MapSimulator/Character/CharacterLoader.cs`, `HaCreator/MapSimulator/Loaders/UIWindowLoader.cs`, `HaCreator/MapSimulator/MapSimulator.cs`) |
