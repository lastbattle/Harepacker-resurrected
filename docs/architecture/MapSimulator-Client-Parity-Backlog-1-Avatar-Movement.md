...
|--------|------|-----|----------------|--------------|
| Partial | Default avatar selection | The simulator still keeps the deterministic, WZ-verified SuperGM/Wizet fallback selection of `1002140` (Wizet Invincible Hat), `1042003` (Wizet Plain Suit), `1062007` (Wizet Plain Suit Pants), `1072005` (Leather Sandals), `1092008` (Pan Lid), and `1322013` (Wizet Secret Agent Suitcase) with matching face, hair, level, and SuperGM job metadata, but login-side avatar ownership is now materially closer to the client: `AvatarLook` decode still validates gender, skin, face, hair, visible equip bodyparts, hidden equip lists, and pet ids, and the login packet seam can now also ingest packet-authored `SelectWorldResult` payloads supplied as hex or Base64 through `/loginpacket` or the inbox, decode the client-owned `result -> count -> GW_CharacterStat -> AvatarLook -> family/rank -> slot metadata` sequence confirmed from `CLogin::OnSelectWorldResult`, and rebuild the character-select roster plus field-entry runtime build from that server-shaped data instead of always fabricating local roster entries. The remaining gap is narrower: the simulator still does not consume live encrypted login traffic directly, still does not mirror other roster packet owners such as `ViewAllCharResult` / `ExtraCharInfoResult`, and still does not apply the broader packet surface such as cash-weapon sticker rendering or full rank/creation-randomization behavior end to end | Default avatars and login previews are no longer owned only by direct preset construction or simulator-authored roster clones: the same `AvatarLook` seam now accepts a decoded `SelectWorldResult` roster payload carrying `GW_CharacterStat` metadata and avatar bytes, which is much closer to the client's `OnSelectWorldResult` -> `GW_CharacterStat::Decode` -> `AvatarLook::Decode` ownership for both preview assembly and selected-character field entry | `CharacterLoader.LoadDefaultMale`, `CharacterLoader.LoadDefaultFemale`, `CharacterLoader.LoadFromAvatarLook`, `LoginAvatarLookCodec`, `LoginSelectWorldResultCodec`, `MapSimulator.InitializeLoginCharacterRoster`, `MapSimulator.TryConfigureLoginPacketPayload`, `LoginCharacterRosterEntry.CreateRuntimeBuild`, `CharacterConfigManager.CreateDefaultMalePreset`, `CharacterConfigManager.CreateDefaultFemalePreset` (`CUser::LoadLayer`, `CUIAvatar::CUIAvatar`, `CLogin::OnSelectWorldResult`, `GW_CharacterStat::Decode`, `AvatarLook::Decode`) |
