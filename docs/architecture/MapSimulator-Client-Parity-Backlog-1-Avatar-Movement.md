...
| Partial | Morph avatar pipeline parity | MapSimulator now resolves `common/morph` template ids into `Morph/*.img`, loads those images as a dedicated avatar render owner, resolves UOL-linked morph frames, preserves direct per-frame anchors like `head`, and switches buff-driven transforms onto morph-specific action playback through the existing assembler/runtime timing path instead of leaving morphs as UI-only suppression metadata. Remaining gap: client-side morph inheritance beyond direct IMG/UOL reuse still is not mirrored from `CActionMan::GetMorphImgEntry`, and hostile polymorph/debuff sources still do not drive the same render-owner swap path. | Morph-heavy jobs now render from a separate morph asset path rather than ordinary body/equip assembly, which closes the biggest parity hole around frame ownership and action timing. The remaining work is the narrower client fallback layer for inherited morph templates plus wiring non-buff polymorph sources into the same runtime owner selection. | avatar transform/runtime assembly (`CActionMan::GetMorphImgEntry`, `CActionMan::LoadMorphAction`) |
