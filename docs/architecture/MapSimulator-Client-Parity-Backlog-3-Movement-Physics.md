...
### 2. Passive transfer-field handoff parity discovered by targeted `CUserLocal` decompile

The latest IDA pass exposed one more movement-to-field seam that the backlog was still only implying through portal and teleport rows.

| Status | Area | Gap | Why it matters | Primary seam |
|--------|------|-----|----------------|--------------|
| Missing | Passive transfer-field handoff parity | `CUserLocal::TryPassiveTransferField` is its own post-action handoff owner rather than a generic portal retry. The decompile shows the client waiting on `m_bTryPassiveTransferField`, refusing to hand off while any one-time action is still active, checking a field-side transfer gate off the current vec-ctrl interface, and only then replaying the up-key path when the local user is neither immovable nor attract-locked before clearing the pending flag. The current movement backlog already tracks explicit portal collision and movement-path timing, but it still does not name this delayed field-transfer seam or the fact that the client keeps it behind local action completion rather than inside the original collision handler. | Without this row, passive transfer-field behavior can look like a minor portal follow-up even though the client treats it as a separate retry owner tied to movement state, one-time action completion, and vec-ctrl field readiness. Tracking it explicitly keeps future parity work anchored to the actual handoff gate instead of folding it into unrelated teleport or portal-collision polish. | delayed field-transfer movement layer (`CUserLocal::TryPassiveTransferField`, `CUserLocal::HandleUpKeyDown`, `CAvatar::GetOneTimeAction`) |
