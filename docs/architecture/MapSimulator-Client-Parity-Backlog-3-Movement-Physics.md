...
Passive transfer-field ownership is now tracked only in backlog 5 under field and portal parity so the delayed `CUserLocal::TryPassiveTransferField` retry owner is not documented twice across movement and field-transition plans.
