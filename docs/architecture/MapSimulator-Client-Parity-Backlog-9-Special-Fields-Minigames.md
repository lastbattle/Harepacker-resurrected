...
|--------|------|-----|----------------|--------------|
| Partial | Guild boss event fields | The simulator now routes special-field ownership through `SpecialFieldRuntimeCoordinator`, auto-binds guild-boss maps into that dedicated runtime seam, configures `GuildBossField` directly from the map's `healer` and `pulley` WZ nodes, loads the exact `Map/Obj/guild.img/syarenian/boss/{0,1}` animated art with origin-aligned placement, exposes a `/guildboss` debug seam that can inject raw client packet types `344` and `345`, decodes those payloads through a dedicated runtime packet path that mirrors `CField_GuildBoss::OnPacket`, applies `CHealer::Move` with the client's immediate absolute-Y result instead of simulator smoothing, cancels packet-owned preview state whenever an external healer or pulley packet arrives, and now mirrors `CField_GuildBoss::BasicActionAttack` more closely by driving the local pulley from the player's normal-attack hitbox instead of `Up`, replaying a local `CPulley::Hit`-style animation restart, and surfacing a pending outbound pulley request seam plus focused `GuildBossField` unit tests. The remaining gap is transport ownership: the simulator still does not send or receive the real guild-boss socket traffic, so the local attack path still falls back to a simulator-owned preview after queuing that request instead of waiting on live server-authored replies | This item now matches the full client-owned guild-boss seam confirmed in IDA more closely: `CField_GuildBoss` only initializes healer plus pulley, consumes packets `344/345`, and uses `BasicActionAttack` to hit the pulley and send the outbound request while idle, so the remaining work is concentrated on real transport wiring rather than an unimplemented hidden event system | `MapSimulator.cs`, `SpecialFieldRuntimeCoordinator.cs`, `SpecialEffectFields.cs`, `UnitTest_MapSimulator/GuildBossFieldTests.cs`, `GuildBossField` (`CField_GuildBoss`, `CField_GuildBoss::Init`, `CField_GuildBoss::OnPacket`, `CField_GuildBoss::BasicActionAttack`, `CHealer::Init`, `CHealer::Move`, `CPulley::Init`, `CPulley::Hit`) |
...
| Partial | Party Raid field flow | The simulator now binds `FIELDTYPE_PARTYRAID`, `FIELDTYPE_PARTYRAID_BOSS`, and `FIELDTYPE_PARTYRAID_RESULT` into a dedicated `PartyRaidField` runtime, renders the field, boss, and result HUDs from the client WZ asset roots (`UI/UIWindow.img/PartyRace/*`, `UI/UIWindow.img/DualMobGauge/*`, `Map/Effect.img/praid/*`), fixes the field and boss point-vs-stage board placement to match the `PartyRace/Stage/backgrd` labels, mirrors the client's team-colored stage-state art, boss red-vs-blue gauge fill, map-owned clear or timeout result effect, and result summary overlay positions, now infers win-vs-fail result mode from the result maps' `info/onUserEnter` scripts (`PRaid_WinEnter`, `PRaid_FailEnter`) and constrains the result panel to the map's `LBSide/LBTop/LBBottom` safe area, carries a type-2 timer lifecycle through the runtime, broadens the existing raw `field|party|session` key seam with numeric team aliases plus session outcome text handling, and in this pass adds a WZ-backed `PartyRace/battery` field HUD plus battery charge-capacity aliases through the same raw key seam with focused Party Raid runtime tests for script-owned outcome binding, battery decode, and session totals, but it still does not consume live Party Raid packets from the simulator network path, decode the exact server-owned Party Raid key names beyond the aliases recovered so far, or confirm any remaining client-only per-map boss layout details that are not explicit in WZ | Party Raid now covers the field battery surface as well as the field board, boss gauge, result overlays, script-backed outcome selection, map-safe result placement, and the stronger raw-key test seam, so the remaining gap is concentrated on live packet ownership, the unrecovered literal key names, and the last client-only per-map boss layout details rather than broad mode presentation holes | `SpecialFieldRuntimeCoordinator.cs`, `PartyRaidField.cs`, `MapSimulator.cs`, `UnitTest_MapSimulator/PartyRaidFieldTests.cs` (`CField_PartyRaid::_UpdatePoint`, `CField_PartyRaid::Init`, `CField_PartyRaidBoss::Init`, `CField_PartyRaidBoss::OnFieldSetVariable`, `CField_PartyRaidBoss::OnPartyValue`, `CField_PartyRaidResult::Init`, `CField_PartyRaidResult::OnSessionValue`) |
