...
|--------|------|-----|----------------|--------------|
| Partial | Local mob combat loop | Attack patterns, cooldowns, targeting helpers, knockback, damage numbers, hit effects, and drop spawning are present, direct melee attacks now use the same delayed action scheduler as projectile and ground-warning families using WZ `attackAfter` timing instead of pure frame polling, and summon skill `200` now resolves its live summon list and limit from `MobSkill.img` so temporary summons enter the field through the mob pool instead of being skipped | Good baseline, but the loop is still simplified vs client `CMob` behavior because non-summon special skill branches and richer transition rules remain missing | `MobAI.cs`, `MobPool.cs`, `MobAttackSystem.cs`, `MapSimulator.cs`, `CombatEffects.cs` |
...
| Partial | Full `CMob` action pipeline | The runtime now carries WZ `rush`, `jumpAttack`, and `tremble` attack flags through the loader and live attack entries, allows rush/jump movement during flagged attack states, recomputes direct-hit rectangles from the mob's live position when the delayed attack entry fires, triggers tremble feedback from flagged impacts, preserves puppet/summon aggro targets through AI updates, and routes delayed mob hits to registered puppet targets instead of always falling back to the local player | Attack timing and branch routing are closer to the client, but `CMob::DoAttack` / `ProcessAttack` still diverge on full summon body-rect fidelity, mob-vs-mob target branches, and other branch-specific side effects | `MobAI.cs`, `MobAttackSystem.cs`, `MobPool.cs`, `MapSimulator.cs` (`CMob::DoAttack`, `CMob::OnNextAttack`, `CMob::ProcessAttack`, `CMob::GetAttackInfo`) |
| Partial | Full mob stat/status parity | Mob statuses now keep concrete temporary-stat entries with expiry metadata, tick values, and reset events, poison/venom/burn deal runtime DoT damage, freeze and stun hold mobs in their incapacitated state until expiry, active temp stats now modify outgoing damage, incoming damage, and chase speed, and active status flags tint affected mobs in-world for visible feedback | Status-heavy encounters are more legible and core temporary stats now affect combat flow instead of existing as display-only flags, but skill-driven stat application coverage and the full client temporary-stat surface still lack fidelity | `MobAI.cs`, `PlayerCombat.cs`, effect loaders |
| Partial | Special mob interactions | WZ `selfDestruction` mobs now trigger reserved bomb/time-bomb actions from HP thresholds or `removeAfter` countdowns, finish through the simulator's special bomb death path, escort/friendly mobs no longer aggro the player, escort spawn points do not auto-respawn, special-death/escort kills suppress reward drops, and WZ `info/damagedByMob` encounter mobs now ignore player-originated basic, skill, projectile, and summon damage so they stay on the client's mob-vs-mob damage lane | This covers the first client-facing slice of escort and bomb encounters, but swallow routing, escort progression/index behavior, and advanced rage/anger boss scripts are still missing | `MobItem.cs`, `MobAI.cs`, `MobPool.cs`, `MapSimulator.cs`, field systems |
