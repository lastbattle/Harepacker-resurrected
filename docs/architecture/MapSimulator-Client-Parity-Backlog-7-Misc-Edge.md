...
|--------|------|-----|----------------|--------------|
| Partial | NPC talk flow | Nearby NPC clicks now open a real simulator dialogue overlay fed from loaded NPC string data including NPC `info/speak` idle lines, quest `Say.img` conversations preserve multi-page WZ ordering, root-level `yes`/`no` branches are now attached to the final numbered page, common MapleStory script markup is stripped into readable text with additional `#q`, `#s`, `#m`, `#z`, and `#v` token cleanup, inline `#L` menu options now surface as clickable read-only choices that route through WZ-backed `stop` answer branches when present or continue along the numbered conversation flow, and quest dialogue entries now also reuse named WZ `stop/*` and `lost` branches such as `item`, `mob`, `monster`, `quest`, `default`, `info`, and lost-item fallback text instead of flattening those failure states into generic simulator prose, but true script execution, automatic branch conditions beyond the current quest requirement mapping, and full client conversation layouts are still missing | Core interaction now covers a much closer read-only version of client conversation flow, including multi-step selection menus, more of the client’s WZ-backed failure and lost-item text, and cleaner MapleStory token rendering, even though the simulator still lacks the full script bridge and broader client conversation window parity | player/NPC/UI layer (`CUserLocal::TalkToNpc`) |
| Partial | Quest lists and quest-only actions | Nearby NPC overlays now list WZ-backed available, locked, in-progress, and completable quests, keep accept/completion state plus quest mob counts, picked-up quest item counts, and quest action item grants/consumes for the simulator session, gate startable quests on `Check.img/0/item` requirements, and now also mirror additional common quest gates from WZ by surfacing `Check.img/0/pop`, trait-min requirements such as `charismaMin` or `craftMin`, and `Check.img/1/endmeso` costs through locked quest rows, NPC quest pages, quest detail requirement lines, completion progress, and accept or complete actions, while common quest actions now also apply trait EXP rewards from `Act.img/*EXP` alongside EXP, fame, next-quest notices, and linked quest-state mutations, but full script branching, SP or skill rewards, and broader inventory-backed quest parity are still incomplete | Progression-facing NPC parity now has a broader WZ-backed simulator baseline for both start and completion requirements plus common reward mutations, so fame, trait, meso-cost, item, and chained-quest state feedback stay in sync across the NPC overlay and quest window even though script-driven branches, SP or skill grants, and the wider inventory model are still missing | NPC/UI layer |
...
| Partial | Quest-alert, balloon, and idle social feedback loops | NPCs with available, in-progress, or completable quests now render the client-backed `QuestIcon` alert art above their heads based on the simulator quest runtime, quest accept/complete actions now queue timed in-world feedback balloons above the speaking NPC instead of collapsing to a single fixed message while also holding the NPC in `speak` until the active balloon expires, nearby NPCs with WZ `info/speak/*` idle lines now periodically surface those lines through the same in-world balloon path, and pets now mirror the `CUser::Update` idle rest-alarm path by preferring structured `String/PetDialog.img` event keys such as `e_rest` when present while still falling back to legacy talk-chat lines, plus the simulator now surfaces the same WZ-backed `PetAutoSpeaking` near-level-up, level-change, low-HP alert, and HP/MP-potion failure lines when those runtime conditions occur by preserving item hotkey bindings at zero count and routing USE-item clicks or hotkeys through WZ `Item/Consume/*/spec` recovery parsing, but broader non-potion specialist pet chatter call sites are still not modeled | The simulator now surfaces both static quest-state alerts and closer client-like NPC and pet idle feedback loops in-world instead of hiding all progression feedback inside the dialogue overlay alone, and the remaining pet auto-speaking gap is narrowed to the wider set of non-potion specialist triggers the current runtime still does not produce | `QuestRuntimeManager.cs`, `NpcFeedbackBalloonQueue.cs`, `NpcItem.cs`, `LifeLoader.cs`, `PetLoader.cs`, `PetController.cs`, `MapSimulator.cs`, `UIWindow*.img/QuestIcon`, `String/PetDialog.img` (`CUserLocal::Update`, `CUser::PetAutoSpeaking`) |
