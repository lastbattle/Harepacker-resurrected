...
- A simulator-owned pet runtime base in `PlayerManager` and `HaCreator/MapSimulator/Companions/`, including an ordered active-pet roster, live pet update loop, WZ-backed pet frame loading, and the client-confirmed pickup-forbidden gate for map `209080000`.
...
| Partial | Active pet lifecycle and multi-pet rendering | The simulator now exposes an ordered active-pet roster with live pet entities, slot reindexing, and WZ-backed in-field pet rendering, but it still lacks the client's fuller lifecycle details such as hang or back variants, random idle actions, and the remaining multi-pet behavior owned by `CPet::Update` and `CUser::SetActivePet` | Companion-heavy gameplay and visual parity are no longer blocked on having no pet entities at all, but the roster still needs more client-specific behavior before multi-pet parity is complete | companion runtime, avatar or field-entity layer (`CPet::Update`, `CUser::SetActivePet`) |
...
| Partial | Pet speech, commands, and idle feedback | The simulator now loads WZ-backed pet command tables and reaction speech from `Item/Pet/*.img` plus `String/PetDialog.img`, routes submitted chat text through the active pet roster, and lets `PetRuntime` own command-triggered reactions, timed auto-speaking balloons, and client-shaped idle action loops instead of leaving pet feedback as a simulator-local placeholder; however, pet-tameness command gating and the adjacent food or slang reaction surfaces are still unmodeled | Companion parity is no longer limited to follow visuals because pets now answer chat-triggered commands and emit in-world speech or idle feedback through the runtime, which makes the field feel much closer to the client even before the remaining tameness and care systems land | `HaCreator/MapSimulator/Companions/`, `MapSimulator.cs`, `MapSimulatorChat.cs` (`CPet::Update`, `CUser::PetAutoSpeaking`) |
