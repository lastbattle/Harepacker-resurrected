...
| Partial | Dragon companion runtime/action parity | `PlayerManager` now owns a dedicated `DragonCompanionRuntime` that activates for Evan job paths, loads and caches in-field dragon action canvases directly from the client-backed `Skill/Dragon/<job>.img` images, resolves dragon-facing base `stand` / `move` state separately from ordinary avatar assembly, promotes dragon-specific skill actions such as `magicmissile`, `fireCircle`, `lightingBolt`, `icebreathe_prepare`, `dragonIceBreathe`, `breathe_prepare`, `dragonBreathe`, `magicFlare`, `magicShield`, `elementalReset`, `magicBooster`, `magicRegistance`, `killingWing`, `dragonThrust`, and `slow` onto a separate owner-followed render pass, and keeps those frames cached per Evan job the same way the client’s `CActionMan::LoadDragonAction` path does instead of leaving the in-field dragon implicit; the remaining gap is the rest of the client-owned dragon position and map-state authority such as exact `CVecCtrl` follow behavior, `vanishDragon` / no-dragon field suppression, and the fuller one-time-action or alpha-layer transitions around `CDragon::Update` and `PrepareActionLayer` | Evan classes now surface a visible secondary dragon actor with its own WZ-backed action cache and render-time action selection instead of collapsing dragon parity to HUD bars and equip panes, which makes dragon skill presentation testable through a distinct companion runtime before the last client-owned movement and field-gating details land | companion actor/runtime layer (`CActionMan::LoadDragonAction`, `CDragon::PrepareActionLayer`, `CDragon::Update`, `CDragon::DrawKeyDownBar`) |
...
| Partial | Portable chair and install-item parity | The simulator now loads WZ-backed setup-chair layers from `Item/Install/0301*.img`, attaches their `effect` or `effect2` animations as avatar-bound sit overlays, splits couple-chair `Effect/ItemEff.img/<itemId>/0` midpoint animations away from the seat-bound layers so `3012xxx` chairs render a local midpoint effect between the seated owner and the preview partner, exposes an owned-item `/chair <itemId|clear>` runtime path, mirrors the client-confirmed riding-chair handoff by equipping `info/tamingMob` mounts while the chair is active and restoring the previous taming-mob state on stand-up, and uses couple-chair `info/distanceX` and `info/distanceY` metadata to render a second seated avatar preview so pair spacing can be checked locally; however, the true two-user `CUserPool` pairing handshake, remote partner resolution, and network-authored couple-chair state and effect ownership behind `CUser::SetCoupleChairEffect` still remain unmodeled | Chair usage now exercises the client-shaped additional-layer seam, riding-chair mount handoff, local pair-chair seat spacing, and the WZ-backed midpoint couple effect instead of collapsing entirely into a plain sit pose, which makes setup-item testing and visible avatar composition checks possible before the remaining cross-user coupling flow lands | avatar additional-layer path, install-item runtime (`CUser::SetActivePortableChair`) |
...
| Partial | Guild BBS parity | The simulator now loads the dedicated `UI/GuildBBS.img` board shell, exposes a simulator-owned guild thread model with guild name/mark header, thread list selection, notice flags, thread/comment paging, keyboard-owned title/body/reply draft entry, WZ-backed basic emoticons plus client-backed cash-emoticon slot count from `UI/GuildBBS.img`, inventory-backed cash-emoticon ownership for item IDs `5290000` to `5290006`, client-shaped notice exclusivity when a notice already exists, and simulator-owned guild-board authority gates derived from the local guild rank and author ownership for write/edit/delete/reply flows; however, the real client textbox control or IME behavior, exact packet-authored guild-board permission masks, and true server-owned cash-emoticon entitlement sync still remain unmodeled | Guild workflows can now be exercised through a much closer board surface with typed drafts, inventory-shaped cash emoticon availability, and authority-gated notice/thread moderation instead of staying at unrestricted sample flows, which narrows this row to the remaining client textbox lifecycle plus exact server-authored permission and entitlement work behind the dedicated guild board seam | social UI layer, guild board model (`CUIGuildBBS::Draw`, `CUIGuildBBS::OnCreate`, `CUIGuildBBS::OnWrite`) |
| Partial | Family chart parity | The simulator now opens dedicated WZ-backed `Family` and `FamilyTree` windows from `BtFamily`, backs them with a simulator-owned family roster, mirrors the client-confirmed split between the compact family-statistics panel and the larger 11-slot tree layout, exposes family-count and junior-count feedback plus current/today reputation and entitlement-use state, uses client-driven tree semantics for empty branch versus junior-entry slots plus online/offline plate selection and child-node alert coloring, renders the family-tree header and counts on the client-confirmed `CUIFamilyChart::Draw` anchors for title, family count, and junior count instead of the earlier custom footer summary, and now exposes packet-shaped roster sync through `/family packet clear|seed|remove|upsert ...` so the simulator-owned roster can be rebuilt, edited, and selected from packet-authored membership data while entitlement execution from `BtSpecial` and privilege-icon cycling remain live; however, the exact client `_DrawChartItem` node-coordinate table, the literal client string-table text behind ids `0x11FD`, `0x1201`, `4608`, `4610`, `4611`, and `4612`, and deeper cross-map/server-authored privilege effects still remain unmodeled | Family progression and social checks are no longer blocked on the complete absence of family surfaces because the simulator now has dedicated family data, packet-shaped roster sync controls, a closer client-anchored tree draw, and locally executable privilege actions that can be exercised through the same two-window seam the client draws, while the remaining gap is narrowed to the last exact draw constants, client-owned string phrasing, and remote/server authority semantics | family runtime, family UI layer (`CUIFamilyChart::Draw`) |
| Partial | MapleTV and broadcast parity | The simulator now loads both the WZ-backed `UIWindow.img/MapleTV` send-board art and `UI/MapleTV.img` broadcast preview assets, exposes a dedicated MapleTV window with the client-shaped `BtOk`, `BtCancel`, and `BtTo` controls, backs it with a simulator-owned MapleTV manager that mirrors the client’s init or set-message or clear-message or send-result lifecycle names, resolves the default media branch from the shipped `TVmedia` set, applies WZ-plus-String-backed profiles for the shipped `5075000` through `5075005` MapleTV cash items so duration plus sender-only or dedication audience rules and non-default media branches are selected from the existing runtime seam, keeps a queue-visible idle state after timeout instead of dropping straight to empty, and now decodes `CMapleTVMan::OnSetMessage`-shaped packet payloads through `/mapletv packet` or `/mapletv packetraw` so remote sender and receiver `AvatarLook` payloads are turned back into previewable `CharacterBuild` data on the same seam the client fills with `AvatarLook`; however, exact client-owned in-world MapleTV overlay placement and text composition, the precise client item-to-media lookup and localized send-result string mapping beyond the current six-item profile set, and the full network-owned broadcast queue or live socket lifecycle still remain unmodeled | MapleTV is no longer a mostly local draft surface because the simulator can now exercise the dedicated message owner with WZ-authored send-board and preview assets, item-backed duration or audience behavior, queue-visible timeout and clear semantics, packet-backed remote avatar reconstruction, and visible sender or receiver presentation from the same lifecycle names the client uses, which narrows the remaining gap to the true world overlay and network-owned ownership path | social broadcast layer, timed overlay UI, broadcast-state manager (`CUIMapleTV::Draw`, `CMapleTVMan::Init`, `CMapleTVMan::OnClearMessage`, `CMapleTVMan::OnSendMessageResult`, `CMapleTVMan::OnSetMessage`) |
