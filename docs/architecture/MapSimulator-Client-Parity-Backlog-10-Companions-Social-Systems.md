...
| Missing | Dragon companion runtime/action parity | The client still keeps Evan dragon presentation on a dedicated action pipeline through `CActionMan::LoadDragonAction`, with its own cached action frames and render-time action selection separate from ordinary avatar assembly, while the current backlog only covers dragon HUD placement and dragon equipment tabs rather than the in-field dragon actor itself | Dragon classes depend on a visible secondary actor with its own action timing, frame data, and position ownership, so leaving this implicit risks landing UI parity while the actual dragon companion still lacks client-owned animation and render behavior | companion actor/runtime layer (`CActionMan::LoadDragonAction`, `CDragon::DrawKeyDownBar`) |
